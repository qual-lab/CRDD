...
| エージェント組織（`Agent Organization`） | `Multi-Agent Runtime` / `Subagent Tree` / `Coordinator Runtime` | 対象Workに必要な専門性、責務、能力、決定権限の境界および検証を組み合わせ、同じ目的と前提へ結果を統合する横断概念。常時Multi-Agent、固定親子構造、特定Coordinator製品またはRuntimeを意味せず、役割、能力および決定権限を同一視しない。詳細は[エージェント組織](04_Agent_Organization.md)を参照する |
...
規則名、節名、契約名を「日本語（English）」の形式で併記することは、正式用語の導入に当たらない。本書への定義追加と用語の境界行の追加を要しない。併記した名称を正式用語として扱う場合だけ、本書へ追加する。正式用語として登録する条件は次のいずれかを満たすことである。複数の規則で独立した概念として使用する。固有の定義または境界を持つ。他概念との区別が処理結果を変える。固有の決定権限、状態、関係、入力・出力を持つ。正本節への参照なしでも名称単独で意味を伝える必要がある。規則名として扱う条件は次のいずれかを満たすことである。ただし下記の優先規則の例外に該当する場合を除く。特定の規範ブロックまたは節を識別する表示名である。意味が参照先の本文だけで完結する。独立した対象、状態、決定権限を持たない。名称の利用目的が規則への到達である。両方の条件集合に該当する名称は、規則名として扱う。ただし、当該名称に固有のスキーマキー、状態値集合、または準拠基準の基準列に判定条件として現れる場合は正式用語として扱う。本節が定める判定条件は、v0.8.0以降に新しく導入する名称、または意味、責務もしくは適用範囲を変更する名称へ適用する。既に本書へ登録されているか否かは、2節または3節に当該名称のエントリが存在するか、前段が定める条件を満たす5節の境界行が存在するかで判定する。
