...
**必須（MUST）:** CRDD標準では、付与基準を満たす`REQ`、`UX`、`IA`、`UI`、`SPEC`だけに使用する。同じ意味の訂正ではIDを維持し、意味の置換では新IDを発行して`supersedes`で接続する。

**禁止（MUST NOT）:** ファイル名、文書番号、`CHG-*`、アーキテクチャ、判断、根拠、テスト、リリース等の成果物IDと同一視してはならない。`@1`等の手動改訂番号を結合して、成果物の固定改訂版や変更トレースと別の履歴正本を作ってはならない。
...
**定義:** 一つの主要な変更意図について、契機、想定影響／実際の影響、関連コンテキスト、実装、検証、リリース帰属を`99_Roadmap/Changes/<CHG-ID>/change.md`で接続するトレース成果物。
