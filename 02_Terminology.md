...
一言で言うと、**同じ1つの実体を表す、複数ある表現形のそれぞれ**。実体は1つのままで、表現形だけが複数ある。

**正式英語名:** `Projection`

**定義:** 同じ1つの実体を、目的ごとに異なる切り口で表した表現形のそれぞれ。実体は1つのままで、表現形だけが複数存在する。例えば、通販サイトの1件の注文という実体は1つだが、それを表す形として、注文履歴の一覧行（商品名と金額だけ）、注文詳細ページ（配送先や決済方法を含む全情報）、配送追跡画面（現在地と到着予定）という3つが存在する。**この一覧行・詳細ページ・追跡画面のそれぞれが投影（Projection）である。** 3つは別々の注文ではなく、同じ1件の注文という一つの実体を、目的ごとに異なる切り口で表しているだけである。
...
投影は、UIが画面上でどう見せるか（Tabsで切り替える、別画面にする、等）を決める前段階の、IAによる意味づけである。「これらは同じ実体の異なる表現形であり、別の実体ではない」と認定するのがIAの責務であり、実際の見せ方をTabsにするか別画面にするかというUI表現の具体化は、投影とは別の概念（[具体的な表示構造（Presentation Realization）](#224-presentation-realization)、UIが所有）が担う。単独の単語からは意味が定まらないため、常に「何の投影か」を示す対象（注文の投影、Evidenceの投影、等）と共に用いる。

**目的:** 「同じ対象の複数の見え方」と「別々の対象」を区別し、IAが誤って一つの対象を複数オブジェクトへ分割したり、UIが投影同士を無関係な画面として実装したりすることを防ぐ。

**決定権限:** IAが、ある情報群を単一オブジェクトの複数投影として扱うか別オブジェクトとして扱うかを決定する。UIは投影間の切替パターン（Tabs等）と同期の見せ方を、振る舞い仕様は同期の発火条件・対象・失敗時の振る舞いを所有する。

**関連用語:** 情報提示の意味構造（Information Presentation Model）、共有すべき選択コンテキスト（Shared Context）、具体的な表示構造（Presentation Realization）

**必須（MUST）:** 特定の投影を指して述べるときは、何の投影かを示す対象を併記しなければならない。同じ対象の投影候補は、IAがオブジェクト境界を確認したうえで、単一オブジェクトの投影か別オブジェクトかを判断しなければならない。投影同期、投影切替パターン、別投影候補等、投影という概念そのものを指す複合語はこの限りではない。

**禁止（MUST NOT）:** 見た目や画面が異なることだけを理由に、同じ対象の投影を別オブジェクトへ分割してはならない。
...
| 投影（`Projection`） | `View` / `Screen` | 同一対象の複数ある表現形のそれぞれを指すIA上の意味づけ。正本成果物の関連表現である`View`とは別概念であり、UIの画面そのものでもない |
...
新しい正式用語（Canonical Term）を導入する場合、使用前または同一の変更内で、本書へ定義、関連・廃止予定の表現、既存用語との境界を追加しなければならない。
