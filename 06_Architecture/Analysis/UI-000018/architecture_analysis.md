...
| UX分析 | このUIで保持する利用者成果 |
|---|---|
| [UX-000027](../../../04_UI/Analysis/UX-000027/ui_analysis.md) | 課題と判断の物語を理解してから、表・図で条件と関係を確認し、次の行動へ進める |
| [UX-000028](../../../04_UI/Analysis/UX-000028/ui_analysis.md) | 工程固有の図から全体像・関係・状態差・未接続を理解し、後工程で意図を再発明せずに引き継げる |

### IA観点の分析結果

| IA分析 | このUIで保持する情報構造 |
|---|---|
| [IA-000018](../../../04_UI/Analysis/IA-000018/ui_analysis.md) | 人が問題から判断まで理解し、工程固有の図と構造から次工程へ意図を渡す。 |

### 両観点の統合判断

| UX分析 | 利用者が得たい結果 | 対応するIA分析 | UIで成立させる対応 |
|---|---|---|---|
| [UX-000027](../../../04_UI/Analysis/UX-000027/ui_analysis.md) | 情報量を失わず短時間で意味と次の行動を理解できる | [IA-000018](../../../04_UI/Analysis/IA-000018/ui_analysis.md) | 物語（Narrative）、構造化した詳細（Structured Detail）、判断（Decision）、検証根拠（Evidence）、正本（Canonical Source）を見分ける。状態は「下書き（draft）／レビュー済み（reviewed）／現在有効（current）／履歴（historical）を必要時に区別」。導線は「問題と目的→判断→構造化詳細→根拠→次工程」 |
| [UX-000028](../../../04_UI/Analysis/UX-000028/ui_analysis.md) | 後工程で意図を再発明せず漏れを発見できる | [IA-000018](../../../04_UI/Analysis/IA-000018/ui_analysis.md) | 図（Diagram）、図の要素（Diagram Element）、上流での意味（Source Meaning）、引継ぎ義務（Handoff Obligation）、処置（Disposition）を見分ける。状態は「作成済み（created）／参照（referenced）／理由付き非該当（not_applicable with reason）」。導線は「上流意図→工程図→要素の意味→下流義務」 |
...
入力Definitionから継承した確認事項の由来: [UX-000027](../../../02_UX/Definitions/UX-000027/ux_definition.md)
...
入力Definitionから継承した確認事項の由来: [UX-000028](../../../02_UX/Definitions/UX-000028/ux_definition.md)
...
入力Definitionから継承した確認事項の由来: [IA-000018](../../../03_IA/Definitions/IA-000018/ia_definition.md)
...
|---|---|---|---|---|
| [機械検査と文書検査のArchitecture定義](../../Definitions/ARCH-000001/architecture_definition.md) | Checker CoreとCRDD現行Profile | UI契約はAuthorityを発行しない。利用者操作: 概要を読む／図から詳細へ進む／正本を開く。 | UI契約はEffectを定義しない。状態・導線: 下書き（draft）／レビュー済み（reviewed）／現在有効（current）／履歴（historical）を必要時に区別 / 問題と目的→判断→構造化詳細→根拠→次工程 / ；作成済み（created）／参照（referenced）／理由付き非該当（not_applicable with reason） / 上流意図→工程図→要素の意味→下流義務 /  | 確認項目順と専門語だけで文書を埋める／必要な図を黙って省略しAIごとに記法が変わる |
...
| Failure／Recovery | 評価済み | 確認項目順と専門語だけで文書を埋める／必要な図を黙って省略しAIごとに記法が変わる。Recoveryは入力定義にある場合だけ保持する。 |
| Security／Trust | 評価済み | 入力定義のAuthority、開示、Effect 0および非推定条件を保持する。 |
| Quality Constraint | 評価済み | 未観測・不明・制限・失敗を成功または不存在へ丸めない。 |
| Human Input | 継承あり | REQ-000031: CRDD作成者・保守者が「課題と判断の物語から構造化詳細へ進む」を行う際の判断基準、許容負担、利用環境および失敗後の選択／REQ-000032: CRDD作成者・保守者が「工程固有の図から状態・関係・未接続を理解する」を行う際の判断基準、許容負担、利用環境および失敗後の選択。 |
...
Human Inputの判断者は「CRDD閲覧者を代表する利用者とQual-Lab。」。再評価契機は「対象利用者による実利用確認、前提変更、または後続工程でこの未確認事項が成立条件へ影響すると判明した時。」。Architectureはこれらを解消済みとせず、入力の意味が変わる場合はOwner工程へ戻す。
