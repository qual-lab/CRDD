...
| UX分析 | 保持する利用者成果 |
|---|---|
| [UX-000008](../../../05_SPEC/Analysis/UX-000008/spec_analysis.md) | 故障した境界と影響範囲を理解する |

### IA観点の分析結果

| IA分析 | 保持する情報構造 |
|---|---|
| [IA-000020](../../../05_SPEC/Analysis/IA-000020/spec_analysis.md) | 実行基盤の故障箇所と利用可能範囲 |
...
| Failure／Recovery | 評価済み | 一つの失敗から全機能停止や原因を断定しない。 |
| Security／Trust | 評価済み | 入力定義のAuthority、開示、Effect 0および非推定条件を保持する |
| Quality Constraint | 評価済み | 未観測・不明・制限・失敗を成功または不存在へ丸めない |
| Human Input | 継承あり | REQ-000023: 実行環境の導入・運用者が「AI提供元固有の一連の状態変化を接続部越しに正確に扱う」を行う際の判断基準、許容負担、利用環境および失敗後の選択 |
