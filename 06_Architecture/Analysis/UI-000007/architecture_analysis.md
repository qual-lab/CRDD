...
| UX分析 | このUIで保持する利用者成果 |
|---|---|
| [UX-000012](../../../04_UI/Analysis/UX-000012/ui_analysis.md) | stdio MCPとlocalhost HTTPのどちらでも、同じ入力・権限・状態・結果で仕事を続けられる |
...
|---|---|---|---|
| [UX-000012](../../../04_UI/Analysis/UX-000012/ui_analysis.md) | stdio MCPとlocalhost HTTPのどちらでも、同じ入力・権限・状態・結果で仕事を続けられる | [IA-000008](../../../04_UI/Analysis/IA-000008/ui_analysis.md) | 公開要求、公開契約、通信手段（Transport）、作用発行状態、結果を見分ける。状態は「受付前／受付済み／作用前失敗／作用後失敗／結果あり」。導線は「入口→同じ公開要求→Runtime→同じ結果」 |
...
|---|---|---|---|---|
| [公開Transportの意味同一性のArchitecture定義](../../Definitions/ARCH-000012/architecture_definition.md) | MCP／CLI Transport Adapter | UI契約はAuthorityを発行しない。利用者操作: 依頼する／結果を受け取る／別入口で続ける。 | UI契約はEffectを定義しない。状態・導線: 受付前／受付済み／作用前失敗／作用後失敗／結果あり / 入口→同じ公開要求→Runtime→同じ結果 /  | 通信方式によって入力・権限判断・状態・結果の意味が変わる |
...
| Failure／Recovery | 評価済み | 通信方式によって入力・権限判断・状態・結果の意味が変わる。Recoveryは入力定義にある場合だけ保持する。 |
