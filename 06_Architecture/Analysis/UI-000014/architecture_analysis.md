...
|---|---|---|---|---|
| [契約移行と利用側閉包のArchitecture定義](../../Definitions/ARCH-000002/architecture_definition.md) | 変更影響分析とConsumer Closure契約 | UI契約はAuthorityを発行しない。利用者操作: 対応を見る／不足箇所へ進む。 | UI契約はEffectを定義しない。状態・導線: 維持／変更／廃止／未確認 / 変更→能力→全利用側→置換→反証根拠 /  | 主経路だけ移行し副次利用側を取り残す |
...
| Failure／Recovery | 評価済み | 主経路だけ移行し副次利用側を取り残す。Recoveryは入力定義にある場合だけ保持する。 |
