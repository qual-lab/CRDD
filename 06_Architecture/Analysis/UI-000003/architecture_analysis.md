...
| UX分析 | このUIで保持する利用者成果 |
|---|---|
| [UX-000004](../../../04_UI/Analysis/UX-000004/ui_analysis.md) | 失敗後に状態確認、再試行、回復および清掃を取り違えず、外部作用（Effect）の二重実行を避けて次の行動を選べる |
...
|---|---|---|---|
| [UX-000004](../../../04_UI/Analysis/UX-000004/ui_analysis.md) | 失敗後に状態確認、再試行、回復および清掃を取り違えず、外部作用（Effect）の二重実行を避けて次の行動を選べる | [IA-000003](../../../04_UI/Analysis/IA-000003/ui_analysis.md) | 失敗、試行（Attempt）、外部作用、結果、残存、回復処置（Recovery Action）、再試行を見分ける。状態は「失敗後の作用なし／作用済み／不明、回復要／不要」。導線は「失敗→作用状態→同じ依頼の結果→回復処置または再試行」 |
