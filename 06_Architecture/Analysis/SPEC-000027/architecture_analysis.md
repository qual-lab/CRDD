...
| Boundary／Component／Interface | 評価済み | 状態Ownerは候補採用Controller。公開境界は入力定義のAuthority・Effect・制約を越えない。 |
| Data／State Ownership | 評価済み | 候補採用ControllerをOwner候補とし、UI表示またはSPEC結果と内部状態を同一視しない。 |
| Failure／Recovery | 評価済み | 結果受領や送信許可を候補採用Authorityへ流用しない。Recoveryは入力定義にある場合だけ保持する。 |
