...
| UX分析 | 保持する利用者成果 |
|---|---|
| [UX-000012](../../../05_SPEC/Analysis/UX-000012/spec_analysis.md) | 入口を変えても同じ仕事を続ける |
...
入力Definitionから継承した確認事項の由来: [UX-000012](../../../02_UX/Definitions/UX-000012/ux_definition.md)
...
入力Definitionから継承した確認事項の由来: [IA-000008](../../../03_IA/Definitions/IA-000008/ia_definition.md)
...
| 観点 | 判定 | 根拠・引渡し |
|---|---|---|
| Responsibility | 評価済み | [公開Transportの意味同一性のArchitecture定義](../../Definitions/ARCH-000012/architecture_definition.md)へ入力Contractを意味変更せず渡す。 |
| Boundary／Component／Interface | 評価済み | 状態OwnerはMCP／CLI Transport Adapter。公開境界は入力定義のAuthority・Effect・制約を越えない。 |
| Data／State Ownership | 評価済み | MCP／CLI Transport AdapterをOwner候補とし、UI表示またはSPEC結果と内部状態を同一視しない。 |
| Failure／Recovery | 評価済み | Transport固有値を意味契約へ混入せず、未対応入口を成立済みと表示しない。Recoveryは入力定義にある場合だけ保持する。 |
| Security／Trust | 評価済み | 入力定義のAuthority、開示、Effect 0および非推定条件を保持する。 |
| Quality Constraint | 評価済み | 未観測・不明・制限・失敗を成功または不存在へ丸めない。 |
| Human Input | 継承あり | REQ-000006: 開発者が「stdioとHTTPで同じ意味の操作を行う」を行う際の判断基準、許容負担、利用環境および失敗後の選択／REQ-000010: プロジェクト運営者／PMが「入口が違っても同じプロジェクト操作を行う」を行う際の判断基準、許容負担、利用環境および失敗後の選択／REQ-000028: 開発者が「入口が違っても同じCRDD正本と判断境界を使う」を行う際の判断基準、許容負担、利用環境および失敗後の選択。 |
