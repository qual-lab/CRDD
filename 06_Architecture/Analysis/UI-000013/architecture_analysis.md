...
| UX分析 | このUIで保持する利用者成果 |
|---|---|
| [UX-000020](../../../04_UI/Analysis/UX-000020/ui_analysis.md) | 準拠、改ざん有無、配布者、公式表示および実行許可を区別し、自分の環境の方針で公式版・派生版・組織版を選べる |
| [UX-000031](../../../04_UI/Analysis/UX-000031/ui_analysis.md) | CRDDの公式入口や素材を視覚的に見分けながら、その表示だけを署名・準拠・品質または発行元への信頼の証明と誤認せず利用できる |

### IA観点の分析結果

| IA分析 | このUIで保持する情報構造 |
|---|---|
| [IA-000015](../../../04_UI/Analysis/IA-000015/ui_analysis.md) | 公式表示だけに頼らず、異なる根拠を分けて実行基盤を信頼するか決める。 |

### 両観点の統合判断

| UX分析 | 利用者が得たい結果 | 対応するIA分析 | UIで成立させる対応 |
|---|---|---|---|
| [UX-000020](../../../04_UI/Analysis/UX-000020/ui_analysis.md) | 信頼の各要素を区別し、自分の環境の方針で実行基盤を選べる | [IA-000015](../../../04_UI/Analysis/IA-000015/ui_analysis.md) | 準拠、完全性、配布者、成果物、信頼方針および導入判断を別々に示す。確認済み／未確認と、信頼済み／非信頼を同じ状態へ畳まず、成果物から各根拠、利用者方針、導入判断へ辿れるようにする |
| [UX-000031](../../../04_UI/Analysis/UX-000031/ui_analysis.md) | 見た目だけを信頼根拠にせず適切な入口を選べる | [IA-000015](../../../04_UI/Analysis/IA-000015/ui_analysis.md) | 公式識別（Official Identity）、配布者（Publisher）、署名（Signature）、準拠（Conformance）、品質主張（Quality Claim）を見分ける。状態は「識別済み（identified）／確認済み（verified）／信頼済み（trusted）／品質確認済み（quality_assured）を別軸にする」。導線は「公式表示→配布者（Publisher）→完全性→準拠→品質主張→利用判断」 |
...
入力Definitionから継承した確認事項の由来: [UX-000020](../../../02_UX/Definitions/UX-000020/ux_definition.md)
...
入力Definitionから継承した確認事項の由来: [UX-000031](../../../02_UX/Definitions/UX-000031/ux_definition.md)
...
入力Definitionから継承した確認事項の由来: [IA-000015](../../../03_IA/Definitions/IA-000015/ia_definition.md)
...
| Failure／Recovery | 評価済み | 一つの署名表示を全保証と誤認する／アイコンや見た目を署名・準拠・品質保証と誤認する。Recoveryは入力定義にある場合だけ保持する。 |
