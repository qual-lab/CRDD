...
公開Processの回復観測は、回復処理を代替する操作やRecovery Authorityを追加しない。Runtimeは各耐久readbackの後に、Project／Milestone／Queue／Task／Operation／Recovery Identityと`required / recovering / settled / acknowledged / verification_resources_finalized / queue_settled / retry_ready`の段階だけを構造化診断として出力できる。診断は書込み要求と書込み完了を区別し、完了、stream失敗または有限時間の終了まで所有する。診断失敗は回復状態を変更しない。最終E2Eは、実Provider開始後に公開MCP親Process treeを強制終了し、freshな署名済み公開MCP Processから同じObjectiveへ再入場して、この順序付き閉集合、同一の非Authority回復参照、再試行完了、正本不変および最終Docker資源不存在を一体で観測した場合だけ、実回復settlementを成立とする。通常終了後のclean観測や自己申告booleanだけでは成立しない。
...
3. Queue選択後、Lease種別、元Queue、owner generation・Process・回復IDを結んだ取得中Markerを一意な準備fileへwrite、flush、close、exact readbackした後、同一Filesystemの排他的hard linkで最終Pathへ完全なbytesとして公開する。別Processが準備中または最終Pathを所有する場合、後着の取得経路は内容を回復残存と推定せず、取得不可、手動回復不要、Effect 0で停止する。取得経路は既存Markerからowner喪失を推定しない。公開Objective入口はLease取得前に共有取得資源を読取り専用で調べ、Markerが示す実際の元Queueを解決してからfreshなowner観測へ渡す。現在の要求Queueを残存資源の所有者と推定しない。正本採用入口も採用Lease取得前に同じ確認を行う。Process消失後のfreshな専用reconciliationだけがownerの不存在とMarker identityを確認し、完全な準備file、公開済みMarkerおよび旧実装の一時fileを同じexact Recoveryへ接続する。owner生存・観測不能では変更せず取得不可へ閉じ、不完全・不正・複数の候補またはIdentity不一致は自動削除せず、exactな非Authority回復参照を返す。その後に物理Lockを作り、Lock作成に成功した同じownerであることを示すMarkerと取得証跡を順に耐久化する。Queue ownerへ結合する前に停止しても、Project Operationと正本採用の両Leaseを共通回復プロトコルで識別する。Project Operationでは回復結果を実際の元Queueの新世代へ書いてreadbackするまで取得中Markerを保持し、最後に除去する。正本採用の回復IDはQueueではなくRepository BindingとProjectへ結ぶ。Lock所有Markerがない既存Lock、不正MarkerまたはIdentity不一致は自動削除せず、決定論的な回復IDを伴う手動回復へ閉じる。

Project Operationの共有取得資源は、MarkerだけでProjectを推定しない。MarkerのQueue IDからQueue正本を読み、Repository Binding、実Project、実Queue、owner generation・Processおよび回復IDが閉じた解決結果になった場合だけ分類する。分類と処置は次に固定する。

| 観測した状態 | 処置 |
|---|---|
| 同じProject、正確なQueue、owner不在 | freshな専用reconciliationだけが自動回復する |
| 同じProject、owner生存 | 資源を変更せず取得不可または待機とする |
| owner観測不能 | 資源を変更せずFail Closedとする |
| 別Project、Queue欠落、Queue履歴不正またはIdentity不一致 | 新しいProject State／Queueを作成せず、既存資源を変更せず、Repository Bindingへ結合したexactな非Authority回復参照を返す |
...
| `IF-QUEUE` | enqueue、再送再利用、世代更新、短時間変更Lock、Runtime発行Leaseとのowner結合、Project Operation Lease、Project単位の正本採用Lease、解放不明Marker、対話Lane優先、Project実行の`queued → leased → running → integration_pending / recovery_required / replan_required / human_decision_required / cancelled`を部分接続。公開Objective入口は共有取得資源から実際の元Queueを解決し、Windows owner観測と型付きRecovery再入場へ接続する。実資源を伴う全Recoveryと電源断の公開Process E2Eは未成立 | 単一・複数Task実行～Recovery | `PR-D-Q-01`、`PR-D-A-01`、`PR-Q-04`、`PR-A-04`、`PR-A-05` |
...
| `IF-INTEGRATION` | 耐久Task候補IDから実Candidate Storeの内容を読み、同じbaseの候補を統合する。Path競合時の判断移送、明示Authority下の採用Lease、取得前のowner確認、fresh Revision／Scope／base内容照合、失敗時rollback、Objective／Milestone Evidence受入を部分接続。公開Project Runtimeは、Task生成と統合が同じRuntime実行Identityに属するCandidate Store境界と同じPlatform owner観測を使う。固定開発版の実測では、未署名の開発SourceへRelease Authorityを与えず、検証済み署名配布のCandidate Storeを依存注入して同じ候補を読み書きする | 統合・採用 | `PR-I-01`、`PR-I-02`、`PR-A-06` |
