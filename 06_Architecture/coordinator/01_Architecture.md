...
開発Sessionの完全Identity観測と進行中作業の生存確認を分ける。Admission前後、Task予約、Operation結合、Native成果物の借用、CLI呼出し予約・消費その他のAuthorityまたはEffect境界では、Source、Native成果物およびRepository Identityを完全に再観測する。既に固定したSession内での進捗参照、取消・期限・Process停止の確認およびread-onlyな利用量集計は、Session bindingと現在時刻へ結合した生存確認だけを行い、配布物全体を再Hashしない。生存確認は新しいAuthority、Native成果物またはProvider Effectを発行できず、後続の完全再観測を代替しない。
