...
公式ReleaseはGitだけで自己完結する二Commit構造を用いる。Release候補Commit AにはSource、文書、試験と、固定Path `90_Release/platform-access/x86_64-pc-windows-msvc/crdd-platform-access.exe`および`90_Release/coordinator/x86_64-pc-windows-msvc/coordinator.exe`のlocked release build成果物を含め、manifestを含めない。Commit Aのblob byteを変換せずRepository-local stagingへ展開し、署名manifestを生成する。Commit BはCommit Aへ固定Path `90_Release/coordinator-package-manifest.json`だけを追加する。公式tagはCommit Bへ付け、manifest内の`crddCommit`／`crddTree`はCommit A／Tree Aを示す。Commit AからCommit Bへの差分がmanifest 1件だけでなければRelease Identityは成立しない。
