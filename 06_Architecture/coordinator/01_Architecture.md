...
manifest revision 5は、Coordinatorの`bin/**`、`src/**`、`runtime/**`、`policies/**`および`package.json`に加え、共通Launcherの正本が選ぶ署名・4経路・Recovery入口と、そこから静的に到達する`script`依存を自動走査する。責務分離されたMCP、Project Runtimeおよび実行知（Execution Intelligence）は、Coordinatorからcanonicalな静的importで到達する`src/**`だけを同じ実行閉包へ含め、Component全体、文書、試験または未利用sourceを一律に含めない。このcontent root、Root Protection／Key Storage Policy Hash、Platform Access成果物のPath・target・protocol・toolchain・byte長・SHA-256からRuntime実行Identityを決定論的に算出し、Ed25519署名へ結合する。共通Launcherの入口表とIdentityのseedを別々の手書き一覧にせず、許可されていない兄弟Componentまたは実行集合外への依存は拒否する。
