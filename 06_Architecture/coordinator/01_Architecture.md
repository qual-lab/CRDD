...
署名Source Aからmanifest carrier Bへの境界は、署名済みManifestをJSON文書として再生成する境界ではなく、不透明なbyte列を固定Pathへ昇格するRelease Effectである。昇格Coreは、staging Root、Source file、Repository Root、配置先親Directoryおよび作成fileのIdentityを保持し、排他的作成、全byte書込み、flush、handle close、再読取り、byte数とSHA-256の一致を順に確認する。公開昇格入口はそれに先立ち、固定公開鍵による署名、Source AのCommit／Tree、閉じたRuntime実行集合、Policy、Native成果物、現在HEADおよび配置先不存在を同じ候補へ結合し、昇格後にも同じ検査をやり直す。API呼出し、write完了またはfile存在だけを昇格完了としない。

昇格前の観測不能は不存在へ畳まずEffect 0で停止する。昇格中または昇格後検査で失敗した場合は、そのOperationが排他的に作成し、file Identityと内容Hashが変わっていない固定Pathだけを回収できる。別主体による置換、内容変化または観測不能があれば削除せず、Filesystem Effect発行済みかつcleanup未確認として人間へ移送する。昇格成功はRelease Authority、Runtime Authority、Capability、Commit Bまたは公開を単独では成立させない。
