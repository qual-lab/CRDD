...
CLI optionは個別の存在だけでなく、同時指定する全optionの組合せと実Workspace変更を固定Binaryの実境界で反証する。Codexは`approval_policy="never"`とRuntime所有のRole別Filesystem権限Profileに加え、非対話実行のSandboxをRoleごとに明示する。Executorは`--sandbox workspace-write`、Reviewerは`--sandbox read-only`とし、Profile側でもExecutorだけがWorkspace書込みを許可する。ReviewerはShell／Filesystem Toolを無効化する。`--approve-for-me`は`--sandbox`との組合せを固定CLIが拒否するため、実行経路へ使用しない。Dockerのread-only root、Capability drop、Network proxyおよびProvider Home保護は維持する。
