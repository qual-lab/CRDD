...
CLI optionは個別の存在だけでなく、同時指定する全optionの組合せと実Workspace変更を固定Binaryの実境界で反証する。Codex Executorの隔離所有者は外側のDocker境界とする。固定CLIには、外部Sandbox内の非対話実行用として提供される`--dangerously-bypass-approvals-and-sandbox`を指定し、Container内で追加のbubblewrapを起動しない。このoption単体を安全根拠にせず、非root user、read-only root、Capability drop、限定Workspace mount、Network proxy、Provider Home保護および終了後資源不存在が全て成立した場合だけExecutorを起動する。Reviewerは`approval_policy="never"`と`--sandbox read-only`を明示し、Shell／Filesystem Toolを無効化する。内外のSandboxを重ねる場合は、対象Docker Engineで内部Sandboxが必要とするKernel Capabilityを実証できない限り成立済みと扱わない。
