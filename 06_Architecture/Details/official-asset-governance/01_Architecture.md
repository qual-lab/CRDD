...
File-backedな共有状態では、用途限定Store RootとKernelの排他作成を用いて`read-check-write`全体を一つの排他区間へ閉じる。単一Process内の順次実行だけを競合保証にせず、別Processから同じ期待revisionで開始した場合も一方だけが確定し、敗者は勝者の状態を保持したままEffect 0で拒否されなければならない。
