...
File-backedな共有状態では、用途限定Store RootとOS Kernel排他を用いて`read-check-publish`全体を一つの排他区間へ閉じる。Filesystem上のLock Recordは回復根拠であって排他そのものではない。Recordの生成、観測および回復対象の確定は同じKernel排他内で行う。Kernel Endpoint解放後のRecord削除は、同じexact IdentityのObligationを再確認する二段階cleanupとして旧Ownerまたは並行回復者から別世代を保護する。確定済みFileへの上書きrenameはOSごとに意味が異なるため使用せず、同一Filesystem内で一時Fileを完全に書いた後、次Revisionの不変世代名へ排他的Hard Linkで公開する。Readerは完全に公開された最大Revisionだけを採用する。単一Process内の順次実行だけを競合保証にせず、別Processから同じ期待revisionで開始した場合も一方だけが確定し、敗者は勝者の状態を保持したままEffect 0で拒否されなければならない。

有効な残存Lock Recordを検出した場合は、Revision競合へ畳まず非nullのexact Recovery Identityを`recovery_required`として搬送する。Identity確定前の取得・観測不能、または空、部分、破損、読取り不能なRecordは`observation_unknown`へ分離し、Recovery Identityを発行しない。有効なIdentityを確認した後に観測者自身のKernel Endpoint解放を確定できない場合は、同じexact Identityを`recovery_required`として保持する。稼働中Ownerとの競合は`unavailable`として分け、いずれも素材判断のStore Effectを発行しない。

素材判断のStore EffectとLock cleanupの完了は同一視しない。Store更新後にKernel Endpointの解放を確認できない場合は、確定済みRevision、`storeEffectIssued: true`およびexact Recovery Identityを`effect_issued_cleanup_required`として返す。Operation自体の成否を確認できない場合は`storeEffectIssued: null`として分け、`false`へ畳まない。同じRecovery Identityでcleanupを完了するまで、後続の判断Effectは発行しない。
...
|---|---|---|---|---|
| [UI／SPEC Detail Architecture Traceability](../../08_UI_SPEC_Detail_Traceability.md) | ARCH-000017のSource Definition | 同Traceability表で上記ARCH-IDへ接続された全Detail ID | Covered | Detailの意味変更はUI／SPECへ、配置責務の変更は該当ARCH定義へ戻す |
