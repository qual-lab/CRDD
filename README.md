...
- [Agent and provider orchestration](99_Roadmap/07_CRDD_v2_Agent_and_Provider_Orchestration.md)
...
| Communication capability | “Prepare external communication for `<audience and purpose>`. Always handle the audience and purpose, claims and evidence, projection and publication, and measurement and learning under the Communication contract. First evaluate two conditions: whether the audience, proposition, acquisition path, channel, advertising, or treatment of market response is being created or changed; and whether the purpose includes awareness, evaluation, trial, adoption, continuation, or advocacy. Use the Discovery market lens only when both are true. If either is false, do not activate that additional exploration even when the other is unknown; continue the ordinary Communication responsibilities only. If neither is false and at least one is unknown, state the missing information and who can confirm it, retain the ordinary Communication responsibilities, and stop unapproved external action pending confirmation. When the additional exploration applies, before selecting a channel or budget, examine the situation that starts adoption, current workarounds and alternatives including doing nothing, users, champions, decision-makers and blockers, conversion barriers, trust requirements, and the distinct roles of articles, landing pages, demos, and sales material. Treat awareness, evaluation, trial, first value, continuation, and advocacy as optional lenses rather than a fixed funnel. Keep Discovery-owned market, segment, and adoption hypotheses separate from Communication-owned audience, message, channel, artifact, path, and measurement hypotheses. Within that additional exploration, separate observed behavior, proxies, inferred attributes, and self-report, and compare qualitative discovery with quantitative confirmation without fixing an interview count or channel order. Do not invent market hypotheses for a factual notice, an approved publication to its designated destination, or a publication-record update. Whether or not additional market exploration is activated, whenever human-subject research is conducted, apply the Discovery-owned evidence-quality boundaries for participant eligibility, selection and recruitment, nonresponse and exclusions, role and context bias, generalization limits, and continuation or termination, as well as the safety boundaries for purpose and use, consent, refusal and withdrawal, response pressure, data minimization, re-identification risk, and freedom from disadvantage; do not require research artifacts when no research is conducted. Before asking for an action outside the artifact, connect the promised path, response, and delivery capacity to evidence; if unavailable, state the impact and stop or offer a truthful alternative. Do not infer whether copy, UX, product value, price, or operations caused a response; separate the observation, cause candidates, responsible contexts, and required evidence. For persuasion, protect autonomy and material conditions; do not use deception, unsupported urgency, excessive fear, or coercive targeting. Connect claims to evidence, distinguish projection from publication, require Human publication approval, and return measurement as learning candidates rather than product truth. If the authorized processing boundary or Human authority is not established, do not conduct external research, contact people, or launch advertising; present only unverified hypotheses, decision-changing questions, required evidence, and an exploration plan.” |
...
- [Forward Compatibility](99_Roadmap/06_CRDD_v2_Forward_Compatibility.md)
- [Agent／Provider Orchestration](99_Roadmap/07_CRDD_v2_Agent_and_Provider_Orchestration.md)
...
| 外部コミュニケーション機能 | 「`<受け手と目的>`向けの外部説明を準備して。通常Communicationとして、受け手と目的、主張と根拠、投影と公開、測定と学びは常に扱って。最初に、受け手・訴求・獲得経路・媒体・広告・市場反応の扱いを新設または変更することと、目的が認知・評価・試行・導入・継続・支持形成を含むことを別々に判定して。両方が成立する場合だけDiscoveryの市場探索レンズを使って。いずれか一方が成立しない場合は、もう一方が不明でも追加探索を発火せず、通常Communicationの責務だけを続けて。成立しない条件がなく、少なくとも一方が不明な場合だけ、不足情報と確認先を示し、通常Communicationの責務を維持したまま確認まで未承認の外部行為を止めて。追加探索が適用される場合は、媒体や予算を先に確定せず、採用が動き始める状況、現在の回避策と何もしない選択を含む代替、利用者・推進者・導入決定者・阻害者、転換障壁、信用条件、記事・LP・デモ・営業資料の役割を確認して。認知、評価、試行、初回価値、継続、支持は固定ファネルではなく必要に応じて使うレンズとして扱い、Discoveryが所有する市場・対象セグメント・採用仮説と、Communicationが所有する受け手・メッセージ・媒体・成果物・導線・測定仮説を分けて。その追加探索では、観測した行動、代理指標、推定属性、本人申告を分け、固定人数や固定媒体順を使わず定性的な発見と定量確認を比較して。事実通知、承認済み成果物の指定先公開または公開記録更新に市場仮説を作らないで。追加市場探索の発火にかかわらず、人間対象調査を行う場合は、Discoveryが所有する対象者の適格条件、選定・募集、無反応・除外、役割・状況の偏り、一般化限界、継続・終了条件という根拠品質と、目的・利用範囲、同意、拒否・中断・撤回、回答圧力、情報最小化、再識別可能性、不利益防止という安全の両方に従い、調査しない場合は空成果物を作らないで。成果物外の行動を求める前に、約束した導線、対応、提供能力を根拠へ接続し、確認できなければ影響を示して停止するか、事実に沿う代替を提示して。反応からコピー、UX、プロダクト価値、価格、運用のどれが原因かを確定せず、観測、原因候補、責務を持つコンテキスト、必要な根拠を分けて。説得では自律性と重要条件を守り、欺瞞、根拠のない緊急性、過度な不安または強制的な対象選別を使わないで。主張を根拠へ接続し、投影と公開記録を分け、人間の公開承認を置き、測定をプロダクト上の事実ではなく学び候補へ接続して。許可した処理境界と人間の権限がなければ、外部調査、対象者への接触、広告実行はせず、未検証仮説、判断を変える質問、必要な根拠、探索計画として提示して。」 |
