...
- 情報源となるUX / IAと対象改訂版。REQは上流トレースに限定する
...
| 観点 | UI契約側 | 振る舞い仕様側 |
|---|---|---|
| 情報源／目的 | 利用者の目標、UX / IAの意図 | システムが成立させる結果、情報源となるUX / IA。REQから直接補完しない |
...
<a id="ui-spec-diagram-profile"></a>

## 2.4.1. UI／SPEC対応図の処置と記法

UIと振る舞い仕様の各固定入口は、操作・表示・System結果の対応図を`作成`、`既存参照`、`非該当`、`作成不能`のいずれかへ処置する。利用者向けの操作、表示、Feedback、ErrorまたはRecoveryとSystemの状態・結果が対応する対象では発火する。直接UIがなく利用側契約だけで完結する場合は、その契約への参照と理由を示して非該当にできる。UIまたはSPECの片側が未作成、根拠不足、対応未確認である状態は非該当へ畳まない。
...
`[UI-*]`はUI成果物の一意な参照、`[SPEC-*]`は振る舞い仕様の一意な参照、`triggers`は操作から振る舞いへの接続、`shows`は状態から表示への接続、`produces`は振る舞いから観測可能な結果への接続を表す。一対多・多対一は線を分け、不一致、未対応および未確認を線なしのまま隠さず対応表へ記録する。図はUIまたはSPECの内容を再定義せず、[標準対応付け表示](#61-標準対応付け表示)を視覚化する投影である。

対応レビューの出口では、対象範囲の操作、表示状態、振る舞い、結果、Error、取消およびRecoveryが両方向に辿れ、図の投影元改訂版と対応表が一致していることを確認する。図で露出した片側欠落は所有工程へ戻し、共同レビュー前に解消するか未解決として明示する。
...
| 確認対象 | 基準 |
|---|---|
| 本共有契約の対応関係 | [必要な対応関係の網羅範囲](#23-required-pair-coverage)と[対応レビュー監査チェックリスト](#27-pair-audit-checklist) |
| UI工程 | 同工程の「必要な責務の網羅」と「工程監査チェックリスト」。別途確認する |
| 振る舞い仕様工程 | 同工程の「必要な責務の網羅」と「工程監査チェックリスト」。別途確認する |

一つの工程移行レビューへ結果を統合してよい。ただし、本共有契約の`Pass`でUIまたは振る舞い仕様の個別確認を代替せず、個別確認の`Pass`だけで対応関係全体を`Pass`にしない。
...
```text
情報源となるUX / IAと判断を確認し、REQはUXを介した上流トレースとして扱う
UI項目かSPEC項目かを確認する
...
実装、検証、運用で対応関係の競合を発見した場合、観察された根拠を保存し、UI／SPECのどちらが誤りか、両方の上流前提が誤りかを判断する。同じ意味の明確化・訂正は既存IDを維持して成果物の固定改訂版とCHG／Gitで追跡し、意味の置換は新IDと`supersedes`を用いる。`pairs_with`関係と影響コンテキストも更新する。
