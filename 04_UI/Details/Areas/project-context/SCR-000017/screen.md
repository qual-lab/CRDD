# SCR-000017 過去情報と現在有効な意図の選択

成果物種別: Logical Screen Detail
Screen ID: `SCR-000017`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 過去の推論や判断と現在有効な意図を混同せず選べる。
- 対象UI Definition: [UI-000017](../../../../Definitions/UI-000017/ui_definition.md)
- Area: [project-context](../area.md)
- 入口: 利用者が「過去情報と現在有効な意図の選択」を必要とし、対象Contextを識別できた時
- 終了条件: 過去の推論や判断と現在有効な意図を混同せず選べる。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000017` | 過去の推論や判断と現在有効な意図を混同せず選べる。 | 対象、状態、根拠、不足、観測時点、次行動 | 履歴を見る／現在値を選ぶ／再評価する。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000017.spec-000022` | PRT-000017 | 履歴を見る／現在値を選ぶ／再評価する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000022](../../../../../05_SPEC/Details/BHV-000022/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 履歴を見る／現在値を選ぶ／再評価する。 | 結果を同じContextで確認 |
