# SCR-000011 実行時データの保持・清掃

成果物種別: Logical Screen Detail
Screen ID: `SCR-000011`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 必要な状態だけを保持し、不要になったデータを安全に清掃できる。
- 対象UI Definition: [UI-000011](../../../../Definitions/UI-000011/ui_definition.md)
- Area: [configuration-trust](../area.md)
- 入口: 利用者が「実行時データの保持・清掃」を必要とし、対象Contextを識別できた時
- 終了条件: 必要な状態だけを保持し、不要になったデータを安全に清掃できる。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000011` | 必要な状態だけを保持し、不要になったデータを安全に清掃できる。 | 対象、状態、根拠、不足、観測時点、次行動 | 保持内容を見る／清掃する／保留する。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000011.spec-000005` | PRT-000011 | 保持内容を見る／清掃する／保留する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000005](../../../../../05_SPEC/Details/BHV-000005/behavior.md) | Covered |
| `PRT-000011.spec-000016` | PRT-000011 | 保持内容を見る／清掃する／保留する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000016](../../../../../05_SPEC/Details/BHV-000016/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 保持内容を見る／清掃する／保留する。 | 結果を同じContextで確認 |
