# SCR-000010 Tool・AIモデル構成の選択

成果物種別: Logical Screen Detail
Screen ID: `SCR-000010`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 仕事に合うToolとAIモデルを根拠付きで選び、安全に変更できる。
- 対象UI Definition: [UI-000010](../../../../Definitions/UI-000010/ui_definition.md)
- Area: [configuration-trust](../area.md)
- 入口: 利用者が「Tool・AIモデル構成の選択」を必要とし、対象Contextを識別できた時
- 終了条件: 仕事に合うToolとAIモデルを根拠付きで選び、安全に変更できる。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000010` | 仕事に合うToolとAIモデルを根拠付きで選び、安全に変更できる。 | 対象、状態、根拠、不足、観測時点、次行動 | 選ぶ／構成を検証する／更新する。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000010.spec-000014` | PRT-000010 | 選ぶ／構成を検証する／更新する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000014](../../../../../05_SPEC/Details/BHV-000014/behavior.md) | Covered |
| `PRT-000010.spec-000015` | PRT-000010 | 選ぶ／構成を検証する／更新する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000015](../../../../../05_SPEC/Details/BHV-000015/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 選ぶ／構成を検証する／更新する。 | 結果を同じContextで確認 |
...
- Area Design Guide参照: [configuration-trust](../area.md)
