# SCR-000013 Runtime信頼判断と公式識別

成果物種別: Logical Screen Detail
Screen ID: `SCR-000013`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 配布元、改ざん有無、準拠、利用者の信頼方針を分けて判断できる。
- 対象UI Definition: [UI-000013](../../../../Definitions/UI-000013/ui_definition.md)
- Area: [configuration-trust](../area.md)
- 入口: 利用者が「Runtime信頼判断と公式識別」を必要とし、対象Contextを識別できた時
- 終了条件: 配布元、改ざん有無、準拠、利用者の信頼方針を分けて判断できる。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000013` | 配布元、改ざん有無、準拠、利用者の信頼方針を分けて判断できる。 | 対象、状態、根拠、不足、観測時点、次行動 | 検証する／信頼方針を選ぶ／詳細を見る。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000013.spec-000018` | PRT-000013 | 検証する／信頼方針を選ぶ／詳細を見る。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000018](../../../../../05_SPEC/Details/BHV-000018/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 検証する／信頼方針を選ぶ／詳細を見る。 | 結果を同じContextで確認 |
