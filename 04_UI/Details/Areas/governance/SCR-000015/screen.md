# SCR-000015 監査・変更・試験・品質の追跡

成果物種別: Logical Screen Detail
Screen ID: `SCR-000015`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 同じ改訂版上で指摘、是正、試験、品質状態を辿れる。
- 対象UI Definition: [UI-000015](../../../../Definitions/UI-000015/ui_definition.md)
- Area: [governance](../area.md)
- 入口: 利用者が「監査・変更・試験・品質の追跡」を必要とし、対象Contextを識別できた時
- 終了条件: 同じ改訂版上で指摘、是正、試験、品質状態を辿れる。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000015` | 同じ改訂版上で指摘、是正、試験、品質状態を辿れる。 | 対象、状態、根拠、不足、観測時点、次行動 | 指摘を見る／根拠を開く／次Gateへ進む。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000015.spec-000020` | PRT-000015 | 指摘を見る／根拠を開く／次Gateへ進む。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000020](../../../../../05_SPEC/Details/BHV-000020/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 指摘を見る／根拠を開く／次Gateへ進む。 | 結果を同じContextで確認 |
...
- Area Design Guide参照: [governance](../area.md)
