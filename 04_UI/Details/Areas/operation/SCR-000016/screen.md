# SCR-000016 外部送信の同意・持帰り・採否

成果物種別: Logical Screen Detail
Screen ID: `SCR-000016`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 外部へ渡す範囲を理解し、戻った候補を採用前に判断できる。
- 対象UI Definition: [UI-000016](../../../../Definitions/UI-000016/ui_definition.md)
- Area: [operation](../area.md)
- 入口: 利用者が「外部送信の同意・持帰り・採否」を必要とし、対象Contextを識別できた時
- 終了条件: 外部へ渡す範囲を理解し、戻った候補を採用前に判断できる。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000016` | 外部へ渡す範囲を理解し、戻った候補を採用前に判断できる。 | 対象、状態、根拠、不足、観測時点、次行動 | 同意する／送信を止める／候補を採用・却下・保留する。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000016.spec-000021` | PRT-000016 | 同意する／送信を止める／候補を採用・却下・保留する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000021](../../../../../05_SPEC/Details/BHV-000021/behavior.md) | Covered |
| `PRT-000016.spec-000026` | PRT-000016 | 同意する／送信を止める／候補を採用・却下・保留する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000026](../../../../../05_SPEC/Details/BHV-000026/behavior.md) | Covered |
| `PRT-000016.spec-000027` | PRT-000016 | 同意する／送信を止める／候補を採用・却下・保留する。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000027](../../../../../05_SPEC/Details/BHV-000027/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 同意する／送信を止める／候補を採用・却下・保留する。 | 結果を同じContextで確認 |
