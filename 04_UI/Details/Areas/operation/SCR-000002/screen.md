# SCR-000002 委任・実行状態・判断

成果物種別: Logical Screen Detail
Screen ID: `SCR-000002`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 任せた範囲と進行状態を理解し、必要な時だけ判断する。
- 対象UI Definition: [UI-000002](../../../../Definitions/UI-000002/ui_definition.md)
- Area: [operation](../area.md)
- 入口: 利用者が「委任・実行状態・判断」を必要とし、対象Contextを識別できた時
- 終了条件: 任せた範囲と進行状態を理解し、必要な時だけ判断する。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000002` | 任せた範囲と進行状態を理解し、必要な時だけ判断する。 | 対象、状態、根拠、不足、観測時点、次行動 | 委任を提案する／受理・拒否・取消・判断返却を行う。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000002.spec-000002` | PRT-000002 | 委任を提案する／受理・拒否・取消・判断返却を行う。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000002](../../../../../05_SPEC/Details/BHV-000002/behavior.md) | Covered |
| `PRT-000002.spec-000003` | PRT-000002 | 委任を提案する／受理・拒否・取消・判断返却を行う。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000003](../../../../../05_SPEC/Details/BHV-000003/behavior.md) | Covered |
| `PRT-000002.spec-000028` | PRT-000002 | 委任を提案する／受理・拒否・取消・判断返却を行う。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000028](../../../../../05_SPEC/Details/BHV-000028/behavior.md) | Covered |
| `PRT-000002.spec-000029` | PRT-000002 | 委任を提案する／受理・拒否・取消・判断返却を行う。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000029](../../../../../05_SPEC/Details/BHV-000029/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 委任を提案する／受理・拒否・取消・判断返却を行う。 | 結果を同じContextで確認 |
