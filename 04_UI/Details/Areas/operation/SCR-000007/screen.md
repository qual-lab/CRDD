# SCR-000007 入口をまたぐ共通依頼・結果

成果物種別: Logical Screen Detail
Screen ID: `SCR-000007`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: CLI、MCP、Workbenchの入口を変えても同じ依頼と結果を扱える。
- 対象UI Definition: [UI-000007](../../../../Definitions/UI-000007/ui_definition.md)
- Area: [operation](../area.md)
- 入口: 利用者が「入口をまたぐ共通依頼・結果」を必要とし、対象Contextを識別できた時
- 終了条件: CLI、MCP、Workbenchの入口を変えても同じ依頼と結果を扱える。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 依頼する／結果を受け取る／別入口で続ける。 | 結果を同じContextで確認 |
