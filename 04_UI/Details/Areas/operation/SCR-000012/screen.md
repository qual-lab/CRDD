# SCR-000012 Agent間の情報引継ぎと再接続

成果物種別: Logical Screen Detail
Screen ID: `SCR-000012`
状態: Canonical
維持責任者: Qual-Lab

## 1. Screenの目的と入口・終了条件

- 利用者の作業・認識目的: 必要な情報を限定して渡し、切断後も同じ仕事へ戻れる。
- 対象UI Definition: [UI-000012](../../../../Definitions/UI-000012/ui_definition.md)
- Area: [operation](../area.md)
- 入口: 利用者が「Agent間の情報引継ぎと再接続」を必要とし、対象Contextを識別できた時
- 終了条件: 必要な情報を限定して渡し、切断後も同じ仕事へ戻れる。 次の安全な行動または所有正本を選べる
...
|---|---|---|---|---|
| `PRT-000012` | 必要な情報を限定して渡し、切断後も同じ仕事へ戻れる。 | 対象、状態、根拠、不足、観測時点、次行動 | 引き継ぐ／再接続する／結果を戻す。 | ready／pending／blocked／unknown／restricted |

## 3. InteractionとBHV対応

| Interaction Key | PRT | 利用者の意図 | Feedback | BHV | Coverage |
|---|---|---|---|---|---|
| `PRT-000012.spec-000017` | PRT-000012 | 引き継ぐ／再接続する／結果を戻す。 | 受付、結果、失敗、判断不能および安全な次行動を区別する | [BHV-000017](../../../../../05_SPEC/Details/BHV-000017/behavior.md) | Covered |

## 4. State／Variant

| State／Variant | 発生条件 | 表示する意味 | 操作 | 回復／次の状態 |
|---|---|---|---|---|
| ready | 必要情報とAuthorityを確認できる | 安全に主要操作へ進める | 引き継ぐ／再接続する／結果を戻す。 | 結果を同じContextで確認 |
