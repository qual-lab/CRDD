...
| 入力 | 件数 | 現在の処置 |
|---|---:|---|
| UX定義 | 31 | 全件を`Analysis/UX-*/ui_analysis.md`で個別分析する |
| IA定義 | 21 | 全件を`Analysis/IA-*/ui_analysis.md`で情報設計観点から個別分析する |
| UI分析（UX観点） | 31 | 全UXを一件ずつ分析し、利用者成果、認識、操作、Feedback、体験差を導く |
| UI分析（IA観点） | 21 | 全IAを一件ずつ分析し、情報、状態、関係、可視性、優先順位、導線を導く |
| UI定義 | 19 | 独立して変更・検証するUI契約へ統合する |
...
31件のUX定義と21件のIA定義を別々の正式入力として全数分析し、19件のUI定義へ統合した。REQはUXを介して追跡するが、UIの直接入力にはしない。独立レビューは、両観点の意味伝播、19 UIの状態適用範囲、37件のUX×IA関係およびSPEC引き渡しの責任分離をCritical／Major／Moderate／Minor 0で確認した。SPECもUX観点とIA観点を別々に分析し、UI定義との対応レビューで操作・表示と振る舞いの不足を突き合わせる。
