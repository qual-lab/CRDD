...
行単位の対話CLIに[デザインシステム参照実装](../25_UI.md#design-system-reference)をどう適用するかは未確定である。GUIがないことだけを理由に`Not Applicable`としない。現行ソースと出力例は確認対象の特定に使えるが、代表HTML、実端末の見た目・操作確認、支援技術確認の代替完了とは扱わない。この適用判断は下記で追跡する。
...
- 読み上げ順序、拡大・折返し、色非依存、対話／非対話の環境差を確認する。WCAG等の適合は本書で宣言しない。採用するアクセシビリティプロファイルと未評価環境は人間の判断対象に残る。
...
| 事項 | 根拠と影響 | 次の処置・再確認契機 |
|---|---|---|
| 表示の意味説明 | 日本語説明、未知値の固定表示、成功した候補操作と停止の分離を実装した | 独立確認待ち。説明未登録の理由は機械結果を担当者が確認する。全理由の翻訳完了とはしない |
| 未取得値と候補操作の表示 | 未取得booleanの三値表示、回復・再起動優先、停止時の候補操作抑止、全回復ID保持を実装した | 表示・取消投影・実子Process接続の28試験を確認。実端末と独立レビューは別途必要 |
| 入力・起動の実体験 | 人間報告に不表示、二重Enter、突然閉じる、文字化けがある。既存試験の合格だけでは端末全体を証明しない | 最新固定候補の起動経路で正常・拒否・時間切れ・取消・終了後表示を確認。ソース例を実測へ流用しない |
| CLI参照媒体の適用 | 25_UIは行単位CLIと画面構成型TUIの扱いを明示していない | 人間へ最小の規則明確化案を提示。現物確認を維持し、未決のままUI工程完了・N/Aにしない |
| 支援技術・環境 | 読み上げ、拡大、折返し、対話期限の十分性は未評価 | 適用プロファイルと対象環境を確定後、操作・認識の専門確認。未評価を適合と表示しない |
| 詳細設計の読み解き | SPEC、実行アーキテクチャ、脅威モデルに履歴・補正・現行条件が長段落で同居 | 操作／状態／責務ごとに再構成し、本表の対応先・検証先を再照合。[CHGの処置](../90_Release/Changes/CHG-000017_Tools_Coding_Standards.md#tool-experience-design)で追跡 |

工程網羅状態は`Blocked`。UIとSPECの共同レビュー、UI専門品質、実端末の確認、Qual-Labの内容・移行判断を別々に満たす必要がある。今回の文書独立レビューは再構成候補の責務と追跡性を確認するもので、実装・利用品質やRuntime全体を完了へ変えない。
