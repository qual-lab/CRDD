...
|---|---|---|---|
| [UX-000002](../../Analysis/UX-000002/ui_analysis.md) | 実行前に誰へ何をどこまで任せるかを理解し、暗黙の範囲拡張なく仕事を委ねられる | [IA-000002](../../Analysis/IA-000002/ui_analysis.md) | 目的、対象範囲、担い手、決定権限、入力、候補、取消、回復を見分ける。状態は「委任提案中／受付可能／受付済み／拒否／範囲見直し／実行中／停止。権限発行前後とTask発行前後を分ける」。導線は「目的→範囲と担い手→許可→受付、拒否時は同じ提案の範囲見直しへ戻る→実行」 |
...
現在状態・不足・制限
        ↓
目的→範囲と担い手→許可→受付、拒否時は同じ提案の範囲見直しへ戻る→実行／Task→現在状態→判断要否→待機・入力・取消・回復
```
...
|---|---|---|---|---|
| UX-000002 | 複数AIへ任せる範囲と権限を理解する | 外部作用（Effect）の前の委任境界 | 提案中、受付可能、受付済み、拒否、範囲見直しを分け、拒否理由と同じ提案への戻り先を示す | 暗黙の範囲拡張、拒否後の別依頼化、回復不能 |
...
|---|---|---|
| UX-000002／IA-000002 | 委任提案中／受付可能／受付済み／拒否／範囲見直し／実行中／停止。権限発行前後とTask発行前後を分ける | 目的→範囲と担い手→許可→受付、拒否時は同じ提案の範囲見直しへ戻る→実行 |
...
|---|---|---|---|
| UX-000002 | IA-000002 | 複数AIへ任せる範囲と権限を理解する。提案、受付可能、受付済み、拒否、同じ提案の範囲見直し、停止理由を行動可能に示す | IA-000002 が示す状態・関係を入力条件、成功・停止条件へ接続し、「暗黙の範囲拡張、拒否後の別依頼化、回復不能」を防ぐ観測可能な結果を確定する |
