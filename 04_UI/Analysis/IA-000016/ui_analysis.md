...
UX観点の分析が求める利用者成果、重要場面、操作およびFeedbackに対し、「変更・指摘・是正・試験・品質」の対象・状態・関係・導線が過不足なく判断材料を提供するかをUI定義で確認する。IAからUIへ引き渡す固有の意図は「QualityはVerification Designと状態評価を、CHGは変更意図と影響ファイルを、Gitは差分履歴を所有する。」であり、利用者の目的をここで推測して先取りしない。
