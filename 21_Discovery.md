...
初期項目の必須性が緩いことは、未評価の項目を無期限に放置してよいことを意味しない。プロジェクト固有ロードマップの決定権限は、宣言した整理契機に基づいて`Unreviewed`の項目を確認し、`Exploring`、`Held`、`Adopted`、`Rejected`または`Superseded`のいずれかへ進める。重複する項目は`Superseded`として統合先を参照する。整理契機は、6.6が定める宣言した契機の一つとして、プロダクトまたはリポジトリの運用で宣言し、取得可能にする。個々の初期項目に担当責任者が未設定であることは許容するが、登録簿全体を整理する決定権限を未設定にしてはならない。宣言した整理契機を持たない運用、または契機へ到達しても処置しない運用によって、`Unreviewed`の項目を無期限に放置してはならない。
