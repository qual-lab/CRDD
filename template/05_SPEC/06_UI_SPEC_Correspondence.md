...
各理由は`UI事実（UI-ID）「具体的事実」／SPEC事実（SPEC-ID）「具体的事実」／対応: 判断理由`の形で記載する。両定義の見出しや名称を言い換えるだけでなく、状態、契機、結果、失敗、回復、Authority、可視性または制約の実際の契約事実を引用・要約する。`一致させる`、`混ぜない`、`隠さない`等の判定規則だけを対応理由にせず、両側のどの状態・条件・結果・戻り先・Authority・表示または制約を、どの向きで対応させるかを同じ行に記録する。題名だけの置換、`…`による省略、`同上`等の参照、途中で切った事実は根拠にしない。複数SPECに対応するUIでは、その組が所有する責務だけを記載し、別の組が所有する操作・状態・結果を混ぜない。Checkerはこの構造、対象IDおよび根拠節との接続を検査し、意味的一致は独立レビューが確認する。差異がある場合は`一致`にせず、GapとOwner工程を記録する。
