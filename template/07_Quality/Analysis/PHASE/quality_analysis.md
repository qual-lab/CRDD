...
この工程が定義した成立条件を全件処置し、適用するDetailがある場合は具体的な観測条件をDefinition由来の意味と区別する。成功の意味、失敗またはRisk、検証義務および統合候補を、後から推測せずQuality Integrationへ渡す。
...
| Source ID | Source Layer | 成功の意味／観測条件 | 検証義務 | 統合先の検証目標 | 試験段階 | 試験種別 | 処置状態 |
|---|---|---|---|---|---|---|---|
| `<Canonical IDまたはDetail IDへのLink>` | `Definition／Detail` | `<成立したと判断できる結果、または具体的観測条件>` | `<防ぐ失敗と、何を確認する必要があるか>` | `<QA-IDのQuality定義へのLinkまたは候補名>` | `<UT／IT／ST／UAT>` | `<Functional／State／Fault等>` | `Mapped／Gap／N/A: 理由` |
...
| Source ID | Source Layer | Obligation Key | 導出元／Relation | 検証目標 | 保持する固有条件 | 試験段階 | 外部境界の段階 | 対応Local Item |
|---|---|---|---|---|---|---|---|---|
| `<Canonical IDまたはDetail IDへのLink>` | `Definition／Detail` | `<局所参照>` | `<Definitionの節、Detail Relation、Architecture Model Itemまたは詳細設計の導出キー>` | `<QA-IDのQuality定義へのLink>` | `<Source固有の成立条件。共通目標名へ置換しない>` | `<UT／IT／ST／UAT>` | `<N/A／Direct Boundary／Adjacent 1 Block／Related 2 Blocks／System/E2E／User Acceptance>` | `<各試験段階を満たすDefinition内Local ID>` |
