...
監査、レビューまたはCHGから人間の判断が必要になった場合、親エージェントは指摘事項をそのまま質問として転送しない。是正・再レビュー後の現在の固定改訂版から判断要求を再計算し、解消済み事項、AIが一意に修正できる事項、報告のみの事項を除く。同じ決定権限者、判断時点を持ち、採否を分離できない事項だけを一つの判断単位へまとめ、独立して保留できる事項は分ける。判断が残らなければ「現在、人間による判断は必要ありません」と明示する。人間には、今回分かったこと、対応結果、実際の影響、現在も決めること、推奨、なぜ今必要か、利用者・業務・プロダクト・計画・費用・リスクへの変化、主な短所、保留または不採用時に残る問題を先に示す。指摘事項ID、対象ファイル、工程、監査等の実行詳細は追跡可能に保ち、必要時に示す。重大リスク、不可逆性、強い不確実性、人間が受容する残存リスク、決定権限の競合は初期表示から隠さない。詳細は`00_CRDD/11_Skill.md`の判断支援契約と`00_CRDD/10_Agent.md`の監査統合規則に従う。
