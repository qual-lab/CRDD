...
Repository全体の現在地、重要Risk、判断待ち、現在の理由または次候補を把握する場合は、最初に`PROJECT_CONTEXT.md`を読む。個別作業では同ファイルだけで完結せず、記載されたOwner Relationと対象の正本成果物を確認する。Project ContextとOwner Artifactが競合する場合はProject Contextを現在値として使わず、Owner Artifactを優先して再投影する。
...
項目は、文書が宣言する目的、対象、決定権限および成果物種別を所有する正本へ置く。別の正本を利用する文書には必要最小限の要約と参照だけを置き、概念、設計、手順、実装、検証または履歴を再定義して第二の正本にしない。配置の判断は`00_CRDD/03_Documentation.md`の成果物とコンテキストに従う。
...
Discoveryで深掘り、再Discoveryまたは認識合わせを行い、AIが既存Contextまたは人間からの事前Inputから課題、必要性、望ましい成果または重要な対象外を再構成する場合は、`00_CRDD/21_Discovery.md`の人間理解の確認に従う。Repositoryから回答できることを人間の問題認識まで理解した根拠にせず、AIの事前理解、事実と推測の境界および暗黙の前提を必要最小限で人間へ返し、修正後の理解を再提示する。人間が抽象的な問題や要求を言語化できることを前提にせず、具体的な出来事、作業、迷い、回避策、反例または選択肢の比較から問題仮説を作り、訂正可能な形で返す。沈黙や回答不能を同意または問題不存在へ読み替えない。現在案を変え得る有力な代替または反証がある場合は、解ける問題、残る問題、費用および失う意図を示して突き合わせる。有力な代替がない場合は理由を示す。理解確認と要求・方針の採用判断を分け、一意な構造化や意味を変えない更新へ質問を追加しない。
...
複数の下位要素から構成され、部分成立を全体完成へ誤認し得る上位のInterface、Capabilityまたは実行境界では、`00_CRDD/27_Architecture.md`が定める必要な保証の一覧、実装接続、観測および利用側がすべて成立した場合だけ完成を表示する。一部の操作、Adapter、単体試験または安全な拒否から上位全体の成立を推定せず、部分接続、本番未接続および未評価を区別する。複数の結果fieldが共同で結果または利用側処置を決める場合は相関不変条件と完成主張を破る反証例を確認する。一意な局所変換、単一結果または独立した説明fieldへ過剰適用せず、対象や必要な保証を特定できなければ上位完成を主張しない。
...
- Architectureの正式な意味入力はCanonicalなUI定義とSPEC定義とし、両系列を別々に全数分析してから責務単位のArchitecture定義へ統合する。適用対象として評価済みのUI／SPEC Detailは、Definitionの意味を実現する配置・操作・状態・観測の具体的制約として併せて引き渡し、`SCR／PRT／Interaction／BHV`とのRelation、Coverageおよび未解決GapをArchitecture側で保持する。Detailを要求源またはDefinition不足の補完に使わない。REQ、UX、IA、現行Architectureまたは実装を正式入力の不足補完に使わず、不足時はUI／SPEC工程へ戻す。現行Architectureと実装は、成立済み能力、移行対象および未接続範囲の照合にだけ使う。
- `07_Quality`には`01_Quality_Center.md`、`02_Quality_Strategy.md`、`03_Verification_Design.md`、`04_Quality_Integration.md`、`05_Current_Implementation_Reality_Audit.md`、`Analysis/`および`Definitions/`を置く。各工程はDefinition作成と並行して`Analysis/REQ|UX|IA|UI|SPEC|ARCH/quality_analysis.md`の該当Canonical IDを全件処置し、成功の意味、失敗またはRisk、検証義務およびQA候補を説明できる状態を工程Readyに含める。UI／SPEC Detailが適用される場合は、Definition由来の成立条件とDetail由来の具体的観測条件を`Source Layer`で区別し、Relationを保持して処置する。個別処置をID別文書の作成と同一視しない。`04_Quality_Integration.md`は工程別義務を`Same／New／Merge`し、独立した品質の意味、成功・失敗境界および検証戦略を持つ`QA-*`へ統合する。`Definitions/QA-*/quality_definition.md`は`Source ID → Source Layer → QA-ID → 試験段階 → Local Item`と`Architecture詳細設計領域 → QA-ID`を保持し、工程別Analysisと導出集合を完全一致させる。各DefinitionはUT／IT／ST／UATとRT／PT／LTを省略せず`Required`、`Conditional`または理由付き`N/A`で判断する。外部境界は直接境界、隣接1 block、関連2 blocks、System／E2E、利用者受入へ段階的に設計する。PT／LTは設計・実装済みでも、人間が対象、環境、時間・反復、費用／Credit、Provider呼出し、中止条件およびcleanupを明示しない限り実行しない。未指示の非実行だけで通常監査を停止せず、明示した受入条件またはRelease Gateに必要な場合は未確認として扱う。`05_Current_Implementation_Reality_Audit.md`はCanonical設計固定後に現行Source、Test、Evidenceを照合する。機械可読なCatalogやTraceabilityがTool利用に必要な場合だけ`Registry/`へ置き、人間可読な設計正本や実行Evidenceとして扱わない。
...
1. 受け手、訴求、獲得経路、媒体、広告または市場反応を新設・変更する。
2. 認知、評価、試行、導入、継続または支持の形成を目的とする。

両方が成立する場合は、Communicationの表現設計だけで閉じず、`00_CRDD/21_Discovery.md`の利用者、顕在化契機、既存代替、回避策、市場・競争、採用行動および判断を変える根拠のレンズを併用する。Discoveryの対象セグメント・採用行動・商業性と、Communicationが所有する受け手・メッセージ・媒体・成果物・導線・測定仮説を分ける。

いずれか一方でも成立しないと確認できる場合は、もう一方が不明でも追加探索を発火しない。成立しない条件がなく、少なくとも一方が不明な場合だけ、不足情報と確認先を示して確認待ちとし、未承認の外部行為を停止する。

認知意図を適用する場合は、根拠付き仮説、必要な根拠／情報、現在有効な意図、履歴およびAIへ投影した対象改訂版を追跡する。誤字、一意な変換または記録更新へ認知状態を作らず、情報不足をAIで補完しない。職種・媒体属性と採用上の役割、観測値・代理指標・推定属性・本人申告を分ける。人数、固定ファネルまたは表示数だけで根拠の十分性を判定しない。外部行動を促す場合は提供能力と停止・代替導線へ接続し、反応から原因を直接確定しない。

追加市場探索の発火にかかわらず、人間対象調査を行う場合は、`00_CRDD/21_Discovery.md`の対象者の適格条件、選定・募集、偏り、対象外、一般化限界、継続・終了条件を確認する。同意、拒否、回答圧力、情報最小化、再識別可能性および不利益防止も確認する。説得、広告、推薦、個別化または再接触を行う場合は、`00_CRDD/17_Communication.md`の自律性、重要条件、対象別リスク、表示の識別可能性、選択・拒否・訂正および不利益を確認する。扱わないRepositoryへ空フォルダまたは空成果物を要求しない。
