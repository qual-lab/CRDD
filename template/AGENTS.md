...
基準版更新に含まれる差分のいずれかが規範もしくは破壊的に分類される場合、またはいずれかのリリースのCHANGELOGが移行を必要と明示する場合は、採用処置が「対応なしで採用」であっても、`00_CRDD/19_Maintenance.md`の移行完了の条件（既存資産の棚卸し、全資産の処遇決定、移行先の追跡、着手待ち資産ゼロ、独立レビューによる意味の欠損確認）を満たすまで採用を完了とみなさない。5点目の独立レビューは文書監査または不足／影響監査で実施し、準拠監査では実施しない。満たさないまま有効化する場合は、未完了事項、対象範囲、既知リスク、担当責任者、再評価契機、復旧条件を人間の決定権限者が明示する部分的な有効化として扱い、その基準版への準拠表明も記録しない。部分的な有効化の未完了事項は、その後の基準版更新へ引き継ぐ。後続の更新が移行完了の条件の対象外であっても、後続の更新について5点を満たしても、引き継いだ未完了事項が残る間は対象範囲全体について同条件を満たしたとみなさない。対象差分の変更分類を確定できないことを、同条件の対象外とする理由にしない。未確定の場合の扱い（対象として扱うか、人間の決定権限者が再評価契機を含む理由を確認して暫定的に対象外とするか）は`00_CRDD/19_Maintenance.md`の移行完了の条件を正本とする。
...
- `01_Discovery`は新しい根拠、不確実性、要求の入口とする。未採用で今は分析しない入力は、必要な場合だけ同工程内で候補として保持できるが、保持だけから採用、優先順位、実行許可を推定しない。`99_Roadmap`は採用済みだが未着手の要求やコンテキストを参照するプロジェクト固有の計画表示とし、ロードマップ項目へCRDD安定コンテキストIDを付与しない。候補保持、ロードマップの登録、主要表示／詳細、再評価、着手、完了、整理は`00_CRDD/21_Discovery.md`を正本とする。
