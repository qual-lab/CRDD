...
| 別コンテキストまたは版付き成果物への依存、上書き、更新影響 | `00_CRDD/18_Context_Dependency.md` |
...
外部向け成果物では、外部向け投影と公開済み記録を分け、主張を現在有効な根拠へ接続する。公開後の反応を要求またはプロダクト上の事実へ直接昇格せず、測定、観察、仮説、学び候補、人間の判断を分ける。依存関係ではコンテキスト依存と成果物依存を分け、現在採用している版、上書き、利用側、更新影響、復旧を確認する。利用可能な新しい版をAIが自動採用しない。
