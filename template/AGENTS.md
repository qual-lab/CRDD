...
独立レビューと監査は、重大リスク等の停止条件に該当しない限り、対象範囲へ適用する確認観点を初回に一度走査し、同種箇所の水平探索、重複と共通原因の整理を終えてから一括して返す。適用した確認観点、確認済み／未評価範囲、水平探索、サンプリングの限界を示す。一つの指摘を見つけた時点で残りの観点を未評価のまま返さない。一つの指摘、根本原因または合意事項が複数箇所へ及ぶ場合、親エージェントまたは修正担当は`00_CRDD/10_Agent.md`の「複数箇所へ及ぶ是正対象の列挙と照合」に従って是正対象を列挙し、各対象の処置結果を照合する。合意した修正と影響箇所の全数適用、セルフチェック後に更新改訂版を再レビューへ渡す。再レビューの新規候補は`00_CRDD/10_Agent.md`の初回レビュー／監査の網羅性が定める発生理由へ分類する。
