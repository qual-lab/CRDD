...
- `07_Quality`には`01_Quality_Center.md`、`02_Quality_Strategy.md`、`03_Verification_Design.md`、`Analysis/`、`Definitions/`を置く。`Analysis/`は`REQ-*`、`UX-*`、`IA-*`、`UI-*`、`SPEC-*`および`ARCH-*`のCanonical集合を正式入力とし、全IDをMapping上で一行以上処置する。個別処置をID別文書の作成と同一視せず、意味の近い検証義務を`Same／New／Merge`で検証目標へ統合する。`Source ID → 検証目標 → Definition内Local Item`と`Architecture詳細設計領域 → 検証目標`を明示し、MappingとDefinitionから導出したRelation集合、Source固有条件および`検証目標 → Local Item`の組を完全一致させる。目標名だけ、代表SourceだけまたはLocal Itemの全体集合だけでは全件処置としない。`Definitions/`は検証目標と、どう確認するかを所有する。検証項目は`Local ID`、分類、事前状態／入力、操作／刺激、観測と期待結果、終了後条件、実行形態の7軸を空欄なく示す。実行形態は`Automated`、`Manual`、`Hybrid`のいずれかとし、試験段階や担当観点を混在させない。試験段階を上流工程へ固定対応させず、検証義務と観測境界から判断する。上位3文書は入口・横断方針・統合投影とし、個別設計を再定義しない。個別の検証結果は直接証明するChangeまたはReleaseの`Evidence/`へ置き、検証義務と根拠を`07_Quality`へ複製しない。
