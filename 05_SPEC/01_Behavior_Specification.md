...
公開版は、署名manifestのrevision 4に`expiresAt: null`を明示することで、有効期限だけを理由とする起動停止を行わない。現行Runtimeはrevision 4だけを受理し、revision 2／3をfallbackまたは互換入力として扱わない。署名・配布Identity・実行権限を省略する設定ではなく、旧配布物の署名改変や期限の無断延長は拒否する。初期同意、操作の許可、候補、準備記録の期限は別契約のまま維持する。期限なしは永久サポートや将来の互換性保証ではない。
