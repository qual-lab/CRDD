...
| SPEC定義 | 26 | 独立して条件・状態・結果を変更／検証できる振る舞い単位へ統合する |
...
| 記録・状態変更 | 000002、000012、000013、000015、000016、000024、000027 | 必要なAuthorityの下で、発行した変更と耐久確定を区別する |
| 外部Effect・結果帰還 | 000005、000017、000021、000026 | 同意、送信、結果帰還、再接続、終了後確認を各SPECの適用範囲で分ける |
...
31件のUX定義と21件のIA定義を別々に分析し、26件の振る舞い仕様へ統合した。現行実装との比較は[現行振る舞い参照](07_Current_Behavior_Reference.md)で行い、分析の不足を現行コードから補完していない。独立レビューとUI／SPEC対応レビューはCritical 0／Major 0／Moderate 0／Minor 0でPassし、27件のUI／SPEC関係を双方から確認した。
