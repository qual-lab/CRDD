...
| 項目 | 対象 |
|---|---|
| 対象改訂版 | UI／SPEC Definition集合 SHA-256: `bca626aed42f4feb2795efb7eda4451909e721331d743300f92fb5fc35079600` |
| 対象関係 | 31組 |
...
| 観点 | UI側の根拠 | SPEC側の根拠 | 判定 | 理由 |
|---|---|---|---|---|
| State | [UI-000002](../04_UI/Definitions/UI-000002/ui_definition.md#状態と表示差) | [SPEC-000002](Definitions/SPEC-000002/spec_definition.md#振る舞い状態結果) | 一致 | UI事実（UI-000002）「委任提案中／受付可能／受付済み／拒否／範囲見直し／実行中／停止。権限発行前後とTask発行前後を分ける」／SPEC事実（SPEC-000002）「目的、範囲、担い手、決定権限、節目を検証し、実行可能な依頼だけを受理する。」／対応: UIの委任提案中はSPECの提案、受付可能は受理可能、受付済みはTask作成済み、拒否は拒否・Task未発行へ対応する。範囲見直しは同じ提案へ戻るUI側の導線であり、実行開始後の状態と分ける。 |
...
| Result | [UI-000002](../04_UI/Definitions/UI-000002/ui_definition.md#操作とfeedback) | [SPEC-000002](Definitions/SPEC-000002/spec_definition.md#振る舞い状態結果) | 一致 | UI事実（UI-000002）「提案中、受付可能、受付済み、拒否、範囲見直しを分け、拒否理由と同じ提案への戻り先を示す」／SPEC事実（SPEC-000002）「受理結果から実行対象・未委任判断・完了条件を一意に確認できる。」／対応: 受理時は発行Taskと未委任判断を返し、拒否時はTask未発行をUIで確認できるようにする。 |
...
| Recovery | [UI-000002](../04_UI/Definitions/UI-000002/ui_definition.md#状態と表示差) | [SPEC-000002](Definitions/SPEC-000002/spec_definition.md#失敗回復副作用) | 一致 | UI事実（UI-000002）「拒否時は同じ提案の範囲見直しへ戻る」／SPEC事実（SPEC-000002）「受理前の拒否またはblockedは提案へ戻せる。」／対応: 受理前の拒否は同じ提案の修正へ、受理後の観測不能は同じ依頼の再観測へ戻す。 |
...
| Recovery | [UI-000016](../04_UI/Definitions/UI-000016/ui_definition.md#状態と表示差) | [SPEC-000026](Definitions/SPEC-000026/spec_definition.md#失敗回復副作用) | 一致 | UI事実（UI-000016）「同じ候補と出所へ戻り、所有正本を変更しない」／SPEC事実（SPEC-000026）「本SPEC固有の回復経路は設けず、失敗理由と安全な戻り先を返す。」／対応: SPECの安全な戻り先をUIの同じ候補と出所へ接続し、送信入口へ巻き戻したりProvider Effectを再発行したりしない。 |
...
| 観点 | UI側の根拠 | SPEC側の根拠 | 判定 | 理由 |
|---|---|---|---|---|
| State | [UI-000016](../04_UI/Definitions/UI-000016/ui_definition.md#状態と表示差) | [SPEC-000027](Definitions/SPEC-000027/spec_definition.md#振る舞い状態結果) | 一致 | UI事実（UI-000016）「採用（adopted）／却下（rejected）／保留（held）」／SPEC事実（SPEC-000027）「候補の出所、対象、判断主体、現在改訂版を確認し、採用時だけ所有正本へ反映する。／採否と反映先を辿れ、却下・保留では所有正本を変更しない。」／対応: 返却済みと候補はPromotion前、採用・却下・保留は判断結果である。SPECどおり採用時だけ所有正本へ反映し、却下・保留では変更しない。 |
...
| Result | [UI-000016](../04_UI/Definitions/UI-000016/ui_definition.md#操作とfeedback) | [SPEC-000027](Definitions/SPEC-000027/spec_definition.md#振る舞い状態結果) | 一致 | UI事実（UI-000016）「投影、採用、却下および保留を分離する」／SPEC事実（SPEC-000027）「採否と反映先を辿れ、却下・保留では所有正本を変更しない。」／対応: SPECの採否と反映先をUIの候補、判断、対象正本へ返す。採用・却下・保留の理由は元候補に結合し、反映先と区別する。 |
