# BHV-000022 過去情報と現在有効な意図を区別して解決する

成果物種別: SPEC Detail
Behavior ID: `BHV-000022`
状態: Canonical
維持責任者: Qual-Lab

## 1. 目的とSource Definition

- 目的: 過去情報と現在有効な意図を区別して解決する。
- Source SPEC: [SPEC-000022](../../Definitions/SPEC-000022/spec_definition.md)
- 対象利用側: UI-000017
...
| 観点 | 判定 | 契約／理由 |
|---|---|---|
| Trigger | Applicable | 過去の推論情報や判断を現在の作業で参照する時 |
| Precondition | Applicable | 情報の出所、発生時点、対象改訂版、置換関係を確認できる |
| Authority | Applicable | 履歴を閲覧する主体。参照から現在方針の採用Authorityを推定しない |
| Input | Applicable | 過去情報と現在有効な意図を区別して解決するに必要な対象Identity、入力値、出所および観測時点 |
| Validation | Applicable | 前提条件、Authority、対象Identity、入力完全性および現在性をEffect前に確認する |
| State／Transition | Applicable | 発生時点、対象改訂版、出所、現在有効性、置換関係を評価して参照結果を返す。 |
| Sequence | Applicable | Trigger→Precondition／Authority／Validation→State／Effect→Resultの順を保つ |
| Effect | Applicable | 読取り解決だけを行い、Gitで再現可能な状態を重複保存・変更しない。 |
| Output | Applicable | Gitで再現できる状態を重複永続化せず、現在値と履歴を区別する。 |
| Failure | Applicable | 古い判断を現在方針へ自動昇格せず、Commit SHAだけを成立条件にしない。 |
| Recovery | Applicable | 失敗理由と安全な戻り先を返す。 |

## 3. Behavior Flow

```text
過去の推論情報や判断を現在の作業で参照する時
  ↓
情報の出所、発生時点、対象改訂版、置換関係を確認できる
  ↓
Authority・対象Identity・入力を検証
  ├─ 不足／不一致 → Effect 0で理由と戻り先
  └─ 成立
       ↓
発生時点、対象改訂版、出所、現在有効性、置換関係を評価して参照結果を返す。
       ↓
...
|---|---|---|---|
| [SCR-000017／PRT-000017.spec-000022](../../../04_UI/Details/Areas/project-context/SCR-000017/screen.md) | 過去情報と現在有効な意図を区別して解決するの操作・状態・結果を利用者へ示す | Success／Reject／Failure／Unknown／Recovery | Covered |

## 5. Verification Intent

| Condition | 観測可能な成立／不成立 | Qualityへの引き渡し |
|---|---|---|
| Normal | Gitで再現できる状態を重複永続化せず、現在値と履歴を区別する。 | Source SPECと同じ正常義務へ統合 |
| Boundary | 対象、Authority、現在性または入力の境界を越えず、対象外へEffectを発行しない | 境界条件を独立観測する |
| Failure | 古い判断を現在方針へ自動昇格せず、Commit SHAだけを成立条件にしない。 | 失敗を成功・未実行・不存在へ畳まない |
...
- Architectureで決める事項: ARCH-000016がComponent、Boundary、Data／State Owner、Failure Boundaryおよび実行環境への配置を所有する。
