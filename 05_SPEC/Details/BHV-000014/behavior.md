# BHV-000014 Repositoryに適合する標準Toolを解決する

成果物種別: SPEC Detail
Behavior ID: `BHV-000014`
状態: Canonical
維持責任者: Qual-Lab

## 1. 目的とSource Definition

- 目的: Repositoryに適合する標準Toolを解決する。
- Source SPEC: [SPEC-000014](../../Definitions/SPEC-000014/spec_definition.md)
- 対象利用側: UI-000010
...
| 観点 | 判定 | 契約／理由 |
|---|---|---|
| Trigger | Applicable | 目的に合う標準Toolを選ぶ時 |
| Precondition | Applicable | Repositoryの固定改訂版、登録済み能力、配布根拠を読める |
| Authority | Applicable | Tool能力一覧を閲覧する主体。一覧取得はTool実行Authorityを発行しない |
| Input | Applicable | Repositoryに適合する標準Toolを解決するに必要な対象Identity、入力値、出所および観測時点 |
| Validation | Applicable | 前提条件、Authority、対象Identity、入力完全性および現在性をEffect前に確認する |
| State／Transition | Applicable | 登録能力、利用可否、固定改訂版、配布根拠、必要Authorityを照合して利用候補を返す。 |
| Sequence | Applicable | Trigger→Precondition／Authority／Validation→State／Effect→Resultの順を保つ |
| Effect | Applicable | 読取り専用で候補を返し、Toolまたは配布物を実行・変更しない。 |
| Output | Applicable | 未登録能力を推測せず、版不一致や欠落を明示する。 |
| Failure | Applicable | Tool一覧の閲覧だけで実行Authorityを発行しない。 |
...
目的に合う標準Toolを選ぶ時
  ↓
Repositoryの固定改訂版、登録済み能力、配布根拠を読める
  ↓
Authority・対象Identity・入力を検証
  ├─ 不足／不一致 → Effect 0で理由と戻り先
  └─ 成立
       ↓
登録能力、利用可否、固定改訂版、配布根拠、必要Authorityを照合して利用候補を返す。
       ↓
...
|---|---|---|---|
| [SCR-000010／PRT-000010.spec-000014](../../../04_UI/Details/Areas/configuration-trust/SCR-000010/screen.md) | Repositoryに適合する標準Toolを解決するの操作・状態・結果を利用者へ示す | Success／Reject／Failure／Unknown／Recovery | Covered |

## 5. Verification Intent

| Condition | 観測可能な成立／不成立 | Qualityへの引き渡し |
|---|---|---|
| Normal | 未登録能力を推測せず、版不一致や欠落を明示する。 | Source SPECと同じ正常義務へ統合 |
| Boundary | 対象、Authority、現在性または入力の境界を越えず、対象外へEffectを発行しない | 境界条件を独立観測する |
| Failure | Tool一覧の閲覧だけで実行Authorityを発行しない。 | 失敗を成功・未実行・不存在へ畳まない |
