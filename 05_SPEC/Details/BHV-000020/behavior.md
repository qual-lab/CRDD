# BHV-000020 変更・監査・試験・品質の閉包を評価する

成果物種別: SPEC Detail
Behavior ID: `BHV-000020`
状態: Canonical
維持責任者: Qual-Lab

## 1. 目的とSource Definition

- 目的: 変更・監査・試験・品質の閉包を評価する。
- Source SPEC: [SPEC-000020](../../Definitions/SPEC-000020/spec_definition.md)
- 対象利用側: UI-000015
...
| 観点 | 判定 | 契約／理由 |
|---|---|---|
| Trigger | Applicable | 変更候補を工程移行または公開判断へ進める時 |
| Precondition | Applicable | 同じ変更改訂版、監査集合、是正、試験層、Evidenceを特定できる |
| Authority | Applicable | 各レビューは所管範囲を評価し、人間が工程移行・採用・Releaseを決める |
| Input | Applicable | 変更・監査・試験・品質の閉包を評価するに必要な対象Identity、入力値、出所および観測時点 |
| Validation | Applicable | 前提条件、Authority、対象Identity、入力完全性および現在性をEffect前に確認する |
| State／Transition | Applicable | 対象改訂版、指摘、是正、試験層、根拠、残るGateを同じ変更へ結合して現在状態を評価する。 |
| Sequence | Applicable | Trigger→Precondition／Authority／Validation→State／Effect→Resultの順を保つ |
| Effect | Applicable | 評価記録は更新できるが、Pass表示から統合・Release Effectを自動発行しない。 |
| Output | Applicable | 未実施・非該当・失敗・Passを区別し、人間判断が残る場合だけ提示する。 |
| Failure | Applicable | 試験件数や一部監査完了から全体Passを推定しない。 |
| Recovery | Applicable | 失敗理由と安全な戻り先を返す。 |

## 3. Behavior Flow

```text
変更候補を工程移行または公開判断へ進める時
  ↓
同じ変更改訂版、監査集合、是正、試験層、Evidenceを特定できる
  ↓
Authority・対象Identity・入力を検証
  ├─ 不足／不一致 → Effect 0で理由と戻り先
  └─ 成立
       ↓
対象改訂版、指摘、是正、試験層、根拠、残るGateを同じ変更へ結合して現在状態を評価する。
       ↓
...
|---|---|---|---|
| [SCR-000015／PRT-000015.spec-000020](../../../04_UI/Details/Areas/governance/SCR-000015/screen.md) | 変更・監査・試験・品質の閉包を評価するの操作・状態・結果を利用者へ示す | Success／Reject／Failure／Unknown／Recovery | Covered |

## 5. Verification Intent

| Condition | 観測可能な成立／不成立 | Qualityへの引き渡し |
|---|---|---|
| Normal | 未実施・非該当・失敗・Passを区別し、人間判断が残る場合だけ提示する。 | Source SPECと同じ正常義務へ統合 |
| Boundary | 対象、Authority、現在性または入力の境界を越えず、対象外へEffectを発行しない | 境界条件を独立観測する |
| Failure | 試験件数や一部監査完了から全体Passを推定しない。 | 失敗を成功・未実行・不存在へ畳まない |
...
- Architectureで決める事項: ARCH-000003がComponent、Boundary、Data／State Owner、Failure Boundaryおよび実行環境への配置を所有する。
