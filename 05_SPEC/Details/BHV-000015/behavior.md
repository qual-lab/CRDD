# BHV-000015 AIモデル構成を検証し実効選択を決める

成果物種別: SPEC Detail
Behavior ID: `BHV-000015`
状態: Canonical
維持責任者: Qual-Lab

## 1. 目的とSource Definition

- 目的: AIモデル構成を検証し実効選択を決める。
- Source SPEC: [SPEC-000015](../../Definitions/SPEC-000015/spec_definition.md)
- 対象利用側: UI-000010
...
| 観点 | 判定 | 契約／理由 |
|---|---|---|
| Trigger | Applicable | AIモデル構成を読込み、更新または選択する時 |
| Precondition | Applicable | 構成Candidate、許可値、利用可能性、用途、優先順位を確認できる |
| Authority | Applicable | 構成管理者が更新を採用し、Runtimeが検証済み構成から選択する |
| Input | Applicable | AIモデル構成を検証し実効選択を決めるに必要な対象Identity、入力値、出所および観測時点 |
| Validation | Applicable | 前提条件、Authority、対象Identity、入力完全性および現在性をEffect前に確認する |
| State／Transition | Applicable | 許可値、利用可能性、用途、費用・品質制約、優先順位を検証し、実効選択と再選定条件を返す。 |
| Sequence | Applicable | Trigger→Precondition／Authority／Validation→State／Effect→Resultの順を保つ |
| Effect | Applicable | 採用時だけ構成を保存する。選択はProvider実行Effectを発行しない。 |
| Output | Applicable | コード変更なしで構成を更新でき、選択理由と適用範囲を確認できる。 |
| Failure | Applicable | 未知モデルや不正構成を暗黙fallbackせず、構成変更を実行許可にしない。 |
| Recovery | Applicable | 失敗理由と安全な戻り先を返す。 |

## 3. Behavior Flow

```text
AIモデル構成を読込み、更新または選択する時
  ↓
構成Candidate、許可値、利用可能性、用途、優先順位を確認できる
  ↓
Authority・対象Identity・入力を検証
  ├─ 不足／不一致 → Effect 0で理由と戻り先
  └─ 成立
       ↓
許可値、利用可能性、用途、費用・品質制約、優先順位を検証し、実効選択と再選定条件を返す。
       ↓
...
|---|---|---|---|
| [SCR-000010／PRT-000010.spec-000015](../../../04_UI/Details/Areas/configuration-trust/SCR-000010/screen.md) | AIモデル構成を検証し実効選択を決めるの操作・状態・結果を利用者へ示す | Success／Reject／Failure／Unknown／Recovery | Covered |

## 5. Verification Intent

| Condition | 観測可能な成立／不成立 | Qualityへの引き渡し |
|---|---|---|
| Normal | コード変更なしで構成を更新でき、選択理由と適用範囲を確認できる。 | Source SPECと同じ正常義務へ統合 |
| Boundary | 対象、Authority、現在性または入力の境界を越えず、対象外へEffectを発行しない | 境界条件を独立観測する |
| Failure | 未知モデルや不正構成を暗黙fallbackせず、構成変更を実行許可にしない。 | 失敗を成功・未実行・不存在へ畳まない |
...
- Architectureで決める事項: ARCH-000010がComponent、Boundary、Data／State Owner、Failure Boundaryおよび実行環境への配置を所有する。
