# BHV-000012 接続資格からWorkspace利用範囲を確定する
...
| 観点 | 判定 | 契約／理由 |
|---|---|---|
| Trigger | Applicable | リモート接続を開始または再接続する時 |
| Precondition | Applicable | 接続資格を検証でき、WorkspaceとRepository Exposureが現行である |
| Authority | Applicable | Credential発行時に固定されたWorkspace Grant。管理Capabilityと内容Grantを分離する |
| Input | Applicable | 接続資格からWorkspace利用範囲を確定するに必要な対象Identity、入力値、出所および観測時点 |
| Validation | Applicable | 前提条件、Authority、対象Identity、入力完全性および現在性をEffect前に確認する |
| State／Transition | Applicable | 接続資格を検証し、現在有効なWorkspace GrantとRepository Exposureから利用可能範囲を確定する。 |
| Sequence | Applicable | Trigger→Precondition／Authority／Validation→State／Effect→Resultの順を保つ |
| Effect | Applicable | 認証済みSessionとGrantを作成・更新する。未Exposure Repositoryへ読取りEffect 0。 |
| Output | Applicable | System管理能力と内容閲覧権限を別に判定する。 |
| Failure | Applicable | 未許可対象の存在を漏らさず、古いGrantや一律Unlockを受理しない。 |
...
リモート接続を開始または再接続する時
  ↓
接続資格を検証でき、WorkspaceとRepository Exposureが現行である
  ↓
Authority・対象Identity・入力を検証
  ├─ 不足／不一致 → Effect 0で理由と戻り先
  └─ 成立
       ↓
接続資格を検証し、現在有効なWorkspace GrantとRepository Exposureから利用可能範囲を確定する。
       ↓
...
|---|---|---|
| Normal | System管理能力と内容閲覧権限を別に判定する。 | Source SPECと同じ正常義務へ統合 |
| Boundary | 対象、Authority、現在性または入力の境界を越えず、対象外へEffectを発行しない | 境界条件を独立観測する |
| Failure | 未許可対象の存在を漏らさず、古いGrantや一律Unlockを受理しない。 | 失敗を成功・未実行・不存在へ畳まない |
