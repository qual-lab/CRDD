...
| `08_Quality` | 品質戦略、検証設計、検証結果の履歴、現在品質状態の統合表示 | [品質保証](16_Quality_Assurance.md)、[検証](29_Verification.md) |
...
実装は、採用技術とアーキテクチャが定める情報源コード、構成、テスト等の通常配置を使用するため、CRDD共通の固定Markdown入口を追加しない。検証の計画、結果、現在状態への入口は`08_Quality`の固定構成を使用する。いずれも案件規模別のCRDD文書構成を新設せず、実装構造の変更は技術上の責務境界、生成・実行方法、レビュー容易性等の理由に基づいて扱う。

根拠の配置は6.2、判断の配置は7.1を正本とする。リポジトリ構造へ中央根拠フォルダまたは中央判断フォルダを追加しない。

`08_Quality`は品質情報を新しい中央正本へ集約する場所ではない。検証義務は各工程成果物、実装と開発者テストは`40_Develop`、根拠は6章の配置を正本とする。`08_Quality`は品質戦略、検証設計、確定済み検証結果と、それらから導く現在状態への固定入口を提供する。適用の深さによってファイル構成を変えず、記述、レビュー、根拠の深さを調整する。品質保証の現在状態、判断理由、未検証範囲、残存リスク、再現方法はリポジトリ内から理解できるようにし、外部ツールのリンクまたは実行IDだけへ依存しない。
...
CRDDが通常要求するのは責務網羅範囲であり、文書を増やすこと自体ではない。対象の正本規則が、探索入口、履歴と現在状態の分離、または外部正本への接続を安定させるために最小構成を明示している場合は、その構成に従う。現在の標準では、[基本構造](#31-basic-structure)が課題探索・要求形成からアーキテクチャまでの固定入口を、[品質保証](16_Quality_Assurance.md#42-fixed-quality-structure)が`08_Quality`の最小構成を定める。固定構成の存在だけを、責務の網羅、品質成立またはCRDD準拠の根拠にしてはならない。
