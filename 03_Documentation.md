...
未採用の入力を今すぐ分析しないが後から再検討したい場合は、[任意の候補保持](21_Discovery.md#24-optional-candidate-holding)として`01_Discovery`の責務内に置いてよい。候補保持は、専用フォルダ、候補ごとのファイル、固定状態または中央登録簿を要求しない。一つの文書内の一覧や外部ツールを使用できる。外部ツールを使う場合は、場所、対象、必要な改訂版または取得時点、アクセス方法、課題探索・要求形成へ戻す経路を辿れるようにする。
