...
Qual-Lab / CRDD公式リポジトリの非自明な変更は、`main`から作成した一つの作業用featureブランチ内で、正本、実装、試験、移行、変更トレース、配布内容および必要なRelease manifestまで完成させ、一つのプルリクエストで`main`へ統合する。実行環境の名前空間prefixは付けてよいが、`develop`、`release/*`その他の多段統合用ブランチを設けない。Release manifestが自己参照を避ける二Commit構成を必要とする場合も、Commit Aとmanifest-onlyのCommit Bを同じfeatureブランチ内で作成・検証し、Commit Bを先端とする一つのプルリクエストへ含める。`main`統合後にmanifest更新だけの別ブランチまたは別プルリクエストを通常手順として追加しない。統合後は確認済み`main`のCommit Bへ公式tagを付ける。この運用プロファイルはCRDD公式リポジトリだけに適用し、採用リポジトリへ同じブランチ名、Commit数またはプルリクエスト構成を要求しない。
