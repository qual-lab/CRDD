...
| AD-21 | 独立レビューと監査は、停止または上位判断への移送が必要な場合を除き、対象範囲へ適用される確認観点を初回に一度走査し、指摘候補の水平探索、重複と共通原因の整理を終えてから一括して返す。確認範囲、未評価範囲、水平探索、サンプリングの限界を取得可能にする。再レビューで見つかった新規候補は、初回レビュー／監査の網羅性が定める4つの発生理由へ分類する。修正担当は、一つの指摘または原因が複数箇所へ及ぶ場合に是正対象を列挙して各処置を照合し、合意した修正の全数適用とセルフチェック後に再レビューへ渡す | [エージェント](10_Agent.md#multi-location-remediation)、[エージェント](10_Agent.md#76-first-pass-review-completeness)、[文書監査](51_Document_Audit.md#3-output) | 適用した確認観点、確認済み／未評価範囲、水平探索、サンプリング根拠と限界、初回の候補整理、是正対象の母集団根拠と対象別処置、修正と確認の全数照合、再レビューの新規指摘分類 |
