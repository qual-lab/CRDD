...
| PL-18 | 別のコンテキストまたは版付き成果物へ依存する対象範囲では、コンテキスト依存と成果物依存を区別し、依存先、参照版、適用範囲、ローカルな上書き、競合、利用側、更新・延期・復旧、現在有効な状態を追跡する。Repository、Submodule、Package等の物理方式から意味上の依存または決定権限を推定せず、依存先の結果をローカルな利用側の成立へ無条件に流用しない。対象範囲に依存関係が存在しない場合は、理由と対象範囲を伴う`Not Applicable`として評価でき、専用Manifest、IDまたは依存成果物を要求しない | [コンテキスト依存](18_Context_Dependency.md)、[品質保証](16_Quality_Assurance.md)、[不足／影響監査](53_Gap_Impact_Audit.md) | 依存種別、依存先、参照版、対象範囲、上書き・非適用・競合、利用側、差分と影響、検証結果、更新判断、移行・復旧・延期時リスク、現在有効な状態、適用しない場合の理由と対象範囲 |
