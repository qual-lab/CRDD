...
| C-11 | 内部コンテキストを外部検索、外部AI、API、MCP、生成サービスその他の外部接続へ直接送信せず、情報分類、派生・組合せ情報、削除・抽象化・最小化した外部向け調査コンテキスト、送信先、最小権限、外部入力の命令分離、供給網、セキュリティ不変条件、拒否・監査・失効・回復を対象リスクに応じて接続する。情報分類または安全な抽象化を確認できない場合は外部送信を行わず、人間の決定権限へ戻す。外部内容を信頼済み命令または権限へ昇格せず、AIがセキュリティリスクを自己受容しない | [外部情報境界](01_Principles.md#external-information-boundary)、[エージェント](10_Agent.md#agent-operation-boundary)、[スキル](11_Skill.md#25-external-research-and-tool-execution)、[品質保証](16_Quality_Assurance.md)、[アーキテクチャ](27_Architecture.md)、[実装](28_Implementation.md)、[検証](29_Verification.md) | 情報分類と継承、派生・組合せ情報の扱い、外部送信の目的・送信先・実送信内容、削除・抽象化・最小化、再識別可能性、残存開示リスク、人間判断、外部入力と命令の分離、エージェント・ツールの権限、供給網の情報源・版・権限・ネットワーク・ライセンス・既知リスク、セキュリティ不変条件、実行時強制、拒否・監査、シークレット隔離、事故時の検出・封じ込め・失効・回復、合成データによる境界検証 |
