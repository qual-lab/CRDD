...
| PL-18 | コンテキスト依存、共有境界をまたぐ成果物依存、または重大な品質・セキュリティ・プライバシー・法務・ライセンス・互換性・復旧リスクにより明示管理する成果物依存では、依存先、参照版、適用範囲、ローカルな上書き、競合、利用側、更新・延期・復旧、現在有効な状態を追跡する。Repository、Submodule、パッケージ等の物理方式から意味上の依存または決定権限を推定せず、依存先の結果をローカルな利用側の成立へ無条件に流用しない。通常または推移的な実装依存は既存のアーキテクチャ、パッケージ管理、lockfile、SBOM等を正本にでき、存在だけで本基準の完全契約、個別CHGまたは都度の人間判断を発火させない。管理対象依存が存在しない場合は、理由と対象範囲、および通常依存を管理する正本を伴う`Not Applicable`として評価でき、専用Manifest、IDまたは依存成果物を要求しない | [コンテキスト依存](18_Context_Dependency.md)、[品質保証](16_Quality_Assurance.md)、[不足／影響監査](53_Gap_Impact_Audit.md) | 管理対象への昇格判断、依存種別、依存先、参照版、対象範囲、上書き・非適用・競合、利用側、差分と影響、検証結果、更新判断、事前承認した自動更新の範囲・停止・復旧条件、移行・復旧・延期時リスク、現在有効な状態、適用しない場合の理由・対象範囲・通常依存の正本 |
