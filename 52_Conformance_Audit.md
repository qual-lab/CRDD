...
| PL-18 | コンテキスト依存、意味または契約の共有、決定権限、独立したリリース、独立管理される複数利用側、再利用の境界をまたぐ成果物依存、または重大な品質・セキュリティ・プライバシー・法務・ライセンス・互換性・復旧リスクにより明示管理する成果物依存では、依存先、参照版、適用範囲、ローカルな上書き、競合、利用側、更新・延期・復旧、現在有効な状態を追跡する。Repository、Submodule、パッケージ等の物理方式から意味上の依存または決定権限を推定せず、依存先の結果をローカルな利用側の成立へ無条件に流用しない。通常または推移的な実装依存は既存のアーキテクチャ、パッケージ管理、lockfile、SBOM等を正本にでき、存在だけで本基準の完全契約、個別CHGまたは都度の人間判断を発火させない。管理対象依存が存在しない場合は、理由と対象範囲を伴う`Not Applicable`として評価でき、通常依存が存在する場合だけその管理正本も示す。管理対象への該当性を判定できていない場合は、不足情報、確認担当、再評価条件を示して`Not Evaluated`とし、`Not Applicable`または完全契約済みと扱わない。専用Manifest、IDまたは依存成果物は要求しない | [コンテキスト依存](18_Context_Dependency.md)、[品質保証](16_Quality_Assurance.md)、[不足／影響監査](53_Gap_Impact_Audit.md) | 管理対象への昇格判断、依存種別、依存先、参照版、対象範囲、上書き・非適用・競合、利用側、差分と影響、検証結果、更新判断、事前承認した自動更新の範囲・停止・復旧条件、移行・復旧・延期時リスク、現在有効な状態、適用しない場合の理由・対象範囲、通常依存が存在する場合の管理正本、適用判定待ちの場合の不足情報・確認担当・再評価条件 |
