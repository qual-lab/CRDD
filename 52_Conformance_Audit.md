...
| AD-17 | 正本コンテキストの意味、対象範囲、責任、既定値、優先順位、リスク受容、下流契約を変える問いでは、エージェント / スキルや質問表示名を問わず判断支援契約を適用する。是正・再レビュー後の現在の対象改訂版から、解消済み、AIが一意に修正可能、報告のみの事項を除いて現在の判断集合を再構成し、監査またはレビューでは現在の固定改訂版を用いる。判断が残らなければ人間判断不要と明示し、残る判断は決定権限者、判断時点、独立して保留できるかによって統合または分離する。最初の表示では、今回分かったこと、対応結果、実際の影響、現在も決めること、推奨、必要性、利用者・業務・プロダクト・計画・費用・リスクへの変化、主な短所、不採用時に残る問題を示し、重大リスク、不可逆性、強い不確実性、残存リスク、決定権限の競合を詳細へ隠さない | [エージェント](10_Agent.md)、[スキル](11_Skill.md) | 現在の判断集合の導出元となる対象改訂版、監査・レビュー時の固定改訂版、除外した解消済み / AI修正可能 / 報告のみの事項、判断なし / 一件 / 複数の代表ケース、判断単位の統合・分離理由、判断支援の初期表示、推奨 / 代替案 / 判断理由 / 根拠 / 確信度 / 再確認条件、初期表示と実行詳細の境界 |
