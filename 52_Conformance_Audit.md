...
| AD-02 | 非自明な変更前に目的、対象内／対象外、依存関係、リスク、検証を明示し、変更分類、影響または再開する工程と共通責務、実行する独立レビューと監査、実行しない主な監査と理由、残る人間判断を人間が確認できる形で示す。調査によって経路が変わった場合は表示を更新し、表示した経路から承認を推定しない | [エージェント](10_Agent.md)、[スキル](11_Skill.md)、[概要の変更経路案内](00_Overview.md#44-change-route-selection) | AI作業計画または同等の実行契約、選択した経路と変更理由、人間の判断事項 |
