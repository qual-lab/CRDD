...
CRDD閲覧者 ──→ CRDD作成者・保守者 ──→ Runtime導入・運用者 ──→ 外部Contextの所有者
```
...
| CRDD閲覧者 | 公式入口や素材を見分けつつ、表示と検証可能な保証を区別して適切な入口を選ぶ | 見た目が公式であることを、署名・準拠・品質・Publisher Trustの証明と誤認したくない | CRDDの文書、Tool、公開案内、視覚素材を初めてまたは再度確認する場面 | 使う入口と、信頼判断に必要な根拠を選ぶ | REQ-000035のDiscovery Definitionから導出。識別表示が入口発見を助けつつTrust誤認を増やさないかは未実測 |
