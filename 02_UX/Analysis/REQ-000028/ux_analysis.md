...
| Outcome | 入口固有差だけを理解し、同じ基準で仕事を続けられる |
| REQ固有の差 | AI固有入口を案内と固有制約に限定し、共通規範を複製しないことが成立条件になる |
...
参照する共通規範・工程正本・AI固有制約を確認する
        │
        ├─ ★ Critical: 入口が適用規則と正本を選ぶ場面
        ├─ ⚠ Failure:  入口文書が第二正本になり同じ作業の判断が分岐する
        └─ ✓ Quality:  同じ正本・適用規則・判断境界と差分理由を追跡できる
        ↓
入口固有差だけを理解し、同じ基準で仕事を続けられる
```
...
```text
[U: 異なるAI入口を利用するDeveloper]
        │ 利用者行動: 入口が違っても同じCRDD正本と判断境界を使う
        ▼
[T: Codex、Claude、Copilot等から作業入口を選ぶ時]
        │
        ├─ 時間差: 入口は別々に更新されても共通正本は一つの現在値を持つ
        ├─ 完了時: 参照正本、適用規則、判断境界、AI固有制約、差分理由
        └─ 失敗時: 正本へ到達不能または入口間判断差をDeveloperへ返す
                     │
                     ▼
             [R: 異なるAI入口を利用するDeveloper]
                     │ 返却された事実と判断不能範囲を確認
...
|---|---|---|
| Developer | 仕事に合うAI入口を選び、提示された正本と固有制約を確認する | 入口固有の説明を共通規範そのものとみなさない |
| AI固有入口 | 共通正本へ案内し固有制約を明示する | 共通規範や工程判断を複製・再定義しない |
| 共通CRDD正本 | 入口に依存しない規範と工程判断を所有する | AI固有文法や制約まで一形式へ強制しない |

### 補足する品質

- 作業を開始する時: 入口文書を第二の正本にしない。（避ける失敗: 入口ごとに複製された規則が食い違い、同じRepositoryで異なる判断になる）
