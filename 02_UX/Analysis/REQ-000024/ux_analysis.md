...
分析対象: [REQ-000024 境界を越えるTask結果の帰還](../../../01_Discovery/Definitions/REQ-000024/requirement.md)
...
```text
[U: 仕事を委ねた人（Project Operator／PM）]
        │ 利用者行動: 境界を越えた結果を同じTaskへ受け取る
        ▼
[T: AgentやRuntimeから結果が戻る時]
        │
        ├─ 時間差: 外部実行完了と結果帰還は別時点で成立し、搬送だけが途切れ得る
        ├─ 完了時: Task Identity、Result、Evidence、未確認範囲、帰還状態
        └─ 失敗時: 不完全または相関不能な帰還結果を仕事を委ねた人へ返す
                     │
                     ▼
             [R: Project Operator／正本Owner（結果採否の判断役割）]
                     │ 返却された事実と判断不能範囲を確認
                     └─ 次の行動: 同じTaskの結果再取得、追加確認、回復または採否判断を選ぶ
