...
分析対象: [REQ-000011 Remote接続のWorkspace限定](../../../01_Discovery/Definitions/REQ-000011/requirement.md)
...
[U: Project Operator／PM]
        ...
        ▼
[T: Remote Sessionを開始・再接続する時]
        │
        ├─ 時間差: Credential発行とSession接続は別時点で行われ、Grant変更は次の認証へ反映される
        ├─ 完了時: 現在SessionのWorkspace Grantと利用不能理由
        └─ 失敗時: 不足・失効したGrantをContentを開示せずCROS管理者へ返す
                     │
                     ▼
             [R: CROS管理者]
                     │ 返却された事実と判断不能範囲を確認
                     └─ 次の行動: 別Credentialを使うか、Grant変更を管理者へ依頼する

---------------- 可視境界 ----------------
                     │ 時間関係: Credential発行とSession接続は別時点で行われ、Grant変更は次の認証へ反映される
                     ▼
[S: 提供System]
