...
分析対象: [REQ-000021 Remote要求結果の同一Identity再取得](../../../01_Discovery/Definitions/REQ-000021/requirement.md)
...
[T: 応答喪失後に再接続する時]
        │
        ├─ 時間差: 応答喪失から再接続まで時間差があり、元Requestは継続し得る
        ├─ 完了時: 同じRequestの状態、結果、Effect状態、回復義務
        └─ 失敗時: Request不一致または状態不明を運用・確認者へ返す
                     │
                     ▼
             [R: 運用・確認者]
                     │ 返却された事実と判断不能範囲を確認
...
---------------- 可視境界 ----------------
                     │ 時間関係: 応答喪失から再接続まで時間差があり、元Requestは継続し得る
                     ▼
[S: 提供System]
