...
| 観点 | 内容 |
|---|---|
| 課題 | Repository名や配置から論理Projectを推測する／横断結果の操作対象が曖昧になる |
...
|---|---|---|---|
| Project Operator、Repository管理者 | Repository名や配置から論理Projectを推測する | 一つのProjectとして見ながら、必要時に各Sourceへ辿る | 探索元の課題と採用要求から導いた設計上の想定。個人属性、利用頻度および許容負担は未実測。 |
...
| 項目 | 内容 |
|---|---|
| 起点 | Repository名や配置から論理Projectを推測する |
| 行動・判断 | 利用者は提示された状態と根拠から、続行、確認、修正または回復を選ぶ。 |
| 接点・期待支援 | 同じProject IDを持つRepositoryを一つのViewで理解できる。／名前の一致だけでRepositoryやRootを同一とみなさない。 |
| 完了または回復 | Project、Repository、検証済みRootを区別して確認できる。成立不能時は不明を成功へ畳まず、理由と次の行動を確認できる。 |
...
|---|---|---|
| 根拠付きProject View | `Same → UX-000002@1` | `REQ-000007`と同じ「根拠付きProject View」を目指す。利用者が得る最終状態と主な失敗条件が同じであり、本要求「Project・Repository・Root Identity分離」はその成立条件を別の責務境界から補う。 |
| Project・Repository・Rootの対象確認 | `New → UX-000029@1` | `REQ-000009`の利用前後、重要場面および失敗条件から、他成果と独立して変更・検証できる利用者成果として確定した。 |
...
| System／AI | 利用者が「一つのProjectとして見ながら、必要時に各Sourceへ辿る」状態へ進めるよう、必要な状態、根拠および選択肢を示し、許可された処理だけを行う。 | 不明状態を成功へ畳まず、権限・対象・意味を無断で拡張しない。 |
