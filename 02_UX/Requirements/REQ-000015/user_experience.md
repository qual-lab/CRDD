...
|---|---|---|
| Runtime Data配置の理解 | `New → UX-000038` | 「何がどのRootへ何の目的で保存されるか理解できる」を独立して変更・検証する成果である。REQ-000015では「Runtime Dataの所有場所とLifecycleを理解する」をGoalとし、永続化または削除の直前にsubdirectoryや別Rootへ同名データを作ることを防ぐため、既存成果への条件追加ではなくNewとして追跡する。 |
| Durable・TemporaryのLifecycle理解 | `New → UX-000039` | 「保持すべき記録と再生成可能な一時物を区別し、清掃条件を理解できる」を独立して変更・検証する成果である。REQ-000015では「Runtime Dataの所有場所とLifecycleを理解する」をGoalとし、永続化または削除の直前にsubdirectoryや別Rootへ同名データを作ることを防ぐため、既存成果への条件追加ではなくNewとして追跡する。 |
...
| 重要場面 | 避ける失敗 | 品質期待 |
|---|---|---|
| 永続化または削除の直前 | subdirectoryや別Rootへ同名データを作る | 用途別領域とcleanup条件を明示する |
