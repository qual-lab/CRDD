...
[U: 外部Contextの所有者]
        ...
        ▼
[T: 別AgentやToolへ仕事を渡す時]
        │
        ├─ 時間差: Context準備、外部実行、結果帰還は別時点で成立する
        ├─ 完了時: 送信Contextの出所・現行性・許可と帰還結果の相関
        └─ 失敗時: 不足Context、拒否された情報、未確認の結果を外部Context所有者へ返す
                     │
                     ▼
             [R: 外部Contextの所有者]
                     │ 返却された事実と判断不能範囲を確認
...
---------------- 可視境界 ----------------
                     │ 時間関係: Context準備、外部実行、結果帰還は別時点で成立する
                     ▼
[S: 提供System／AI]
        └─ 提供責務: 必要最小限のContextを出所付きで搬送し候補として戻す
```
