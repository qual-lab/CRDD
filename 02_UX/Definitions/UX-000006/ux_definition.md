...
- 実行基盤ごとに異なるLogを読み、事実と推定を混ぜて比較する
        │
        │ 記録済みの実行事実を判断へ再利用できる体験
        ▼
変更後
────────────────
- 記録済みの実行事実を実行主体に依存しない同じ意味で読み、評価や改善候補と区別して比較・判断に使える
```

この変化が扱うのは、すでに記録された実行事実を読む側の体験である。実行基盤運用者・改善担当が、出所と観測時点を保った事実を実行主体の違いを越えて比較し、推測・評価・改善候補と混ぜず判断に使えることを守る。実行事実を書き込む体験は`UX-000032`が所有する。
