...
基本Viewは、対象となる問題と業務過程がある個別探索で作成する。本表は図を一か所へ再集約せず、CRDD標準自身の全探索を横断した現在の処置を示す。
...
| Actor別Process（Swimlane） | Actor間受渡しを扱う`EXP-*` | 活動、判断および受け渡し | `作成不能` | 探索全体に単一Actor列を置くと、後工程の完成System像を先取りする | 各探索 | 対象別 | 採用した要求を統合したActor間体験 | UXの[Service Blueprint](../02_UX/04_Service_Blueprint.md)で統合する |
| Value Stream | 待機・滞留・手戻りを扱う`EXP-*` | 処理、待機、滞留および手戻り | `作成不能` | 全探索共通の処理時間・待機・流量は観測していない | 各探索 | 対象別 | 時間・頻度・手戻り量 | 実運用で対象値を観測した時 |
| As-Is／To-Be | 各`EXP-*` | 現状の困りごとと望ましい変化 | `既存参照` | [探索台帳](#探索台帳)から各記録の問題、仮説、選択へ進む | 各探索 | 現行 | 実利用での変化量 | UXのExperience Changeと実測で具体化する |
...
本書の28探索と36要求は、既存CHG、Evidence、公開記録および保守対話から2026-09-13に再構成し、人間が課題、要求、対象外およびUXへの移行を確認した。理解確認の具体的な問題・仮説・修正は各`exploration.md`が所有し、本書へ複製しない。新しい探索では、AIが課題認識を再構成した場合に該当記録へ確認内容を残し、要求採用とは別に扱う。
