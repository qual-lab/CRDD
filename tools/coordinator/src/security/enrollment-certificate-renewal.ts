...
const INPUT_KEYS = new Set([
  ...
  "previousIssuerSpkiDer",
  "nextCertificateEnvelope",
  "nextIssuerSpkiDer",
  "evaluationTime",
]);
...
function ownedSpki(raw: unknown) {
  try {
...
function ownedEnvelopes(previous: unknown, next: unknown) {
...
export function verifyEnrollmentCertificateRenewalCandidate(rawInput: unknown) {
  try {
    const input = snapshotPlainRecord(rawInput, INPUT_KEYS);
    if (!input || !utc(input.evaluationTime)) {
      return response(
        "blocked",
        "enrollment_certificate_renewal_input_invalid",
      );
    }
    const envelopes = ownedEnvelopes(
      input.previousCertificateEnvelope,
    ...
    const previousResult = verifyInitialEnrollmentCertificateCandidate({
      certificateEnvelope: envelopes.previous,
      issuerSpkiDer: previousIssuer,
    });
    const nextResult = verifyInitialEnrollmentCertificateCandidate({
      certificateEnvelope: envelopes.next,
      issuerSpkiDer: nextIssuer,
    });
    const previous = envelopes.previous?.payload;
    const next = envelopes.next?.payload;
    if (
      previousResult.status !== "candidate" ||
      nextResult.status !== "candidate" ||
      !previous ||
      !next
    ) {
      return response(
        "blocked",
        "enrollment_certificate_renewal_certificate_invalid",
      );
    }
    if (
      previous.enrollmentId !== next.enrollmentId ||
      ...
      return response(
        "blocked",
        "enrollment_certificate_renewal_identity_mismatch",
      );
    }
    ...
    if (
      now < previousExpiry - 30 * DAY ||
      now >= previousExpiry ||
      nextIssued < previousExpiry - 30 * DAY ||
      nextIssued >= previousExpiry ||
      previousExpiry - nextIssued > 30 * DAY ||
      Date.parse(next.expiresAt) <= previousExpiry
    ) {
      return response(
        "blocked",
        "enrollment_certificate_renewal_window_invalid",
      );
    }
    return response(
      "candidate",
