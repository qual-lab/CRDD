...
const ISSUING_PAYLOAD_KEYS = new Set([
  "contract",
  "contractRevision",
  "caSeriesId",
  "role",
...
const CERTIFICATE_PAYLOAD_KEYS = new Set([
  "contract",
  "contractRevision",
  "enrollmentId",
  "platformScopeId",
...
const PAYLOAD_KEYS = new Set([
  ...
  "requestEnvelope",
  "certificateEnvelope",
  "onlineIssuingCertificateEnvelope",
  "offlineIssuingCertificateEnvelope",
  "revocationEnvelope",
  "issuedAt",
  "expiresAt",
]);
...
function signature(raw: unknown) {
...
function payload(raw: unknown) {
  ...
  if (
    !value ||
    value.contract !== OFFLINE_ENROLLMENT_BUNDLE_CONTRACT ||
    value.contractRevision !== OFFLINE_ENROLLMENT_BUNDLE_CONTRACT_REVISION ||
    typeof value.bundleId !== "string" ||
    !HEX32.test(value.bundleId) ||
    typeof value.requestHash !== "string" ||
    !HEX64.test(value.requestHash) ||
    typeof value.platformScopeId !== "string" ||
    !HEX32.test(value.platformScopeId) ||
    typeof value.provisionerIdentityHash !== "string" ||
    !HEX64.test(value.provisionerIdentityHash) ||
    typeof value.installationKeyId !== "string" ||
    !HEX64.test(value.installationKeyId) ||
    !utc(value.issuedAt) ||
    !utc(value.expiresAt) ||
    Date.parse(value.expiresAt) <= Date.parse(value.issuedAt) ||
    Date.parse(value.expiresAt) - Date.parse(value.issuedAt) > 7 * 86_400_000
  )
    return null;
  return Object.freeze({
    ...
    onlineIssuingCertificateEnvelope: value.onlineIssuingCertificateEnvelope,
    offlineIssuingCertificateEnvelope: value.offlineIssuingCertificateEnvelope,
...
function envelope(raw: unknown) {
  ...
  if (
    !value ||
    value.contract !== OFFLINE_ENROLLMENT_BUNDLE_ENVELOPE_CONTRACT ||
    value.contractRevision !== OFFLINE_ENROLLMENT_BUNDLE_CONTRACT_REVISION ||
    !normalizedPayload ||
    !signatures
  )
...
function frame(domain: string, value: unknown) {
...
function ownedPlainInput(raw: unknown) {
  const canonical = canonicalizeProvisioningJsonValueCandidate(raw);
  if (canonical.status !== "candidate") return null;
  try {
    return JSON.parse(canonical.canonicalBytes.toString("utf8"));
  } catch {
    return null;
  }
}

function issuingPayload(rawEnvelope: unknown, role: string) {
  const outer = snapshotPlainRecord(rawEnvelope, ENVELOPE_KEYS);
  const value =
    outer && snapshotPlainRecord(outer.payload, ISSUING_PAYLOAD_KEYS);
  if (
    !value ||
    value.role !== role ||
    typeof value.spkiDer !== "string" ||
    typeof value.keyId !== "string" ||
    !HEX64.test(value.keyId) ||
  ...
  if (
    inspected.status !== "candidate" ||
    inspected.spkiSha256Digest.toString("hex") !== value.keyId
  )
    return null;
  return Object.freeze({
    value: Object.freeze({
...
function requestHash(requestEnvelope: unknown) {
  const outer = snapshotPlainRecord(requestEnvelope, ENVELOPE_KEYS);
  const requestFrame =
    outer && frame(INITIAL_ENROLLMENT_DOMAINS.request, outer.payload);
  return requestFrame?.hash ?? null;
}

export function verifyOfflineEnrollmentBundleCandidate(rawInput: unknown) {
  try {
    ...
    const online = issuingPayload(
      bundle.payload.onlineIssuingCertificateEnvelope,
      "online_enrollment_issuer",
    );
    const offline = issuingPayload(
      bundle.payload.offlineIssuingCertificateEnvelope,
      "offline_bundle_issuer",
    );
    if (
      !online ||
      !offline ||
      online.value.caSeriesId !== offline.value.caSeriesId ||
    ...
    if (
      onlineState.status !== "candidate" ||
      offlineState.status !== "candidate"
    ) {
    ...
    if (flow.status !== "candidate") {
      return response(
        "blocked",
        "offline_enrollment_bundle_enrollment_flow_invalid",
      );
    }
    const requestOuter = snapshotPlainRecord(
      bundle.payload.requestEnvelope,
      ENVELOPE_KEYS,
    );
    const certificateOuter = snapshotPlainRecord(
      bundle.payload.certificateEnvelope,
      ENVELOPE_KEYS,
    );
    const request =
      requestOuter &&
      snapshotPlainRecord(requestOuter.payload, REQUEST_PAYLOAD_KEYS);
    const certificate =
      certificateOuter &&
      snapshotPlainRecord(certificateOuter.payload, CERTIFICATE_PAYLOAD_KEYS);
    const calculatedRequestHash = requestHash(bundle.payload.requestEnvelope);
    if (
      !request ||
      !certificate ||
      !calculatedRequestHash ||
      typeof request.platformScopeId !== "string" ||
      typeof request.provisionerIdentityHash !== "string" ||
      typeof request.installationKeyId !== "string" ||
      ...
      !utc(certificate.issuedAt) ||
      !utc(certificate.expiresAt) ||
      bundle.payload.requestHash !== calculatedRequestHash ||
      bundle.payload.platformScopeId !== request.platformScopeId ||
      bundle.payload.provisionerIdentityHash !==
        request.provisionerIdentityHash ||
    ...
    if (
      now < Date.parse(bundle.payload.issuedAt) ||
      now >= Date.parse(bundle.payload.expiresAt) ||
    ...
    if (
      !framed ||
      bundle.signature.keyId !== offline.value.keyId ||
      verifyProvisioningEd25519Base64urlCandidate({
...
export function describeOfflineEnrollmentBundlePureCoreContract() {
  return Object.freeze({
    ...
    caChain:
      "exact_online_enrollment_issuer_then_offline_bundle_issuer_same_root_series_and_epoch",
