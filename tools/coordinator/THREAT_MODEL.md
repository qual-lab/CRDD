...
承認済みのProvisioning方針は、CRDD配布物の`tools/coordinator`に内包するprivate `.mjs` packageを将来のRuntime Trust Policyで検証し、有効な準備Identityが維持されるPlatform scopeへ初回の管理者操作を限定する目標contractである。version 1のEffect前条件は、検証済みCRDD Revision、Qual-Lab署名済みpackage manifest、およびRuntime所有のpackage内容／Filesystem Identity確認が同じCRDD配布物へ結合して成立することである。manifestのexact object、CRDD Revision、Ed25519署名、file一覧content rootおよびcaller supplied観測のpure一致は候補実装したが、caller supplied signer、Revision verdictまたはfile metadataをRuntime所有Trustへ昇格しない。単独npm公開／個別install、専用native executableおよびOSネイティブコード署名はversion 1で要求せず、単独取得packageをEffectへ使用しない。未知Revision、改変、権限不一致または別source／方式へのfallbackをEffect前に拒否する。未検証source checkoutはdry-run／test専用で、TrustまたはGateを開かず、明示`provision`以外から昇格、鍵生成、ACL変更または保存を発火しない。

package Trust Gateのpure集約はmanifest、CRDD Revision、package content root、file Identity安定性およびpermission policy一致を同時に要求する。これは将来Adapterの出力契約を固定するだけで、caller supplied観測値または`verified_crdd_bundle`値をRuntime所有観測へ昇格しない。観測値を偽装できる同一権限code、Path検査と実行間の置換または配布物自体の侵害はこのCoreの保証外であり、実AdapterとEffect controllerの同一制御経路が必要である。
