...
pure Core候補は署名領域分離（domain separation）を固定ASCII prefix `CRDD\0PROVISIONING-RECORD\0V1\0`、payload JCS byte長の符号なし64-bit big-endian値、payload JCS bytesの順で構成する。鍵識別子（key ID）はexact RFC 8410 SPKI DERのSHA-256 lowercase hexadecimal 64文字、署名はEd25519のexact 64 byteをpaddingなしcanonical base64urlで表す。raw decoderはBufferだけをowned copyし、strict UTF-8、BOMなし、exact Schema、JCS再符号化完全一致、件数・深さ・node・byte上限を要求する。集約候補は全署名entryが既知、非失効、期間内かつ暗号学的に正常な場合だけ成立し、無効entryを無視しない。入力鍵集合、失効一覧および評価時刻は暗号条件を再現する候補であり、Qual-Lab同梱Trust、現在版、rollback防止またはRuntime時計Authorityではない。実鍵、端末enrollment、Filesystem、Lifecycle、resolverおよびactivation確認が未実装のため、候補一致もAuthority、CapabilityまたはGateを開かない。raw payload、canonical byte、Path、SPKI、signatureまたはkey IDをdoctor、log、Evidence、ProviderまたはOperationへ出さない。
