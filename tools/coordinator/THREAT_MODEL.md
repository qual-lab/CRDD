...
端末導入鍵（installation key）の承認済み目標は、プラットフォーム準備ツールがPlatform scopeごとのEd25519秘密鍵をOS管理の鍵保管境界で生成・保持し、Qual-Lab準備認証局（Qual-Lab Provisioning CA）がその公開鍵へ短期証明書を発行する質的topologyである。OS鍵保管領域（OS keystore）、TPMおよびSecure Enclaveはplatform別の承認済みbackend候補群であり、同時必須、Runtimeの自由選択／fallbackまたは同等強度確認済みではない。Qual-Lab秘密鍵をCoordinator、Repository、準備記録またはオフライン初回登録束へ埋め込まず、秘密鍵materialをCoordinator Runtimeの入力、出力または成果物へ渡さない。初回登録（enrollment）は明示オンラインまたは管理者持込みoffline bundleの二方式だけを将来候補とし、片方の不成立から他方へsilent fallbackしない。caller supplied certificate、bundle、公開鍵、鍵保管らしい状態または自己申告をTrustへ昇格せず、検証済み登録へ結合された公開鍵だけを将来の準備記録署名鍵候補とする。通常runは検証済み登録状態が維持される間networkを要求しない一方、各runのProvisioning Record／Provisioner Trust根拠には鍵保護、登録、認証局Trust、失効およびPlatform scopeの再検証目標を含める。未知、期限切れ、失効、置換、不一致または検証不能ではfail closedで`blocked`とし、再Provisionへ戻して自動回復しない。承認済みなのは短期証明書topology、OS管理鍵保管境界およびbackend候補群までである。証明書のexact Schema／wire encoding／field、具体期間、更新／overlap／失効／rollback／replay、platform別backend選択と必要保護強度／exportability、認証局署名方式、challenge／attestation、offline bundle、保存Path／Lifecycle／recoveryは未決である。鍵生成、鍵保護確認、証明書契約／検証、認証局Trust／失効、登録交換、Record結合、network、keystoreおよび保存Lifecycleも未実装である。実行するPlatform Provisioner実体のTrustは`platform_provisioner_verification`だけが所有し、鍵生成／登録交換は`platform_provisioner_effect`、証明書契約は`provisioning_record_contract`、鍵保護／証明書／認証局Trust／失効／Record結合の検証は`provisioning_record_verification`へ分離する。
