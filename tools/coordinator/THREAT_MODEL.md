...
ローカル導入の目標contractは、無効なRepositoryではRuntime固有Effectを発火せず、初回Platform setupをTrust検証済みnpm package内の明示Provisioner経路と共有Authority Root Provisionへ限定し、Repositoryごとの`coordinator activate`を唯一の有効化入口とする。有効なProvisioning／activationと保護Identityを再確認できる将来状態では通常実行／再起動ごとの昇格、Path再入力またはACL手動設定を要求しないが、再確認自体を省略しない。Platform Provisionerの署名／Trust、principal、Root Identityまたは保護metadataの変化を検知した場合は必ずfail closedでreverificationを開始する。再確認によってRoot欠落／置換、排他的writerまたはAuthority RootのRuntime read-only不一致、もしくは将来の検証済みProvisioning記録に対するAuthority Root Identity不一致が確定した場合だけreprovisionを案内する。判定不能時も`blocked`とし、Runtimeは自動修復、Root作成、chmod／chownまたはACL／DACL変更を行わない。現段階ではPlatform Provisioner検証／Effect、Provisioning記録、Authority Root resolverおよびactivation Effectが未実装であり、これらの再Provision条件を実評価せず、Directory、Platform Provisionerの存在またはcaller claimをProvisioning成立へ流用しない。現行の明示Authority Root Path契約を維持し、OS暗黙Pathを導入しない。

明示`coordinator provision`のcommand grammarは実装済み候補だが、現ローカル／開発buildでは常にEffect前に`blocked`となる。結果の`dryRunOnly`、registry署名／provenance／package Filesystem未確認またはEffect 0という表示は検証tokenではなく、別入口へ持ち回ってTrustを成立させられない。実配布packageの全Trust条件とpackage Identityが同一制御経路へ結合されるまでRoot作成、鍵生成、ACL変更または保存を行わない。

承認済みのProvisioning方針は、Coordinatorの`.mjs`を含むnpm packageを将来のRuntime Trust Policyで検証し、有効な準備Identityが維持されるPlatform scopeへ初回の管理者操作を限定する目標contractである。version 1のEffect前条件は、npm registry署名、生成元証明（provenance）、Qual-Lab署名済みpackage manifest、およびRuntime所有のpackage内容／Filesystem Identity確認が同じpackageへ結合して成立することである。manifestのexact object、Ed25519署名、file一覧content rootおよびcaller supplied観測のpure一致は候補実装したが、caller supplied signer、registry／provenance verdictまたはfile metadataをRuntime所有Trustへ昇格しない。専用native executableとOSネイティブコード署名はversion 1で要求しない。未知署名、生成元不一致、改変、権限不一致または別source／方式へのfallbackをEffect前に拒否する。ローカルsource checkoutはdry-run／test専用で、TrustまたはGateを開かず、明示`provision`以外から昇格、鍵生成、ACL変更または保存を発火しない。

package Trust Gateのpure集約はmanifest、npm registry署名、生成元repository、package content root、file Identity安定性およびpermission policy一致を同時に要求する。これは将来Adapterの出力契約を固定するだけで、caller supplied観測値または`verified`値をRuntime所有観測へ昇格しない。観測値を偽装できる同一権限code、Path検査と実行間の置換またはpackage manager自体の侵害はこのCoreの保証外であり、実AdapterとEffect controllerの同一制御経路が必要である。
...
確認日（reviewedAt）は暗号規格入力が`2026-08-15`、npm配布入力が`2026-08-16`である。Runtimeは規格本文をネットワーク取得または自動更新せず、下表で採用した内部contractを拘束点とする。RFC errata、Node.js crypto／npm検証仕様の挙動、または採用範囲が変わる場合は人間の判断と再監査を行う。
...
| [npm package provenance](https://docs.npmjs.com/generating-provenance-statements/)（reviewedAt 2026-08-16） | publish時の生成元証明、Sigstore署名および透明性log | packageとsource repository／build environmentの結合確認目標 | package内容の無害性、Runtime Authority、実publish workflow、外部CIの自動実行 |
| [npm registry signatures](https://docs.npmjs.com/verifying-registry-signatures/)（reviewedAt 2026-08-16） | registry署名とprovenance attestationの検証入口 | install時receiptのRuntime所有検証目標 | npm CLI出力の自己申告、実Adapter、release Trust成立 |
