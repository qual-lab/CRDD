...
署名基礎Core候補は、RFC 8785 JCSのI-JSON値canonicalization、RFC 8410 Ed25519 SPKI DERのexact再符号化一致、canonical SPKI DERのSHA-256 digest、およびRFC 8032 Ed25519個別署名の暗号学的一致だけを評価する。caller supplied公開鍵、messageまたは署名はTrust Anchor、key ID、非失効またはProvisioning Record Authorityではない。JCS Coreはraw JSON decoderを所有しないため重複propertyを検査済みと主張せず、既存成果物のcanonical方式またはHashをJCSへ暗黙移行しない。個別primitive合格からRecord成立へ昇格させず、CRDD固有署名領域分離のexact framing、Record payload、複数署名Envelope、同梱鍵集合、失効manifest、aggregate受理規則および統合Verifierが未実装の間は`blocked`を維持する。raw payload、canonical byte、SPKI、signatureまたはdigestをdoctor、log、Evidence、ProviderまたはOperationへ出さない。
