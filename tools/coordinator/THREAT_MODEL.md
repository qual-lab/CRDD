...
Authority RecordとRepository generationはimmutable成果物とatomic pointerへ分離し、cross-volume atomicityを主張しない。順序はimmutable file fsync、generation directory fsync、pointer temporary file fsync、pointer atomic replace、pointer parent directory fsync、再読取Identity確認とする。いずれかの同期失敗、曖昧なjournalまたは再読取Identity不一致は推測回復せず`blocked`とする。権限Effect ownerは公式署名済みPlatform Provisionerだけで、Runtimeによるpermission mutationを禁止する。Windowsの継承／広範write ACE、POSIXの未承認group／other write、およびlocal相当の安定Identity・durability・ACLを証明できないnetwork／removable／special／unknown volumeは拒否する。
