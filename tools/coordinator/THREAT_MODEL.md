...
承認済みのProvisioning方針は、Coordinatorと共に配布する公式署名済みProvisionerを将来のRuntime Trust Policyで検証し、有効な準備Identityが維持されるPlatform scopeへ初回の管理者操作を限定する目標contractである。ローカルでは選択利用者、serverでは専用サービスアカウント（service account）をRuntime principal種別とするが、自己申告principal、Platform Provisionerの存在または署名らしい値をAuthorityへ昇格しない。共有Authority Rootの通常run再利用は、将来の署名・Trust検証済みProvisioning Recordへ結合された明示Pathに限る。Provisioning RecordはPlatform scopeのRuntime向け信用判断を担う中心成果物で、Provisioner Identityと署名metadataをRecordへ結合し、セキュリティ上重要な全項目を一つのcanonical JSON署名対象へまとめる目標とする。payloadと複数署名は分離Envelopeへ置き、署名値そのものを循環して署名対象に含めない。署名値そのもののencodingはpaddingなしcanonical base64urlとして実装済み候補である。未決／未実装なのは、Envelope全体のexact wire Schema、field名と配置、payloadおよびkey ID等の署名値以外のfield encoding、CRDD固有domain framing、ならびに1件以上の既知・非失効・有効署名を要求し、重複・未知・失効・不正・形式不明を全体拒否する承認済み目標方針のaggregate実判定である。Provisioning Receiptまたは独立helper Manifestを別のRuntime Authority成果物として要求せず、補助文書、Platform Provisionerの存在または自己申告HashをRecord検証の代用にしない。Authority File Bundle Manifestは既存の別成果物であり、Recordから置換・流用しない。Trust AnchorはCoordinatorと共に配布するQual-Lab公開鍵集合を将来使用し、複数key ID、切替重複期間および明示失効による旧鍵拒否を要求する。

署名基礎Core候補は、RFC 8785 JCSのI-JSON値canonicalization、RFC 8410 Ed25519 SPKI DERのexact再符号化一致、canonical SPKI DERのSHA-256 digest、RFC 8032 Ed25519個別署名の暗号学的一致、およびRFC 4648のpaddingなしcanonical base64urlで表したexact 64-byte署名の厳格な内部復号だけを評価する。padding、非alphabet文字、長さ不一致または非canonical pad bitはfail closedで拒否する。caller supplied公開鍵、message、署名文字列または復号署名はTrust Anchor、key ID、非失効またはProvisioning Record Authorityではない。JCS Coreはraw JSON decoderを所有しないため重複propertyを検査済みと主張せず、既存成果物のcanonical方式またはHashをJCSへ暗黙移行しない。将来の複数署名評価は、1件以上の既知・非失効・有効署名を要求し、重複、未知、失効、不正または形式不明なentryが1件でもあれば全体をfallbackなしで拒否する。ただしこの受理方針は目標contractであり、CRDD固有署名領域分離のexact framing、key ID encoding、Record payload、複数署名Envelope、同梱鍵集合、失効manifest、aggregate受理の実判定および統合Verifierが未実装の間は`blocked`を維持する。raw payload、canonical byte、SPKI、signature、署名文字列またはdigestをdoctor、log、Evidence、ProviderまたはOperationへ出さない。
