...
Provider隔離ProfileはAuthorityと強制結果を分離する。Profileが表すのは、期限付きの人間承認、Provider、Credential Broker参照および許可HTTPS Origin集合であり、Credential値または実行可能Capabilityではない。Profile自身、Agent出力、環境変数またはProviderの自己申告だけからFilesystem、Credential、EgressまたはProcess境界を`confirmed`へ上げない。Runtimeが所有するEnforcer／Adapterの観測結果が、同じ固定Profile HashとOperationへ結び付いた場合だけ各Gateを評価する。
