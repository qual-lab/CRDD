...
端末導入鍵（installation key）の初回オンライン登録pure Core候補は、30分のオンライン登録チャレンジ（online enrollment challenge）、チャレンジのdomain-framed payload Hashを含む登録要求（enrollment request）、端末導入鍵による所有証明、およびPlatform scope・Provisioner Identity・公開鍵を結ぶ登録証明書（enrollment certificate）を検査する。チャレンジ／登録要求／登録証明書のexact object Schema、成果物別domain、JCS署名messageおよび数学的署名一致は実装済み候補である。署名前payloadのcanonical raw byte decoderは実装済み候補である。署名Envelope／transport、Runtime所有時計、一回消費台帳、CA Trust／失効、Network、Filesystem、keystore、Record結合、証明書更新およびオフライン束は未実装である。caller supplied inputからTrustを推定せず、候補一致もAuthority、Capability、EffectまたはGateを開かない。
