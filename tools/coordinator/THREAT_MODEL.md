...
端末導入鍵（installation key）の承認済み目標は、プラットフォーム準備ツールがPlatform scopeごとのEd25519秘密鍵をOS管理の鍵保管境界で生成・保持し、Qual-Lab準備認証局（Qual-Lab Provisioning CA）が独自JCS JSON形式の登録証明書をEd25519で署名して発行することである。証明書のkey IDはexact SPKI DERのSHA-256 lowercase hexadecimal 64文字、有効期間は180日、更新開始は期限30日前、新旧併用期間は最大30日とする。期限切れ、失効、rollback、置換、不一致または検証不能はfail closedで`blocked`とし、更新失敗時にも自動fallbackしない。WindowsはCNG/KSPのTPM-backed鍵を優先し明示的software KSPを代替候補、macOSはKeychainのSecure Enclave-backed鍵を優先しsoftware-backed Keychainを代替候補、LinuxはTPM 2.0を優先しroot-owned software keystoreを代替候補とする。初回setupで選択backendと確認できた保護強度を表示し、通常runでは再選択、管理者操作またはnetworkを要求しない。候補間のsilent fallback、同等強度の推定、秘密鍵materialのCoordinator Runtimeへの入出力または成果物化、Qual-Lab秘密鍵の埋込みは禁止する。初回登録は明示オンラインまたは管理者持込みoffline bundleだけを候補とする。オンライン方式はone-time challenge、nonce、Platform scopeおよびinstallation public key、オフライン束はrequest hash、登録証明書、Provisioning CA chain、失効snapshotおよび束の期限を将来結合する。replay、別machine、別Platform scope、期限切れ入力を拒否し、片方の不成立から他方へfallbackしない。caller supplied certificate、bundle、公開鍵、鍵保管らしい状態または自己申告をTrustへ昇格させない。未決なのは証明書のexact field Schema／wire encoding／domain bytes、proof-of-possession、challenge TTL、offline CA chainのexact topology／署名充足、replay消費台帳、CA root／intermediate Lifecycle、保存Path／recoveryおよびplatform adapter実装である。鍵生成、鍵保護確認、証明書codec／検証、認証局Trust／失効、自動更新、登録交換、replay永続化、Record結合、network、keystoreおよび保存Lifecycleも未実装である。実行するPlatform Provisioner実体のTrustは`platform_provisioner_verification`だけが所有し、鍵生成／登録交換／更新Effectは`platform_provisioner_effect`、証明書／offline bundle契約は`provisioning_record_contract`、鍵保護／証明書／認証局Trust／失効／Record結合／replay検証は`provisioning_record_verification`へ分離する。
