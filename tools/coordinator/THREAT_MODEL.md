...
Runtime Rootの既定候補はRepository直下の`.crdd-runtime`、明示overrideの指定契約と優先順はCLI、環境、Repository既定とする。選択は絶対Pathの構文候補だけを扱い、絶対Path自体をdoctor／Evidenceへ出力しない。OS別の暗黙rootを設けず、serverでは同じ契約へ永続Volumeの絶対Pathを指定できる。現在のCLIおよび環境読取りはこのCoreへ未接続である。Directoryの存在、`--runtime-root`、環境変数、ignored状態またはRepository内ファイルをactivationの根拠にしない。明示enable要求も候補にすぎず、Runtime所有activation記録の原子的永続化と再検証が成立するまでCapabilityを発行しない。
...
Runtime 1.0のRepository形態候補は通常worktree、linked worktreeおよび対象自身がsubmodule等の`.git` file形式であるworktreeとし、bare Repositoryを拒否する。Filesystem解決Core候補は`.git` directory／fileと`commondir`を上限付きで読み、non-linkなGit directoryおよびcommon Git directoryを解決し、既存`info/exclude`境界のlinkを拒否する。これはRepository Identity、親chain、case／Unicode alias、Git拡張またはmetadata書込みAuthorityの成立を意味しない。実AdapterはGitの最終解決結果とFilesystem実体を再照合するまで書き込まない。
...
Qual-Labは、Runtime 1.0のGit metadata配置Authorityを外部Git CLIへ依存させず、内蔵の限定Filesystem parserで成立させる方針を承認した。parserは通常worktree、linked worktreeおよび対象自身がsubmodule等の`.git` file形式であるworktreeに限定し、common Git directoryの`config`を上限付き安定読取りする。Repository format version 0、非bare、`extensions`／`include`／`includeIf`／`core.worktree`なしだけを受理し、未知構文、重複したAuthority関連key、未対応formatまたはextensionを自動fallbackせず`blocked`にする。これによりGit実行ファイルのPath／Hash／更新承認をOS別に持たない一方、Gitとして有効な特殊構成を保守的に拒否する。このparserはGit全仕様、case／Unicode alias、parent chain、owner／ACLまたは同一権限Host主体への完全防御を主張しない。

local exclude書込みAdapter候補は、解決済みcommon Git directoryの既存`info` directoryだけを対象とし、caller指定のGit metadata Pathを受理しない。既存`exclude`は131072 byteを上限として同一handleから所有copyへ読み、non-link、実体Identity、size、modeおよび更新時刻の前後一致を要求する。完全一致entryがなければ、同一`info` directory内の固定lock名を排他的に取得し、既存内容を保持した一時fileを同期してから置換し、再読取りbyte一致とexact entryを確認する。既存lockは別主体または中断処理の可能性があるため推測削除せず`blocked`とする。処理中に作成したlockだけは実体Identityが同じ場合に限り失敗時に除去する。置換後の検証失敗は書込み済み事実を隠さず`blocked`へ閉じ、暗黙rollbackしない。これは同一権限Host主体による検査直後の敵対的置換、parent chain、owner／ACL、crash durabilityを保証しない。CLI／環境接続、activation、Candidate／Operation／Provider除外の実強制およびCapabilityが未実装の間、Gateは`blocked`を維持する。
