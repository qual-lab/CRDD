...
ローカル導入の目標contractは、無効なRepositoryではRuntime固有Effectを発火せず、初回Platform setupを署名済み専用Provisionerの確認と共有Authority Root Provisionへ限定し、Repositoryごとの`coordinator activate`を唯一の有効化入口とする。有効なProvisioning／activationと保護Identityを再確認できる将来状態では通常実行／再起動ごとの昇格、Path再入力またはACL手動設定を要求しないが、再確認自体を省略しない。Platform Provisionerの署名／Trust、principal、Root Identityまたは保護metadataの変化を検知した場合は必ずfail closedでreverificationを開始する。再確認によってRoot欠落／置換、排他的writerまたはAuthority RootのRuntime read-only不一致、もしくは将来の検証済みProvisioning記録に対するAuthority Root Identity不一致が確定した場合だけreprovisionを案内する。判定不能時も`blocked`とし、Runtimeは自動修復、Root作成、chmod／chownまたはACL／DACL変更を行わない。現段階ではPlatform Provisioner検証／Effect、Provisioning記録、Authority Root resolverおよびactivation Effectが未実装であり、これらの再Provision条件を実評価せず、Directory、Platform Provisionerの存在またはcaller claimをProvisioning成立へ流用しない。現行の明示Authority Root Path契約を維持し、OS暗黙Pathを導入しない。

承認済みのProvisioning方針は、Coordinatorと共に配布する公式署名済みProvisionerを将来のRuntime Trust Policyで検証し、有効な準備Identityが維持されるPlatform scopeへ初回の管理者操作を限定する目標contractである。ローカルでは選択利用者、serverでは専用サービスアカウント（service account）をRuntime principal種別とするが、自己申告principal、Platform Provisionerの存在または署名らしい値をAuthorityへ昇格しない。共有Authority Rootの通常run再利用は、将来の署名・Trust検証済みProvisioning Recordへ結合された明示Pathに限る。準備記録（Provisioning Record）のrevision 1 payload、署名包絡（signature Envelope）、信頼起点鍵集合（Trust Anchor Set）および失効一覧（Revocation Manifest）のexact pure codecと集約署名検証（aggregate signature verification）候補を実装した。Provisioning Receiptまたは独立helper Manifestを別のRuntime Authority成果物として要求せず、Authority File Bundle Manifestは既存の別成果物として維持する。

pure Core候補は署名領域分離（domain separation）を固定ASCII prefix `CRDD\0PROVISIONING-RECORD\0V1\0`、payload JCS byte長の符号なし64-bit big-endian値、payload JCS bytesの順で構成する。鍵識別子（key ID）はexact RFC 8410 SPKI DERのSHA-256 lowercase hexadecimal 64文字、署名はEd25519のexact 64 byteをpaddingなしcanonical base64urlで表す。準備記録はcanonical UTCの`issuedAt`より`expiresAt`が後で、その差が最大180日以内の場合だけ候補化する。集約評価では評価時刻が`issuedAt`以上かつ`expiresAt`未満であることを要求し、期限外または180日超過をfail closedで拒否する。この180日は準備記録の有効期間上限で、鍵の有効期間、鍵切替期間または失効一覧の保持期間ではない。raw decoderはBufferだけをowned copyし、strict UTF-8、BOMなし、exact Schema、JCS再符号化完全一致、件数・深さ・node・byte上限を要求する。集約候補は全署名entryが既知、非失効、期間内かつ暗号学的に正常な場合だけ成立し、無効entryを無視しない。署名鍵が失効一覧へ列挙されていれば`revokedAt`が評価時刻より過去、同時または未来のいずれでも拒否し、`revokedAt`を予約失効の発効時刻として扱わない。入力鍵集合、失効一覧および評価時刻は暗号条件を再現する候補であり、Qual-Lab同梱Trust、現在版、rollback防止またはRuntime時計Authorityではない。実鍵、端末enrollment、Filesystem、Lifecycle、resolverおよびactivation確認が未実装のため、候補一致もAuthority、CapabilityまたはGateを開かない。raw payload、canonical byte、Path、SPKI、signatureまたはkey IDをdoctor、log、Evidence、ProviderまたはOperationへ出さない。

端末導入鍵（installation key）の承認済み目標は、プラットフォーム準備ツールがPlatform scopeごとのEd25519秘密鍵をOS管理の鍵保管境界で生成・保持し、Qual-Lab準備認証局（Qual-Lab Provisioning CA）が独自JCS JSON形式の登録証明書をEd25519で署名して発行することである。証明書のkey IDはexact SPKI DERのSHA-256 lowercase hexadecimal 64文字、有効期間は180日、更新開始は期限30日前、新旧併用期間は最大30日とする。検証済み自動更新が成功した場合は利用者／管理者操作を要求しない。期限切れ、失効、rollback、置換、不一致または検証不能はfail closedで`blocked`とし、更新失敗時にも自動fallbackしない。WindowsはCNG/KSPのTPM-backed鍵を優先し明示的software KSPを代替候補、macOSはKeychainのSecure Enclave-backed鍵を優先しsoftware-backed Keychainを代替候補、LinuxはTPM 2.0を優先しroot-owned software keystoreを代替候補とする。初回setupで選択backendと確認できた保護強度を表示し、通常runでは再選択、管理者操作またはnetworkを要求しない。候補間のsilent fallback、同等強度の推定、秘密鍵materialのCoordinator Runtimeへの入出力または成果物化、Qual-Lab秘密鍵の埋込みは禁止する。初回登録は明示オンラインまたは管理者持込みの署名済みオフライン初回登録束（offline bundle）だけを候補とする。オンライン方式は一回限りのチャレンジ（one-time challenge）、ノンス（nonce）、プラットフォーム範囲（Platform scope）および端末導入公開鍵（installation public key）を結合する。オフライン束は登録要求ハッシュ（enrollment request hash）、登録証明書、準備認証局チェーン（Provisioning CA chain）、失効スナップショット（revocation snapshot）および束の期限（bundle expiry）を将来結合する。束の署名者、署名対象、exact topologyおよび署名充足規則は未実装で、署名らしい値からTrustを推定しない。再送（replay）、別machine、別Platform scope、期限切れ入力を拒否し、片方の不成立から他方へfallbackしない。caller supplied certificate、bundle、公開鍵、鍵保管らしい状態または自己申告をTrustへ昇格させない。未決なのは証明書、challenge、requestおよびbundleのexact field Schema／wire encoding／domain bytes、オフライン認証局チェーンのexact topology／署名充足、再送消費台帳（replay ledger）の保存形式、CA root／intermediateの実配布Lifecycleおよびplatform adapter実装である。鍵生成、鍵保護確認、証明書codec／検証、認証局Trust／失効、自動更新、登録交換、replay永続化、Record結合、network、keystoreおよび保存Lifecycleも未実装である。実行するPlatform Provisioner実体のTrustは`platform_provisioner_verification`だけが所有し、鍵生成／登録交換／更新Effectは`platform_provisioner_effect`、証明書／offline bundle契約は`provisioning_record_contract`、鍵保護／証明書／認証局Trust／失効／Record結合／replay検証は`provisioning_record_verification`へ分離する。
...
| [RFC 4648: The Base16, Base32, and Base64 Data Encodings](https://www.rfc-editor.org/rfc/rfc4648.html)（2006-10、Standards Track） | §3.2–§3.5、§5 | exact 64-byte Ed25519署名のpaddingなしcanonical base64url、非alphabet／非canonical表現の拒否 | key ID encoding、Envelope wire Schema、他のbase encoding |
...
後続の一括承認により、オンラインchallengeを10分有効・所有証明成功後の一回消費、署名済みoffline bundleを7日有効・一回消費とした。準備認証局はoffline rootとonline issuing keyを分離し、issuing keyは最大365日、切替overlapは30日、失効情報freshnessは24時間とする。Trust epoch／revisionの低下または同revision別Hashはrollbackとして拒否するが、単調floorの永続化は未実装である。

Authority RecordとRepository generationはimmutable成果物とatomic pointerへ分離し、cross-volume atomicityを主張しない。順序はimmutable file fsync、generation directory fsync、pointer temporary file fsync、pointer atomic replace、pointer parent directory fsync、再読取Identity確認とする。各fsync、atomic replace、再読取またはIdentity確認のfailure／unknown／mismatch、および結果分類不能では、利用可能な成果物と既存journalだけを保持して`blocked`とし、明示的回復を要求する。journal不存在または保持確認不能も`blocked`のままとし、推測rollback、自動retryまたは旧pointerへのfallbackを行わない。明示的回復の具体手順とEffectは未実装である。権限Effect ownerは公式署名済みPlatform Provisionerだけで、Runtimeによるpermission mutationを禁止する。Windowsの継承／広範write ACE、POSIXの未承認group／other write、およびlocal相当の安定Identity・durability・ACLを証明できないnetwork／removable／special／unknown volumeは拒否する。

これらは質的な安全契約である。成果物のexact field Schema／wire encoding／domain bytes、CA chainのexact topology／署名充足、replay台帳の保存形式、CA実配布Lifecycle、実OS／Filesystem Adapterは未決または未実装である。caller claim、Pathやcertificateの存在またはcandidate contractだけからTrust、Authority、Capabilityまたはreadyを成立させない。
