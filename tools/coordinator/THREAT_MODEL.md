...
POSIX Runtime Root mode precheck候補は、Root Path Identity Coreが所有する同一run内のprivate sessionからだけ発火し、`lstat`／`realpath`で固定したRootを`O_DIRECTORY`／`O_NOFOLLOW`で開き、同一handleの`fstat`前後とRoot／parent／Repositoryの再照合を行う。現在processのeffective UIDとRoot ownerが一致し、owner read／write／executeがあり、group／other write bitがないことだけを受動確認する。追加ACL／xattr、将来service principal、Filesystem class、Authority Rootのprovisioner集合および各system call間の最終raceは確認できないため、precheck合格も保護結果の確認またはAuthorityではなく総合`blocked`の診断根拠に限る。Windows、persistent volume、非opt-inでは発火しない。Path／UID／GID／mode／descriptorを公開せず、任意callbackまたは再利用tokenを追加しない。
