...
端末導入鍵（installation key）の承認済み目標は、プラットフォーム準備ツールがPlatform scopeごとのEd25519秘密鍵をOS鍵保管領域（OS keystore）、TPMまたはSecure Enclave内で生成・保持し、Qual-Lab準備認証局（Qual-Lab Provisioning CA）がその公開鍵へ短期証明書を発行することである。Qual-Lab秘密鍵をCoordinator、Repository、準備記録またはオフライン初回登録束へ埋め込まず、秘密鍵materialをCoordinator Runtimeの入力、出力または成果物へ渡さない。初回登録（enrollment）は明示オンラインまたは管理者持込みoffline bundleの二方式だけを将来候補とし、片方の不成立から他方へsilent fallbackしない。caller supplied certificate、bundle、公開鍵、鍵保管らしい状態または自己申告をTrustへ昇格せず、検証済み登録へ結合された公開鍵だけを将来の準備記録署名鍵候補とする。通常runは検証済み登録状態が維持される間networkを要求しない一方、各runのProvisioning Record／Provisioner Trust根拠には鍵保護、登録、認証局Trust、失効およびPlatform scopeの再検証目標を含める。未知、期限切れ、失効、置換、不一致または検証不能ではfail closedで`blocked`とし、再Provisionへ戻して自動回復しない。鍵生成、鍵保護確認、証明書Schema／検証、認証局Trust／失効、登録交換、Record結合、network、keystoreおよび保存Lifecycleは未実装であり、証明書形式、認証局署名方式、期限、challenge／attestation、offline bundleおよびOS backendをこの方針から推定しない。
