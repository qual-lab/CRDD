...
承認済みの端末導入鍵（installation key）方針は、Platform scopeごとのEd25519鍵をプラットフォーム準備ツールがOS管理の鍵保管境界で生成・保持し、その公開鍵へQual-Lab準備認証局（Qual-Lab Provisioning CA）が短期証明書を発行する質的topologyを目標とする。OS鍵保管領域（OS keystore）、TPMおよびSecure Enclaveはplatformごとに選ぶ承認済みbackend候補群であり、全環境での同時必須、Runtimeによる自由選択／fallback、同等の保護強度確認済みを意味しない。Qual-Lab秘密鍵をCoordinator、Repository、準備記録または持込み束へ埋め込まず、秘密鍵materialをCoordinator Runtimeの入力、出力または成果物にしない。初回登録（enrollment）は明示的なオンライン登録または管理者が持ち込むオフライン初回登録束（offline enrollment bundle）に限り、失敗時に別方式へ無言でfallbackしない。検証済み登録へ結合された公開鍵だけを将来の準備記録署名鍵候補とし、通常runは検証済み登録状態が維持される間networkを要求しない。鍵、証明書、認証局Trust、失効、Platform scopeまたは結合が未知、期限切れ、失効、置換、不一致もしくは再検証不能なら`blocked`として再Provisionへ戻し、自動回復しない。承認済みなのはこの短期証明書topologyとbackend候補群までである。証明書のexact Schema／wire encoding／field、具体的な有効期間、更新／overlap／失効／rollback／replay規則、platform別backend選択と必要保護強度／exportability、認証局署名方式、challenge／attestation、持込み束Schema、保存Path／Lifecycle／recoveryは未決である。鍵生成、鍵保護確認、証明書契約／検証、認証局Trust／失効確認、初回登録交換および準備記録との結合検証もすべて未実装である。この方針投影は入力、network、暗号鍵操作、Filesystem Effect、AuthorityまたはCapabilityを発行せず、既存12阻害依存と6件の現在run根拠を減らさない。実行するPlatform Provisioner実体の署名／Trust検証は`platform_provisioner_verification`、鍵生成と初回登録交換は`platform_provisioner_effect`、証明書契約は`provisioning_record_contract`、鍵保護・証明書・認証局Trust／失効・Record結合の検証は`provisioning_record_verification`が将来所有し、実行物TrustとRecord署名者Trustを混同しない。
