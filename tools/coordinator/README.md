...
Runtime所有Operation context Capabilityは、Runtime生成Operation IDと生成時刻を、factory所有object、Operation root／parent／prefix、rootと既知6 childのstable Filesystem Identity、およびHost recovery nonceで区別したprocess-local世代へ結合する。同じ世代のcontext Capabilityとmount Capabilityが揃う場合だけ、後続store用のopaqueなmanagement binding Capabilityを発行し、Path、Filesystem Identityまたはplainな権限tokenを返さない。plain copy、偽造、別Operationの組合せおよび置換済みIdentityは拒否し、確認済み置換では同一世代のcontext／mount／management aliasを不可逆に失効する。正常cleanupまたは検証済みHost recoveryが同一世代のroot消滅を確定した場合も全aliasを失効する一方、Identityが正常なままの未知entry、access error、marker不成立等で処置が停止しただけならOperation終了や失効を先取りしない。これはOperation bindingの内部Capabilityに限られ、Provider Home保護、Mount Grant発行、実mount、Provider process、Network、OAuth、課金、Gate、StableまたはReleaseを成立させない。

Providerライフサイクル基盤（Provider Lifecycle Foundation）は、Codexでは既存ChatGPTプランのsubscription OAuth、Claude Codeでは既存subscription OAuthだけを認証方針として許可する。API key、従量APIへの切替、追加credit購入、Hostの既定HomeやCredentialのコピーは許可しない。認証状態はローカルOS userとProviderの組合せごとに分離した永続専用Homeへ将来の明示loginで作成し、RepositoryやOperationごとにloginし直さない方針である。専用Homeは通常Operationの一時`provider-home/`と別物であり、他Providerと共有せず、Operation cleanupの所有対象に含めない。専用Provider Home配置候補（Dedicated Provider Home Layout Candidate）の現在Coreは、caller supplied Windows絶対Pathをlocal app data Rootの非Authority字句候補としてだけ評価し、固定`Qual-Lab/CRDD/ProviderHomes/{codex|claude}`への配置契約を示す。将来の明示bootstrap Effectは、Windows既知フォルダーのローカルアプリデータ（Windows Known Folder local app data）からRootを独立して取得し、selected local user、handle／Identity、non-link／non-reparseおよびowner／DACLへ結合しなければならない。caller Pathとの文字列一致だけでOS由来、保護、AuthorityまたはCapabilityは成立せず、Pathも表示しない。

専用Provider Homeマウント許可（Provider Home Mount Grant）の現在Coreは、`prepared`、`issued`、`consumed`、`revoked`の構造、最長5分、使用上限1回、およびProvider・Profile・Operation・Provider Home Identity／保護状態・selected local userへのbinding候補を検証する。使用可能区間は`issuedAt <= observedAt < expiresAt`で、消費時刻も同じ半開区間に限り、`revoked` recordは観測時刻にかかわらず使用候補にならない。使用候補ではProfile／Authority contextが選ぶGrant参照候補と、使用直前のProvider Home Identity／保護状態／selected local user観測Hash候補をGrant recordへ完全一致させるが、これはcaller supplied plain data間の非Authority比較であり、Runtime所有観測の成立ではない。Runtime所有clock、atomic store、issuer、mount Adapterおよび終了時revocation Effectは未実装であるため、正しい候補でもMount Authorization、Filesystem Effect、Runtime AuthorityまたはOperation Capabilityを発行せず、token、Credential、session内容またはPathを返さない。

Claude実行計画候補（Claude Execution Plan Candidate）は、最初の外部ExecutorをClaude Codeへ限定する。公式release manifestのLinux x64 binary `2.1.220`、upstream commit、SHA-256、byte長および固定絶対path `/opt/crdd/providers/claude/2.1.220/claude`を一つのartifact Identity候補として結合し、固定image digest、manifest署名、exact versionでのargv互換性、利用条件、専用Provider Home、Egressおよび認証が完成するまで起動しない。binary配布物の条件と認証済みservice／アカウントの適用条件は別axisであり、Pro／Maxをindividual offering候補、Team／Enterpriseをorganization offering候補として列挙しても、適用条件、自動subscription利用許可、選択アカウントまたは人間の権限確認は成立しない。最初のprobe argvは`--bare`、JSON出力、turn上限1、session非永続、built-in tool 0要求、MCP tool拒否要求、slash command無効およびplan permissionへ固定するが、現在はそのexact versionでの制約成立も未検証である。親process環境は継承せず、Runtime所有の`HOME`、`TMPDIR`、`HTTPS_PROXY`と固定3変数だけで完全置換する必要があるが、そのAdapterは未実装である。user／project／local／managed settingsとProvider Home settingsの分離も未検証であり、任意prompt、PATH lookup、workspace mount、resumeまたはHost customizationを受理しないという要求を実測済みと読み替えない。実行時にはProviderへの外部要求と既存subscription利用枠の消費が起こり得る。API key、Console API account、第三者API Provider、追加credit購入および自動plan切替は許可しない。現在は非実行候補であり、read-onlyという名称をNetwork Effect、課金Effect、認証済み、追加費用不要またはOperation Capabilityの成立へ読み替えない。

現在のFake境界は三層に分かれる。合成Fake観測候補（Synthetic Fake Observation Candidate）のpure Coreはprocessを実行せず、caller claimを非Authorityの`candidate`へ限定する。明示`doctor --isolation`の動的Fakeは診断用Docker／一時Filesystem Effectを伴い、固定Docker Desktop Linux Engine上の正常scenarioについて、Fake限定結果、container／process tree不存在およびHost cleanupを同じrunで確認済みである。専用verificationでは同じ固定経路のtimeout、出力上限超過、不正結果およびnonzero exitに加え、固定Fakeへの`SIGTERM`取消要求、Fakeの受領確認と終了、ホスト側Docker CLI attachプロセス（Host Docker CLI Attach Process）のclose、container不存在およびHost cleanupを実Dockerで確認済みである。通常の`doctor --isolation`は同期経路のままで実行中取消を受理せず、実Provider取消、任意signal、意図的なcleanup失敗または残存containerの実測は`Not Verified`のままである。通常の`doctor`は動的Fakeを発火しない。実Codex／Claude Code、専用Homeの保護Effect／観測、実login、Provider endpoint限定Egressおよび実Provider起動は未実装で、spawn前に`blocked`となる。
...
`doctor --json`はprivate `reportVersion: 6`だけを生成し、revision 5以前のaliasまたはfallbackを持たない。Repository内にproduction decoder／consumerはなく、contract testはproducer schemaのexact assertionであってRuntime consumerではない。入力CLI grammar、公開Checkerおよび採用Repositoryの公開Schemaは変更していない。Mount Grantライフサイクル契約追加とprivate移行の現在正本は[CHG-000029](../../90_Release/Changes/CHG-000029_Provider_Home_Mount_Grant_Lifecycle_Foundation.md)であり、専用Home配置・保護基盤の履歴は[CHG-000026](../../90_Release/Changes/CHG-000026_Provider_Home_Protection_Foundation.md)に保持する。
...
承認済みのプラットフォーム準備方針は、CRDD配布物の`tools/coordinator`に内包するprivate packageのプラットフォーム準備処理（Platform Provisioner）を明示`provision`からだけ呼び出し、初回setupだけに管理者操作を集約する目標である。package manifestのexact object、Ed25519署名、package content root、固定公開鍵、Policy Identity、canonical loader、Release Identity照合およびRust成果物の署名済みHash結合は実装済み候補である。Windows v1で許可するRuntime主体は明示provisionで選択したローカル対話ユーザー1名だけだが、現在実装したのは現在process `TokenUser`のIdentity Hash観測だけで、選択userとのbinderは未実装である。別資格情報による昇格など一致を確認できない場合は処置前に`blocked`とする。Rust wire revision 2は現在tokenのuser SIDを外へ出さずdomain-separated Hashへ変換し、選択済みRuntime modeを発行しない。サービスアカウントは将来候補に限り、v1では未実装で`blocked`とする。WindowsのDACL構造claim評価はcaller claimをPolicyと比較するpure候補に限り、ACE集計や主体Hashだけから実効権限を成立させない。明示Release署名commandだけがmanifestをstaging Rootへ配置するが、DACL設定・再検証、native durable有効ポインター切替、保護済みactive、検証済み実行イメージおよびプロセス起動は未実装である。`provision`は最初のRuntime／Platform Provisioning Filesystem Effect前に停止し、権限変更、staging配置またはactive state保存を行わない。結果はfile Path、生byte、descriptor、生SID、ACLまたは署名を公開しない。
...
Profile revision 2はProvider、Operation、subscription OAuth、許可済みAuthority Registryを選ぶためのAuthority Grant参照候補、専用Provider Homeマウント許可（Provider Home Mount Grant）参照候補、および要求されたHTTPS Originの完全一致集合だけを保持する。専用Homeマウント許可は対象Provider・Profile・Operationに結合した保護済み専用Homeのhandle／mount許可だけを表し、token、session、PathまたはCredential値を含めない。Operation終了時に失効させるのは許可、handleおよびmountであり、永続HomeまたはOAuth sessionではない。旧revision 1のgeneric Credential Grantと`credential_broker`はsubscription OAuth経路から削除し、alias／fallbackせず拒否する。構造検証結果は`candidate`であり、人間承認、許可発行、Authority成立、`accepted`、`confirmed`または実行可能の別名ではない。正規化後のProfile Hashは要求候補の同一性だけを固定し、Authorityの証明には使用しない。
...
Authority Grant Verifier Core候補は、Authority Registry revision 2候補を固定契約、内容版Registry revision、UTC観測時刻、Grant集合およびSHA-256へ正規化する。Profile revision 2候補との照合では、Authority Grant参照と専用Homeマウント許可参照を別namespaceに保ち、Grantのactive状態、有効期間、Provider、Profile、Operation、要求Origin、ScopeおよびProfile Hashの完全一致を要求する。専用Homeマウント許可の`grantIssued:false`と`verification:not_implemented`は要求候補であり、許可の発行または失効確認を表さない。構造と内容の照合結果は`candidate`であり、自己申告Registryを信頼済み正本へ昇格させない。
