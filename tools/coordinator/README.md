...
オンボーディング準備状態の投影は、共有Authority RootをPlatform単位、Runtime RootをRepository単位のactivation前提として区別し、現在の阻害依存を一覧化するだけである。Platform ProvisionerのTrust検証とEffect、Provisioning記録の契約と検証、Authority Root resolver、OS別Root Protection Adapter、両Root Provisioning Effect、activation Effect、Path Identity結合、原子的永続化およびrun-scoped Capabilityのいずれも未実装であるため、準備状態は`blocked`である。この一覧は実装順、Provisioning成立、AuthorityまたはCapabilityではなく、外部入力、Receipt Schema、Manifest Schema、PathまたはEffectを追加しない派生表示である。
