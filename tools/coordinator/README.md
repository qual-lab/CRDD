...
`coordinator provision`は将来の唯一のPlatform Provisioning Effect入口としてgrammarだけを実装した。version 1の配布単位は専用EXEではなく`.mjs`を含むnpm packageとし、OSネイティブコード署名を必須条件にしない。現ローカル／開発buildではnpm registry署名、生成元証明（provenance）、Runtime所有release Trust、インストール済みpackageのFilesystem Identity確認およびEffect Adapterが未実装なため、常にdry-run相当の安全要約を返して`blocked`となり、Root、鍵またはACLを変更しない。`activate`、`doctor`または通常runからこの入口を暗黙発火しない。
...
承認済みのプラットフォーム準備方針は、Coordinatorのnpm packageに含まれるプラットフォーム準備処理（Platform Provisioner）を明示`provision`からだけ呼び出し、有効な準備Identityが維持されるPlatform scopeでは初回setupだけに管理者操作を集約する目標である。配布版はnpm registry署名、生成元証明、Qual-Lab署名済みpackage manifest、およびRuntimeが再取得するpackage内容／Filesystem Identityの全条件をEffect前に要求する。どれか一つの欠落、未知署名、生成元不一致、改変、権限不一致または別sourceへのfallbackでは`blocked`とする。package manifestのexact object、Ed25519署名、package file一覧のcontent rootおよびcaller supplied観測を束ねるpure一致検査は実装済み候補だが、Runtime所有のregistry／provenance検証、release Trust選択、package file読取り／Identity確認およびEffect controllerは未実装である。専用native executableやWindows／macOSの有料コード署名証明書はversion 1の必須条件ではない。ローカルsource checkoutはdry-runとtest専用で、Trust、GateまたはFilesystem Effectを成立させない。昇格、鍵生成、ACL変更および保存は将来の明示`provision`だけが発火できる目標である。

package Trust Gateのpure集約候補はmanifestを内部で再検証し、npm registry署名、生成元repository、同じpackage content root、安定file Identityおよびpermission policy一致を一つの観測契約へ結ぶ。ただし観測値はcaller suppliedであり、正常一致も`effectAuthorizationIssued: false`である。Runtime所有Adapter、release Identity選択およびpackage Filesystem確認が同じ制御経路を所有するまで、この結果を別入口へ渡してEffectを許可できない。
