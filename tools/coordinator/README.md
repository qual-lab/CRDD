...
共通Root保護方針Core候補（Root Protection Policy Core Candidate）は、WindowsのDACLとmacOS／Linuxのowner／modeを別々の正本にせず、同じ保護結果へ写像する。Runtime Rootは選択された利用者またはサービス実行主体を`runtime_principal_only`のwriterとして読取り／書込みでき、非承認主体が書き込めないことを要求する。Authority Rootは承認済みadmin／installerの集合を表す`provisioner_principal_only`だけが書込み、Runtime主体は読取り専用で、非承認主体が書き込めないことを要求する。どちらも事前Provision済みの既存non-link Rootと安定Identityを必要とし、Runtime自身はRoot作成や権限変更を行わない。`local`と`persistent_volume`だけを候補とし、network、removable、specialまたは分類不明のFilesystemはfail closedにする。pure Coreへ直接渡す観測値はcaller supplied claimなので単独では実保護を成立させない。一方、Windows Adapter候補は固定drive上の既存non-link Rootについて安定IdentityとDACLを読み取り専用で観測し、同じCoreへ閉じたsnapshotを渡す。返却する`requiredWriteAuthority`は照合したPolicy要求であり、Windows実測候補もAuthorityまたはCapabilityではない。Path、SID、UID、GID、mode、DACLまたはraw errorを返さず、POSIX観測、activation、Capability、ProviderまたはOperationを成立させない。
...
Authority Root結合候補は、明示選択した絶対Path、観測したRoot Identity Hashおよび保護policy Hashを署名済みRecord内の3値と完全一致させる。Windowsでは、検索票が指す固定drive上のnon-link Rootを読み取り専用で実観測し、検索票、永続Trust、current Recordおよび実測2 Hashを同じ候補へ結合する。結果にはRecord Hashと一致状態だけを含め、絶対Path、SIDまたはACLを公開しない。これはWindowsの選択済みRootに対する実測結合候補であり、POSIX探索、初期Trust、権限変更、activationまたはAuthority発行を代替しない。
