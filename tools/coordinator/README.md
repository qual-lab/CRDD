...
承認済みのプラットフォーム準備方針は、公式署名済みのプラットフォーム準備ツール（Platform Provisioner）をCoordinatorと共に配布し、有効な準備Identityが維持されるPlatform scopeでは初回setupだけに管理者操作を集約する目標である。ローカル実行のRuntime principalは選択利用者、server実行は専用サービスアカウント（service account）とし、どちらも既存Root Protection Policyの排他的writer条件を上書きしない。共有Authority Rootは、将来の署名・Trust検証済み準備記録（Provisioning Record）へ結合された明示Pathだけを通常runで再利用する。準備記録はPlatform scopeのRuntime向け信用判断を担う中心成果物とし、Provisioner Identityと署名metadataを一つのJCS payloadへ結合する。署名包絡（signature Envelope）はpayloadと複数署名を分離し、署名値を署名対象へ循環包含しない。CRDD固有の署名領域分離（domain separation）、鍵識別子（key ID）、準備記録、署名包絡、信頼起点鍵集合（Trust Anchor Set）、失効一覧（Revocation Manifest）および集約署名検証（aggregate signature verification）のrevision 1 pure Core候補を実装した。Provisioning Receiptと独立helper Manifestを別のRuntime Authority成果物として要求せず、Authority File Bundle Manifestは既存の別成果物として維持する。

準備記録のpure Core候補は、RFC 8785 JCS、RFC 8410 Ed25519 SPKI DER、RFC 8032 Ed25519およびRFC 4648のpaddingなしcanonical base64urlを組み合わせる。署名messageは固定ASCII prefix `CRDD\0PROVISIONING-RECORD\0V1\0`、payload JCS byte長の符号なし64-bit big-endian値、payload JCS bytesの順で構成する。鍵識別子はexact SPKI DERのSHA-256 lowercase hexadecimal 64文字である。準備記録はcanonical UTCの`issuedAt`より`expiresAt`が後で、その差が最大180日以内の場合だけ候補化する。集約評価では評価時刻が`issuedAt`以上かつ`expiresAt`未満であることを要求し、期限外または180日超過をfail closedで拒否する。この180日は準備記録の有効期間上限であり、鍵の有効期間、鍵切替期間または失効一覧の保持期間ではない。exact plain-data Schema、strict UTF-8、canonical byte完全一致、件数・深さ・node・byte上限を要求し、重複、未知、失効、不正または形式不明な署名entryを一件でも含めば全体を拒否する。ただし入力された鍵集合と失効一覧はまだQual-Lab同梱のRuntime所有Trustとして取得・rollback防止されたものではない。したがって正常結果も暗号条件の候補に限り、Filesystem、Authority、Capability、GateまたはOperationを成立させない。実鍵、端末enrollment、保存、resolverおよびLifecycleは未実装である。適用する外部規格の正本、節および採用境界は[脅威モデルの外部規格入力](THREAT_MODEL.md#provisioning-signature-external-standards)を参照する。

承認済みの端末導入鍵（installation key）方針は、Platform scopeごとのEd25519鍵をプラットフォーム準備ツールがOS管理の鍵保管境界で生成・保持し、その公開鍵へQual-Lab準備認証局（Qual-Lab Provisioning CA）が独自JCS JSON形式の登録証明書をEd25519で署名して発行する目標である。証明書のkey IDはexact SPKI DERのSHA-256 lowercase hexadecimal 64文字、有効期間は180日、更新開始は期限30日前、新旧併用期間は最大30日とする。検証済み自動更新が成功した場合は利用者／管理者操作を要求しない。期限切れ、失効、rollback、置換、不一致または検証不能では`blocked`として再Provisionへ戻し、更新失敗時にも別sourceへ無言でfallbackしない。WindowsはCNG/KSPのTPM-backed鍵を優先し明示的software KSPを代替候補、macOSはKeychainのSecure Enclave-backed鍵を優先しsoftware-backed Keychainを代替候補、LinuxはTPM 2.0を優先しroot-owned software keystoreを代替候補とする。初回setupで選択backendと確認できた保護強度を表示し、検証済み状態が有効な通常runでは再選択、管理者操作またはnetworkを要求しない。候補間のsilent fallback、同等強度の推定、秘密鍵materialのCoordinator Runtimeへの入出力または成果物化、Qual-Lab秘密鍵の製品・Repository・準備記録・持込み束への埋込みは禁止する。初回登録は明示オンラインまたは管理者持込みの署名済みオフライン初回登録束だけを候補とする。オンライン方式は一回限りのチャレンジ（one-time challenge）、ノンス（nonce）、プラットフォーム範囲（Platform scope）および端末導入公開鍵（installation public key）を結合する。オフライン束は登録要求ハッシュ（enrollment request hash）、登録証明書、準備認証局チェーン（Provisioning CA chain）、失効スナップショット（revocation snapshot）および束の期限（bundle expiry）を結合する目標である。束の署名者、署名対象、exact topologyおよび署名充足規則は未実装で、署名らしい値からTrustを推定しない。再送（replay）、別machine、別Platform scope、期限切れ入力は拒否し、片方の不成立から他方へfallbackしない。承認済みなのはこの形式、期間、backend優先候補および登録方式の役割までである。証明書、challenge、requestおよびbundleのexact field Schema／wire encoding／domain bytes、オフライン認証局チェーンのexact topology／署名充足、再送消費台帳（replay ledger）の保存形式、CA root／intermediateの実配布Lifecycleおよびplatform adapter実装は未決である。鍵生成、鍵保護確認、証明書codec／検証、認証局Trust／失効確認、自動更新、初回登録交換、replay永続化および準備記録との結合検証もすべて未実装である。この方針投影は入力、network、暗号鍵操作、Filesystem Effect、AuthorityまたはCapabilityを発行せず、既存12阻害依存と6件の現在run根拠を減らさない。実行するPlatform Provisioner実体の署名／Trust検証は`platform_provisioner_verification`、鍵生成・登録交換・更新Effectは`platform_provisioner_effect`、証明書とオフライン束の契約は`provisioning_record_contract`、鍵保護・証明書・認証局Trust／失効・Record結合・replayの検証は`provisioning_record_verification`が将来所有し、実行物TrustとRecord署名者Trustを混同しない。
...
後続の一括承認により、従来の未決列挙のうち所有証明、challenge有効期間、CA運用期間、保存／recoveryおよびOS保護の質的規則を固定した。オンラインchallengeは10分有効で、端末秘密鍵による所有証明の検証成功後に一度だけ消費する。署名済みoffline bundleは7日有効で一度だけ利用する。準備認証局はoffline rootとonline issuing keyを分離し、issuing keyの最大期間365日、切替overlap 30日、失効情報freshness 24時間とする。Trust epoch／revisionとsame-revision Hashの単調floorを要求する。

保存目標は、Authority Root内の準備記録をimmutable content-addressed成果物とatomic current pointerへ分離し、Repository側のactivationと検索票を同じimmutable generationと単一pointerで可視化する。Authority Rootの確定を先行させ、cross-volume atomicityは主張しない。immutable file fsync、generation directory fsync、pointer temporary file fsync、pointer atomic replace、pointer parent directory fsync、再読取Identity確認の順を要求し、各fsync、atomic replace、再読取またはIdentity確認のfailure／unknown／mismatch、および結果分類不能では、利用可能な成果物と既存journalだけを保持して`blocked`とし、明示的回復を要求する。journal不存在または保持確認不能も`blocked`のままとし、推測rollback、自動retryまたは旧pointerへのfallbackを行わない。明示的回復の具体手順とEffectは未実装である。disableはinactiveな検索票を同じgenerationへ保持し、再有効化は新activation IDを要求する。

Root権限の変更主体は公式署名済みPlatform Provisionerだけとし、Runtime自身によるpermission mutationを禁止する。WindowsはRuntime Rootをruntime SID read/write、Authority Rootをprovisioner／承認admin write＋runtime SID read-onlyとし、継承と広範なwrite ACEを拒否する。POSIXはRuntime Rootをruntime UID owner／mode `0700`、Authority Rootをprovisionerまたはroot owner＋runtime read/traverse ACLとし、未承認group／other writeを拒否する。local相当の安定Identity、durable atomic replaceおよび同等ACLを実証できないnetwork／removable／special／unknown volumeは`blocked`とする。

未決として残るのは各成果物のexact field Schema／wire encoding／domain bytes、CA chainのexact topology／署名充足、replay台帳の保存形式、CA実配布LifecycleおよびOS／Filesystem Adapter実装である。現在の投影は入力、Network、Filesystem、鍵操作、AuthorityまたはCapabilityを発行せず、12 blockerと6 run根拠を維持する。
