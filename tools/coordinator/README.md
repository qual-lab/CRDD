...
Claude実行計画候補（Claude Execution Plan Candidate）は、最初の外部ExecutorをClaude Codeへ限定する。公式release manifestのLinux x64 binary `2.1.220`、upstream commit、SHA-256およびbyte長をexact候補として固定し、固定Digest imageへの格納、manifest署名確認、利用条件の人間による有効化、専用Provider Home、Egressおよび認証が完成するまで起動しない。最初のprobe argvは`--bare`、JSON出力、turn上限1、session非永続、built-in tool 0、MCP tool拒否、slash command無効およびplan permissionへ固定する。これは任意prompt、PATH lookup、workspace mount、resumeまたはcustomizationを受理しないが、実行時にはProviderへの外部要求と既存subscription利用枠の消費が起こり得る。API key、Console API account、第三者API Provider、追加credit購入および自動plan切替は許可しない。現在は非実行候補であり、read-onlyという名称をNetwork Effect、課金Effect、認証済み、追加費用不要またはOperation Capabilityの成立へ読み替えない。
