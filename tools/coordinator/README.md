...
現在実装したのはexact plain-data、上限、current platformのcanonicalな絶対Path文法、canonical JSONおよび内容Hashを検査するpureなCore候補だけである。検索票のFilesystem読取り／書込み、原子的永続化、resolver、Provisioning Record検証、Authority Root Identity検証およびactive activation結合は未実装で、検索票の存在、Pathらしい値または内容HashだけではAuthority、Capability、activationまたはオンボーディング準備を成立させない。将来の利用時も対象runで全Identityと保護を再確認し、不一致、改変、移動または別PCではfail closedで再Provision／再activationへ戻し、自動Path書換えを行わない。このCore候補は既存12阻害依存のいずれも解除しない。
