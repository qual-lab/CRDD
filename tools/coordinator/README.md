...
`<absolute-staging-root>`には対象Git Treeのblob byteを変換せず展開したCRDD配布物が必要である。Windowsを含め、Release archiveは`git -c core.autocrlf=false archive`相当で生成し、改行変換された作業Treeを流用しない。`releaseSequence`は1以上のsafe integerとしてReleaseごとに単調増加させ、同じ値を別Identityへ再利用しない。commandは同Rootの`tools/coordinator`を安定したnon-link fileとして再走査し、package content root、Root保護Policyおよび鍵保管Policyのcanonical SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。Release Identity候補は配布Root全体のGit Object IDをnon-linkの同一handle読取りから再計算し、後置manifestだけを除外したRoot Treeを署名済み`crddTree`へ一致させる。rollback floorのpure遷移は、低いSequenceと同一SequenceのIdentity差替えを拒否し、初回または増分では永続化要求だけを返す。Windowsのpackage書込み保護は、固定Windows PowerShell 5.1を読み取り専用で呼び出すDACL事前検査候補が、Rootの継承保護、全要素のowner、reparse point不在、および`SYSTEM`／machine Administrators以外へのwrite ACE不在をSIDとnumeric access maskで確認する。現時点ではrollback floor永続化、Runtime read／execute主体とのDACL結合、保護済み配置を作るEffect controllerが未実装であるため、manifest生成、暗号的一致またはDACL書込み保護候補だけでGate、Authority、Capability、統合またはReleaseは成立しない。実鍵による生成は、対象Sequence／Version／Commit／Tree、有効期間および配布ステージングRootをRelease判断で固定した後にだけ行う。
...
POSIX Runtime Root precheck入口候補は、明示opt-in時に評価するが、信頼できるFilesystem classifierが未実装のため、POSIXでもPath解決、`lstat`／`realpath`／`open`／`fstat`、process identityおよびmodeの観測前に固定理由で`blocked`へ閉じる。WindowsではこのRuntime Root入口はplatform未対応、非opt-inでは`not_evaluated`とする。別の同梱package Filesystem AdapterではWindowsの書込みDACL事前検査候補を実装したが、Runtime read／execute主体との結合、Root作成または権限変更は実装していない。現段階では`local`と`persistent_volume`を実体から識別できず、POSIX owner／mode観測、POSIX ACL確認およびpersistent volume Adapterも未実装である。Path、UID、GID、SID、modeまたはACEを返さず、Root作成、chmodまたはACL変更を行わず、いずれの入口候補もクロスプラットフォーム保護成立やRuntime有効化として扱わない。
