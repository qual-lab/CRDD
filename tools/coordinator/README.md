...
現在実装したのはexact plain-data、上限、current platformで対応する保守的なcanonical lexical subset、canonical JSONおよび内容Hashを検査するpureなCore候補だけである。Windowsは大文字drive letterのdrive-absolute形式だけを候補化し、UNC／device namespace、ADS、禁止文字、segment末尾dot／space、予約device basenameとその拡張子付き別名を拒否する。これはlexical候補判定だけで、Pathの存在、case／Unicode alias、link／reparse、Filesystem class、Identity、ownerまたはACLを確認しない。POSIXは既存のcanonical absolute lexical Path契約を維持する。検索票のFilesystem読取り／書込み、原子的永続化、resolver、Provisioning Record検証、Authority Root Identity検証およびactive activation結合は未実装で、検索票の存在、Pathらしい値または内容HashだけではAuthority、Capability、activationまたはオンボーディング準備を成立させない。将来の利用時も対象runで全Identityと保護を再確認し、不一致、改変、移動または別PCではfail closedで再Provision／再activationへ戻し、自動Path書換えを行わない。これらの未実装軸は同じlocator contract snapshotから既存のAuthority Root resolutionおよびactivation atomic persistence阻害依存へ結合し、Core候補だけで既存12阻害依存のいずれも解除しない。
