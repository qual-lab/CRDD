...
準備記録の署名基礎Core候補は、RFC 8785のJSON正規化方式（JCS）によるplain JSON値のcanonical byte化、RFC 8410のEd25519 SubjectPublicKeyInfo DERだけを受理する鍵素材検査、exact DERのSHA-256 digest、およびRFC 8032 Ed25519個別署名の暗号学的一致をpureに評価する。JCSはUnicodeを正規化せず、lone surrogate、非有限数、動的／非plain入力、循環および上限超過をfail closedにする。既存のactivation、Locator、Trust Policy、RegistryおよびFile Bundleのcanonical形式はJCSへ暗黙移行しない。個別署名の一致、SPKI digestまたはJCS HashだけではTrust Anchor、key ID、非失効、準備記録成立、AuthorityまたはCapabilityを意味しない。CRDD固有署名領域分離のexact framing、準備記録payload、複数署名Envelopeのexact Schema、Qual-Lab同梱公開鍵集合、明示失効記録のSchema、署名充足規則および統合Verifierは未実装である。適用する外部規格の正本、節および採用境界は[脅威モデルの外部規格入力](THREAT_MODEL.md#provisioning-signature-external-standards)を参照する。
