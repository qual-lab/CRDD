...
Authority Root検索票（Authority Root Locator）は、Repository直下の固定`.crdd-runtime/authority-root-locator.json`へ置き、Runtime Rootを外部overrideしても配置を移さない。専用Store候補は初回検索票をcanonical byteでpendingへ`fsync`し、原子的に配置して安定再読取りする。pendingが残る場合は通常読取りを停止し、明示復旧だけがtarget欠落時の適用または同一targetのpending除去を行う。既存検索票から別内容への遷移はactivation側の原子的更新が未実装なので推測上書きしない。Windows内部resolver候補は記録Pathのnon-link Directory Identityを処置前後で固定し、fixed drive、継承保護DACL、信頼済みowner、Runtime主体のrole別accessおよびwriter排他を読み取り専用で観測する。検索票、永続Trust、current Record、実測Identity HashおよびProtection Hashを完全一致させるが、Path、SID、ACL、raw recordまたはcanonical byteを公開しない。検索票はProvisioning Recordとは別成果物でそのHashだけを参照し、Provisioning Receiptまたは独立helper Manifestを必須Authority成果物として要求しない。

Root Identity観測成果物はWindowsのdevice ID、file IDおよびbirthtime nanosecondsを、Root保護観測成果物はfixed drive、Root role、Runtime主体Hash、role別accessおよびwriter排他を、それぞれ固有domain、uint64be JCS byte長、canonical payloadおよびSHA-256へ固定する。絶対Path、SIDおよびACL実値はHash成果物や公開結果へ含めない。Windows Adapterは観測前後の同一Directory Identityを再確認し、link／reparse、network等の非fixed drive、保護されないDACL、未信頼owner、Runtime Deny、role不一致writeまたは非承認writerを拒否する。POSIX観測、検索票とactivationの原子的遷移および実active activation結合は未実装であり、Windows候補一致だけではAuthority、Capability、activationまたはオンボーディング準備を成立させない。
