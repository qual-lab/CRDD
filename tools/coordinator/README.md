...
端末導入鍵（installation key）はOS管理境界で保持し、初回オンライン登録では30分のチャレンジ、登録要求の所有証明、登録証明書の署名とIdentity結合をpure Core候補で検査する。Challenge／Request／Certificateのexact object Schema、成果物別domain、JCS署名messageおよび数学的署名一致は実装済み候補である。raw byte decoder／transport、Runtime所有時計、一回消費台帳、CA Trust／失効、鍵生成、keystore、Network、Filesystem、Record結合、証明書更新およびオフライン束は未実装である。候補一致もAuthority、Capability、EffectまたはGateを開かず、既存12阻害依存と6件の現在run根拠を減らさない。
