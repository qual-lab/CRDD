...
専用Provider Homeマウント許可（Provider Home Mount Grant）の現在Coreは、`prepared`、`issued`、`consumed`、`revoked`の構造、最長5分、使用上限1回、およびProvider・Profile・Operation・Provider Home Identity／保護状態・selected local userへのbinding候補を検証する。許可Recordと遷移・使用評価はplain dataの非Authority候補に限り、Runtime所有clock、atomic store、issuer、mount Adapterおよび終了時revocation Effectは未実装である。このため正しい候補でもMount Authorization、Filesystem Effect、Runtime AuthorityまたはOperation Capabilityを発行せず、token、Credential、session内容またはPathを返さない。
...
`doctor --json`はprivate `reportVersion: 5`だけを生成し、revision 4以前のaliasまたはfallbackを持たない。Repository内にproduction decoder／consumerはなく、contract testはproducer schemaのexact assertionであってRuntime consumerではない。入力CLI grammar、公開Checkerおよび採用Repositoryの公開Schemaは変更していない。専用Home契約追加とprivate移行の正本は[CHG-000026](../../90_Release/Changes/CHG-000026_Provider_Home_Protection_Foundation.md)である。
