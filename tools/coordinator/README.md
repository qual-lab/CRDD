...
承認済みの端末導入鍵（installation key）方針は、Platform scopeごとのEd25519鍵をプラットフォーム準備ツールがOS鍵保管領域（OS keystore）、TPMまたはSecure Enclaveで生成・保持し、その公開鍵へQual-Lab準備認証局（Qual-Lab Provisioning CA）が短期証明書を発行する目標である。Qual-Lab秘密鍵をCoordinator、Repository、準備記録または持込み束へ埋め込まず、秘密鍵materialをCoordinator Runtimeの入力、出力または成果物にしない。初回登録（enrollment）は明示的なオンライン登録または管理者が持ち込むオフライン初回登録束（offline enrollment bundle）に限り、失敗時に別方式へ無言でfallbackしない。検証済み登録へ結合された公開鍵だけを将来の準備記録署名鍵候補とし、通常runは検証済み登録状態が維持される間networkを要求しない。鍵、証明書、認証局Trust、失効、Platform scopeまたは結合が未知、期限切れ、失効、置換、不一致もしくは再検証不能なら`blocked`として再Provisionへ戻し、自動回復しない。現在は鍵生成、鍵保護確認、証明書契約／検証、認証局Trust／失効確認、初回登録交換および準備記録との結合検証がすべて未実装で、証明書形式、認証局署名方式、短期の具体値、challenge／attestation、持込み束Schema、保存LifecycleおよびOS backendも未決である。この方針投影は入力、network、暗号鍵操作、Filesystem Effect、AuthorityまたはCapabilityを発行せず、既存12阻害依存と6件の現在run根拠を減らさない。
