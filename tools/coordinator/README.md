...
`<absolute-staging-root>`には対象Git Treeのblob byteを変換せず展開したCRDD配布物が必要である。Windowsを含め、Release archiveは`git -c core.autocrlf=false archive`相当で生成し、改行変換された作業Treeを流用しない。`releaseSequence`は1以上のsafe integerとしてReleaseごとに単調増加させ、同じ値を別Identityへ再利用しない。commandは同Rootの`tools/coordinator`を安定したnon-link fileとして再走査し、package content root、Root保護Policyおよび鍵保管Policyのcanonical SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。Release Identity候補は配布Root全体のGit Object IDをnon-linkの同一handle読取りから再計算し、後置manifestだけを除外したRoot Treeを署名済み`crddTree`へ一致させる。rollback floorのpure遷移は、低いSequenceと同一SequenceのIdentity差替えを拒否し、初回または増分では永続化要求だけを返す。Windows DACL事前検査候補は、固定Windows PowerShell 5.1を読み取り専用で呼び出し、Rootの継承保護、全要素のowner、reparse point不在、`SYSTEM`／machine Administrators以外へのwrite ACE不在、および選択したRuntime主体の全要素read／execute成立とwrite／Deny ACE不在をSIDとnumeric access maskで確認する。Runtime主体は既定でsetup processのWindows Identity、サービス運用では明示service SIDとする。SID、Path、ACEおよびraw errorは公開しない。実装、設定、試験および相対レイアウトはRepository内を正本とし、端末固有の導入状態だけを`%ProgramData%\Qual-Lab\CRDD\Coordinator`配下の`releases\<releaseSequence>`と`state`へ置く。秘密鍵、実導入状態、旧配置aliasまたはshimをRepositoryへ含めない。現時点ではrollback floor永続化と保護済み配置を作るEffect controllerが未実装であるため、manifest生成、暗号的一致またはDACL候補だけでGate、Authority、Capability、統合またはReleaseは成立しない。実鍵による生成は、対象Sequence／Version／Commit／Tree、有効期間および配布ステージングRootをRelease判断で固定した後にだけ行う。
