...
実際の有効化は、診断とは別の専用`activate`操作でRepository単位に永続化する方針である。Runtime Rootには固定名`activation.json`を置く候補とし、Repository、Root、Authority Bundle、Trust PolicyおよびRegistryのIdentity／revision／Hashをcanonical recordへ結合する。Bundle、PolicyまたはRegistryのIdentityが変わった場合は古い有効化を自動流用せず、再activationを要求する。現在はrecordの構造／canonical byte Core候補までで、原子的書込み、Path／owner／ACL照合、run-scoped Capabilityおよび専用CLI Effectは未実装である。
