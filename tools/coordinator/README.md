...
`<absolute-staging-root>`には対象Git Treeから展開したCRDD配布物が必要である。commandは同Rootの`tools/coordinator`を安定したnon-link fileとして再走査し、package content root、Root保護Policyおよび鍵保管Policyのcanonical SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。現時点ではCRDD Release Identity loader、Windows DACL AdapterおよびEffect controllerが未実装であるため、manifest生成や暗号的一致だけでGate、Authority、Capability、統合またはReleaseは成立しない。実鍵による生成は、対象Version／Commit／Tree、有効期間および配布ステージングRootをRelease判断で固定した後にだけ行う。
