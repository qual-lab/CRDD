...
承認済みのプラットフォーム準備方針は、公式署名済みのプラットフォーム準備ツール（Platform Provisioner）をCoordinatorと共に配布し、有効な準備Identityが維持されるPlatform scopeでは初回setupだけに管理者操作を集約する目標である。ローカル実行のRuntime principalは選択利用者、server実行は専用サービスアカウント（service account）とし、どちらも既存Root Protection Policyの排他的writer条件を上書きしない。共有Authority Rootは、将来の署名・Trust検証済み準備記録（Provisioning Record）へ結合された明示Pathだけを通常runで再利用する。準備記録はPlatform scopeのRuntime向け信用判断を担う中心成果物とし、Provisioner Identityと署名metadataをRecordへ結合し、セキュリティ上重要な全項目を一つのcanonical JSON署名対象へまとめる目標である。payloadと複数署名は分離Envelopeへ置き、署名値自体を循環して署名対象へ含めない。署名値そのもののencodingはpaddingなしcanonical base64urlとして実装済み候補である。未決／未実装なのは、Envelope全体のexact wire Schema、field名と配置、payloadおよびkey ID等の署名値以外のfield encoding、CRDD固有domain framing、ならびに1件以上の既知・非失効・有効署名を要求し、重複・未知・失効・不正・形式不明を全体拒否する承認済み目標方針のaggregate実判定である。Provisioning Receiptと独立したhelper Manifestを別のRuntime Authority成果物として要求せず、補助文書が将来存在しても準備記録を代用しない。Authority File Bundle Manifestは既存の別成果物であり、準備記録と同一化または流用しない。Trust AnchorはCoordinatorと共に配布するQual-Lab公開鍵集合を将来使用し、複数key ID、切替時の重複期間および明示失効による旧鍵拒否を必須方針とする。

準備記録の署名基礎Core候補は、RFC 8785のJSON正規化方式（JCS）によるplain JSON値のcanonical byte化、RFC 8410のEd25519 SubjectPublicKeyInfo DERだけを受理する鍵素材検査、exact DERのSHA-256 digest、RFC 8032 Ed25519個別署名の暗号学的一致、およびRFC 4648のpaddingなしcanonical base64urlで表したexact 64-byte署名の厳格な内部復号をpureに評価する。JCSはUnicodeを正規化せず、lone surrogate、非有限数、動的／非plain入力、循環および上限超過をfail closedにする。署名文字列はpadding、非alphabet文字、長さ不一致または非canonical pad bitを処置前に拒否し、復号した署名byteや入力文字列を公開結果へ出さない。既存のactivation、Locator、Trust Policy、RegistryおよびFile Bundleのcanonical形式はJCSへ暗黙移行しない。個別署名の一致、SPKI digestまたはJCS HashだけではTrust Anchor、key ID、非失効、準備記録成立、AuthorityまたはCapabilityを意味しない。将来の複数署名評価は、1件以上の既知・非失効・有効署名を必要とし、重複、未知、失効、不正または形式不明なentryを1件でも含む場合は全体をfallbackなしで拒否する方針である。ただしCRDD固有署名領域分離のexact framing、key ID encoding、準備記録payload、複数署名Envelopeのexact Schema、Qual-Lab同梱公開鍵集合、明示失効記録のSchema、署名充足規則の実判定および統合Verifierは未実装である。適用する外部規格の正本、節および採用境界は[脅威モデルの外部規格入力](THREAT_MODEL.md#provisioning-signature-external-standards)を参照する。
