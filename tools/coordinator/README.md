...
端末導入鍵（installation key）はOS管理境界で保持し、初回オンライン登録では30分のオンライン登録チャレンジ（online enrollment challenge）、登録要求（enrollment request）の所有証明、登録証明書（enrollment certificate）の署名とIdentity結合をpure Core候補で検査する。チャレンジ／登録要求／登録証明書のexact object Schema、成果物別domain、JCS署名messageおよび数学的署名一致は実装済み候補である。署名前payloadのcanonical raw byte decoderに加え、登録要求と登録証明書の`payload`とEd25519署名exact 1件を分離するobject Envelope、およびEnvelope全体を上限131072 byteのcanonical JCS UTF-8として受理するraw byte decoderも実装済み候補である。独自header／length prefixを付けず、入力bytesと再生成canonical bytesの完全一致を要求する。transport、Runtime所有時計、一回消費台帳、CA Trust／失効、鍵生成、keystore、Network、Filesystem、Record結合、証明書更新およびオフライン束は未実装である。decoder候補や数学的一致もAuthority、Capability、EffectまたはGateを開かず、既存12阻害依存と6件の現在run根拠を減らさない。
...
後続の人間判断により、一回限りのオンライン登録チャレンジ（online enrollment challenge）は発行時から30分有効とする。nonce、installation public key、Platform scopeおよび登録要求へ結合し、最初の検証試行が成功でも失敗でも消費して再利用しない。期限切れは`blocked`とし、新しいchallengeを要求するが、offline方式へ自動fallbackしない。この期限は登録通信だけに適用し、通常runや利用者への細切れの指示を発火させない。端末秘密鍵による所有証明を要求し、登録要求object Envelopeはpayloadの端末導入鍵IDと一致するEd25519署名exact 1件だけを受理する。登録証明書object Envelopeも準備認証局候補鍵に結合する署名exact 1件だけを受理するが、その鍵のRuntime所有Trustは未実装である。両Envelopeのcanonical JCS UTF-8 raw byte decoderは実装済み候補であるが、transportと消費台帳は未実装である。署名済みoffline bundleは7日有効で一度だけ利用する。準備認証局はoffline rootとonline issuing keyを分離し、issuing keyの最大期間365日、切替overlap 30日、失効情報freshness 24時間とする。Trust epoch／revisionとsame-revision Hashの単調floorを要求する。
...
初回オンラインのチャレンジ／登録要求／登録証明書についてはexact object Schema、成果物別domainおよびJCS署名messageを実装済み候補とする。署名前payloadのcanonical raw byte decoder、登録要求／登録証明書のexact object Envelope、および両Envelope全体のcanonical JCS UTF-8 raw byte decoderも実装済み候補である。未決または未実装として残るのはtransport、更新・オフライン成果物とCA chainのexact topology／署名充足、replay台帳の保存形式、CA実配布LifecycleおよびOS／Filesystem Adapter実装である。現在の投影は入力、Network、Filesystem、鍵操作、AuthorityまたはCapabilityを発行せず、12 blockerと6 run根拠を維持する。
