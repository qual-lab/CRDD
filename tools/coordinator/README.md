...
承認済みのプラットフォーム準備方針は、公式署名済みのプラットフォーム準備ツール（Platform Provisioner）をCoordinatorと共に配布し、有効な準備Identityが維持されるPlatform scopeでは初回setupだけに管理者操作を集約する目標である。ローカル実行のRuntime principalは選択利用者、server実行は専用service accountとし、どちらも既存Root Protection Policyの排他的writer条件を上書きしない。共有Authority Rootは、将来の検証済み準備記録（Provisioning Record）へ結合された明示Pathだけを通常runで再利用する。現在の選択は引き続きCLI、環境の順であり、記録resolverの優先順やSchemaを今回追加しない。各runでは準備記録とProvisioner Trust、Authority／Runtime RootのIdentityと保護、Runtime principal、Repositoryに結合したactiveなactivation、およびhelper／署名／保護metadataの不変性を再確認する。12件の実装依存が十分条件を満たすことと、この現在run根拠の確認を両方満たした場合だけオンボーディング準備状態を`ready`候補にできる。現在は十分値、resolver、検証器およびready遷移が未実装なので`blocked`であり、この方針自体はAuthorityまたはCapabilityを付与しない。
