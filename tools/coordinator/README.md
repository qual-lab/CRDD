...
`state/release-floor.json`と`state/active-release.json`の専用Storeは、canonical UTF-8、同一handle再読取り、pending fileの`fsync`、原子的置換および置換後再読取りを実装済み候補とする。rollback floorは単調増加だけを許し、有効世代ポインターは署名済みRelease Identity、package content rootおよび同Sequenceのfloor Hashを結合する。Windowsがparent directoryの`fsync`を提供しない場合はその事実を結果へ保持し、pending stateと再読取りを省略して成功扱いにしない。端末状態はRepositoryへ複製せず、旧state aliasも作らない。両状態ファイルをまたぐクラッシュ整合とRuntime側の有効世代読取りは後続の状態transactionへ接続するまで未完了であり、単独Storeや配置成功だけでGate、AuthorityまたはCapabilityを成立させない。
...
オンボーディング準備状態の投影は、共有Authority RootをPlatform単位、Runtime RootをRepository単位のactivation前提として区別し、現在の阻害依存を一覧化するだけである。Platform ProvisionerのTrust検証とWindows導入Effectは実装済み候補へ進んだが、候補値を十分条件とする承認はなく、Provisioning Recordの契約／Lifecycleおよび署名／鍵／失効／読取り検証、Authority Root resolver、残るOS別Root Protection Adapter、両Root Provisioning Effect、状態transaction、Runtime有効世代読取り、activation Effect、Path Identity結合、原子的永続化およびrun-scoped Capabilityが未完了なので準備状態は`blocked`である。阻害一覧と公開状態は同じ実装状態snapshotから派生し、候補値、未知値または一部実装を十分条件として扱わない。将来阻害一覧が空になっても、明示的なready遷移が別途承認・実装されるまでは`ready`と推定しない。この一覧は実装順、Provisioning成立、AuthorityまたはCapabilityではなく、外部入力、Provisioning Record Schema／署名Envelope、独立Receipt／helper Manifest Authority Schema、PathまたはEffectを追加しない派生表示である。
