...
`<absolute-staging-root>`には対象Git Treeのblob byteを変換せず展開したCRDD配布物と、固定Path `90_Release/platform-access/x86_64-pc-windows-msvc/crdd-platform-access.exe`へ配置したlocked release build成果物が必要である。Windowsを含め、Release archiveは`git -c core.autocrlf=false archive`相当で生成し、改行変換された作業Treeを流用しない。`releaseSequence`は1以上のsafe integerとしてReleaseごとに単調増加させ、同じ値を別Identityへ再利用しない。commandは同Rootの`tools/coordinator`とRust成果物を安定したnon-link fileとして再走査し、package content root、Root保護Policy、鍵保管Policy、Rust target／protocol／toolchain／byte長／SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。Release Identity候補は配布Root全体のGit Object IDをnon-linkの同一handle読取りから再計算し、後置manifestと後置Rust成果物を除外したRoot Treeを署名済み`crddTree`へ一致させる。rollback floor、active releaseおよびstate transactionのpure／Store component候補は残す。署名manifestへ結合するRust成果物の観測・署名候補は実装したが、保護済み有効世代と検証済み実行imageの結合、および上限付きプロセス（Bounded Process）による起動は未実装である。現在はPath入力やprocess起動より前に`blocked`となる。Root観測への接続、DACL適用、世代配置、Effect controllerおよびRuntime readerも未実装である。署名commandの正常終了は観測区間中のmanifestと成果物の一致候補だけを示し、Release採用、継続的同一性または実行許可を与えない。実鍵によるmanifest生成後も、保護済み有効世代と検証済み実行image方式が判断・実装・検証されるまでproduction handoffを`blocked`とする。

## 現在利用できるコマンド

`state/release-floor.json`と`state/active-release.json`のcodec、専用Storeおよび`state/provision-transaction.json`のcomponent候補は、canonical UTF-8、pending fileの`fsync`、原子的置換、置換後再読取りおよび明示復旧を実装済み候補とする。ただしEffectからは到達不能であり、端末状態を作成・更新しない。署名manifestへ結合する成果物観測・署名候補はあるが、保護済み有効世代、検証済み実行image、process起動およびRuntime readerへのRoot観測／Protection接続は未実装である。readerはProgramData探索、transaction／floor／active／manifest／package読取りより前に`blocked`となる。
...
ローカル導入の目標UXは、機能を使わないRepositoryへRuntime固有Effectを発生させず、最初の有効なPlatform ProvisioningだけでOS権限確認と共有Authority Rootの準備を行い、その後はRepositoryごとの`coordinator activate`一回を入口とすることである。現在は署名済みPlatform Provisionerのpure検証、state component候補、Rust Coreの読み取り専用Windows access観測候補、および署名manifestへ結合する成果物観測・署名候補までである。保護済み有効世代、検証済み実行image、process起動、Root観測への写像、DACL適用、導入Effect、Runtime有効世代reader、Provisioning記録、共有Root作成およびactivation Effectは未実装である。そのため`provision`と`activate`は引き続き`blocked`である。

承認済みのプラットフォーム準備方針は、CRDD配布物の`tools/coordinator`に内包するprivate packageのプラットフォーム準備処理（Platform Provisioner）を明示`provision`からだけ呼び出す目標である。package manifestのexact object、Ed25519署名、package content root、固定公開鍵、Policy Identity、canonical loader、Release Identity照合およびRust成果物の署名済みHash結合は実装済み候補である。Rust Coreの現在process token観測もcomponent候補だが、保護済み有効世代、検証済み実行image、TypeScript process Adapter、Root観測への写像、DACL設定・再検証、Effect接続およびEffect controllerは未実装である。`provision`は最初のFilesystem Effect前に停止する。
...
準備記録のpure Core候補は、RFC 8785 JCS、RFC 5480 P-256 SPKI DER、ECDSA with SHA-256、固定64-byte IEEE P1363署名およびRFC 4648のpaddingなしcanonical base64urlを組み合わせる。専用Storeへ永続化したfloor、Trust成果物、current RecordおよびRuntime取得時刻を結ぶ集約検証も実装済み候補である。検索票resolverのnon-link Directory Identity、Repositoryとの相互非包含、永続Trustおよびcurrent Record結合、Rust観測Core、および署名manifestへ結合する成果物観測・署名処理はcomponent候補として実装済みである。保護済み有効世代、検証済み実行image、process起動、Root観測成果物への写像、完全Protection、初期Trust、POSIX観測およびactivationは未実装である。このため正常結果もAuthority、Capability、GateまたはOperationを成立させない。
...
準備記録の正本は共有Authority Rootと同じPlatform scopeへ置き、管理者／provisionerだけが書込み、Runtimeは読取り専用とする目標であり、Repositoryへ正本を複製しない。現在はimmutable Record、Trust成果物Store、current Record集約、Repository検索票との結合、Windows実効accessのRust観測Core、および署名manifestへ結合する成果物観測・署名候補までを実装した。保護済み有効世代、検証済み実行image、process起動、Root観測成果物への写像、Root削除可否、初期Trust、POSIX Adapter、完全Lifecycle、active activationおよびready遷移は未実装なので`blocked`である。

オンボーディング準備状態の投影は現在の阻害依存を一覧化するだけである。Trust／manifest／package、Rust Core、および成果物観測・署名処理はcomponent候補だが、保護済み有効世代、検証済み実行image、process起動、Root観測への写像、導入EffectおよびRuntime readerは未実装である。既存12 blocker、6 current-run evidenceおよびGate `blocked`を維持する。

Authority Root検索票（Authority Root Locator）は、Repository直下の固定`.crdd-runtime/authority-root-locator.json`へ置く。resolver候補はRepositoryとの相互非包含、non-link安定Identity、永続Trustおよびcurrent Record結合までを行う。Rust Coreと成果物観測・署名処理はcomponent候補だが、process起動、Root観測成果物への写像と実測Protection Hash結合は未実装で、観測済みRecord入口は`blocked`である。

Root Identity／Protection観測成果物のexact pure contract、domainおよびHash生成候補は実装済みである。Rust Coreは同一handleのRoot Identityを固定して現在process tokenをWindows `AccessCheck`へ渡す候補を持ち、TypeScript側は成果物観測・署名候補を持つ。ただしprocess起動とRoot観測成果物への写像は未実装である。component候補だけではProtection Hash、Authority、Capabilityまたはactivationを成立させない。
...
共通Root保護方針Core候補（Root Protection Policy Core Candidate）は、WindowsのDACLとmacOS／Linuxのowner／modeを同じ保護結果へ写像する。Rust Coreは現在process tokenの実効accessを観測するcomponent候補で、TypeScript側は署名manifestへ結合する成果物観測・署名候補までを実装する。保護済み有効世代、検証済み実行image、process起動、Root観測成果物への写像、writer排他判定、全tree観測およびProtection Hash結合は未実装であり、本番入口は常に`blocked`とする。
...
Authority Root結合pure候補は、明示選択した絶対Path、観測済みRoot Identity Hashおよび保護policy Hashを署名済みRecord内の3値と完全一致させる。Rust Coreと成果物観測・署名候補は存在するが、process起動、Root観測成果物への写像とProtection Hash生成が未実装のため観測済みRecord結合入口は`blocked`である。
...
Trust成果物Store候補は、信頼起点鍵集合と失効一覧をcanonical byteのimmutable成果物として保存する。Windowsでは検索票resolver、Repositoryとの相互非包含、安定Root IdentityおよびRecord Hash結合までを候補接続した。Rust Coreと成果物観測・署名処理はcomponent候補だが、process起動、Root観測成果物への写像と署名済み保護Hashへの実測結合は未実装である。
