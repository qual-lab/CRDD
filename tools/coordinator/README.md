...
`<absolute-staging-root>`には対象Git Treeのblob byteを変換せず展開したCRDD配布物が必要である。Windowsを含め、Release archiveは`git -c core.autocrlf=false archive`相当で生成し、改行変換された作業Treeを流用しない。`releaseSequence`は1以上のsafe integerとしてReleaseごとに単調増加させ、同じ値を別Identityへ再利用しない。commandは同Rootの`tools/coordinator`を安定したnon-link fileとして再走査し、package content root、Root保護Policyおよび鍵保管Policyのcanonical SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。Release Identity候補は配布Root全体のGit Object IDをnon-linkの同一handle読取りから再計算し、後置manifestだけを除外したRoot Treeを署名済み`crddTree`へ一致させる。rollback floorのpure遷移は、低いSequenceと同一SequenceのIdentity差替えを拒否し、初回または増分では永続化要求だけを返す。Windows DACL検査と設定の候補は、固定Windows PowerShell 5.1を呼び出し、Rootの継承保護、全要素のowner、reparse point不在、`SYSTEM`／machine Administrators以外へのwrite ACE不在、および選択したRuntime主体の全要素read／execute成立とwrite／Deny ACE不在をSIDとnumeric access maskで確認する。DACL変更は署名済み配布物を検証した明示`provision`から固定導入Rootだけへ発火し、変更後に全要素を再検証する。Runtime主体は既定でsetup processのWindows Identity、サービス運用では明示service SIDとする。SID、Path、ACEおよびraw errorは公開しない。実装、設定、試験および相対レイアウトはRepository内を正本とし、端末固有の導入状態だけを`%ProgramData%\Qual-Lab\CRDD\Coordinator`配下の`releases\<releaseSequence>`と`state`へ置く。秘密鍵、実導入状態、旧配置aliasまたはshimをRepositoryへ含めない。署名済み配布物の世代別配置、配置後のmanifest再検証、rollback floorおよびactive releaseの専用Store、DACL設定と再検証、および明示`provision`接続は実装済み候補である。これらはRuntime AuthorityやCapability、統合またはReleaseを成立させず、実鍵による生成は対象Sequence／Version／Commit／Tree、有効期間および配布ステージングRootをRelease判断で固定した後にだけ行う。

## 現在利用できるコマンド

`state/release-floor.json`と`state/active-release.json`の専用Storeは、canonical UTF-8、同一handle再読取り、pending fileの`fsync`、原子的置換および置換後再読取りを実装済み候補とする。rollback floorは単調増加だけを許し、有効世代ポインターは署名済みRelease Identity、package content rootおよび同Sequenceのfloor Hashを結合する。両状態は`state/provision-transaction.json`の永続intentで結び、明示`provision`がfloor、activeの順に確定して全Hashを再読取りし、途中停止時は次の明示`provision`が同じintentだけを復旧する。Windowsがparent directoryの`fsync`を提供しない場合もpending stateと再読取りを省略しない。端末状態はRepositoryへ複製せず、旧state aliasも作らない。Runtime側の読取り専用候補は未完了transactionの不在、floor／active一致、署名済み導入世代およびDACLを再検証するが、通常runのAuthority接続は未実装であり、単独Store、transactionまたはreader成功だけでGate、AuthorityまたはCapabilityを成立させない。
...
`coordinator provision`は唯一のPlatform Provisioning Effect入口として実装済み候補である。version 1はCRDD配布物に内包したprivate TypeScript sourceとして提供し、単独npm公開、個別install、専用EXEまたはOSネイティブコード署名を要求しない。内部Scriptは現行フォルダ配置のまま`.ts`であり、Node.js 24.12以上のnative TypeScript実行でCRDD配布物内の固定Pathからだけ起動する。`package.json`は実行commandを`bin`として公開しない。固定公開鍵による署名、CRDD Tree、package content rootおよびPolicy Identityの全一致を先に確認し、通常のsource checkoutではProgramData探索または書込みより前に`blocked`となる。検証済み配布物だけを世代別pendingへコピーし、同じ署名Manifestで再検証してから配置、DACL、rollback floorおよびactive releaseを更新する。`activate`、`doctor`または通常runからこの入口を暗黙発火しない。
...
準備記録の正本は共有Authority Rootと同じPlatform scopeへ置き、管理者／provisionerだけが書込み、Runtimeは読取り専用とする目標であり、Repositoryへ正本を複製しない。RepositoryのAuthority Root検索票は準備記録Hashだけを参照する信用前のヒントで、準備記録の代わりにならない。初回setupまたは再設定は明示CLI、通常runは検証済み準備記録と検索票の両方、環境変数は互換／自動化向けの明示overrideという役割に分ける。現在の実選択は引き続きCLI、環境の順であり、未実装resolverの厳密な優先状態機械を先取りしない。不一致、欠落、失効または再識別不能では別候補や低優先sourceへ無言でfallbackせず`blocked`とし、自動修復せず再Provisionへ戻す。実行するProvisioner実体の署名・Trust確認と、生成済みProvisioning Recordの署名検証は別責務とする。各runでは準備記録とProvisioner Trust、Authority／Runtime RootのIdentityと保護、Runtime principal、Repositoryに結合したactiveなactivation、およびPlatform Provisioner署名／Trust／保護metadataの不変性を再確認する。12件の実装依存が十分条件を満たすことと、6件の現在run根拠の確認を両方満たした場合だけオンボーディング準備状態を`ready`候補にできる。現在はimmutable RecordのFilesystem読取り／書込み、atomic current pointerと明示復旧、trust floorのpure遷移／専用Store、content-addressed Trust成果物Store、永続floorとTrust成果物を使ってcurrent RecordをRuntime取得時刻で検証する集約経路、およびRepository検索票をその検証済みcurrent Recordへ結ぶ候補経路が実装済みである。結合候補は検索票のRecord Hashと、Recordへ署名されたAuthority Root絶対Path／Identity Hashを一致させる。Runtime所有Trustの初期導入元、OSが実測するAuthority Root Identity／保護との結合、完全Lifecycle、十分値およびready遷移は未実装なので`blocked`であり、この方針自体はAuthorityまたはCapabilityを付与しない。

オンボーディング準備状態の投影は、共有Authority RootをPlatform単位、Runtime RootをRepository単位のactivation前提として区別し、現在の阻害依存を一覧化するだけである。Platform ProvisionerのTrust検証、Windows導入Effect、状態transactionおよびRuntime有効世代readerは実装済み候補へ進んだが、候補値を十分条件とする承認はなく、Provisioning Recordの契約／Lifecycleおよび署名／鍵／失効／読取り検証、Authority Root resolver、残るOS別Root Protection Adapter、両Root Provisioning Effect、readerの通常run Authority接続、activation Effect、Path Identity結合、activation側の原子的永続化およびrun-scoped Capabilityが未完了なので準備状態は`blocked`である。阻害一覧と公開状態は同じ実装状態snapshotから派生し、候補値、未知値または一部実装を十分条件として扱わない。将来阻害一覧が空になっても、明示的なready遷移が別途承認・実装されるまでは`ready`と推定しない。この一覧は実装順、Provisioning成立、AuthorityまたはCapabilityではなく、外部入力、Provisioning Record Schema／署名Envelope、独立Receipt／helper Manifest Authority Schema、PathまたはEffectを追加しない派生表示である。

Authority Root検索票（Authority Root Locator）は、Repository直下の固定`.crdd-runtime/authority-root-locator.json`へ置き、Runtime Rootを外部overrideしても配置を移さない。専用Store候補は初回検索票をcanonical byteでpendingへ`fsync`し、原子的に配置して安定再読取りする。pendingが残る場合は通常読取りを停止し、明示復旧だけがtarget欠落時の適用または同一targetのpending除去を行う。既存検索票から別内容への遷移はactivation側の原子的更新が未実装なので推測上書きしない。検索票はRepository、Runtime Root、Authority Root、Provisioning Recordおよび現在のactivation ID／revision／record Hashを結ぶ、信用前の検索ヒントに限る。内部resolver候補は記録Pathのnon-link Directory実体と安定性だけを確認し、記録済みIdentity Hashや保護を検証済みにしない。資格情報は含めないが端末固有の絶対Pathを含む保護対象metadataなので、Path、raw recordまたはcanonical byteをdoctor、通常／JSON出力、log、Evidence、Provider、OperationまたはCandidate Revisionへ出さない。検索票はProvisioning Recordとは別成果物でそのHashだけを参照し、Provisioning Receiptまたは独立helper Manifestを必須Authority成果物として要求しない。将来補助資料が存在してもRecordを代替せず、Authority File Bundle Manifestは既存の別成果物として維持する。

exact plain-data、上限、current platformで対応する保守的なcanonical lexical subset、canonical JSONおよび内容Hashを検査するpureなCore候補に加え、Repository固定位置の初回永続化、安定読取り、明示復旧、Root Directory実体までのresolver候補、および同じRoot内の永続Trustでcurrent Recordを再検証して検索票のRecord Hash／署名済みPath／署名済みIdentity Hashへ結ぶ候補を実装した。Windowsは大文字drive letterのdrive-absolute形式だけを候補化し、UNC／device namespace、ADS、禁止文字、segment末尾dot／space、予約device basenameとその拡張子付き別名を拒否する。lexical候補とresolver候補はPathのcase／Unicode alias、Filesystem class、OS実測のIdentity Hash、ownerまたはACLを確認済みにせず、POSIXは既存のcanonical absolute lexical Path契約を維持する。有効activation–検索票結合候補（Active Activation–Locator Binding Candidate）は、既存の初版activation候補と検索票候補について、Repository／Runtime Root Identity Hash、activation ID／revisionおよび再計算したactivation record Hashの5項目だけをpureに比較する。ここで`active`は入力recordの状態値だけを表し、実行時の有効性、AuthorityまたはCapabilityを意味しない。両者は将来同じ原子的更新で成立させる目標だが、現版は別内容への一部更新、Hash／Identity不一致または判定不能を`blocked`にして自動修復せず、再Provisionへ戻す関係だけを固定する。検索票の別内容への原子的遷移、OS実測のAuthority Root Identity／保護検証および実active activation結合は未実装で、検索票の存在、Root Directory実体、永続Trust／current Recordとの候補一致、内容Hashまたはpure比較一致だけではAuthority、Capability、activationまたはオンボーディング準備を成立させない。将来の利用時も対象runで全Identityと保護を再確認し、不一致、改変、移動または別PCではfail closedで再Provision／再activationへ戻し、自動Path書換えを行わない。これらの未実装軸は同じcontract snapshotから既存のAuthority Root resolutionおよびactivation atomic persistence阻害依存へ結合し、Core候補だけで既存12阻害依存のいずれも解除しない。Provisioning Receiptおよび独立helper Manifestは別のRuntime Authority成果物として要求せず、Authority File Bundle Manifestは別成果物のままであり、質的規則としてprivate-owned transaction、期待previous／next Hash、曖昧時の成果物保持＋`blocked`、inactive locator保持および新activation IDを承認済みとする。未決・未実装なのはexact transaction／journal Schema、ID形式、配置、具体的回復手順およびAdapter／Effectである。
...
POSIX Runtime Root precheck入口候補は、明示opt-in時に評価するが、信頼できるFilesystem classifierが未実装のため、POSIXでもPath解決、`lstat`／`realpath`／`open`／`fstat`、process identityおよびmodeの観測前に固定理由で`blocked`へ閉じる。WindowsではこのRuntime Root入口はplatform未対応、非opt-inでは`not_evaluated`とする。別の同梱package Filesystem AdapterではWindowsの書込み保護とRuntime read／executeを結合するDACL事前検査候補を実装したが、Root作成または権限変更は実装していない。現段階では`local`と`persistent_volume`を実体から識別できず、POSIX owner／mode観測、POSIX ACL確認およびpersistent volume Adapterも未実装である。Path、UID、GID、SID、modeまたはACEを返さず、Root作成、chmodまたはACL変更を行わず、いずれの入口候補もクロスプラットフォーム保護成立やRuntime有効化として扱わない。
...
Authority Root結合候補は、明示選択した絶対Path、観測したRoot Identity Hashおよび保護policy Hashを署名済みRecord内の3値と完全一致させる。結果にはRecord Hashと一致状態だけを含め、絶対Pathを公開しない。これは選択済みRootの結合検証であり、未実装のRoot探索、Filesystem Identity観測またはAuthority発行を代替しない。
