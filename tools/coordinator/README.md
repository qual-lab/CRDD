...
```powershell
npm run release-manifest:sign --prefix tools/coordinator -- --distribution-root <absolute-staging-root> --private-key C:\project\key\CRDD\crdd-release-v1-private.pem --crdd-version <vX.Y.Z> --release-sequence <positive-safe-integer> --crdd-commit <commit-id> --crdd-tree <tree-id> --issued-at <canonical-utc> --expires-at <canonical-utc>
```

`<absolute-staging-root>`には対象Git Treeのblob byteを変換せず展開したCRDD配布物が必要である。Windowsを含め、Release archiveは`git -c core.autocrlf=false archive`相当で生成し、改行変換された作業Treeを流用しない。`releaseSequence`は1以上のsafe integerとしてReleaseごとに単調増加させ、同じ値を別Identityへ再利用しない。commandは同Rootの`tools/coordinator`を安定したnon-link fileとして再走査し、package content root、Root保護Policyおよび鍵保管Policyのcanonical SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。Release Identity候補は配布Root全体のGit Object IDをnon-linkの同一handle読取りから再計算し、後置manifestだけを除外したRoot Treeを署名済み`crddTree`へ一致させる。rollback floorのpure遷移は、低いSequenceと同一SequenceのIdentity差替えを拒否し、初回または増分では永続化要求だけを返す。現時点ではその永続化、Windows DACL AdapterおよびEffect controllerが未実装であるため、manifest生成や暗号的一致だけでGate、Authority、Capability、統合またはReleaseは成立しない。実鍵による生成は、対象Sequence／Version／Commit／Tree、有効期間および配布ステージングRootをRelease判断で固定した後にだけ行う。
