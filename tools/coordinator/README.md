...
`<absolute-staging-root>`には対象Git Treeのblob byteを変換せず展開したCRDD配布物が必要である。Windowsを含め、Release archiveは`git -c core.autocrlf=false archive`相当で生成し、改行変換された作業Treeを流用しない。`releaseSequence`は1以上のsafe integerとしてReleaseごとに単調増加させ、同じ値を別Identityへ再利用しない。commandは同Rootの`tools/coordinator`を安定したnon-link fileとして再走査し、package content root、Root保護Policyおよび鍵保管Policyのcanonical SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。Release Identity候補は配布Root全体のGit Object IDをnon-linkの同一handle読取りから再計算し、後置manifestだけを除外したRoot Treeを署名済み`crddTree`へ一致させる。rollback floor、active releaseおよびstate transactionのpure／Store component候補は残すが、Windows実効アクセス確認（effective access verification）、DACL適用、世代配置、Effect controllerおよびRuntime readerは未実装である。DACL構造claimのpure評価だけでは保護成立を主張せず、`coordinator provision`は配布物、時刻、PathまたはFilesystemへ触れる前に`blocked`となる。SID、Path、ACEおよびraw errorは公開しない。実装、設定、試験および相対レイアウトはRepository内を正本とし、端末固有の導入状態、秘密鍵、旧配置aliasまたはshimをRepositoryへ含めない。実鍵によるmanifest生成は、対象Sequence／Version／Commit／Tree、有効期間および配布ステージングRootをRelease判断で固定した後にだけ行う。

## 現在利用できるコマンド

`state/release-floor.json`と`state/active-release.json`のcodec、専用Storeおよび`state/provision-transaction.json`のcomponent候補は、canonical UTF-8、pending fileの`fsync`、原子的置換、置換後再読取りおよび明示復旧を実装済み候補とする。ただしEffectからは到達不能であり、端末状態を作成・更新しない。Runtime readerもWindows実効アクセス確認が未実装のため、ProgramData探索、transaction／floor／active／manifest／package読取りより前に`blocked`となる。単独component候補をGate、AuthorityまたはCapabilityの成立根拠へ流用しない。
...
`coordinator provision`はPlatform Provisioningの明示command grammar候補であり、実Effectは未実装である。version 1の目標はCRDD配布物に内包したprivate TypeScript sourceとして提供し、単独npm公開、個別install、専用EXEまたはOSネイティブコード署名を要求しないことである。内部Scriptは現行フォルダ配置のまま`.ts`で、`package.json`は実行commandを`bin`として公開しない。現在はWindows実効アクセス確認が未実装のため、検証済み配布物であっても配布物、時刻、ProgramData、PathまたはFilesystemへ触れる前に固定理由で`blocked`となり、copy、DACL変更、state更新および復旧を発火しない。`activate`、`doctor`または通常runからこの入口を暗黙発火しない。
...
準備記録の正本は共有Authority Rootと同じPlatform scopeへ置き、管理者／provisionerだけが書込み、Runtimeは読取り専用とする目標であり、Repositoryへ正本を複製しない。RepositoryのAuthority Root検索票は準備記録Hashだけを参照する信用前のヒントで、準備記録の代わりにならない。初回setupまたは再設定は明示CLI、通常runは検証済み準備記録と検索票の両方、環境変数は互換／自動化向けの明示overrideという役割に分ける。不一致、欠落、失効または再識別不能では別候補や低優先sourceへ無言でfallbackせず`blocked`とし、自動修復せず再Provisionへ戻す。現在はimmutable Record、atomic current pointerと明示復旧、trust floor、content-addressed Trust成果物Store、Runtime時計によるcurrent Record集約、Repository検索票との結合、およびRoot Identity／保護HashをRecordの署名済み3値へ結ぶpure候補までを実装した。Windows実効access観測、初期Trustの承認済み導入元、POSIX Adapter、完全Lifecycle、active activation、十分値およびready遷移は未実装なので`blocked`であり、候補一致はAuthorityまたはCapabilityを付与しない。

オンボーディング準備状態の投影は、共有Authority RootをPlatform単位、Runtime RootをRepository単位のactivation前提として区別し、現在の阻害依存を一覧化するだけである。Platform ProvisionerのTrust／manifest／package component候補は実装済みだが、Windows実効アクセス確認、導入EffectおよびRuntime有効世代readerは未実装である。これらは既存の`platform_provisioner_verification`／`platform_provisioner_effect`阻害依存へ残り、第13 blockerを作らない。阻害一覧と公開状態は同じ実装状態snapshotから派生し、既存12 blocker、6 current-run evidenceおよびGate `blocked`を維持する。

Authority Root検索票（Authority Root Locator）は、Repository直下の固定`.crdd-runtime/authority-root-locator.json`へ置く。resolver候補はRepositoryとの相互非包含、non-link安定Identity、永続Trustおよびcurrent Record結合までを行う。Windows実効アクセス確認と実測Protection Hash結合は未実装で、観測済みRecord入口は`blocked`である。Path、SID、ACL、raw recordまたはcanonical byteを公開しない。

Root Identity／Protection観測成果物のexact pure contract、domainおよびHash生成候補は実装済みである。Filesystemから観測値を生成するWindows Adapterは未実装であり、DACL構造確認と現在process tokenによる実効アクセス確認を分離する。caller claim、ACE集計またはopen失敗から保護成立を主張しない。絶対Path、SIDおよびACL実値は公開せず、候補一致だけではAuthority、Capabilityまたはactivationを成立させない。
...
共通Root保護方針Core候補（Root Protection Policy Core Candidate）は、WindowsのDACLとmacOS／Linuxのowner／modeを別々の正本にせず、同じ保護結果へ写像する。Runtime Rootは選択された利用者またはサービス実行主体を`runtime_principal_only`のwriterとして読取り／書込みでき、非承認主体が書き込めないことを要求する。Authority Rootは承認済みadmin／installerの集合を表す`provisioner_principal_only`だけが書込み、Runtime主体は読取り専用で、非承認主体が書き込めないことを要求する。どちらも事前Provision済みの既存non-link Rootと安定Identityを必要とし、Runtime自身はRoot作成や権限変更を行わない。`local`と`persistent_volume`だけを候補とし、network、removable、specialまたは分類不明のFilesystemはfail closedにする。pure Coreへ直接渡す観測値はcaller supplied claimなので単独では実保護を成立させない。Windowsの実効access Adapterは、現在process tokenのgroup、deny-only group、restricted SIDおよびACE順序をOSに評価させる固定方式が未実装である。ACE集計やhandle open失敗だけを実効拒否の根拠にせず、未実装中は常に`blocked`とする。Path、SID、UID、GID、mode、DACLまたはraw errorを返さず、POSIX観測、activation、Capability、ProviderまたはOperationを成立させない。
...
Authority Root結合pure候補は、明示選択した絶対Path、観測済みRoot Identity Hashおよび保護policy Hashを署名済みRecord内の3値と完全一致させる。Windows resolverはRepositoryとAuthority Rootのlexical／realpath相互非包含およびnon-link Directory IdentityをTrust／Record読取りの前後で固定するが、実効access Adapterが未実装のため実測2 Hashを生成せず、観測済みRecord結合入口は`blocked`である。結果には絶対Path、SIDまたはACLを公開しない。POSIX探索、初期Trust、権限変更、activationまたはAuthority発行を代替しない。
