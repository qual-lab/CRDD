...
承認済みの端末導入鍵（installation key）方針は、Platform scopeごとのEd25519鍵をプラットフォーム準備ツールがOS管理の鍵保管境界で生成・保持し、その公開鍵へQual-Lab準備認証局（Qual-Lab Provisioning CA）が独自JCS JSON形式の登録証明書をEd25519で署名して発行する目標である。証明書のkey IDはexact SPKI DERのSHA-256 lowercase hexadecimal 64文字、有効期間は180日、更新開始は期限30日前、新旧併用期間は最大30日とする。期限切れ、失効、rollback、置換、不一致または検証不能では`blocked`として再Provisionへ戻し、更新失敗時にも別sourceへ無言でfallbackしない。WindowsはCNG/KSPのTPM-backed鍵を優先し明示的software KSPを代替候補、macOSはKeychainのSecure Enclave-backed鍵を優先しsoftware-backed Keychainを代替候補、LinuxはTPM 2.0を優先しroot-owned software keystoreを代替候補とする。初回setupで選択backendと確認できた保護強度を表示し、検証済み状態が有効な通常runでは再選択、管理者操作またはnetworkを要求しない。候補間のsilent fallback、同等強度の推定、秘密鍵materialのCoordinator Runtimeへの入出力または成果物化、Qual-Lab秘密鍵の製品・Repository・準備記録・持込み束への埋込みは禁止する。初回登録は明示オンラインまたは管理者持込みのオフライン初回登録束だけを候補とする。オンライン方式はone-time challenge、nonce、Platform scopeおよびinstallation public keyを結合し、オフライン束はrequest hash、登録証明書、Provisioning CA chain、失効snapshotおよび束の期限を結合する目標である。replay、別machine、別Platform scope、期限切れ入力は拒否し、片方の不成立から他方へfallbackしない。承認済みなのはこの形式、期間、backend優先候補および登録方式の役割までである。証明書のexact field Schema／wire encoding／domain bytes、proof-of-possession、challenge TTL、offline CA chainのexact topology／署名充足、replay消費台帳、CA root／intermediate Lifecycle、保存Path／recoveryおよびplatform adapterの実装は未決である。鍵生成、鍵保護確認、証明書codec／検証、認証局Trust／失効確認、自動更新、初回登録交換、replay永続化および準備記録との結合検証もすべて未実装である。この方針投影は入力、network、暗号鍵操作、Filesystem Effect、AuthorityまたはCapabilityを発行せず、既存12阻害依存と6件の現在run根拠を減らさない。実行するPlatform Provisioner実体の署名／Trust検証は`platform_provisioner_verification`、鍵生成・登録交換・更新Effectは`platform_provisioner_effect`、証明書とオフライン束の契約は`provisioning_record_contract`、鍵保護・証明書・認証局Trust／失効・Record結合・replayの検証は`provisioning_record_verification`が将来所有し、実行物TrustとRecord署名者Trustを混同しない。
