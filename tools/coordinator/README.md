...
```powershell
npm run release-manifest:sign --prefix tools/coordinator -- --distribution-root <absolute-staging-root> --private-key C:\project\key\CRDD\crdd-release-v1-private.pem --crdd-version <vX.Y.Z> --crdd-commit <commit-id> --crdd-tree <tree-id> --issued-at <canonical-utc> --expires-at <canonical-utc>
```

`<absolute-staging-root>`には対象Git Treeのblob byteを変換せず展開したCRDD配布物が必要である。Windowsを含め、Release archiveは`git -c core.autocrlf=false archive`相当で生成し、改行変換された作業Treeを流用しない。commandは同Rootの`tools/coordinator`を安定したnon-link fileとして再走査し、package content root、Root保護Policyおよび鍵保管Policyのcanonical SHA-256を署名payloadへ入れる。固定Path loaderはBOMなしRFC 8785 canonical UTF-8だけを同一file handleで読み、署名Coreへ渡す。Release Identity候補は配布Root全体のGit Object IDをnon-linkの同一handle読取りから再計算し、後置manifestだけを除外したRoot Treeを署名済み`crddTree`へ一致させる。現時点ではrollback floor永続化、Windows DACL AdapterおよびEffect controllerが未実装であるため、manifest生成や暗号的一致だけでGate、Authority、Capability、統合またはReleaseは成立しない。実鍵による生成は、対象Version／Commit／Tree、有効期間および配布ステージングRootをRelease判断で固定した後にだけ行う。
...
`coordinator provision`は将来の唯一のPlatform Provisioning Effect入口としてgrammarだけを実装した。version 1はCRDD配布物に内包したprivate TypeScript sourceとして提供し、単独npm公開、個別install、専用EXEまたはOSネイティブコード署名を要求しない。内部Scriptは現行フォルダ配置のまま`.ts`へ移行済みであり、Node.js 24.12以上のnative TypeScript実行でCRDD Repository内の固定Pathからだけ起動する。`package.json`は実行commandを`bin`として公開しない。固定された`tools/coordinator`の読取りと内容Hash計算は候補実装したが、現ローカル／開発buildではCRDD配布Revision、Runtime所有release Trust、owner／permission確認およびEffect Adapterが未実装なため、常にdry-run相当の安全要約を返して`blocked`となり、Root、鍵またはACLを変更しない。`activate`、`doctor`または通常runからこの入口を暗黙発火しない。
...
承認済みのプラットフォーム準備方針は、CRDD配布物の`tools/coordinator`に内包するprivate packageのプラットフォーム準備処理（Platform Provisioner）を明示`provision`からだけ呼び出し、有効な準備Identityが維持されるPlatform scopeでは初回setupだけに管理者操作を集約する目標である。配布版は検証済みCRDD Revision、Qual-Lab署名済みpackage manifest、およびRuntimeが再取得するpackage内容／Filesystem Identityの全条件をEffect前に要求する。どれか一つの欠落、未知Revision、改変、権限不一致、単独取得packageまたは別sourceへのfallbackでは`blocked`とする。package manifestのexact object、CRDD Revision、Ed25519署名およびpackage file一覧のcontent rootを検査するpure Coreに加え、module相対で固定した`tools/coordinator`だけを再帰走査し、non-linkのRoot／file Identityを前後確認しながら同じhandleからbyte数とSHA-256を取得してcontent rootを再計算するFilesystem Adapter候補を実装した。開発依存の`node_modules`とroot `.gitignore`は配布package内容から除外する。結果はfile Path、生byte、descriptorまたは署名を公開しない。release Trustは、外部確認済みCRDD ReleaseへQual-Labの有効なEd25519公開鍵exact 1本をCRDD所有sourceへ固定し、CRDD version、Commit、TreeおよびCoordinator content rootを固定Path`90_Release/coordinator-package-manifest.json`の署名対象へ結ぶ。秘密鍵はRepositoryへ保存しない。公開鍵固定、外部ステージングRoot向けmanifest署名command、Policy Identity算出、固定Path canonical loader、および配布Root全体のGit Tree再計算によるRelease Identity照合は実装済み候補である。manifestは対象Git Treeへ含めない後置配布成果物とし、自己参照を避ける。rollback floor永続化、Windows DACL AdapterおよびEffect controllerが未実装なので現在も`blocked`である。POSIXの保護候補はroot所有、directory `0755`、file `0644`を全package treeへ要求する。Windowsは`SYSTEM`／Administratorsだけがwrite、Runtime利用者はread／executeだけとする。専用native executableやWindows／macOSの有料コード署名証明書はversion 1の必須条件ではない。未検証source checkoutはdry-runとtest専用で、Trust、GateまたはFilesystem Effectを成立させない。昇格、鍵生成、ACL変更および保存は将来の明示`provision`だけが発火できる目標である。

package Trust Gateのpure集約候補はmanifestを内部で再検証し、CRDD Revision、同じpackage content root、安定file Identityおよびpermission policy一致を一つの観測契約へ結ぶ。callerが任意Rootを指定する既存観測入口は引き続き非Authorityである。新しい固定Root Adapterはpackage fileの実byteと安定IdentityをRuntime側で取得するが、release signer／manifest／期待Revisionは未信頼入力であり、owner／permissionも未確認なので、正常一致も`effectAuthorizationIssued: false`である。CRDD release Identity、release Trust、permission確認およびEffect controllerが同じ制御経路へ接続されるまで、この結果を別入口へ渡してEffectを許可できない。
