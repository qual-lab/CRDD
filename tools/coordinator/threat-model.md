...
承認済みのProvisioning方針は、CRDD配布物の`tools/coordinator`に内包するprivate TypeScript sourceを固定Release Trustで検証し、OS固有観測だけを`tools/platform-access/`のprivate Rust componentへ限定する目標contractである。manifest、Ed25519署名、content root、固定公開鍵、Policy Identity、canonical loaderおよびGit Tree照合は候補実装済みである。active pointerのcanonical codec、単調な直前Hash遷移、local user Identity Hashおよび安定同一file読取りもcomponent候補である。Windows DACL構造claimは非Authorityのpure評価で、Rust Coreも現在process tokenの実効accessとuser SIDのdomain-separated Hashを観測するcomponent候補に限る。署名manifestへのRust成果物Hash結合は候補実装済みであるが、native durable pointer、保護済みactiveと検証済み実行イメージへのRust成果物の実行時結合、プロセス起動、DACL適用、staging copy、state更新およびEffect controllerは未実装である。`coordinator provision`はcommand grammar候補に限り、配布物やFilesystemへ触れる前に停止する。固定公開鍵以外のcaller supplied signer、manifest、SIDまたは任意観測をRuntime所有Trustへ昇格しない。
