...
このCoreはPolicy要求と観測claimを照合するpureな候補であり、caller suppliedのIdentity、DACL構造またはaccess可否をAuthorityへ昇格させない。Rust CoreはWindows実効アクセス確認（effective access verification）の読み取り専用観測候補を実装する。TypeScript側は固定成果物のHash、target、protocol revision、toolchainおよびbyte長を署名manifestへ結合する観測・署名候補を実装する。保護済み有効世代、検証済み実行image、process起動、Root観測成果物への写像、DACL適用、Platform Provisioner EffectおよびRuntime readerは未実装である。現在はPath入力やprocess起動より前に`blocked`となる。
...
ローカル導入の目標contractは、無効なRepositoryではRuntime固有Effectを発火せず、初回Platform setupをTrust検証済みCRDD配布物内の明示Provisioner経路へ限定することである。現段階は署名／manifest／package／state、Rust Core、および署名manifestへ結合する成果物観測・署名候補までである。保護済み有効世代、検証済み実行image、上限付きプロセス（Bounded Process）による起動、Root観測成果物への写像、DACL適用、導入EffectおよびRuntime readerは未実装である。
...
承認済みのProvisioning方針は、CRDD配布物の`tools/coordinator`に内包するprivate TypeScript sourceを固定Release Trustで検証し、OS固有の読み取り専用観測だけを`tools/platform-access/`のprivate Rust componentへ限定する目標contractである。manifest、Ed25519署名、content root、固定公開鍵、Policy Identity、canonical loaderおよびGit Tree照合は候補実装済みである。rollback floor、active releaseおよびtransactionのcodec／Store候補もcomponentとして残る。Windows DACL構造claimは非Authorityのpure評価で、Rust Coreも現在process tokenの実効accessを観測するcomponent候補に限る。Rust binaryのRelease Identity結合、DACL適用、世代copy、state更新およびEffect controllerは未実装である。`coordinator provision`はcommand grammar候補に限り、配布物やFilesystemへ触れる前に停止する。固定公開鍵以外のcaller supplied signer、manifestまたは任意観測をRuntime所有Trustへ昇格しない。
...
署名payloadは単調増加する`releaseSequence`もRelease Identityへ結合する。rollback floor、active releaseおよび永続transactionのpure／Store候補は残るが、Effectから到達不能である。Runtime readerもRelease Identityへ結合したWindows実効access Adapterが未実装のため、transaction、floor、active、署名済み導入世代またはDACLを読まずに`blocked`となる。Rust Core単体やcaller supplied stateをAuthorityへ昇格しない。
...
pure Core候補は署名領域分離（domain separation）、exact鍵・署名形式、期限、strict UTF-8、canonical byte完全一致および入力上限を要求する。永続floorへ結合したTrust成果物、current RecordおよびRuntime取得時刻を使う集約検証候補も実装した。Rust Coreと署名manifestへ結合する成果物観測・署名処理はcomponent候補だが、保護済み有効世代、検証済み実行image、process起動、Root観測成果物への写像、初期Trust、POSIX観測、実端末登録Effect、完全Lifecycleおよびactivation確認は未実装である。候補一致もAuthority、CapabilityまたはGateを開かない。
...
固定`state/release-floor.json`、`state/active-release.json`および`state/provision-transaction.json`のcodec／Storeはcomponent候補である。Rust Coreの読み取り専用観測候補は存在するが、Release Identityへ結合したWindows実効access AdapterとEffect controllerが未実装のため、これらを作成・更新する実EffectとRuntime readerは未実装である。component結果をRuntime AuthorityまたはCapabilityへ昇格しない。
...
Authority Root検索票（Authority Root Locator）は、Repository直下の固定`.crdd-runtime/authority-root-locator.json`に置く信用前検索ヒントである。resolver候補はRepositoryとAuthority Rootの相互非包含、non-link実体およびIdentity安定を要求する。Root Identity／Protection成果物のpure contract、Rust Coreおよび署名manifestへ結合する成果物観測・署名処理はcomponent候補だが、保護済み有効世代、検証済み実行image、process起動、Root観測成果物への写像、全tree／writer排他確認および実測Protection Hash結合は未実装であり、観測済みRecord結合入口は常に`blocked`とする。

検索票Core候補はexact shape、byte上限、保守的なcanonical lexical subset、canonical JSONおよび内容Hashを検査する。resolver候補はRepositoryとの相互非包含、non-link Directory Identity、永続Trustおよびcurrent Record結合までを行う。Rust Coreと成果物観測・署名処理はcomponent候補だが、process起動、Root観測成果物への写像と実測Protection Hash結合は未実装である。有効activation–検索票結合候補はpure比較に限り、Filesystem書込みや実active activation bindingを行わない。これらは第13 blockerを作らず、既存12 blocker、6 current-run evidenceおよびGate `blocked`を維持する。
