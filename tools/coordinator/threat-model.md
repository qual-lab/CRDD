...
本文で「検証済み実行イメージ」または「プロセス起動は未実装」と記す箇所は通常Runtime／Adapter側を指す。CHG-000036のpre-active native source候補は別の限定入口であり、Minimum Trust Boundary採用後のsourceからmapped supervisor image専用blockerを除去した。固定commit `f5f2517`の署名済みrunではPA03／PR03、Job／tree終了およびRegistry復元を確認したが、全module集合とNetwork非発火は未検証であり、通常Runtime、Authority、Capability、Gateまたは実Providerを有効化しない。
