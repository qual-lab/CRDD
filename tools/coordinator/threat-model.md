...
Windows v1のRuntime主体は明示provisionで選択したローカル対話ユーザー1名だけである。管理者昇格は同じuserの`TokenUser`一致を要求し、別資格情報による昇格など選択userとの一致を決定論的に確認できない場合は処置前に`blocked`とする。elevated group、admin groupまたはcaller指定SIDを選択userの代用にはしない。サービスアカウントは未実装境界に残す。
