...
Provider隔離ProfileはAuthority候補と強制結果を分離する。Profileが表すのは、Provider、Authority RegistryのGrant参照候補、genericな短期Credential用Broker参照候補および要求Origin集合であり、人間承認、Credential値または実行可能Capabilityではない。revision 1のCredential Grantはsubscription OAuth v1へ適用しない。subscription OAuth用のProvider Home mount Grantは別の将来契約であり、Profile自身、Agent出力、環境変数またはProviderの自己申告だけから発行しない。Runtimeが所有するAuthority Grant Verifier、Enforcer／Adapterの観測結果が、同じ固定Profile Hash、OperationおよびScopeへ結び付いた場合だけ各Gateを評価する。
...
revision 1のCredential GrantはBrokerとGrantの用途別namespaceを持つgenericな短期Credential参照候補だけをProfileへ保持し、subscription OAuth v1には適用しない。generic Credentialを使用する将来経路では対象ProviderとProfileへ限定した短期Credentialの破棄を要求する。一方、subscription OAuthは永続専用HomeのsessionをOperationごとにcopy／inject／破棄せず、短命なProvider Home mount Grant、handleおよびmountだけを終了時に失効させる。Secret、Token、API Key、Credential Store Path、Provider Home PathまたはCredential値をProfile、Prompt、通常Event、ProjectionまたはEvidenceへ保存しない。いずれのBroker／Grant発行部も未実装で、失効を確認できなければGateを開かない。
