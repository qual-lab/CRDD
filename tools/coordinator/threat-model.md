...
このCoreはPolicy要求と観測claimを照合するpureな候補であり、caller suppliedのIdentity、DACL構造またはaccess可否をAuthorityへ昇格させない。同梱package向けのDACL構造claim evaluatorもpure候補に限る。Windows実効アクセス確認（effective access verification）はgroup、deny-only group、restricted token、ACE順序およびgeneric mappingをOSに評価させる必要があり、現Adapterは未実装である。PowerShell ACE集計、caller SIDまたはhandle open失敗から成立を推定しない。観測、DACL適用、Platform Provisioner EffectおよびRuntime readerは下流のPath／Filesystem処理前に`blocked`となり、Path、SID、ACEまたはraw errorを公開しない。
...
ローカル導入の目標contractは、無効なRepositoryではRuntime固有Effectを発火せず、初回Platform setupをTrust検証済みCRDD配布物内の明示Provisioner経路へ限定することである。現段階は署名／manifest／package／stateのcomponent候補までで、Windows実効アクセス確認、DACL適用、導入EffectおよびRuntime有効世代readerは未実装である。`provision`は最初のFilesystem Effect前、readerはProgramData探索前に`blocked`となる。component候補やcaller claimをProvisioning成立、AuthorityまたはCapabilityへ流用しない。
...
承認済みのProvisioning方針は、CRDD配布物の`tools/coordinator`に内包するprivate TypeScript sourceを固定Release Trustで検証する目標contractである。manifest、Ed25519署名、content root、固定公開鍵、Policy Identity、canonical loaderおよびGit Tree照合は候補実装済みである。rollback floor、active releaseおよびtransactionのcodec／Store候補もcomponentとして残る。ただしWindows DACL構造claimは非Authorityのpure評価だけで、実効アクセス確認、DACL適用、世代copy、state更新およびEffect controllerは未実装である。`coordinator provision`はcommand grammar候補に限り、配布物やFilesystemへ触れる前に停止する。固定公開鍵以外のcaller supplied signer、manifestまたは任意観測をRuntime所有Trustへ昇格しない。

署名payloadは単調増加する`releaseSequence`もRelease Identityへ結合する。rollback floor、active releaseおよび永続transactionのpure／Store候補は残るが、Effectから到達不能である。Runtime readerもWindows実効アクセス確認が未実装のため、transaction、floor、active、署名済み導入世代またはDACLを読まずに`blocked`となる。component候補やcaller supplied stateをAuthorityへ昇格しない。
...
固定`state/release-floor.json`、`state/active-release.json`および`state/provision-transaction.json`のcodec／Storeはcomponent候補である。Windows実効アクセス確認とEffect controllerが未実装のため、これらを作成・更新する実EffectとRuntime readerは未実装である。component結果をRuntime AuthorityまたはCapabilityへ昇格しない。
