...
Claude実行計画候補（Claude Execution Plan Candidate）は、公式release manifestで観測したLinux x64 binaryのexact version、upstream commit、SHA-256およびbyte長を保持するが、manifest署名の実検証、binary取得、固定image buildまたは起動を行わない。将来のbuildはAnthropic release signing keyのfingerprintとmanifest署名を確認し、manifestのplatform checksumをbinaryへ照合してから固定image digestへ結合しなければならない。npm package、PATH上のHost CLI、native installerの自動更新または`latest` channelへfallbackしない。固定probeはworkspaceをmountせず、built-in toolを0、MCP toolを拒否、project instruction、customization、resumeおよびsession永続化を無効にする。それでもProvider request、専用HomeへのProvider内部書込み、subscription利用枠消費およびTelemetryの可能性は残るため、専用Home Mount Grant、Proxy強制、Telemetry判断、認証観測およびterms有効化の前にspawnを許可しない。
