...
function fixture() {
  ...
  const payload = {
    ...
    authorityRootAbsolutePath: process.platform === "win32" ? "C:\\CRDD-Authority" :
...
function compiled(f) {
  ...
  const revocations = compileProvisioningRevocationManifestCandidate(f.revocations);
  assert.equal(envelope.status, "candidate");
  assert.equal(keyset.status, "candidate");
  assert.equal(revocations.status, "candidate");
  return { envelope, keyset, revocations };
}

function signingEntry(payload, privateKey, keyId) {
  const domain = buildProvisioningRecordDomainMessageCandidate(payload);
  assert.equal(domain.status, "candidate");
  return {
    keyId,
...
test("各codecはcanonical bytesだけをround-tripし、非canonical raw JSONを拒否する", () => {
  ...
  for (const [result, decode] of [
    ...
    assert.equal(decode(result.canonicalBytes).status, "candidate");
    assert.equal(decode(Buffer.concat([result.canonicalBytes, Buffer.from("\n")])).status,
      "blocked");
    assert.equal(decode(Buffer.concat([Buffer.from([0xef, 0xbb, 0xbf]),
      result.canonicalBytes])).status, "blocked");
    assert.equal(decode(new Uint8Array(result.canonicalBytes)).status, "blocked");
  }
  const duplicate = Buffer.from('{"contract":"crdd-coordinator/provisioning-revocation-manifest",' +
    '"contract":"crdd-coordinator/provisioning-revocation-manifest","contractRevision":1,' +
    '"revocationRevision":1,"revoked":[],"trustEpoch":1}');
  assert.equal(decodeProvisioningRevocationManifestCandidate(duplicate).status, "blocked");
});

test("exact schemaはextra、accessor、Proxy、revision、並び、上限をfail closedにする", () => {
  const f = fixture();
  assert.equal(compileProvisioningRecordEnvelopeCandidate({ ...f.envelope, extra: true }).status,
    "blocked");
  const accessor = { ...f.envelope };
  let getterCalls = 0;
  Object.defineProperty(accessor, "payload", {
    enumerable: true,
    get() { getterCalls += 1; return f.payload; }
  });
  assert.equal(compileProvisioningRecordEnvelopeCandidate(accessor).status, "blocked");
  assert.equal(getterCalls, 0);
  let proxyCalls = 0;
  const proxy = new Proxy(f.envelope, {
    ownKeys() { proxyCalls += 1; return Reflect.ownKeys(f.envelope); }
  });
  ...
  assert.equal(compileProvisioningTrustAnchorSetCandidate({
    ...f.keyset, keys: [...f.keyset.keys, f.keyset.keys[0]]
  }).status, "blocked");
  assert.equal(compileProvisioningRecordEnvelopeCandidate({
    ...f.envelope,
    signatures: Array(PROVISIONING_RECORD_PURE_CORE_LIMITS.signatures + 1)
      .fill(f.envelope.signatures[0])
  }).status, "blocked");
  assert.equal(compileProvisioningTrustAnchorSetCandidate({
    ...f.keyset,
    keys: Array(PROVISIONING_RECORD_PURE_CORE_LIMITS.keys + 1).fill(f.keyset.keys[0])
  }).status, "blocked");
  assert.equal(compileProvisioningTrustAnchorSetCandidate({
    ...f.keyset,
  ...
  assert.equal(compileProvisioningRevocationManifestCandidate({
    ...f.revocations,
    revoked: Array(PROVISIONING_RECORD_PURE_CORE_LIMITS.revocations + 1).fill({
      keyId: "8".repeat(64), revokedAt: "2026-01-01T00:00:00.000Z", reasonCode: "test"
    })
  }).status, "blocked");
});
...
test("複数署名は全entryが既知・期間内・正常な場合だけ候補になる", () => {
  ...
  const entries = [
    { keyId: first.keyId, privateKey: first.privateKey, spkiDer: first.spkiDer,
      enrollmentCaId: "7".repeat(32) },
    { keyId: secondKeyId, privateKey: secondPair.privateKey, spkiDer: secondSpki,
      enrollmentCaId: "8".repeat(32) }
  ].sort((left, right) => left.keyId.localeCompare(right.keyId));
  const envelope = {
    ...first.envelope,
    signatures: entries.map((entry) => signingEntry(first.payload, entry.privateKey, entry.keyId))
  };
  const keyset = {
    ...first.keyset,
  ...
  const envelopeBytes = compileProvisioningRecordEnvelopeCandidate(envelope).canonicalBytes;
  const trustAnchorSetBytes = compileProvisioningTrustAnchorSetCandidate(keyset).canonicalBytes;
  const revocationManifestBytes = compileProvisioningRevocationManifestCandidate(
    first.revocations).canonicalBytes;
  const input = {
    envelopeBytes,
    trustAnchorSetBytes,
  ...
  assert.equal(verifyProvisioningRecordAggregateCandidate(input).verifiedSignatureCount, 2);
  ...
  const invalidEnvelope = compileProvisioningRecordEnvelopeCandidate({
    ...envelope, signatures: invalidEntries
  });
  assert.equal(invalidEnvelope.status, "candidate");
  assert.equal(verifyProvisioningRecordAggregateCandidate({
    ...input, envelopeBytes: invalidEnvelope.canonicalBytes
  }).reason, "provisioning_record_aggregate_signature_invalid");
});
...
test("aggregate候補は全署名が既知・非失効・期間内・正常な場合だけ通す", () => {
  ...
  const revokedBytes = compileProvisioningRevocationManifestCandidate(revoked).canonicalBytes;
  assert.equal(verifyProvisioningRecordAggregateCandidate({
...
test("unknown、改変、epoch差、余分な不正署名を無視せず全体拒否する", () => {
  ...
  const unknownCompiled = compileProvisioningRecordEnvelopeCandidate(unknownEnvelope);
  ...
  assert.equal(verifyProvisioningRecordAggregateCandidate(base).reason,
    "provisioning_record_aggregate_unknown_key");

  const changed = { ...f.envelope.signatures[0] };
  ...
  const changedEnvelope = compileProvisioningRecordEnvelopeCandidate({
    ...f.envelope, signatures: [changed]
  });
  assert.equal(verifyProvisioningRecordAggregateCandidate({
    ...base, envelopeBytes: changedEnvelope.canonicalBytes
  }).reason, "provisioning_record_aggregate_signature_invalid");

  const epoch = compileProvisioningRevocationManifestCandidate({ ...f.revocations, trustEpoch: 2 });
  assert.equal(verifyProvisioningRecordAggregateCandidate({
    ...base,
    envelopeBytes: values.envelope.canonicalBytes,
    revocationManifestBytes: epoch.canonicalBytes
  }).reason, "provisioning_record_aggregate_trust_epoch_mismatch");
});
