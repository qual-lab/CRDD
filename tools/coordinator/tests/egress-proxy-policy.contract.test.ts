...
test("自己構築した検証結果やcaller指定HashをPolicy候補へ昇格しない", () => {
  ...
  for (const forged of [
    ...
    { ...validation, profileHash: "a".repeat(64) },
    {
      ...
      profile: {
        ...validation.profile,
        egress: { origins: ["https://other.example.test"] },
      },
    },
    {
      status: "candidate",
...
test("不正ProfileとOriginは例外を漏らさずblockedへ閉じる", () => {
  for (const candidate of [
    null,
    {},
    rawProfile({ egress: { origins: ["not a URL"] } }),
    rawProfile({ extra: true }),
  ]) {
    assert.doesNotThrow(() => compileEgressProxyPolicyCandidate(candidate));
    assert.equal(
      compileEgressProxyPolicyCandidate(candidate).status,
      "blocked",
    );
  }
});
...
test("Profile accessorはPolicy経路でも実行しない", () => {
  ...
  Object.defineProperty(value, "egress", {
    ...
    get() {
      calls += 1;
      return { origins: ["https://api.example.test"] };
    },
  });
...
test("CONNECTはcanonical hostnameと文字列443の完全一致だけを候補にする", () => {
  ...
  assert.equal(
    evaluateProxyConnectForFixture(policy, {
      method: "CONNECT",
  ...
  for (const authority of [
    ...
    assert.equal(
      evaluateProxyConnectForFixture(policy, { method: "CONNECT", authority })
        .decision,
      "deny",
      authority,
    );
...
test("IANA最長prefixでglobal例外とspecial範囲を区別する", () => {
  for (const address of [
    ...
    assert.equal(
      evaluateResolvedAddressesForFixture([address]).decision,
      "candidate",
      address,
    );
  }
  ...
  for (const [address, expected] of [
    ["100.63.255.255", "candidate"],
    ["100.64.0.0", "deny"],
    ["100.127.255.255", "deny"],
    ["100.128.0.0", "candidate"],
    ["3ffe:ffff:ffff:ffff:ffff:ffff:ffff:ffff", "deny"],
    ["3fff::", "deny"],
    ["3fff:fff:ffff:ffff:ffff:ffff:ffff:ffff", "deny"],
    ["3fff:1000::", "deny"],
  ])
    assert.equal(
      evaluateResolvedAddressesForFixture([address]).decision,
...
test("IPv4とIPv6のspecial、mapped、compatible、zone表記を拒否する", () => {
  ...
  assert.equal(
    evaluateResolvedAddressesForFixture(["::ffff:8.8.8.8"]).decision,
    "candidate",
  );
  assert.equal(
    evaluateResolvedAddressesForFixture(["8.8.8.8", "2606:4700:4700::1111"])
      .decision,
    "candidate",
  );
  assert.equal(
    evaluateResolvedAddressesForFixture(["8.8.8.8", "127.0.0.1"]).decision,
    "deny",
  );
});
...
test("IANA snapshot metadataとTopologyは実強制Capabilityではない", () => {
  ...
  assert.equal(
    registry.snapshotSha256,
    "a0c9de6e6f84a76c75cbdb85b3d3f01e76c965691211b469374a117fab351f2f",
  );
