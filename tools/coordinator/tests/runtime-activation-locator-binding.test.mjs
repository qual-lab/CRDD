...
function locator(record, overrides = {}) {
...
function evaluate(record, locatorOverrides = {}, previousActivationCanonicalBytes = null) {
...
test("initial active activation and matching locator remain a candidate", () => {
  ...
  assert.equal(result.reason, "runtime_initial_activation_locator_binding_candidate");
  ...
  assert.equal(JSON.stringify(result).includes(record.repositoryIdentityHash), false);
...
test("disable and reactivation locator lifecycles remain unsupported", () => {
  ...
  assert.equal(evaluate(disabled, {}, active.canonicalBytes).reason,
    "runtime_activation_locator_transition_not_supported");
  assert.equal(evaluate(activation({
    activationRevision: 2,
    previousActivationHash: active.recordHash
  }), {}, active.canonicalBytes).reason, "runtime_activation_locator_transition_invalid");
});
...
test("outer accessors and proxies fail closed without executing dynamic input", () => {
  ...
  assert.equal(evaluateInitialActivationLocatorBindingCandidate(input).reason,
  ...
  const proxy = new Proxy(raw, {
    ownKeys() { proxyCalls += 1; return Reflect.ownKeys(raw); }
  });
  assert.equal(evaluateInitialActivationLocatorBindingCandidate(proxy).reason,
    "runtime_activation_locator_binding_input_invalid");
  assert.equal(proxyCalls, 0);
  assert.equal(evaluateInitialActivationLocatorBindingCandidate({
    ...raw,
    extra: true
  }).reason, "runtime_activation_locator_binding_input_invalid");
});
