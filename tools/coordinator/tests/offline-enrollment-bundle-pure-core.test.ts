...
function framed(domain: string, value: Record<string, unknown>) {
...
const fixturePrivateKeys = new WeakMap<
  object,
  { online: KeyObject; offline: KeyObject }
>();
...
function fixture() {
  ...
  const issuing = (role: string, id: string, spki: Buffer) => {
    const value = {
      contract: "crdd-coordinator/provisioning-ca-issuing-certificate",
      contractRevision: 1,
      caSeriesId: "a".repeat(32),
      role,
      keyId: id,
      algorithm: "Ed25519",
      ...
      signatures: [
        {
          ...
          signature: sign(
            null,
            framed(PROVISIONING_CA_DOMAINS.issuingCertificate, value),
            root.privateKey,
          ).toString("base64url"),
        },
      ],
  ...
  const onlineIssuingCertificateEnvelope = issuing(
    "online_enrollment_issuer",
    onlineKeyId,
    onlineSpki,
  );
  const offlineIssuingCertificateEnvelope = issuing(
    ...
    signatures: [
      {
        ...
        signature: sign(
          null,
          framed(PROVISIONING_CA_DOMAINS.revocationManifest, revocation),
          root.privateKey,
        ).toString("base64url"),
      },
    ],
  ...
  if (challengeCandidate.status !== "candidate") {
    assert.fail(
      `fixture challenge did not compile: ${challengeCandidate.reason}`,
    );
  }
  const challengeHash = challengeCandidate.challengeHash;
  const request = {
  ...
  const requestHash = createHash("sha256")
    .update(framed(INITIAL_ENROLLMENT_DOMAINS.request, request))
    .digest("hex");
  const bundlePayload = {
    contract: OFFLINE_ENROLLMENT_BUNDLE_CONTRACT,
    contractRevision: 1,
    bundleId: "6".repeat(32),
    requestHash,
    platformScopeId: challenge.platformScopeId,
    provisionerIdentityHash: challenge.provisionerIdentityHash,
    installationKeyId,
    ...
    revocationEnvelope,
    issuedAt: "2026-08-15T00:15:00.000Z",
    expiresAt: "2026-08-22T00:15:00.000Z",
  };
  const bundleEnvelope = {
    contract: OFFLINE_ENROLLMENT_BUNDLE_ENVELOPE_CONTRACT,
    contractRevision: 1,
    ...
    signatures: [
      {
        ...
        signature: sign(
          null,
          framed(OFFLINE_ENROLLMENT_BUNDLE_DOMAIN, bundlePayload),
          offline.privateKey,
        ).toString("base64url"),
      },
    ],
  ...
  fixturePrivateKeys.set(result, {
    online: online.privateKey,
    offline: offline.privateKey,
  });
  return result;
}
...
test("signed offline bundle binds request, certificate and exact two-role CA chain", () => {
  const value = fixture();
  const result = verifyOfflineEnrollmentBundleCandidate({
    ...value,
...
test("wrong role, binding, signature, expiry and dynamic input fail closed", () => {
  ...
  const changedSignature = changed.bundleEnvelope.signatures[0];
  assertPresent(changedSignature);
  changedSignature.signature = "A".repeat(86);
  assert.equal(
    verifyOfflineEnrollmentBundleCandidate({
      ...changed,
      evaluationTime: "2026-08-15T12:00:00.000Z",
  ...
  assert.equal(
    verifyOfflineEnrollmentBundleCandidate({
      ...fixture(),
      evaluationTime: "2026-08-22T00:15:00.000Z",
    }).status,
    "blocked",
  );
  const certificateExpiresFirst = fixture();
  const privateKeys = fixturePrivateKeys.get(certificateExpiresFirst);
  assertPresent(privateKeys);
  ...
  const certificateSignature =
    certificateExpiresFirst.bundleEnvelope.payload.certificateEnvelope
      .signatures[0];
  assertPresent(certificateSignature);
  certificateSignature.signature = sign(
    null,
    framed(
  ...
  const bundleSignature = certificateExpiresFirst.bundleEnvelope.signatures[0];
  assertPresent(bundleSignature);
  bundleSignature.signature = sign(
    null,
    framed(
      OFFLINE_ENROLLMENT_BUNDLE_DOMAIN,
      certificateExpiresFirst.bundleEnvelope.payload,
    ),
    privateKeys.offline,
  ).toString("base64url");
  assert.equal(
    verifyOfflineEnrollmentBundleCandidate({
      ...certificateExpiresFirst,
      evaluationTime: "2026-08-15T12:00:00.000Z",
    }).status,
    "blocked",
  );
  ...
  Object.defineProperty(dynamic, "rootTrustSet", {
    enumerable: true,
    get() {
      calls += 1;
      return fixture().rootTrustSet;
    },
  });
  dynamic.bundleEnvelope = fixture().bundleEnvelope;
  dynamic.evaluationTime = "2026-08-15T12:00:00.000Z";
  assert.equal(
    verifyOfflineEnrollmentBundleCandidate(dynamic).status,
    "blocked",
  );
...
test("contract keeps replay, Trust, clock and import effects unimplemented", () => {
  ...
  assert.match(
    contract.caChain,
    /online_enrollment_issuer_then_offline_bundle_issuer/u,
  );
