...
import { evaluateRuntimeActivationTransitionCandidate } from "../src/security/runtime-activation-transition.ts";
...
function transition(previousCanonicalBytes: unknown, nextRecord: unknown) {
  return evaluateRuntimeActivationTransitionCandidate({
...
function activePrevious(overrides = {}) {
  const compiled = compileRuntimeActivationRecordCandidate(record(overrides));
...
function disabledNext(
  previous: ReturnType<typeof activePrevious>,
  overrides: Record<string, unknown> = {},
) {
  assertPresent(previous.record);
  const activationRevision = previous.record.activationRevision;
  const activatedAt = previous.record.activatedAt;
  if (
    typeof activationRevision !== "number" ||
    typeof activatedAt !== "string"
  ) {
    assert.fail("fixture activation record fields were not normalized");
  }
  assertPresent(previous.recordHash);
  return record({
...
test("初版はnullからactive revision 1だけを候補化する", () => {
  ...
  assertPresent(result.recordHash);
...
test("disable遷移はrevision、前版Hashおよび固定Identityの差を拒否する", () => {
  ...
  assert.equal(
    transition(
      ...
      disabledNext(previous, {
        activationRevision: 1,
      }),
    ).reason,
    "runtime_activation_next_record_invalid",
  );
  assert.equal(
    transition(
      previous.canonicalBytes,
  ...
  for (const overrides of [
    ...
    assert.equal(
      transition(previous.canonicalBytes, disabledNext(previous, overrides))
        .reason,
      "runtime_activation_disable_transition_invalid",
    );
});
...
test("再activationとdisabled起点の遷移は未実装としてfail closedにする", () => {
  ...
  for (const overrides of [
    ...
    assert.equal(
      transition(active.canonicalBytes, record(overrides)).reason,
      "runtime_reactivation_transition_policy_not_implemented",
    );

  const disabled = compileRuntimeActivationRecordCandidate(
    disabledNext(active),
  );
  assert.equal(
    transition(
      disabled.canonicalBytes,
      record({
        activationRevision: 3,
...
test("前版revision上限と不正canonical bytesを例外なく拒否する", () => {
  ...
  assert.equal(
    transition(penultimate.canonicalBytes, disabledNext(penultimate)).status,
    "candidate",
  );
  ...
  assert.equal(
    ...
    "runtime_activation_revision_exhausted",
  );

  const disabledMaximum = compileRuntimeActivationRecordCandidate(
    record({
      activationRevision: Number.MAX_SAFE_INTEGER,
      status: "disabled",
      previousActivationHash: "a".repeat(64),
  ...
  assert.equal(
    transition(
      disabledMaximum.canonicalBytes,
      record({
        activationRevision: Number.MAX_SAFE_INTEGER,
  ...
  for (const value of invalidPrevious) {
    assert.equal(
      transition(value, record()).reason,
...
test("外側入力と次recordの動的shapeを実行せずblockedへ閉じる", () => {
  ...
  assert.equal(
    evaluateRuntimeActivationTransitionCandidate(nullPrototype).status,
    "candidate",
  );
  assert.equal(
    evaluateRuntimeActivationTransitionCandidate(
      Object.freeze({
        previousCanonicalBytes: null,
  ...
  assert.equal(
    evaluateRuntimeActivationTransitionCandidate(outer).reason,
  ...
  Object.defineProperty(previousAccessor, "previousCanonicalBytes", {
    enumerable: true,
    get() {
      previousGetterCalls += 1;
      return null;
    },
  });
  assert.equal(
    evaluateRuntimeActivationTransitionCandidate(previousAccessor).reason,
    "runtime_activation_transition_input_invalid",
  );
  ...
  for (const value of [
    ...
    assert.equal(
      evaluateRuntimeActivationTransitionCandidate(value).reason,
      "runtime_activation_transition_input_invalid",
    );
});
