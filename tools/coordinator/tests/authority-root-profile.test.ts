...
test("Authority RootはCLI、環境の順で明示絶対Pathだけを候補化する", () => {
  ...
  const cliResult = selectAuthorityRootCandidate(
    request({ cliOverride: cli, environmentOverride: environment }),
  );
  assert.equal(cliResult.status, "candidate");
  assertPresent(cliResult.selection);
  ...
  const environmentResult = selectAuthorityRootCandidate(
    request({ environmentOverride: environment }),
  );
  assert.equal(environmentResult.status, "candidate");
  assertPresent(environmentResult.selection);
...
test("Authority RootにOS暗黙値、相対Pathまたはenable診断intentを使わない", () => {
  assert.equal(
    selectAuthorityRootCandidate(request()).reason,
    "authority_root_explicit_path_required",
  );
  assert.equal(
    selectAuthorityRootCandidate(request({ cliOverride: "relative" })).reason,
  ...
  assert.equal(
    ...
    "authority_activation_intent_invalid",
  );
});
...
test("Authority Root入力のaccessor、Proxy、余分fieldおよび欠落fieldを拒否する", () => {
  ...
  assert.equal(
    selectAuthorityRootCandidate({ ...raw, extra: true }).status,
    "blocked",
  );
