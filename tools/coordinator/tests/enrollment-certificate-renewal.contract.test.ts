...
function frame(payload: Record<string, unknown>) {
  ...
  if (canonical.status !== "candidate") {
    assert.fail(`fixture payload was not canonicalizable: ${canonical.reason}`);
  }
  const bytes = canonical.canonicalBytes;
  const length = Buffer.alloc(8);
  length.writeBigUInt64BE(BigInt(bytes.length));
  return Buffer.concat([
    Buffer.from(INITIAL_ENROLLMENT_DOMAINS.certificate, "ascii"),
    length,
    bytes,
  ]);
}
...
function fixture() {
  ...
  const envelope = (
    ...
    issuer: { publicKey: KeyObject; privateKey: KeyObject },
  ) => {
      ...
      signatures: [
        {
          ...
          algorithm: "Ed25519",
          signature: sign(null, frame(payload), issuer.privateKey).toString(
            "base64url",
          ),
        },
      ],
  ...
  return {
    previousCertificateEnvelope: envelope(previous, previousIssuer),
    previousIssuerSpkiDer: previousIssuer.publicKey.export({
      format: "der",
      type: "spki",
    }),
    nextCertificateEnvelope: envelope(next, nextIssuer),
    nextIssuerSpkiDer: nextIssuer.publicKey.export({
      format: "der",
      type: "spki",
    }),
...
test("early, expired, identity-changing and invalidly signed renewals fail closed", () => {
  const early = fixture();
  early.evaluationTime = "2026-05-30T23:59:59.999Z";
  assert.equal(
    verifyEnrollmentCertificateRenewalCandidate(early).status,
    "blocked",
  );
  const expired = fixture();
  expired.evaluationTime = "2026-06-30T00:00:00.000Z";
  assert.equal(
    verifyEnrollmentCertificateRenewalCandidate(expired).status,
    "blocked",
  );
  const identity = fixture();
  identity.nextCertificateEnvelope.payload.platformScopeId = "9".repeat(32);
  assert.equal(
    verifyEnrollmentCertificateRenewalCandidate(identity).status,
    "blocked",
  );
  const changed = fixture();
  const changedSignature = changed.nextCertificateEnvelope.signatures[0];
  assert.ok(changedSignature);
  changedSignature.signature = "A".repeat(86);
  assert.equal(
    verifyEnrollmentCertificateRenewalCandidate(changed).status,
    "blocked",
  );
});
...
test("contract separates pure transition from issuance and persistence effects", () => {
  ...
  assert.equal(
    contract.certificateIssuanceAndAutomaticRenewalEffect,
    "not_implemented",
  );
