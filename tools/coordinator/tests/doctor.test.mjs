...
test("production doctorはpassiveかつ未実装境界をReadyにしない", () => {
  ...
  assert.deepEqual(report.runtimeActivation.provisioningRecordTrustAndSelectionPolicy, {
  ...
  assert.equal(Object.isFrozen(
    report.runtimeActivation.provisioningRecordTrustAndSelectionPolicy), true);
  assert.equal(Object.isFrozen(report.runtimeActivation.installationKeyEnrollmentPolicy),
    true);
  assert.deepEqual(report.runtimeActivation.provisioningStorageAndLifecyclePolicy, {
    policy: "human_approved_candidate_contract_only",
  ...
  true);
  assert.equal(Object.isFrozen(
    report.runtimeActivation.installationKeyEnrollmentPolicy.platformKeyStoragePolicies), true);
  assert.equal(Object.isFrozen(
    report.runtimeActivation.installationKeyEnrollmentPolicy.platformKeyStoragePolicies.windows),
  true);
  assert.equal(Object.isFrozen(
    report.runtimeActivation.installationKeyEnrollmentPolicy.onlineEnrollmentRequiredInputs),
  true);
  assert.equal(Object.isFrozen(
    report.runtimeActivation.installationKeyEnrollmentPolicy
      .offlineEnrollmentBundleRequiredContents), true);
  assert.equal(Object.isFrozen(
    report.runtimeActivation.installationKeyEnrollmentPolicy
      .implementationDependencyRelationships), true);
  ...
  assert.equal(report.runtimeActivation.rootProtectionPolicy.completeOsOrVerifierCompromiseProtection,
    "not_guaranteed");
  assert.equal(report.runtimeActivation.rootProtectionPolicy.protectionEffectOwner,
    "official_signed_platform_provisioner_only_target");
  assert.equal(report.runtimeActivation.rootProtectionPolicy.runtimePermissionMutation,
    "prohibited");
  assert.deepEqual(report.runtimeActivation.rootProtectionPolicy.windowsProtectionTarget, {
  ...
  assert.deepEqual(report.runtimeActivation.rootProtectionPolicy.posixProtectionTarget, {
  ...
  assert.equal(report.runtimeActivation.rootProtectionPolicy.persistentVolumeEligibility,
    "local_equivalent_stable_identity_durable_atomic_replace_and_equivalent_acl_required_target");
  assert.equal(report.runtimeActivation.rootProtectionPolicy.unsupportedVolumeBehavior,
  ...
  assert.equal(report.runtimeActivation.provisionReceiptRelationship,
    "not_separate_runtime_authority_artifact_target");
  assert.equal(report.runtimeActivation.platformProvisionerManifestRelationship,
    "not_separate_runtime_authority_artifact_target");
  ...
  assert.equal(report.runtimeActivation.installationKeyProtectionVerification,
    "not_implemented");
  assert.equal(report.runtimeActivation.provisioningEnrollmentCertificateContract,
    "not_implemented");
  assert.equal(report.runtimeActivation.provisioningEnrollmentCertificateVerification,
    "not_implemented");
  assert.equal(report.runtimeActivation.provisioningCaTrustAndRevocationVerification,
    "not_implemented");
  ...
  assert.equal(report.runtimeActivation.platformKeyStorageAdapterVerification,
    "not_implemented");
  assert.equal(report.runtimeActivation.enrollmentReplayProtectionPersistence,
    "not_implemented");
  assert.equal(report.runtimeActivation.automaticEnrollmentRenewalEffect,
    "not_implemented");
  ...
  assert.equal(
    ...
    report.runtimeActivation.provisioningRecordPureCore.aggregateCryptographicCondition);
