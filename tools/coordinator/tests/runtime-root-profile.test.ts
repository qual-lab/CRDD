...
import { assertPresent, assertRecord } from "./test-support.ts";
...
test("既定RootはRepository配下の固定directory候補にする", () => {
  const result = selectRuntimeRootCandidate(
    input({ activationIntent: "explicit_enable_request" }),
  );
  assert.equal(result.status, "candidate");
  assertPresent(result.selection);
  assert.equal(result.selection.source, "repository_default");
  assert.equal(
    result.selection.repositoryRelativeDefault,
    DEFAULT_REPOSITORY_RUNTIME_DIRECTORY,
  );
...
test("CLI、環境、Repository既定の順でRoot候補を選ぶ", () => {
  const environment = selectRuntimeRootCandidate(
    input({
      environmentOverride: environmentRoot,
      activationIntent: "explicit_enable_request",
    }),
  );
  assertRecord(environment.selection);
  ...
  const cli = selectRuntimeRootCandidate(
    input({
      cliOverride: cliRoot,
      environmentOverride: environmentRoot,
      activationIntent: "explicit_enable_request",
...
test("相対Path、NUL、長すぎるPathおよび未知activationを拒否する", () => {
  assert.equal(
    selectRuntimeRootCandidate(input({ repositoryRoot: "relative" })).reason,
    "runtime_root_input_invalid",
  );
  assert.equal(
    selectRuntimeRootCandidate(input({ cliOverride: "relative" })).reason,
    "runtime_root_override_invalid",
  );
  assert.equal(
    selectRuntimeRootCandidate(
      input({ environmentOverride: `${environmentRoot}\0bad` }),
    ).reason,
    "runtime_root_override_invalid",
  );
  assert.equal(
  ...
  assert.equal(
    selectRuntimeRootCandidate(
      input({ cliOverride: path.resolve("a".repeat(4_096)) }),
    ).reason,
    "runtime_root_override_invalid",
  );
  assert.equal(
    selectRuntimeRootCandidate(input({ activationIntent: "enabled" })).reason,
...
test("Root選択Coreはignore、Path保護、activationまたはCapabilityを成立させない", () => {
  ...
  assert.equal(contract.cliOverrideIntegration, "implemented_candidate");
  assert.equal(
    contract.environmentOverrideIntegration,
    "implemented_candidate",
  );
  assert.equal(contract.diagnosticRequestIntegration, "implemented_candidate");
