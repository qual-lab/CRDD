...
fn request(path: &str, identity: [u32; 3], nonce: [u8; 32]) -> Vec<u8> {
    ...
    bytes.extend_from_slice(b"CRDDPA01");
...
fn binary_reports_candidate_blocked_and_invalid_requests() {
    ...
    assert_eq!(candidate.stdout.len(), 50);
    assert_eq!(&candidate.stdout[..8], b"CRDDPR01");
    ...
    assert_eq!(blocked.stdout.len(), 50);
    ...
    assert_eq!(invalid.stdout.len(), 50);
