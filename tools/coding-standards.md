...
Trust、Authority、Recoveryまたは安全上重要な結果をAPI、IPC、callback、event、return値、fileまたは永続記録で搬送する試験は、production wiringから実producerと全consumerを特定する。肯定入力は実producer出力、またはproductionと共有するCanonical validator／generatorから作り、手書きfixtureだけでproducerとの結合成立を主張しない。後続EffectまたはRecoveryを可能にする耐久intent、receipt、pointer、recordはAuthority-bearing stateとして、失敗呼出しが新規Authorityを残さないこと、exact既存状態だけが再開できること、partial／mismatch／unknownでは処置0とEvidence保持になることを確認する。
