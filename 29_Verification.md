...
[任意機能と必須評価](03_Documentation.md#mandatory-applicability-evaluation)に従い、検証方法または補助ツールを採用しない選択と、品質上重要な確認を評価しないことを分ける。試験Level、外部境界の段階、状態・失敗・Recovery、PT／LT、根拠の主張軸、未取得範囲および人間判断は必ず適用可否を評価し、非該当または未解決には理由を残す。
...
<a id="verification-diagram-profile"></a>

## 検証基本図の処置と記法

固定入口では、検証義務・試験Level／Boundary対応図、状態・分岐・Block別Coverage図および検証結果・判断接続図を、`作成`、`既存参照`、`非該当`、`作成不能`のいずれかへ処置する。複数工程の義務、複数試験Level、外部境界、状態分岐、複数Blockまたは複数の品質判断がある場合に発火する。単一義務を一つの決定論的確認だけで閉じられる場合は、根拠を示して不要な図を非該当にできる。根拠不足や未実行は非該当へ畳まない。
...
`[V:]`は工程が所有する検証義務、`[UT／IT／ST／UAT／PT／LT:]`は適用する試験Levelまたは判断、`[E:]`は対象改訂版に結合した根拠、`[D:]`は品質状態または人間判断を表す。回帰試験（RT）は矢印で選択・再実行する既存試験として表し、独立Levelにしない。図は義務、試験結果または品質判断を再定義せず、検証設計と結果の対応を反証する投影である。

工程出口では、各義務を実在する確認、結果、未確認範囲および判断へ接続し、図と試験カタログ／結果の対象改訂版を一致させる。図で露出した未対応状態、分岐、Block、利用側または終了後条件は、検証項目へ追加するか理由付き未確認として保持する。
...
適用される重要なシステム経路は、実際の契機または人間の入口から開始し、最終結果の公開と、必要なcleanupまたはRecovery状態の観測までを一つの検証対象として扱う。対象に実在する場合は、Authority、実行、結果搬送、独立レビューまたは是正、最終確認、資源回収、公開を経路へ含める。単純な処理へ存在しない段階を追加しないが、部品試験、契約試験、coverageまたは各段階の個別`Pass`だけから、重要経路のEnd-to-End成立を推定しない。

安全性に影響する層間契約は、実際のproducerから本番の搬送・変換を通り、本番consumerと外部契約へ到達する経路で確認する。consumerへ直接与えた手書きfixtureだけでは、producerが生成し得る値、搬送中の欠落・変形、または到達不能な分岐を網羅したとみなさない。正常だけでなく、対象に実在する準正常、拒否、部分状態、判定不能および異常を含め、資源不存在、Authority失効、Effect 0またはRecovery完了は、成功状態名ではなく独立した終了後観測から判定する。

結果母集団の意味は[振る舞い仕様](26_Behavior_Specification.md#phase-process-contract)、状態・資源・Authority・Effectの成立方法は[アーキテクチャ](27_Architecture.md#phase-process-contract)、検証義務、観測方法および合否判定は[品質保証の検証設計](16_Quality_Assurance.md#52-verification-design)を正本とする。本節はそれらを接続する検証上の完了境界を定めるものであり、固定Workflow、専用成果物、SchemaまたはCheckerを一律に要求しない。
...
認知意図を適用した対象では、設計時に仮説とした認知状態、障壁、必要な根拠／情報、判断／行動および観測方法に対し、成果物が必要な理解・比較・判断を支えられるかを認知品質確認（Cognitive QA）として評価する。これは利用者の内面を直接証明する検査ではない。専門家レビュー、利用者評価、行動指標、エラー、後戻り、離脱、支援利用、判断時間等から対象に適した根拠を選び、観測結果を仮説の支持・反証・更新へ戻す。クリック率、完了率、AI／VLMの評価または一回の成功だけから意図した理解が成立したと推定しない。
...
- 接続母集団と許可した処理境界の識別、境界内で許可された最小情報だけを送ること、境界外での外部向け調査コンテキストへの抽象化・最小化、および未許可の検索・外部AI・API・MCP・生成サービス等への直接送信の拒否
- 外部入力中の直接・間接プロンプトインジェクション、権限要求、ツール操作、内部情報要求を、エージェント契約、認証済み主体および許可した目的・操作による確認なしに命令として扱わないこと
...
外部情報境界を検証する場合は、実際の機密情報やシークレットを外部へ送って試験しない。合成した機密相当データ、識別可能な組合せ、悪意ある外部文書、未許可送信先、権限逸脱、拒否不能、ログ漏洩、回復不能等の試験データと攻撃経路を用いる。接続なし、許可した処理だけ、境界外接続、判定情報不足を分け、境界内では目的に許可された最小情報だけが送られ、境界外調査では削除・抽象化・最小化済み内容だけが送られ、拒否条件では送信前に閉じることを確認する。注意書きやエージェントの拒否申告だけでなく、利用可能な実行時強制、外部側へ到達した内容、監査記録、境界条件失効時の停止および迂回経路を確認する。
