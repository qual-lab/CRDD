...
多数の義務を継続管理する場合は、実装状態と検証状態を別項目として持つ検証マトリクスを使ってよい。検証義務の正本は所有工程へ残し、検証マトリクスは`08_Quality`の検証設計またはQuality Centerから参照できる索引・派生表示として扱う。
...
検証成果物の入口と物理構成は[品質保証の固定構成](16_Quality_Assurance.md#42-fixed-quality-structure)に従う。対象範囲について次を`08_Quality`の該当成果物から参照可能にし、適用の深さではファイル構成でなく記述、専門確認、根拠の深さを調整する。
