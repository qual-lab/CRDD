...
外部情報境界を対象に含む場合は、情報分類と継承、派生・組合せ情報、内部／承認済み内部ツール／外部サービス／公開先等の信頼領域、検索・生成・送信・公開・実行等の操作、外部向け調査コンテキスト、送信先、実送信内容、外部から戻る根拠、エージェント・ツール権限、供給網、実行時強制、監査、失効、封じ込め、回復の関係を探索する。各情報クラスと操作から、検索、ブラウザ、外部AI、API、MCP、生成、計測、問い合わせ、公開等の利用側へ辿り、削除だけで再識別可能な組合せを残していないか、分類不明を公開扱いへ丸めていないか、外部内容を命令・権限・正本へ昇格していないか、委譲で権限を拡大していないかを確認する。特定ツールの導入またはChecker合格を境界全体の成立へ流用しない。
