...
複数の下位要素から構成され、部分成立を全体完成へ誤認し得る対象では、一部の操作、Adapter、分岐、単体試験または安全な拒否だけを根拠に、上位のインターフェース、Capabilityまたは実行境界全体を実装済み、利用可能、検証済みまたは完成と表示していないか確認する。部分接続では、接続済み範囲、未成立の必要な保証、本番経路への接続状態、未評価範囲および利用側の停止または代替が区別されているか確認する。一意な局所変換、単一結果または上位完成を主張しない内部実装へ同じ表示を要求しない。

複数の状態項目が共同で結果または利用側の処置を決める文書では、成功／完了表示がcleanup未確認、Effect不明、Authority未成立、未解決Recoveryその他の状態と矛盾していないか確認する。個別fieldまたは個別試験が正しいことを、相関不変条件や上位成果の成立へ読み替えない。独立した説明項目へ存在しない相関を要求せず、共同で決める処置が不明なら確認済みと表示しない。
