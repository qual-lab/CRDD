...
外部コミュニケーションを対象に含む場合は、現在生成できる外部向け投影と公開済み記録が分離され、主張から根拠と正本へ辿れ、公開後の測定、観察、仮説、学び候補、人間判断が同じ状態へ丸められていないか確認する。受け手、訴求、獲得経路、媒体、広告または市場反応を新設・変更し、かつ認知、評価、試行、導入、継続または支持の形成を目的とする場合は、課題探索・要求形成の市場探索レンズへ接続し、採用契機、関係者、現在の代替、転換障壁、信用条件、チャネル意図、成果物の役割、反証および収束根拠を取得できるか確認する。職種・媒体属性と採用上の役割、観測値・代理指標・推定属性・本人申告、意味の異なる転換行動を混同せず、人間対象調査の選定偏りと一般化限界を隠していないかも確認する。追加探索の適用条件を判定できない場合は、通常のCommunication責務まで未評価へ戻さず、不足情報、確認先および再開条件を伴う確認待ちとしているか確認する。管理対象のコンテキスト依存または成果物依存を含む場合は、依存の種別、参照版、上書き、競合、利用側、更新判断を取得でき、参照元とローカル正本が無言で競合していないか確認する。通常または推移的な実装依存の存在だけで完全契約を要求せず、[コンテキスト依存](18_Context_Dependency.md)が定める独立管理利用側間の横断調整または重大リスクを必要とする依存を無言で通常依存へ落としていないかも確認する。
