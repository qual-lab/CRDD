...
| 文書の所有範囲 | 各項目がこの文書の目的、対象、決定権限および成果物種別に属するか、別文書の第二の正本を作っていないか |
...
対象リリース候補を監査する場合は、CRDD正本文書全体で`Version`、`Status: Candidate`、`Released Baseline`および`Last Updated`が対象改訂版と整合し、候補内容を公開済み基準または準拠可能なリリースとして誤認させないことを確認する。
...
人間のRelease判断は既に存在する統合後の確認済み最終Identityを一意に対象とし、確認後に対象branchまたはIdentityが変われば再固定する。承認時だけ同じIdentityへタグを付ける。
...
| 状態 | 意味 |
|---|---|
| `Pass` | 表示した対象範囲と必須観点を評価し、未解決重大 / メジャーがなく、残るマイナー / 情報に処置がある |
...
発行後の`finding_id`を別の不備へ再利用せず、指摘事項の対象、判定規則、確認した事実、根本原因および期待状態を履歴として保持する。訂正が必要な場合は元の記録を現在の意味へ上書きせず、訂正理由、正しい意味、影響する是正と根拠、再評価範囲、決定権限を持つ訂正または置換の記録へ接続する。置換した元の指摘事項は`Superseded`としてよいが、元の指摘事項と訂正後の指摘事項、是正対象および根拠の対応関係を失わない。
...
適用される標準と読み込み集合
状態 / 要約
適用した確認観点 / 実施した確認 / 未評価 / 適用外
確認した対象範囲 / 水平探索 / サンプリングと限界
指摘事項
追跡可能性不足
直接伝播の不足
未決事項 / 阻害条件
...
初回監査と再監査は[初回レビュー／監査の網羅性](10_Agent.md#76-first-pass-review-completeness)に従う。初回監査では、指摘事項を見つけた順に確定して返さず、適用する監査区分を一度走査し、候補の水平探索と共通原因の整理を終えてから一括して返す。

適用条件、例外、非該当、停止または正式結果を変更する文書では、発火例だけで可読性と整合を判定しない。非発火例、境界例、判定情報不足例から、定義、発火条件、判定不能時の扱い、発火後の処置または結果を読者が区別できるか確認する。代表例の説明が正本より強いまたは弱い、別の状態軸の語を流用する、主要ロケールやひな型で条件が縮退する場合は指摘候補とする。

会話、会議記録、観察またはAgent出力から成果物を作った対象では、情報源の口語や一人称を既定の本文として転記せず、事実、判断、理由、制約、仮説および未確定事項へ変換しているか確認する。曖昧な発話を成果物数、削除範囲、公開範囲または決定権限の最大解釈へ拡張していないかも確認する。

採用または公開へ進む成果物では、題材候補、構成案、本文Draft、レビュー済みDraft、公開候補および公開済み等の現在状態を、単一の`Draft`または完成度の印象へ丸めていないか確認する。外部表現への変換では、量、頻度、因果、比較、保証、確実性または一般化可能性が情報源より強くなっていないか確認する。レビュー記録では、未評価の必須観点を残したまま対象全体の`Pass`、公開準備完了または工程完了を表示していないか確認する。
...
複数の下位要素から構成され、部分成立を全体完成へ誤認し得る対象では、[アーキテクチャ](27_Architecture.md#23-インターフェースと統合契約)が定める必要な保証の一覧に対して、一部の操作、Adapter、分岐、単体試験または安全な拒否だけを根拠に、上位のインターフェース、Capabilityまたは実行境界全体を実装済み、利用可能、検証済みまたは完成と表示していないか確認する。部分接続では、接続済み範囲、未成立の必要な保証、本番経路への接続状態、未評価範囲および利用側の停止または代替が区別されているか確認する。上位完成を主張しない内部実装、一意な局所変換または単一結果へ同じ表示を要求しない。

複数の状態項目が共同で結果または利用側の処置を決める文書では、[アーキテクチャ](27_Architecture.md#24-状態処理順序副作用)が定める相関不変条件に照らして、成功／完了表示がcleanup未確認、Effect不明、Authority未成立、未解決Recoveryその他の状態と矛盾していないか確認する。個別fieldまたは個別試験が正しいことを、相関不変条件や上位成果の成立へ読み替えない。独立した説明項目へ存在しない相関を要求せず、共同で決める処置が不明なら確認済みと表示しない。
...
人間可読性は、少なくとも次の観点で確認する。

- **表示名の直感性**: 主要ロケールの表示名から概念の大まかな役割を推測できる。正式用語を変更できない場合は、近接する一言説明で補える
- **一言説明**: 正式定義を読む前に概念の輪郭を理解でき、説明自体が複数の未説明用語へ依存していない
- **用語依存**: 主要な結論を読むために別文書や別用語を連鎖的に解決する必要がある場合、最低限の意味、読書順または参照関係をその場で追える
- **文の複雑さ**: 前提、複数条件、責務、例外、判断および結果が一文へ過剰に詰め込まれず、意味を保った単位へ分けられている
- **情報構造**: 結論、原則、条件、例外、処置、期待結果および詳細が、見出し、箇条書き、表または段階的な開示から区別できる
- **表現形式**: 状態、条件、比較、責務、決定権限、Input／Output、遷移、履歴または受入条件が長い文章へ埋め込まれず、[構造を先に選ぶ文書表現](03_Documentation.md#104-structured-first)に従って直接比較・追跡できる
- **意味目的**: 一つの段落が一つの意味目的を持ち、定義、条件、例外、処置、主体、結果および理由の係り先が混線していない

文字数、段落長、用語数または表の数だけを指摘または合否の根拠にしない。機械的な長さ制限に合わせた結果、条件、例外、決定権限または規範強度が失われる修正を要求しない。

全体的な構造改善では、対象文書を省略せず母集団化し、各文書を少なくとも次へ分類する。

| 処置 | 判定 |
|---|---|
| 構造是正 | 長文のため条件、責務、状態、履歴または判断を誤読し得る |
| 正本移動または参照化 | 内容は正しいが、別成果物が所有すべき詳細または重複正本である |
| 変更不要 | 文章であることに意味があり、対象読者が必要事項を読み分けられる |
| 履歴保全付き可読性是正 | 原記録自身の変更または、固定原記録と現行正本を分ける構造化索引のいずれかで読取り負荷を下げる |
| 確認不能 | 対象読者、決定権限または意味保持を判定する根拠が不足する |

公開済みの`CHG-*`、Evidence、CHANGELOGまたはRelease記録も監査母集団から除外せず、現在文書と同じく一件ずつ表現構造を評価する。過去tag上の原記録、当時の事実、判断、改訂版および結果は変更しない。意味、規範強度、数値、時点および根拠Identityを保持できる場合は原記録自身を構造是正できる。

文書自身のbyte列が署名、Hash、公開履歴のIdentityまたは外部契約のEvidenceである場合は、原記録を書き換えない。その場合も可読性評価を省略せず、次を一件ずつ確認する。

- 読む目的から原記録へ到達できる構造化索引がある。
- 現在状態、現行正本および当時の記録を識別できる。
- 原記録のPath、固定Identityおよび表示変更しない理由を追跡できる。
- 原記録の内容を別文書へ複製し、第二の履歴正本を作っていない。

固定履歴であることだけを`変更不要`の理由にせず、可読性是正の結果を`原記録直接是正`または`固定原記録＋構造化索引`として記録する。Directoryや成果物種別だけで同じ処置を一括適用しない。

冒頭の要約だけでなく、本文から適用条件、処置、例外の係り先を読み分けられるかを[文書化の表示規則](03_Documentation.md#481-locale-first-display)に照らして確認する。長さや表の有無だけで合否を決めず、構造変更の前後で条件の論理、対象範囲、決定主体、時点および規範の強さを照合する。

人間向けの判断支援要約では、現在の対象改訂版と現在状態から判断要求を再構成し、監査またはレビューでは現在の固定改訂版を使っているか確認する。今回分かったこと、対応結果、実際の影響、現在も必要な判断、推奨および保留時の影響が読み取れるか、解消済み事項、AIが一意に修正できる事項または報告のみの事項を質問へ戻していないか、現在の作業を阻害せず安全に独立保留できる将来判断を担当責任者・再評価契機・保留影響・元根拠へ接続して現在の質問から外しているか、判断が残らない場合に人間判断不要と明示しているか、独立して保留できる判断と不可分な判断を適切に分けているかを確認する。条件不明な将来判断または現在必要な重大リスクを退避した表示、指摘事項ID、ファイル、監査順が利用者の判断より先に並ぶ表示、または未解決の重大リスクを詳細へ埋める表示は可読性と決定権限の指摘候補とする。

状態、バージョン、最終更新済み、リリース／CHANGELOGの関係を適用範囲で確認する。AIや文書編集者が状態を自己昇格していないか、非推奨／置換済み成果物の後継参照と移行があるか、削除が文書化の廃止規則と人間の承認に従っているかを確認する。

## 4.4. 決定権限・重複・情報保持の監査

各項目が、対象文書の目的、対象、決定権限および成果物種別に属するか確認する。内容が正しくても、別の文書が所有すべき概念、設計、手順、実装、検証または履歴を抱え込んでいる場合は、配置と責務境界の指摘事項とする。

この確認は「記載内容が正しいか」とは別に実施する。監査対象の文書名、冒頭で宣言した目的、所有しない情報の参照先および同じ内容を所有する候補文書を照合し、次の問いへ回答できなければ配置を未確認とする。

```text
この項目は、なぜこの文書に置かれるのか
この文書が持つ決定権限または成果物責務から導けるか
別の正本へ移した場合に意味を失うのか、それとも参照だけで足りるのか
この記載によって、別文書と更新元・判断元・検証元が分岐しないか
```

共通原則、工程規則、製品またはToolの設計、反復手順、実装規約、検証設計、検証結果および変更履歴を区別する。参照のための要約が必要な場合も、参照元が固定Path、期間、技術選定、状態機械、操作手順または合否結果を再定義して第二の正本になっていないか確認する。Tool固有条件をCRDD全体または採用Repositoryへ一般化していないか、反対に共通原則をTool固有文書だけへ閉じ込めていないかも確認する。
...
実装方式や操作手順が、保守、原則または工程規則の文書へ混入していないか
文書の見出しと本文責務が一致し、途中から別成果物種別の正本へ変わっていないか
...
品質保証成果物を対象に含む場合は、検証義務の正本が各工程成果物へ残り、品質戦略、検証設計、検証結果、現在の品質状態、Quality Centerの責務が[品質保証](16_Quality_Assurance.md)どおり分離されているか確認する。

各工程が所有する品質保証への寄与が、その工程の必要な責務の網羅、工程移行の判定基準、工程監査チェックリストへ接続され、専門品質確認から実際に評価できるかも確認する。

Quality Centerでは、人間が結論、計画対実績、分母、分子、割合、差異理由、重大な未成立、残存リスクと詳細参照を読み取れ、不成立、停止、要再確認の対象ごとに理由、影響、担当責任者、解除または再検証条件を確認でき、詳細または根拠を複製して第二の正本にしていないか確認する。

単体試験が適用される場合は、分岐網羅率`100%`の目標、測定対象、分母、分子、実測値、除外または未達の理由、残るリスク、代替確認、担当責任者、必要な人間判断、再確認条件が、検証設計、検証結果、Quality Centerで矛盾していないか確認する。

専門判断の探索と収束を対象に含む場合は、[スキル](11_Skill.md#23-expert-exploration-and-convergence-contract)の活動名や「探索済み」「批評済み」という表示だけで成立としない。判断を変え得る不確実性、工程固有のレンズ、参照したパターン・原理・反例、探索した母集団・方法・対象外、合成した有力な代替または有力でない理由、批評または反証、保持条件、残存不確実性、再評価契機および追加探索を終了した根拠へ到達できるか確認する。課題探索・要求形成から検証まで、各工程が定める専門レンズが共通契約の複製ではなく、工程固有の探索対象、候補合成、批評方法および収束阻害条件として接続されているか確認する。

外部コミュニケーションを対象に含む場合は、次の順序で確認する。

1. **記録と根拠**: 現在生成できる外部向け投影と公開済み記録が分離され、主張から根拠と正本へ辿れる。公開後の測定、観察、仮説、学び候補および人間判断を同じ状態へ丸めていない。
2. **認知意図**: 適用対象では、根拠付き仮説、必要な根拠／情報、当時と現在の意図、観測、履歴およびAIへ投影した対象改訂版を辿れる。非発火例へ認知状態を作らず、判定情報不足をAIで補完していない。
3. **市場・採用探索の発火**: 受け手、訴求、獲得経路、媒体、広告または市場反応を新設・変更し、かつ認知、評価、試行、導入、継続または支持の形成を目的とする場合だけ、課題探索・要求形成の市場探索レンズへ接続している。採用契機、関係者、現在の代替、転換障壁、信用条件、チャネル意図、成果物の役割、反証および収束根拠を取得できる。
4. **非発火と判定不能**: 追加探索条件のいずれかが不成立なら、もう一方が不明でも非発火としている。成立しない条件がなく、少なくとも一方を判定できない場合だけ、不足情報、確認先および再開条件を伴う確認待ちとしている。通常のCommunication責務全体を未評価へ戻していない。
5. **責務と観測値**: Discoveryの対象セグメント・採用行動・商業性と、Communicationの受け手・メッセージ・媒体・成果物・導線・測定仮説を分けている。段階例を固定状態遷移にせず、職種・媒体属性と採用上の役割、観測値・代理指標・推定属性・本人申告、および意味の異なる転換行動を混同していない。
6. **人間対象調査**: 市場探索の発火にかかわらず、人間対象調査を行う場合は、[課題探索・要求形成](21_Discovery.md)が所有する対象者の適格条件、選定・募集、偏り、対象外、一般化限界、継続・終了条件、同意、拒否、回答圧力、再識別可能性および不利益防止と、[外部コミュニケーション](17_Communication.md)での利用方法へ到達できる。
7. **行動喚起と説得**: 公開または行動喚起の前に提供能力と停止・代替導線へ接続し、反応から表現、UX、プロダクト、価格または運用の原因を直接確定していない。説得、広告、推薦、個別化または再接触が、欺瞞、重要条件の隠蔽、過度な不安、根拠のない緊急性、意思を妨げる操作または対象者の脆弱性の不当利用へ依存せず、表示の識別可能性、選択・拒否・訂正および不利益を対象リスクに応じて確認している。
8. **管理対象依存**: 管理対象のコンテキスト依存または成果物依存を含む場合は、依存の種別、参照版、上書き、競合、利用側および更新判断を取得できる。参照元とローカル正本が無言で競合せず、通常または推移的な実装依存の存在だけで完全契約を要求していない。[コンテキスト依存](18_Context_Dependency.md)が定める横断調整または重大リスクを必要とする依存を、無言で通常依存へ落としていない。
...
工程固有の専門探索レンズ / 品質保証の検証設計
情報分類 / 外部情報境界 / エージェント・ツール権限 / セキュリティ検証
...
専門探索または視覚制作を変更する場合は、共通骨格と工程固有レンズを重複正本化していないか、2D／3Dまたは各工程の適用境界が縮退していないかを確認する。差別化提案を扱う文書では、提案と採用済み方向を分け、人間判断前の提案が既存原則、契約、ブランドまたは工程成果物を上書きする表現になっていないかを確認する。

外部情報境界またはエージェント・ツールのセキュリティを変更する場合は、原則からエージェント、スキル、文書化、依存、外部コミュニケーション、外部視覚成果物、アーキテクチャ、実装、検証、品質保証、AI入口、公開案内、CHANGELOG、準拠基準まで直接伝播しているか確認する。接続母集団、許可した処理境界の目的・操作・送信先・利用主体・保持・二次利用・決定権限、境界外への内部コンテキスト直接送信禁止、削除だけでなく抽象化・最小化する境界、承認の非流用、派生・組合せ情報、分類不明時の停止、外部内容の指示権限、最小権限、供給網、セキュリティ不変条件、実行時強制、事故時の失効・回復が別文書で弱められていないか確認する。全ファイルへの固定ラベル、特定仲介ツールまたは実在シークレットによる試験を無条件に要求する表現も指摘候補とする。

指摘事項または是正を扱う文書では、次も確認する。

- 元の指摘事項の意味が、後続の是正、検証結果または根拠取得へ置き換えられていない
- 指摘事項を統合または分割しても、元の指摘事項と各是正対象の対応関係を辿れる
- 訂正または置換は元の履歴を保持し、影響した是正、根拠および再評価範囲へ接続している
- 固定時点の内容、固定後の結果、現在状態および履歴が同じ状態値として混在していない
- 現在状態の記録更新が意味更新を含まない場合、参照実在性、対象同一性および改変有無の軽量確認で終了し、その確認を無限に新しい監査対象へしていない
...
| 外部コミュニケーション | [17_Communication.md](17_Communication.md) |
| コンテキスト依存／成果物依存 | [18_Context_Dependency.md](18_Context_Dependency.md) |
...
- 適用した確認観点、実際に確認した対象範囲、水平探索した範囲、サンプリングを使用した場合の母集団／選択方法／限界が示されている
- 停止条件に該当しない限り、適用する監査区分の一次走査が完了し、指摘候補の重複と共通原因が整理されている
...
本監査が複数監査の一つとして実行される場合は、[複数監査の統合と監査間是正方針レビュー（Cross-Audit Remediation Reconciliation）](10_Agent.md#75-audit-aggregation-and-reconciliation)に従う。対象成果物を修正せず、他の監査が終了する前に自身の指摘事項だけを根拠として是正を開始しない。親エージェントが統合修正方針を再提示した場合は、文書構造、参照、用語、決定権限、重複、直接伝播の観点から、自身の指摘事項が解消されるか、重複を消した結果として必要な意味まで失われないかを確認して回答する。一つの原因が複数箇所へ及ぶ場合は、是正対象一覧が本監査で確認した参照元／参照先、派生文書、ひな型、例示、同義表現を処置対象または理由付き対象外として扱っているかも確認する。
...
- 不変・非active歴史参照がある場合、通常の破損link 0という主張へ含めず、観測数・Identity検証数・旧Pathの物理再出現を示すactive数・Git index再出現数を分離し、symlink、directoryまたはindexだけの再出現も隠さず、情報不足または集合の過不足をFail Closedにしている
- 適用する決定権限、安定コンテキストID、工程情報源と矛盾しない
- 直接伝播漏れがない
- 未確認対象範囲、マイナー / 情報、例外に処置がある
- 今回の受入条件に必要な確認観点がすべて評価され、未評価の必須観点を対象全体の`Pass`へ含めていない
- 情報源から成果物への意味変換、成果物成熟度および外部表現の主張強度が、確認できた決定権限と根拠を越えていない
- 指摘事項を`Resolved`とする場合は、[複数箇所へ及ぶ是正対象の列挙と照合](10_Agent.md#multi-location-remediation)が定める受入条件、合否判定方法、新しい根拠、同一固定改訂版の独立再レビュー、現在状態への伝播を確認できる
- 指摘事項の意味、訂正または置換、是正対象および根拠の対応関係を確認できる
- 個別指摘事項の`Resolved`だけから対象全体を`Pass`とせず、修正による新規候補、初めて確認可能になった候補、既存の見落とし、未評価範囲を分類・処置している
...
決定権限と直接依存を特定する
適用する監査区分を列挙する
全区分を一次走査し、根拠と指摘候補を収集する
同種の箇所、参照元／参照先、ひな型、AI入口を水平探索する
重複と共通原因を整理し、指摘事項を確定する
監査状態を判定する
是正と引き渡しを返す
許可がある場合は監査実行の外で是正する
合意した修正と影響箇所の全数適用、処置結果の照合、セルフチェック後に再監査する
