...
外部情報境界またはエージェント・ツールのセキュリティを変更する場合は、原則からエージェント、スキル、文書化、アーキテクチャ、実装、検証、品質保証、AI入口、公開案内、CHANGELOG、準拠基準まで直接伝播しているか確認する。内部コンテキストの直接送信禁止、削除だけでなく抽象化・最小化する境界、派生・組合せ情報、分類不明時の停止、外部内容の命令分離、最小権限、供給網、セキュリティ不変条件、実行時強制、事故時の失効・回復が別文書で弱められていないか確認する。全ファイルへの固定ラベル、特定仲介ツールまたは実在シークレットによる試験を無条件に要求する表現も指摘候補とする。
