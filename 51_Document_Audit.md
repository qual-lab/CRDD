...
人間向けの判断支援要約では、現在の対象改訂版と現在状態から判断要求を再構成し、監査またはレビューでは現在の固定改訂版を使っているか確認する。今回分かったこと、対応結果、実際の影響、現在も必要な判断、推奨および保留時の影響が読み取れるか、解消済み事項、AIが一意に修正できる事項または報告のみの事項を質問へ戻していないか、判断が残らない場合に人間判断不要と明示しているか、独立して保留できる判断と不可分な判断を適切に分けているかを確認する。指摘事項ID、ファイル、監査順が利用者の判断より先に並ぶ表示、または未解決の重大リスクを詳細へ埋める表示は可読性と決定権限の指摘候補とする。
