...
外部コミュニケーションを対象に含む場合は、現在生成できる外部向け投影と公開済み記録が分離され、主張から根拠と正本へ辿れ、公開後の測定、観察、仮説、学び候補、人間判断が同じ状態へ丸められていないか確認する。管理対象のコンテキスト依存または成果物依存を含む場合は、依存の種別、参照版、上書き、競合、利用側、更新判断を取得でき、参照元とローカル正本が無言で競合していないか確認する。通常または推移的な実装依存の存在だけで完全契約を要求せず、[コンテキスト依存](18_Context_Dependency.md)が定める独立管理利用側間の横断調整または重大リスクを必要とする依存を無言で通常依存へ落としていないかも確認する。
