...
Reference Runtime Architecture: [状態・資源・Lock・Recovery・検証接続](../../../06_Architecture/Details/coordinator/01_Architecture.md)
脅威境界: [Threat Model](../../../06_Architecture/Details/coordinator/02_Threat_Model.md)
...
開発検証はNode 24.19.0で起動契約5件、既存Coordinator全試験、strict／tests型検査、変更4TSのBiomeを終了コード0で確認した。実ConPTY端末では実CLIのhelp、automationのTTY拒否、署名の非TTY入力拒否を確認した。限定独立レビューではproductionの境界は整合し、試験fixtureが拒否の退行時にも無作用になるよう`doctor`を`--help`へ置き換えるMinor指摘を反映した。試験・静的検査と独立確認は、実Provider E2E・全体完成監査・Release判断を代替しない。

開発E2Eの定常コマンド`development-e2e:verify`にも起動契約試験を接続した。既存9試験ファイルと新入口の試験を同時に実行して終了コード0を確認し、実端末3ケースも再確認した。これは固定Fakeと契約・結合試験による検証であり、Release鍵・実Provider・新しい外部送信承認を使用していない。

### 既存の実測と現在状態

2026-08-31、直接端末へ表示したE2E結果を保存しておらず、画面閉鎖後に当該runの合否を取得できなかった。ユーザー承認に基づき、同じ変更内で4経路／復旧の通常CLIへ限定結果保存を接続する。対話stdoutを維持し、Repository-localの開始記録と終了記録、実行と保存の成否、非Authorityな診断記録と正式Evidenceを分離する。旧runは結果未確認のまま保持し、新たな成功を推定しない。設計・UI・仕様・検証・手順を同期し、公式署名版は変更せず開発試験で確認する。

保存処理の9試験と、それを含む開発E2Eの11試験fileをNode 24.19.0で通過した。開始／終了保存失敗、実行停止／例外、実子の途中終了、同時run、置換、上限、限定投影、未署名の公開Recovery入口を確認した。結果のflush・read-back成立後にhash付き完了記録を追加するため、JSONの構文だけから保存完了を推定しない。strict／tests型検査と変更TSのBiomeを確認し、全体Checkerは指摘0。実Provider、新しい正式署名、電源断後の永続性は未検証であり、この記録を正式E2Eの合格へ流用しない。

その後のproducer照合で、Recovery Matrixの単一回復IDにはTask形式のほかprobe／host形式があることを確認し、既存の生成・文法判定を再利用して全形式を保存対象へ接続した。保存試験へ実formatter由来のIDと不正・過長値を追加した。また全Coordinator試験で、開発E2Eの固定コマンドを検査する既存回帰試験が旧9fileを期待していた1件を検出した。期待する許可集合を同期し、このguard自体も定常E2Eへ接続した。回復文法、実回復、外部送信、署名の許可は変更していない。

追加是正後の全Coordinator試験、実装・試験の型検査、変更6TSのBiomeおよびRuntime traceabilityは成功した。全体Checkerは2026-08-31T13:00:12.348Zに383文書・2,636リンク・879アンカー・固定履歴24件を確認し、エラー／警告0。限定独立再レビューは追加指摘0で、回復IDの保持と定常試験への接続不足を解消した。この確認は起動・保存機能の開発検証であり、閉じた画面の実測結果や最新の正式4経路E2E成功を復元するものではない。次の固定配布候補で実測を再開し、工程強化・実務評価の残る検証と最終一括監査へ接続する。
...
2026-08-30、実務Reviewerのturn上限停止を受け、基準Commit `57ccb71`から推論強度と作業量を分離した。Windows Job Objectの上限変更ではない。検証済みTask Packetを消費したRuntimeが読取り・変更範囲、受入条件、是正指摘の件数を導出し、[実行Architectureの有限見積り](../../../06_Architecture/Details/coordinator/01_Architecture.md#task-turn-budget)に従ってClaudeの実行上限を決める。最大16を超える見積りは分割要求として停止する。Provider Authority発行・起動は行わず、有効化済みMountは既存cleanupへ返す。推論を上げることで作業枠を増やす旧結合は廃止し、結果検証とDocker argv再構成も同じ作業量へ接続した。同じ上限になる作業量の差替えも実行計画Identityで拒否する。
...
| 実務自己適用・有用性評価 | 親Coordinator。上記の計測不足を処置した後 | 既存の[有用性評価](../../../01_Discovery/Analysis/EXP-000004/exploration.md)へ結果を接続し、採用可能な成果、人間負荷、不要な往復、Provider利用集中を評価する。今回の2Task成功だけで全件完了にしない |
...
| 有用性全体の評価 | 親Coordinator。実務自己適用の継続時 | [有用性評価](../../../01_Discovery/Analysis/EXP-000004/exploration.md)に沿って人間負荷・採用可能な成果・品質・利用分散を評価。今回の記録だけで全体完了にしない |
...
- 次版の検討材料は[Discoveryの既存候補](../../../01_Discovery/Analysis/EXP-000013/exploration.md)とCHG-000055から追跡する。呼出し元別計測と共有単位の設計は未採用であり、遅延を理由にTTL cacheや検証省略を追加しない。
...
終了履歴の読取りに現在ログオン一致まで要求していた実装を、[設計](../../../06_Architecture/Details/coordinator/01_Architecture.md#22-docker-desktop最終復旧時の起動環境)に従って是正する。通常の終了記録と明示引継ぎ・終了済み記録を対象とし、未終了記録の権限は引き継がない。元の履歴・署名配布物・OS状態は書き換えず、新規操作には現在境界を使う。新しいCHGへ分離せず、本変更の復旧契約の是正として扱う。
...
追加の反例では、連鎖Hashと引継ぎ・終了receiptの参照Hashを整合させ、途中recordのログオンHashだけ異なる場合も拒否することを確認した。通常終了3状態、未終了の拒否、新ログオンでの新規記録、実StoreからRuntimeの終了表示までを確認した限定独立レビューは指摘0件。Coordinator全体試験はpackage配下からの実行で成功した。先行するRepository Rootからの誤起動によるCLI参照失敗は成功結果に算入しない。起動場所を規定した検証入口の使用が必要であり、任意の作業Directoryで全試験が成立するという主張はしない。

## Docker起動前のOS環境取得の是正（2026-08-31）

署名候補`109b33f`では過去の終了履歴検証を通過し、承認済み修復でDocker停止・退避を確認した。しかし`docker_desktop_restart_unconfirmed`で停止し、今回ID `docker-desktop-repair.f8d393f1de388670efd7b9da52ad3d57`の耐久台帳はlauncherを`not_issued`と記録した。退避物と記録は保持し、削除・Provider送信・追加修復を実行していない。Docker Engineは未起動であり、正式E2Eは未完了。

読取り専用の実OS試験では、production helper環境の空`SYSTEMDRIVE`により`FOLDERID_ProgramData`が相対Pathを返し、実体一致確認で停止することを再現した。同じ環境の一変数だけをOS確認済みdriveへ置換すると絶対Pathと実体の一致が成立した。親の環境継承や固定Pathで回避せず、Docker専用helperとnative launcherの二段でOS由来driveを渡す是正とする。他用途の環境、既知フォルダー検証、署名・権限・回復契約は変更しない。

既存の環境Hash一致試験は中間helperの必須OS文脈を確認せず、子側のcanonicalize比較も相対Pathを絶対化して不足を隠していた。非C drive・相対／UNC拒否、親値偽装、実子のneutral保持、既知フォルダー取得時点の絶対Path性を回帰確認へ追加する。工程強化への学び候補は「最終出力だけでなく中間実行環境の前提を確認し、正規化前に禁止入力を検証する」とし、本CHGの既存ドッグフーディングへ接続する。担当はRuntime保守。実修復の再開・終了、正式E2Eおよび完成監査が残り、読取り試験だけをDocker復旧成功としない。開発反復に追加署名や秘密入力は要求しない。

是正後の確認では、Coordinator全体の回帰試験、TypeScriptの実装・試験型検査、変更箇所のBiome、Runtime traceability確認が成功した。Rust workerは19件成功し、直接実行しない子試験1件も親試験から起動して成功した。Rust整形とClippyも成功。production helper用の実環境からRust試験実行物を起動し、同じ`CreateProcessW`経路で子の既知フォルダー取得・終了を10回連続確認した。これは今回の限定実測であり、恒久CI接続またはDocker本体の起動成功とは扱わない。全体Checkerは383文書・2,629リンク・876アンカーを確認し、error 0／warning 0。変更6ファイルを固定した限定独立安全レビューは指摘0件だった。正式配布反映と保持中の回復記録の処置を経て、未完了の実修復・E2Eへ戻る。
