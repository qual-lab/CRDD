...
Source A署名後のManifest取込みでは、stagingからRepositoryへ人手で内容を転記した結果、編集処理が末尾LFを1 byte追加し、署名済みfileのSHA-256と不一致になった。同じ種類の問題は過去にもEvidence保存時の末尾LF追加として観測され、今回も手動copyを正規経路に残したことでRelease境界へ再発した。個別にLFを削る処置ではなく、A→Bを専用の昇格Effectへ変更した。昇格入口は署名、Source A Commit／Tree、Runtime実行集合、Policy、Native成果物、現在HEADをEffect前に結合し、Manifestをparse／serializeせず不透明な安定byteとして扱う。最終Pathへ段階writeせず、同一Filesystemの排他的hard linkで完成済みfileだけをatomicに公開し、開始時sourceと公開後の二名が同一file objectであること、byte数、SHA-256および署名済み配布条件を再確認する。Path検査後のsource削除は別主体のfileを消し得るため公開Effectから除外し、staging名は所有Rootを確認する別の明示破棄へ委ねる。中断後はsourceのみ、同じfile objectを指す二名、明示破棄後のdestinationのみから再開し、別Identity、内容変化または観測不能を削除しない。これにより、編集器、Shell encoding、末尾改行、部分file、削除競合および手作業の取り違えを正式経路から除く。
