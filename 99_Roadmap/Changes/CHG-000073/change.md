...
| 第10回指摘 | 原因 | 構造是正と反証 |
|---|---|---|
| UI-000002内に旧委任状態と導線が残存 | 詳細節だけを更新し、統合判断と表示面をConsumerとして閉じなかった | Definition全体を提案・受付可能・受付済み・拒否・同じ提案の範囲見直しへ統一し、対応Evidenceの引用も現行正本へ再固定した |
| UI-000016に採用・却下・保留の三結果が未投影 | SPEC-000027の三分岐をEvidenceで補い、UI Ownerへ戻さなかった | UIの操作、状態、Feedback、戻り先、Handoffへ三結果を正本化し、却下・保留では同じ候補と出所へ戻り所有正本を変更しない契約を保持した |
