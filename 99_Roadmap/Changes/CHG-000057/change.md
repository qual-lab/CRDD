...
Linux／macOS Runtimeの実装、配布、認証、回復および実Provider E2Eはv0.19の対象外とする。一方、Coordinator／Platform責務分離では、Project Runtime Coreへ新しいWindows固有依存を持ち込まず、現在実在するOS依存を[参照アーキテクチャ](../../../06_Architecture/project-runtime/01_Architecture.md)のPlatform Contractへ閉じる。MCPはTransport AdapterとしてPlatform Contractと直交させ、stdio／HTTP等の搬送方式や対象OSが変わってもProject Model、Authority、状態遷移および成功条件を変えない。
