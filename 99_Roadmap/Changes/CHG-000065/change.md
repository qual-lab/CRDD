...
v0.20.0のリリース前に、CRDD Repository内の人間可読文書を全数対象として、文章中心の説明を意味の種類に合う構造へ変更する。情報量を削減すること、表の数を増やすこと、または過去tag上の原記録を変更することは目的にしない。当時の全数処置対象と分類は[Disposition Inventory](../../../07_Quality/07_Structured_Document_Disposition_Inventory.json)へ品質Evidenceとして固定し、本書へPath一覧を複製しない。Inventoryは現在文書の常時台帳または通常Checkerの入力ではない。
