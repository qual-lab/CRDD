...
状態: Stable遷移前のRelease計画。Release判断、main統合、tag付与は未実施
...
| 統合先 | `main` |
| 計画固定時の候補Commit | `9663764a0b52a40e2eed4f80d9fdb680de36d054` |
| 計画固定時の候補Tree | `d7dae5bf54685a7f8d80ae5ee6df2f0a43433aee` |
...
- 本記録へ最終候補Commit／Tree、Checker、独立監査および残る操作を追記する。
...
1. 宣言済みPath以外の差分がないことを確認する。
2. Repository全体Checkerを実行し、Candidate／Released Baseline残存、README、CHANGELOG英日、Change状態およびLinkを確認する。
3. 文書監査、準拠影響確認、不足／影響監査を同じ最終候補Commit／Treeへ実行する。
4. 最終候補Commit／TreeとPR対象branchを固定する。
5. PRでは内容を変更せず`main`へ統合する。統合方式はCommit／Treeを維持できる方式を選ぶ。
6. 統合後の`main`が最終候補と同一なら、同じCommitへ`v0.21.0`tagを付ける。異なる場合はtagを停止して再固定する。
...
- [ ] Stable遷移後の最終Commit／Treeを記録する。
- [ ] Repository Checkerと独立監査を最終候補へ実行する。
- [ ] PR、main統合、統合後Identity確認およびtag付与を完了する。
