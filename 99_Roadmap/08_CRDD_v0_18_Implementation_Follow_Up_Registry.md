...
- Codex／Claude Codeの両Provider Adapterは一般Task経路へ接続済みである。両経路とも正式署名一般Task実run前は`Completed`へ昇格しない。
