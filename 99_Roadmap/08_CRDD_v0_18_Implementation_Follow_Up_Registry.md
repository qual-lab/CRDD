...
| `FU-018-PROVIDER-HOME` | Provider Home保護とMount Grant | In Progress | local OS user単位の専用Home、settings分離、opaqueな一回限りmount許可と失効 | [CHG-000029](../90_Release/Changes/CHG-000029_Provider_Home_Mount_Grant_Lifecycle_Foundation.md) | 構造・遷移・使用候補Coreは実装済み。次はselected-user binderと保護Effectを先に実装し、Runtime所有issuer／atomic store／clock、mount／失効Effectへ接続する |
