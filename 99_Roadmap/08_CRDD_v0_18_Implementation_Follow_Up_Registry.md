...
| `FU-018-PROVIDER-HOME` | Provider Home保護とMount Grant | In Progress | local OS user単位の専用Home、settings分離、opaqueな一回限りmount許可と失効 | [CHG-000030](../90_Release/Changes/CHG-000030_Provider_Home_Mount_Grant_Runtime_Store.md) | 構造Coreに加えてRuntime所有issuer／atomic store／clock、一回消費および明示失効を実装中。次は固定版の確認後、selected-user binderと保護Effect、実mountおよびOperation終了時自動失効へ接続する |
