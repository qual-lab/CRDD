...
|---|---|---|---|---|
| Coordinator Runtime 1.0の完成固定 | —（非適用） | In Progress | [CHG-000015](../90_Release/Changes/CHG-000015_Coordinator_Runtime_1_0.md)、[有用性評価](../01_Discovery/01_CRDD_Product_Discovery.md#runtime-utility-evaluation) | Docker復旧後の固定署名版0c3e6d2の4経路4/4・復旧7/7の結果はCHGへ移管済み。これをRuntime全体の完成へ読み替えず、実務自己適用で採用可能な結果までの時間、人間の実作業時間、AI処理量、不要Loop、Provider集中、品質および処理量の最小Baselineを取得する。現行正本、残存リスク、完了条件と実装依存閉包を再計算し、必要な是正と固定版の一括監査後、人間のPR／統合判断へ渡す |
...
| v0.18内部Tool命名Baselineの収束 | —（非適用） | In Progress | [CHG-000017](../90_Release/Changes/CHG-000017_Tools_Coding_Standards.md)、[内部Tool規約](../06_Architecture/99_Coding_Standards.md) | 所有件数とRepository-local `.crdd`、版固定Policy、Plain text成果物およびspawn Workerの所有漏れを是正し、全数検査で顕在化した243件のsource／test命名違反を抑制・allowlistなしで0件へ収束した。公開Schema keyを維持したまま、命名契約7 / 7、Coordinator全試験1,211 / 1,211、Trace／型／Lint／FormatおよびRepository Checkerを確認済み。最新固定改訂版の独立レビューと必要監査へ渡し、結果をCHG-000017へ反映する |
