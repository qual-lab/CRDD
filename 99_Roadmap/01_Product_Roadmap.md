...
|---|---|---|---|---|
| v0.19 Minimum AI-native Project Runtime | Adopted | In Progress | [Discoveryの採用境界](../01_Discovery/01_CRDD_Product_Discovery.md#v019-minimum-project-runtime)、[CHG-000057の依存順と完了条件](../90_Release/Changes/CHG-000057_Minimum_AI_Native_Project_Runtime.md#9-設計確定からリリース判断までの実行計画)、[Project Runtime詳細設計](../06_Architecture/coordinator/03_Project_Runtime_Design.md) | 設計とCoordinator／Platform責務分離の確認を完了し、耐久基盤候補を部分接続した。Process owner喪失後のRecovery公開入口は未成立である。次は単一Task縦断へ接続し、その後に複数Task、再計画、人間判断、統合、結合確認、自己適用、リリース判断準備を進める。Linux／macOSは実装せず、Project Runtime CoreとMCPのPlatform非依存を維持する |
...
1. 確認済みProject Runtime設計を境界付きTaskへ分け、部分接続済みのCoordinator／Platform責務分離と耐久基盤を、単一Task縦断、複数Task、再計画、統合へ順に接続する。
