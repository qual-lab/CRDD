...
2026-09-01、検証済み候補の内容と移行方針を人間が採用し、PR #32をmainへ統合してv0.18.0を公開した。Runtime完成、工程接続、根拠駆動リファクタリング、命名、本文可読性およびツール配置は、根拠を各CHG・品質記録へ接続して本登録簿から除去した。v0.18.1のCoordinator新規採用入口是正は、最終監査でRuntime実行Identityの依存閉包不足を検出したため、修正・再検証中である。公開状態は公式タグまたは同等の不変なRelease識別子で確認する。

| 作業 | 判断状態 | 対応状態 | 情報源 | 次の処置／再評価契機 |
|---|---|---|---|---|
| Coordinator新規採用入口とRuntime実行Identityの是正 | —（非適用） | In Progress | [CHG-000056](../90_Release/Changes/CHG-000056_Coordinator_Adoption_Interface_Correction.md) | 共通Launcherから到達する署名・4経路・Recovery依存をIdentityへ結合し、固定候補の機械試験、署名、fresh clone／submodule一般Task、4経路、Recoveryおよび独立確認を完了した後にRelease判断へ戻す |
