...
|---|---|---|---|---|
| v0.19 Minimum AI-native Project Runtime | Adopted | In Progress | [Discoveryの採用境界](../01_Discovery/01_CRDD_Product_Discovery.md#v019-minimum-project-runtime)、[CHG-000057の依存順と完了条件](../90_Release/Changes/CHG-000057_Minimum_AI_Native_Project_Runtime.md#9-設計確定からリリース判断までの実行計画) | Project Runtime設計、機械可読な設計対応、Checker、契約試験、独立Architecture／Securityレビューおよび文書Scope／Gap／Impact確認を完了した。次はCoordinator／Platform責務分離へ進み、耐久State・Queue・Lease、単一Task縦断、複数Task制御、進捗／再計画、人間判断、統合、結合確認、自己適用、リリース判断準備の順で接続する。Linux／macOSは実装せず、Project Runtime CoreとMCPへOS固有依存を持ち込まない境界までを今回成立させる。1 Project／1 Repository／1 Parent Coordinator、人間起点、最大同時実行5を維持する |
...
1. 確認済みProject Runtime設計を境界付きTaskへ分け、Coordinator／Platform責務分離から耐久基盤、単一Task縦断、複数Task、再計画、統合へ順に接続する。
2. 各段階で設計対応と実装・試験・終了後観測を照合し、段階単位の結合結果へ統合する。
3. E2Eと自己適用後、各変更の独立Release Gateを評価し、v0.19の収載内容を人間が確定する。
