...
1. Project RuntimeのD0設計を固定し、機械可読な設計対応と独立確認を完了する。
2. 固定設計を境界付きTaskへ分け、Coordinator責務分離から耐久基盤、単一Task縦断、複数Task、再計画、統合へ順に接続する。
