...
|---|---|---|---|---|
| v0.20 実行知（Execution Intelligence） | Adopted | Ready for Verification | [CHG-000062](../90_Release/Changes/CHG-000062_Execution_Intelligence.md) | Coordinatorから独立した共通コンポーネントとして、仕事Identityへ結合した閉Event、Git管理外の不変Store、欠測を保持する集約、非Authority改善候補、およびexact Identity・Hash・未解決参照0・耐久Evidenceを要求する物理清掃を実装した。Project Runtimeは専用AdapterからEventを発行し、Single Task Runtimeが返す検証済み実効Executor Providerだけを観測する。未提供のModel／利用量／人間時間は未観測として保持する。品質受入、Viewer、Project State投影、共有Store、運用・事業結果は未接続であり、本項の成立から完成を推定しない。固定候補の独立レビューと全自動回帰を次の処置とする |
