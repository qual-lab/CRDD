...
|---|---|---|---|---|
| Coordinator Runtime 1.0の完成固定 | —（非適用） | In Progress | [CHG-000015](../90_Release/Changes/CHG-000015_Coordinator_Runtime_1_0.md)、[有用性評価](../01_Discovery/01_CRDD_Product_Discovery.md#runtime-utility-evaluation) | 成功Vertical SliceをRuntime全体の完成へ読み替えず、署名入力なしの開発E2Eで4経路・一般Task・Recoveryを収束させる。全機械確認後に凍結した候補だけを正式署名E2Eへ一度通し、現行正本、残存リスクおよび完了条件を再計算する。Dogfoodingでは安全な起動数だけでなく、採用可能な結果までの時間、人間の実作業時間、AI処理量、不要Loop、Provider集中、品質および処理量の最小Baselineを取得する。必要な是正と固定版確認後、人間のPR／統合判断へ渡す |
...
| v0.18内部Tool命名Baselineの収束 | —（非適用） | In Progress | [CHG-000017](../90_Release/Changes/CHG-000017_Tools_Coding_Standards.md)、[内部Tool規約](../tools/coding-standards.md) | 所有件数とRepository-local `.crdd`、版固定Policy、Plain text成果物およびspawn Workerの所有漏れを是正し、全数検査で顕在化した243件のsource／test命名違反を抑制・allowlistなしで0件へ収束した。公開Schema keyを維持したまま、命名契約7 / 7、Coordinator全試験1,211 / 1,211、Trace／型／Lint／FormatおよびRepository Checkerを確認済み。最新固定改訂版の独立レビューと必要監査へ渡し、結果をCHG-000017へ反映する |
| 既存・過去CHGを含む人間可読文書の意味構造改善 | Adopted | In Progress | [人間可読文書の改善候補](../01_Discovery/01_CRDD_Product_Discovery.md#733-人間可読文書の意味構造改善候補)、[文書化](../03_Documentation.md#481-locale-first-display)、[CHG-000055](../90_Release/Changes/CHG-000055_CRDD_Long_Term_Evolution_Roadmap.md) | 統合台帳へ「現在のCanonical CHG→旧ID復元→機械検証詳細」の順で読む入口を追加した。公開済みCHG本文や固定Evidenceは改稿せず、まず索引と補助投影で現在地を理解できる形にした。最終固定版でREADME、現行正本、監査文書および過去CHGの水平確認を行い、意味・Authority・Evidence・時系列を変えない追加是正が必要か判定する |
