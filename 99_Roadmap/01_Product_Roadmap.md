...
|---|---|---|---|---|
| Coordinator Runtime 1.0の正式署名一般Task実行 | —（非適用） | In Progress | [CHG-000015](../90_Release/Changes/CHG-000015_Coordinator_Runtime_1_0.md)、[CHG-000038](../90_Release/Changes/CHG-000038_Claude_Subscription_OAuth_Vertical_Slice.md) | 正式署名配布物から、一般Task、両Provider経路、Provider Home、Mount Grant、限定Egress、終了、回復および残存0を同一runで確認する。人間だけが扱う署名鍵passphrase、必要なOAuth再認証または外部Provider送信が生じる時点で人間へ返す |
| 上流工程エージェントと課題探索対話Loopの強化 | Adopted | Planned | [CRDD標準自身の課題探索・要求形成](../01_Discovery/01_CRDD_Product_Discovery.md) | Coordinator Runtimeの正式署名一般Task実行と完成固定版確認後、現行正本と影響を再確認して人間の着手判断を得る。その後、このRuntimeを使って一つの主変更意図からCHG、専門探索、実装および独立レビューを行う |
...
| CHG-000055による長期発展方針の記録 | —（非適用） | In Progress | [CHG-000055](../90_Release/Changes/CHG-000055_CRDD_Long_Term_Evolution_Roadmap.md) | 同一固定版の文書監査、アーキテクチャ／セキュリティ確認および不足／影響監査をFinding 0で完了し、Release引き渡し状態を記録する |
