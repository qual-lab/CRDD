...
| v0.18内部Tool命名Baselineの収束 | —（非適用） | In Progress | [CHG-000017](../90_Release/Changes/CHG-000017_Tools_Coding_Standards.md)、[内部Tool規約](../tools/coding-standards.md) | 所有件数を現行Treeへ再固定し、Repository-local `.crdd`、版固定Policy、Plain text成果物およびspawn Workerの所有漏れを是正した。全数検査で顕在化した243件のsource／test命名違反を、machine contractを変えず変更単位で収束させ、Checker契約試験を全件再実行する。抑制またはallowlistでPassにしない |
