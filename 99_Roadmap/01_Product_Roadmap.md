...
| v0.20 Runtime責務分離 | Adopted | Ready for Independent Review | [CHG-000063](../90_Release/Changes/CHG-000063_Runtime_Responsibility_Separation.md)、[Project Runtime設計](../06_Architecture/project-runtime/01_Architecture.md)、[MCP設計](../06_Architecture/mcp/01_Architecture.md) | Project RuntimeをProject-level execution lifecycleのApplication Core、Coordinatorを実行編成、MCPをTransport、Execution Intelligenceを観測・分析、Platform AccessをOS／Platform境界へ分ける。Project RuntimeはExecution Port等の必要能力だけを要求し、Coordinator／Persistence／Platform／実行知Adapterが実装する。Project状態機械、公開契約、Application調停、必要Port、構成RootおよびMCP Transportの移行を完了し、Coordinator側のObjective入口は未信頼入力・Binding・Host Adapter構成だけを担う。Project Runtime全ソースの推移的依存検査、CLI／MCP stdioを含む利用側回帰、全体Checkerは成立済み。分離完了表示は独立レビューと公開入口の最終総合確認まで保留する |
| v0.20 限定分散実行と統合結果の評価 | Adopted | Implementation in Progress | [CHG-000062](../90_Release/Changes/CHG-000062_Execution_Intelligence.md)、[Discoveryの採用境界](../01_Discovery/01_CRDD_Product_Discovery.md#bounded-distributed-execution-candidate) | v0.19で成立したTask Graph、最大5並列、再計画および統合を作り直さず、一つの目的の予定Task、実Attemptおよび統合後の受入結果を同じ評価へ接続する。個別Taskの成功を統合受入と同一視せず、欠測、Provider利用、完成時間、人間の実作業時間、競合、手戻り、不要Loopおよび後工程品質を観測できた範囲で保持する。共通評価契約と決定論的試験を実装中であり、実務上の速度・費用改善は限定実測と独立レビューまで未成立とする |
| v0.20 Project Stateの読み取り専用投影 | Adopted | Implementation in Progress | [CHG-000064](../90_Release/Changes/CHG-000064_Project_State_and_Local_MCP_HTTP.md)、[Discoveryの採用境界](../01_Discovery/01_CRDD_Product_Discovery.md#v020-read-only-project-state-projection) | Project RuntimeのState Portから`readState`だけを受け取る公開Applicationと、`observed / absent / unknown`を区別するcanonical結果を実装した。MCP stdio／HTTPは同じ結果を利用し、ProjectionからTask、判断、Authority、成功、進捗率または正本変更を生成しない。独立したPM台帳、予測機能、Topic管理またはフルProject Management機能を意味しない。MCP公開LauncherとCoordinator公開Adapterの縦断、独立レビューおよび最終総合確認まで完成表示を保留する |
| v0.20 ローカルMCP Streamable HTTP接続 | Adopted | Implementation in Progress | [CHG-000064](../90_Release/Changes/CHG-000064_Project_State_and_Local_MCP_HTTP.md)、[Discoveryの採用境界](../01_Discovery/01_CRDD_Product_Discovery.md#v020-mcp-streamable-http) | `2026-07-28`のPOST単位・SessionなしのTransportを、`127.0.0.1`固定、Bearer認証、Origin確認、mirror header照合、bounded UTF-8、切断取消および終了時join付きで実装した。stdioと同じObjective、Decision、Project State公開契約へ到達し、TransportからProject Authority、成功、Repository操作権限またはRecovery Authorityを生成しない。LAN／Internet公開、Remote常設運用、複数Repository、Organization RuntimeまたはSelf-hosted Providerを意味しない。公開Process縦断、独立レビューおよび最終総合確認まで完成表示を保留する |
...
| OSS Runtimeの利用者所有Trust Policy | Exploring | Unscheduled | [OSS Runtime Trust Policy候補](../01_Discovery/01_CRDD_Product_Discovery.md#oss-runtime-trust-policy-candidate) | Qual-Lab公式署名を公式Buildの発行者・完全性Evidenceへ限定し、CRDD準拠性、Artifact完全性、発行者信頼および配置先の実行許可を分離する候補。Fork／組織Buildの独自署名や用途限定の未署名Local開発を、Deployment Ownerが所有するPolicyで扱える可能性をv1前に再評価する。現行v0.20の署名・実行Authorityは変更せず、例示設定を採用済みSchemaまたは未署名実行許可として扱わない |
