...
振る舞い仕様の正式入力は、UX DefinitionとIA Definitionである。次の一覧では、正式入力が所有する意味と、後で独立に照合する比較候補を分ける。

### 正式入力

- 情報源となるUX / IA、保持する意図、目指さないこと、検証義務
...
### UI／SPEC対応レビューで照合するもの

- 同じUX／IA Definitionから独立に導出されたUI契約候補
- UIの操作・Feedback候補と、SPECの契機・結果・失敗・回復の対応

### Canonical化後のReality Auditで照合するもの

- 既存の振る舞い、コード、旧文書およびその改訂版
- 現行利用側、運用Feedbackおよび既存試験

通常は[IAの完了条件と引き渡し](23_IA.md#exit-and-handoff)からUX定義とIA定義の組を受け取り、UIと並行・反復して具体化する。REQはUXを形成した上流トレースとして辿れるが、振る舞い仕様分析の正式入力にせず、REQから振る舞い、例外または受入条件を直接補完しない。UXまたはIAに必要な意味が不足する場合は、その工程へ戻してから分析を再開する。アクター／決定権限、対象範囲、人間レビューが不足する場合もUX、IAまたは人間による判断へ戻す。IAが`Partial — Human Authorized`の場合は、承認された対象範囲だけを扱い、未網羅項目、リスク、後続担当責任者を引き継ぐ。
...
振る舞い仕様は、UX定義とIA定義を別々の正式入力として全数分析し、SPEC定義で初めて統合する。
...
- `Analysis/UX-*/spec_analysis.md`は、利用者、状況、目的、得たい結果、重要場面、避ける失敗および品質期待を、観測可能な契機・結果・不成立へ変換する。IA、UIまたはREQから不足を補完しない。
- `Analysis/IA-*/spec_analysis.md`は、対象、識別、関係、状態、可視性、導線および責任を、入力・条件・状態・結果・開示境界へ変換する。UX、UIまたはREQから不足を補完しない。
- `Definitions/SPEC-*/spec_definition.md`は両分析を、契機、事前条件、決定権限、状態、振る舞い、結果、失敗、回復、副作用および受入条件へ統合する。
- REQはUXを介した上流トレースとしてだけ辿る。UXまたはIAに必要な意味がなければ、現行コード、旧SPECまたはREQから補完せず、該当工程を再開する。
- 現行実装と旧SPECは、統合後の欠落・競合を探す独立した照合入力であり、望ましい振る舞いを決める正本ではない。
...
対象範囲の各UX / IAの組、ユースケース、利用者操作、振る舞い上の義務について、次を網羅する。

| 責務 | 必要なコンテキスト |
|---|---|
| 情報源と対象範囲 | 情報源となるUX / IA、目的、保持する意図、目指さないこと、検証義務、改訂版、関係。REQは上流トレースに限定し、SPECの直接入力にしない |
...
- 情報源となるUX / IA、対象ユースケース / 利用者操作へトレースでき、上流REQはUXを介して辿れる
...
`SPEC-*`は、同じ契機・事前条件のもとで一つの観測可能な結果を成立させ、失敗・回復・副作用を他と分けて変更・検証できる振る舞い単位へ付与する。文書名・機能名・UI ID・EARS文・テスト名・文書番号と安定コンテキストIDを同一視しない。一つのUIが複数SPECを利用する場合、一つのSPECを複数UIから利用する場合、直接UIを持たないSPECがあるため、UIとSPECを機械的な一対一にしない。

次のものだけにはSPEC IDを発行しない。

- 画面、UI部品、表示領域または操作手段
- 情報項目、状態値、Error CodeまたはSchema field
- 品質期待、検証観点、テストケースまたは実装関数
- Architecture方式、Process、Adapterまたは保存技術

迷う場合は、「Actorが特定の状況で契機を発生させ、Systemが条件と状態に応じて独立した結果を返す」と自然に記述でき、その成功と失敗を他の振る舞いから分けて受入判定できるかを確認する。
...
```text
対象範囲 / 網羅範囲要約 / 未解決不足
情報源となるUX / IA / 対応づけ単位 / UXを介した上流REQトレース
SPEC ID / 状態 / 情報源
目的 / 保持する意図 / 目指さないこと
...
UI、アーキテクチャ、実装、検証、運用から、システム状態、失敗、回復、互換性、処理能力、受入条件に関する学びが得られた場合はSPECへ戻す。同じ意味の明確化・訂正は同じ`SPEC-*`を維持し、成果物の固定改訂版とCHG／Gitで追跡する。意味を置換する場合は新しい`SPEC-*`を発行して`supersedes`で接続する。
