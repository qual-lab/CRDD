...
| 対象 | Working directory | 結果 |
|---|---|---|
| Coordinator `check` | `tools/coordinator` | TypeScript typecheck、Biome Warning拒否、format確認成功（152 files） |
| Coordinator全試験 | `tools/coordinator` | 435件成功、失敗0 |
| Checker `check` | `tools/checker` | TypeScript typecheck、Biome Warning拒否、format確認成功（152 files） |
| Checker全試験 | `tools/checker` | 151件成功、失敗0 |
| Rust format | `tools/coordinator` | 成功 |
| Rust lint | `tools/coordinator` | workerとrelease-only native bootstrapのClippy Warning拒否で成功 |
| Rust試験 | `tools/coordinator` | unit 8件、worker CLI 1件、native Core 2件の計11件成功、失敗0 |
| Rust release build | `tools/coordinator` | workerとfeature付きnative bootstrapのfrozen release build成功 |
| TypeScript coverage | `tools/coordinator` | 固定source 24、test 24、2回のpayload完全一致 |
| Rust coverage | `tools/coordinator` | instrument対象source 6、除外source 1、2回のJSON payload完全一致 |
| Native PE／実CLI確認 | `tools/coordinator` | 連続2runのstdout完全一致、各runの2 clean buildも一致 |
| Repository全体Checker | `.` | Markdown 352、link 2030、anchor 581、error 0、warning 0 |
...
- run 1生stream: 4378 bytes、SHA-256 `931b3e783003eedc2b89c5ef9ec78e60d8673e4c96da0a782332b3681cf0f990`
- run 2生stream: 4378 bytes、SHA-256 `1131d1d8f4efb84d990d06ba462d160393e1efe7887ce537722d42b5885d0526`
