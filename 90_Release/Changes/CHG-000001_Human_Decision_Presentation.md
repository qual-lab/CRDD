...
- 機械確認への伝播（当時）: `template/tools/crdd_check.ts`と`tools/crdd_check.test.ts`。現在の試験移設先は[`tools/checker/crdd_check.test.ts`](../../tools/checker/crdd_check.test.ts)
- 初回監査結果:
...
- 機械確認（当時）: `template/tools/crdd_check.ts`、`tools/crdd_check.test.ts`。現在の試験移設先は[`tools/checker/crdd_check.test.ts`](../../tools/checker/crdd_check.test.ts)
