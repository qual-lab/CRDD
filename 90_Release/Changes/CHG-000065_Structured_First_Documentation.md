...
v0.20.0のリリース前に、CRDD Repository内の人間可読文書を全数対象として、文章中心の説明を意味の種類に合う構造へ変更する。情報量を削減すること、表の数を増やすこと、または過去tag上の原記録を変更することは目的にしない。

規範と監査観点は[文書化](../../03_Documentation.md#104-structured-first)と[文書監査](../../51_Document_Audit.md)へ追加した。初回処置では全427文書を現在文書84件と固定履歴343件へ分け、固定履歴を一括して無変更とした。しかし、固定履歴であることは事実保全の条件であり、可読性評価の免除理由ではない。2026-09-11の人間判断に基づき、過去CHG、Evidenceおよび検証結果を含む全427件を再評価した。byte列が公開履歴のIdentityとなる文書は原記録を保全し、目的別の構造化索引から現行正本と固定原記録を分けて読めるようにした。初回監査Passは拡張後の完了根拠へ流用せず、新しい固定改訂版を再監査する。
...
| 現在の正本、案内、ひな型、未公開CHG、v0.20統合検証結果 | 84 | 全件再評価 | 構造・配置是正の対象と、既に意味に合う表・箇条書き・テキスト図を持つ変更不要対象を分離。固定質問票や長さ基準は追加しない |
| 合計 | 427 | 未処置0件 | 機械確認はError 0／Warning 0。独立再監査は新しい固定改訂版で行う |
...
| Roadmap | 1 | 未完了作業、現在状態、次のGate、詳細正本参照 | Runtime責務分離の旧Gateを更新 |
...
| 項目 | 内容 |
|---|---|
| 対象 | 427文書。現在性と成果物種別は区別するが、固定履歴を可読性評価から除外しない |
...
| 結果 | 全427件の処置を閉じ、固定履歴の直接書き換えではなく、目的別の構造化索引で現在と履歴を分離 |
