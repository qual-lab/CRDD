...
最終固定前の変更集合は基準Commitとの差分から再導出する。現在の実差分は、内容／入口／Checker等35ファイル、初回固定候補の履歴根拠3ファイル、第2固定候補の履歴根拠3ファイルの計41ファイルであり、上表の変更、既存参照で追従、理由付き変更不要のいずれかへ対応している。確認待ち0件、未解消不一致0件である。今後の修正で集合が変わった場合は、この件数と処置を固定前に再照合する。
...
第2監査集合の統合修正を適用中である。現在状態は`Ready for Verification`であり、新しい固定コミット、固定後Checker、独立確認、統合、リリース判断および公開識別子は未確定である。
