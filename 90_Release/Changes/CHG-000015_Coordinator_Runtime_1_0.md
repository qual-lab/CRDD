...
続く固定候補の監査は、旧Coordinator facadeがProcess Controllerの実`recoveryId`を読まず手書きcallback fixtureだけで成立を推定していたこと、Host先行回収後の耐久finalization intentを安全predicate前に書いて失敗呼出しが次回Recovery Authorityをmintし得たこと、およびTask Runtime contract fixtureの実Filesystem観測を公開入口・実Processまで確認した根拠へ昇格していたことを検出した。同じ未リリース変更内で、実producer shapeと全consumerを共通Projectorへ接続し、Effect前／Host cleanup／Host削除後Docker finalizeのRecovery ID保持を分離する。finalization intentはHost／submission／active不存在とexact committed pointer、Recovery ID、nonce、base／Home hash、initial Host IDを確認した後だけ発行し、拒否呼出しでは新規Authorityを残さない。Evidenceは入口形態、観測基盤、成果物Identityおよびlifecycleを独立軸として、当該fixtureを「Task Runtime契約＋実Host Filesystem」へ限定した。あわせてCRDD標準の着手前整合、Architecture、Implementation、品質保証およびひな型へ、このproducer／transport／consumer、耐久Authority、Evidence軸の条件付き確認を一般化した。単純な局所処理へ新しい重い成果物または監査を要求しない。更新固定版の機械確認と独立再監査前にRuntime完成またはRelease根拠へ昇格しない。
