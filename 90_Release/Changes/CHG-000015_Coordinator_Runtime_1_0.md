...
停止理由の内訳を生出力なしで識別できるよう、結果形式、turn数、費用メタデータ、Reviewer本文の搬送不適合を固定理由へ分離した。受理条件や上限を緩和する修正ではなく、今回の根本原因が解消したという証明でもない。更新版の実測と最終一括監査を残す。

固定開発版`56af5a1`による[逆方向・順方向の追加実測](Evidence/CHG-000015_Development_Routes_56af5a1.md)では、順方向の一般Taskが完了し、逆方向はClaudeレビュー段階で上限関連の理由により停止した。両Taskともcleanup確認済み、手動回復不要、正本Repository変更なし。実測後の設計照合で、Reviewerへ数値検証を重複要求する固定Taskの指示を既存の責務分離へ戻す是正と、CLIの上限停止／成功結果の回数不整合を区別する是正を行う。上限、モデル、権限、Runnerのbyte完全一致条件は変えない。直接原因と是正効果は再実測まで未確定とする。
