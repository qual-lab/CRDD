...
同じHost復旧を通常Operationへ混入させず再現可能にするため、Windows専用の明示`doctor --repair-docker-desktop-runtime`候補を追加した。発火はEngine停止と固定`dockerInference` socketの既知アクセス不能へ限定し、署名済みCRDD配布物、native selected-user／Known Folder照合済みLocal App Data、保護Runtime State、固定Docker Desktop 4.41.2成果物のPath／size／SHA-256、専用Host Operation lockおよびEffect前の段階記録を要求する。公式shutdown後に残るProcessは固定allowlistと一致するPIDだけを強制終了し、WSLは`docker-desktop`だけをterminateする。固定`Docker\run`は同一親の一意な退避名へrenameして削除せず、Identityを前後照合する。Engine再応答とlock解放まで確認できた場合だけ`recovered`とし、退避Directoryと記録は明示的な後続処置まで保持する。通常Taskからの自動fallback、`wsl --shutdown`、CRDD RuntimeState、Provider Home、container、image、volumeまたは別WSL distributionの削除を許可しない。現固定版では依存差替えによる正常・非発火・境界・失敗・cleanup不明の契約試験を追加したが、再度Hostを破損させる実測は行っていない。この候補は別の`0xC0000409`、Docker Task Recovery、残存0、DogfoodingまたはRuntime完成を成立させない。
