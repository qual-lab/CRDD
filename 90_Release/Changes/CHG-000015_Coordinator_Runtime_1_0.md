...
後続監査是正では記録をrev3とし、shutdown、native termination、WSL termination、record write、renameおよびlauncherを操作別の追記Effectへ分離した。既知の発行事実を後退させず、後続Effectが不明なら以前の成功で隠さず全体確認を`unknown`にする。CRDD manifest Hash／Release Sequence／Tree／package content rootへの再開結合、旧rev2の非移行停止、段階ごとのEngine／Process／live run／stale再観測、および過去Effect不明・staleなしの専用pending／terminalを追加した。各Effect／record直前、再開、終端投影直前およびhelper解放後にpackage／Policy世代を再確認し、非同期待機中の取消またはhelper喪失後は新しいEffectを0にする。Nodeの非同期launcher起動は廃止し、native helperがKnown Folder由来の最小Unicode環境で固定launcherを`CreateProcessW`し、返された同じprocess handleでimageと作成Identityを確認する。生成後のIdentity確認不明もProcess Effect発行済みとして保持する。durable terminalはEvidence dispositionだけを表し、helper cleanupは現在runで`Q`、exit 0、child `close`、stdin／stdout／stderrおよびprocess handle回収後にだけ公開成功へ投影する。契約試験は取消、helper喪失、package世代差、後続Effect不明、全再開stage、terminal再表示、遅延exit／stdio回収、公開repair／close表示およびnative同一CreateProcess primitiveの実child観測を含む。保証対象は直接Effectに使う固定executable集合とEngine応答版であり、未列挙DLL、resource、loader依存、installation全体または供給経路のAttestationは主張せず、人間が確認した公式Docker配布物と正常なupdaterをT1–T2のTCBに含める。
