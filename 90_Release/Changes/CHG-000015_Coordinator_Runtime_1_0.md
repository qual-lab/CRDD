...
##### Windows Known Folder展開用の最小ユーザー文脈を閉じるrevision 2是正

固定Commit `b57a297efcf0845f1f64e40fc025b2a73a8173fc`／Tree `3a45661e5c705ff0de0226743e908eda11dec8b9`をRelease順序`2026082604`、manifest SHA-256 `a57f4f69ce8d6f4f880615079e996cbca4213723e02c5dbfa4b94f023f4cc1cf`として署名し、正式Runnerを実行した。Runnerは対話Console Gateを通過した後、`docker_task_runtime_state_unavailable`、`manualRecoveryRequired: true`、Recovery IDなし、正規Repository変更なし、生Provider出力・Host Path・Credential報告なし、exit `2`で停止した。Provider request、Network、Docker resource、workspaceまたはCandidate Effectは開始していない。

Native応答の固定protocolを安全な数値だけで再構成し、Runtime State種別の失敗理由を`KnownFolderUnavailable`へ特定した。署名済みWorkerを同じ通常ローカルユーザーTokenで比較すると、現行の固定最小環境と`LOCALAPPDATA`だけを追加した環境は失敗し、OSユーザープロファイル相当の`USERPROFILE`だけを追加した環境と両方を追加した環境は成功した。したがって必要条件は親環境全体や`LOCALAPPDATA`の継承ではなく、`SHGetKnownFolderPath`がCurrent Userの登録値を展開するための選択ユーザーProfile Pathである。Codex Sandboxの制限Tokenでは同じWorkerが`PrincipalNotSelectedLocalUser`として拒否され、selected-user Gateを緩めず区別できることも確認した。

Windows内部子Process環境契約をrevision 2とし、Runtime所有・署名検証済みNative Helperに限り、Nodeの`os.userInfo()`がOSから返すProfile Pathを絶対Directory、non-linkおよびnative real path完全一致へ検証して`USERPROFILE`へ設定する。Provider Home要求は読み取り専用観測、Candidate Store／Runtime State要求は読み取り専用観測または`initializeIfMissing: true`の固定Root明示初期化Filesystem Effectである。Profile PathはKnown Folder展開Contextに限り、Authority、selected-user成立または初期化許可ではない。親Processの`USERPROFILE`、`LOCALAPPDATA`、Home、Credential、Proxy、PATHまたはNode injection値は受理せず、Console readerとDocker CLIの既存用途別環境を変更しない。Native Helperは展開後のKnown Folderから固定segmentを再構成し、selected-user token、fixed volume、全segmentのnon-reparse、stable Identity、ownerおよびexact protected DACLを独立に確認する。OS由来Profileを安全に取得・検証できない場合はHelperを起動せず、読み取り専用観測と明示初期化の双方をProcess／Filesystem／Network EffectおよびAuthority 0へ閉じる。

発火例は通常の選択ローカル対話ユーザーとしてOS由来ProfileとKnown Folder観測が一致する場合、非発火例はNative Helperを使わないConsole readerまたはDocker CLI、境界例は親環境の`USERPROFILE`が別Pathへ差し替えられた場合、判定情報不足例はOS user info、Directory、real pathまたはNative Known Folder取得が不成立の場合である。親環境差替えは無視し、判定情報不足ではfallbackせずEffect前に停止する。修正後の実WorkerはRuntime Stateのprincipal flags、Home flagsおよび四つのbinding Hashを満たす`candidate`を返した。source checkout Adapterは署名Release Authority不成立によりHelper spawn前に停止し、観測成功だけをAuthorityへ昇格しない。

本処置は`Applied`／`Self-checked`である。Node.js 24.19.0でWindows内部子Process／Console境界19試験とCoordinator全806試験、Coordinatorのstrict typecheck、Biome Lint／FormatterおよびRepository全体CheckerをPassした。全体Checkerは709 files／384 Markdown／2,237 links／652 anchors／29 Related／28 versioned documents／8 stable IDs／74 remediation rows／Error 0／Warning 0である。新固定Commit／TreeへのArchitecture／Security、Document／Gap／Impact／ConformanceおよびTest／UX独立再レビューがすべて`Pass`する前に`Resolved`、再署名、正式一般Task、Provider EffectまたはRuntime完成へ昇格しない。Release順序`2026082604`は修正前Identityへ結合済みであり再利用しない。

固定Commit `09960ba1441c4431fc77acf911f396503b167c9c`／Tree `4e305038d5ba8b47de773260d52c67781798c521`への3独立確認は、Architecture／SecurityがMinor 1件、Document／Gap／Impact／ConformanceがMajor 1件、Test／UXがMinor 1件を検出して`Fail`とした。共通根因は、共有Native Helper環境factoryをread-only observer専用と誤認し、Candidate Store／Runtime Stateの明示初期化利用側、両Adapterのmachine descriptorおよび情報不足時の回帰試験を同じ契約母集団へ含めなかったことである。3監査へ統合是正方針を再提示し、競合なしの`Accept with Conditions`を得た。追加の人間判断は不要である。

是正ではProvider Home Adapter contractをrevision 5へ更新し、両Adapterのmachine descriptorを、loaded `kernel32.dll`由来Windows Directory、`os.userInfo()`由来の検証済みProfile Path、その他ambient名のfixed neutral化および親環境非Authorityへ完全一致させる。Candidate Store Adapterに新しい版概念は追加しない。OS user info例外、Profile不存在、linkおよびnative real path不一致をfresh child内のNode builtin故障注入で別々に発火し、有効な親`USERPROFILE`へfallbackしないことを確認する。正常なOS Profileが成立する場合は親値を無視してOS値を使う反例を維持する。Provider Home、Candidate Store／Runtime Stateのread-only観測と明示初期化の全5利用形態で、環境不成立時のHelper spawn、Filesystem／Network Effect、観測／Root CapabilityおよびRuntime Authorityを0へ固定する。故障注入はtest-only fresh childへ閉じ、production dependency injection、caller callback、動的Path、環境override、Registry、ACLまたはProvider Home変更を追加しない。新固定Commit／Treeへの全機械確認と3独立再確認が完了する前に指摘を`Resolved`としない。

本処置は`Applied`／`Self-checked`である。Node.js 24.19.0でOS Profile故障と全5利用形態のEffect前停止を含む重点30試験、Coordinator全808試験、Checker全153試験、Coordinatorのstrict typecheck、Biome Lint／Formatter、Repository全体Checkerおよび`git diff --check`をPassした。全体Checkerは709 files／384 Markdown／2,237 links／652 anchors／29 Related／28 versioned documents／8 stable IDs／74 remediation rows／Error 0／Warning 0である。追加した2 fixtureも共通Path／型付きsource命名契約の所有母集団へ含めた。新固定Commit／Treeへの3独立再確認がすべて`Pass`する前に指摘を`Resolved`、再署名、正式一般Task、Provider EffectまたはRuntime完成へ昇格しない。Release順序`2026082604`は再利用しない。

固定Commit `15983f864a21cd6c8d30708baa3b39a258c9f3c6`／Tree `c92d8c99ac1758ab26bd8e335e4e4a9eb9f73c38`への3独立再確認は、旧Majorの実装・文書・machine contract部分を`Resolved`とした一方、Capability 0のsummaryが実outcomeではなく固定`false`から生成される共通Minor 1件と、Profile故障fixtureのsetup途中失敗がcleanup所有範囲へ入らないTest／UX Minor 1件を検出して`Fail`とした。現行productionのCapability誤発行、Authority拡張またはEffect拡大は確認されていない。3監査へ統合是正方針を再提示し、競合なしの`Accept with Conditions`を得た。追加の人間判断は不要である。

是正では、Provider HomeのObservation Capability、Runtime-owned発行状態、Mount Grant、Operation Capabilityおよびmanual recoveryと、Candidate Store／Runtime State全4利用形態のRoot Capabilityおよびmanual recoveryを各実outcomeから直接検査する。summaryは5 outcomeの実値からだけ導出し、既存のHelper spawn、Process／Filesystem／Network Effect、Runtime AuthorityおよびPath／Principal／ACL報告0の検査を維持する。Profile故障fixtureは一時Root取得直後からDirectory作成、環境変更、builtin差替え、module importおよび結果判定を単一cleanup所有範囲へ入れる。3 builtin propertyは元descriptorを完全復元し、USERPROFILE復元とexact一時Root削除を、一つの失敗で後続処置を止めず全試行する。cleanup失敗は成功結果へ流さない。production実装、machine descriptor、OS Profile契約、Authority／Effect、Console readerおよびDocker CLI環境は変更しない。新固定Commit／Treeへの全機械確認と3独立再確認が完了する前に両Minorを`Resolved`としない。

本処置は`Applied`／`Self-checked`である。Node.js 24.19.0でCapability実outcome、manual recoveryおよびProfile fixture cleanupを含む対話境界重点21試験、Coordinator全808試験、Checker全153試験、Coordinatorのstrict typecheck、Biome Lint／Formatter、Repository全体Checkerおよび`git diff --check`をPassした。全体Checkerは709 files／384 Markdown／2,237 links／652 anchors／29 Related／28 versioned documents／8 stable IDs／74 remediation rows／Error 0／Warning 0である。新固定Commit／Treeへの3独立再確認がすべて`Pass`する前に両Minorを`Resolved`、再署名、正式一般Task、Provider EffectまたはRuntime完成へ昇格しない。

固定Commit `f8c0e3d9d2b3e93419433ff8295792d31236e706`／Tree `70c632d14eb13330f4567d2f65e4cae465e80f9e`へのArchitecture／SecurityとDocument／Gap／Impact／Conformance独立再確認は`Pass`、Test／UXはCapability、Effect、Authority、manual recoveryおよびProfile cleanupの旧Findingを解消済みとした一方、統合条件に含めたProvider outcomeの`helperSpawned === false`直接assertが未反映のMinor 1件を検出した。productionの実欠陥は確認されていない。監査集合の解消判定へ部分Passを流用せず、同じfixtureで実outcomeの`helperSpawned`を直接検査し、新固定Commit／Treeへ全3確認を再実行する。

固定Commit `5e116a438e8e252b1a871c925fd27da157d4c90a`／Tree `555b162a569445666cca2077ba6b4c4cbe659385`へのArchitecture／Security、Document／Gap／Impact／ConformanceおよびTest／UX独立再確認は、3件とも`Pass`、Critical／Major／Minor Finding `0`を得た。共有Native Helper利用側とmachine contract、情報不足時のEffect／Authority／Capability 0、Capability summary、Profile fixture cleanupおよびProvider `helperSpawned`直接検査の旧Findingは同固定範囲で`Resolved`である。共通入力はCoordinator `808 / 808 Pass`、Checker `153 / 153 Pass`、strict typecheck、Biome Lint／Formatter、Repository全体Checker Error `0`／Warning `0`およびworktree cleanである。

確認済み範囲はProvider Home、Candidate Store／Runtime Stateの全5利用形態、OS Profile正常・例外・不存在・link・realpath差、親環境fallback禁止、Helper spawn、Process／Filesystem／Network Effect、Capability、Authority、manual recovery、fixture cleanup、machine descriptor、README、CHG、全production consumer、Console reader／Docker CLI非変更、利用側、移行および準拠影響である。新固定版の再署名、正式一般Task完走、実Provider／Docker／Network Effect、将来Node版、POSIX実環境およびT1–T2外は未評価であり、後続Gateまたは保証範囲外として維持する。現在、この是正について追加の人間判断はない。Release順序`2026082604`は再利用しない。

監査Pass後の固定Commit `252563f731a1fdf489cf229191652d359ee525a4`／Tree `62f8cee46dc5b0d5ac151fef7fe7079f4947af07`をRelease順序`2026082605`で署名しようとしたが、passphrase入力後に`release_manifest_payload_invalid`で停止した。manifest配置、署名Release成立、Runtime AuthorityまたはProvider／Network Effectは0である。原因は署名wrapperが`issued-at`／`expires-at`を秒精度で渡した一方、manifest契約がミリ秒を含むcanonical UTCを要求したことである。同時に、Versionと時刻の公開入力がpassphraseおよびFilesystem観測より前に完全検証されないSigner側の順序不足を確認した。順序`2026082605`は安全側で再利用しない。

是正ではRelease Identity grammarをVersion、Git Object IDおよびcanonical UTC timestampの共通正本とし、manifest CoreとSignerが同じ時刻判定を使う。SignerはVersion、issued／expires時刻および有効期間順序をpassphrase入力・配布Root／秘密鍵Filesystem観測より前に検証し、Version、時刻または期間ごとの固定reasonで停止する。秒精度、2桁ミリ秒、offset形式、実在しない日付および非正順期間を反証し、CLIへpassphrase相当入力を与えても無効な公開入力では読まずに終了することを確認する。秘密鍵、署名方式、配布Root、成果物、Authority／Effectまたは正式Task契約は変更しない。新固定Commit／Treeへの全機械確認と3独立確認前に本処置を`Resolved`、再署名または正式一般Task成功へ昇格しない。

本処置は`Applied`／`Self-checked`である。Node.js 24.19.0でRelease公開入力順序を含む重点22試験、Coordinator全809試験、Checker全153試験、Coordinatorのstrict typecheck、Biome Lint／Formatter、Repository全体Checkerおよび`git diff --check`をPassした。全体Checkerは709 files／384 Markdown／2,237 links／652 anchors／29 Related／28 versioned documents／8 stable IDs／74 remediation rows／Error 0／Warning 0である。新固定Commit／Treeへの3独立確認がすべて`Pass`する前に本処置を`Resolved`、再署名、正式一般TaskまたはRuntime完成へ昇格しない。Release順序`2026082605`は再利用しない。

固定Commit `891fa68a2cc53c095eef676d35ba5be5e19a9d2a`／Tree `cac6756396a1128054a954d28bd776fd54568be2`へのArchitecture／SecurityとDocument／Gap／Impact／Conformance独立確認は`Pass`、Test／UXはproductionの共通timestamp predicateと事前検証順序を正しいと評価した一方、Signer境界の直接反証が秒精度と同時刻windowだけで、2桁ミリ秒、offset、不存在日および逆転windowを含まないMinor 1件を検出した。監査集合の解消判定へ部分Passを流用しない。同じ不存在配布Root／秘密鍵Path対照へ4例を追加し、各固定reasonがFilesystem errorより先に返ることを確認して、新固定Commit／Treeへ全3確認を再実行する。production predicate、reason、入力順序、鍵・署名・Authority／Effect契約は変更しない。
...
固定Commit `866c57f67c602d0a15f1aa15fbd0d06b1c41e063`／Tree `d9d4731ca1a77dbf09d83e7cd9b8978a33ebf7bd`へのArchitecture／SecurityとTest／UX独立監査はCritical／Major／Minor `0`で`Pass`した。Document／Gap／Impact／Conformance監査は本記録自体を正確と確認した一方、現行README、Threat ModelおよびProduct Roadmapが正式署名実runを未完了のまま表示する直接伝播不足をMajor 1件として検出した。部分Passを現在の解消根拠へ流用しない。

三監査へ統合是正方針を再提示し、競合なしの`Accept with Conditions`を得た。過去CHGの時点別履歴、production、Authority／Effect、Schema、成功契約、Subscription-only、API key／従量API／追加購入／有料fallback禁止およびCHANGELOGは変更しない。現行READMEとThreat Modelだけを、固定1 Pathの`Codex Front → Claude Code Executor → Codex Independent Reviewer`成功経路、Candidate破棄、cleanupおよび残存0の成立へ更新し、逆方向／他経路、正式署名E2Eでの是正・取消・失敗Recovery、Runtime全体、T3–T4、統合およびReleaseへ一般化しない。Product Roadmapは実runの固定Evidence監査、Runtime全体の完成固定およびFront Claude Codeを含む逆方向経路を別の未完了項目として追跡する。全監査が新固定版へ`Pass`する前にMajorを`Resolved`とせず、Evidence監査項目を主要表示から除去しない。

固定Commit `2dd9b18a4d2adac9b54bbe5034e0be7d5c2fa3d3`／Tree `6bfb1e589b5e1a16fadcfd62fc086f555f76da26`へのArchitecture／Security、Document／Gap／Impact／ConformanceおよびTest／UX独立再監査は、3件とも`Pass`、Critical／Major／Minor Finding `0`を得た。正式署名E2E成功の現行README、Threat ModelおよびProduct Roadmapへの直接伝播不足は同固定範囲で`Resolved`である。固定Vertical Slice Evidence監査は完了したためProduct Roadmapの一時項目を除去し、Runtime 1.0の完成固定とFront Claude Codeを含む逆方向経路の正式署名E2Eを独立した未完了項目として維持する。今回の成功を正式署名E2Eでの是正・取消・失敗Recovery、Runtime全体、T3–T4、統合またはReleaseへ昇格しない。現在、このEvidence固定について追加の人間判断はない。
