...
Windows内部子Process環境契約をrevision 2とし、読み取り専用Native Helperに限り、Nodeの`os.userInfo()`がOSから返すProfile Pathを絶対Directory、non-linkおよびnative real path完全一致へ検証して`USERPROFILE`へ設定する。親Processの`USERPROFILE`、`LOCALAPPDATA`、Home、Credential、Proxy、PATHまたはNode injection値は受理せず、Console readerとDocker CLIの既存用途別環境を変更しない。Native Helperはこの値をAuthorityへ昇格せず、展開後のKnown Folderから固定segmentを再構成し、selected-user token、fixed volume、全segmentのnon-reparse、stable Identity、ownerおよびexact protected DACLを独立に確認する。OS由来Profileを安全に取得・検証できない場合はHelperを起動しない。
