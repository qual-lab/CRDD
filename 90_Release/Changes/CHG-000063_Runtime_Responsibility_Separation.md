...
同固定版への二つの独立再レビューでは、loader取得のnamespace／default／再export／bracket表記、Authority証明と実行呼出しの結合、`dependencies.startProcess`を含むwrapper／Lifecycle利用側、公開利用側の構造的導出、および型専用star re-exportの分類に未完を検出した。固定改訂版`98146b3b70295bc122784871233d0bd7cf58c423`では、実ソース、公開入口、Capability、同梱・固定開発版、署名Manifestおよび配置後検証から利用側を導出し、独立した正規集合と双方向に一致しない不足・余分を拒否する。各外部Process呼出しはexactな実行対象、引数構造、同じ所有関数内の先行Authority証明およびhandle所有へ結合し、importや宣言だけを実行成立の根拠にしない。Canonical Path／Identity／Stateを利用側で再解釈せず、正当な型専用star re-exportだけをEffectなしとして許可する。重点126件、制限Process 1,743件、Windows実Process 7件、静的検査およびRepository全体Checkerは成功済みであり、独立再レビュー、正式署名および正式E2Eは後続Gateとして保持する。手書き一覧は証拠または正本ではなく、実ソースから独立導出した利用側集合との完全一致によってだけ閉包を判定する。
