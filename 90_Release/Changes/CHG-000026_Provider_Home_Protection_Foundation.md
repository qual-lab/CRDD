...
対象contract testは方針、Codex／Claude正例、Path非出力、Host／Operation Home非移行、unsupported Provider、Path境界、余分field、accessor、Proxyおよび非昇格を確認する。`npm run provider-home:coverage --prefix tools/coordinator`はWindows Path validator、plain snapshot、Provider Home、Provider lifecycle、doctor、共有exact LCOV parserおよびrunner自身の固定7 production sourceと固定7 testを連続2回測定する。runnerはNode 24.12.0未満を処置前に拒否し、実行Node版と要求下限を同じstdout JSONへ結合する。source別line／function／branchのcovered／total、全未到達BRDA Identity、reason、risk、代替確認、Owner、human decisionおよびrecheckをcompact JSON UTF-8＋末尾LF exact 1件へ保存する。platform dispatchの反対OS分岐、実Docker／Filesystem failure、複合短絡およびrunner failureは、実OS／Effectを捏造して100%へ上げず個別義務として残す。

基準適合Node.js `v24.19.0`で再取得した自己確認の合計はlines `2045/2240`、functions `82/89`、branches `403/491`で、未到達branchは88件である。新規`provider-home.ts`はlines `135/135`、functions `6/6`、branches `17/17`の全到達、`provider-lifecycle.ts`と`plain-data-snapshot.ts`も全到達である。未到達は反対OS dispatch 1件、doctorの環境／Docker／cleanup分岐58件、共有parser 26件、runnerの環境／failure分岐3件で、全件を決定論的出力の義務へ接続した。連続2回のpayload SHA-256は`fef274a509cbfc3354dd54a193fb9e8d07ce528229b72f4568bd6eb45b470920`、runtime Identityを含むstdoutは62,399 byte、SHA-256は`6eec9cf410e1672454e744d1fba95b45ee468cd6d55ec8993f49a5044ea5b49c`、stderrは0 byteである。これらは同一の直接runner stdoutから再計測した一組の根拠である。100%未達はProvider Home Coreの未検証分岐ではなく、母集団に含めた共有利用側と品質生成器のOS／環境／防御分岐である。

Node.js v24.19.0でCoordinator check、Coordinator全contract test 386／386、Checker contract test 151／151、Provider Home coverage、tools命名／source closure 5／5およびfull checkerを確認対象とする。source closureの固定値はCoordinator production 66、Coordinator test 60、Checker／template 5、Rust 4、unique total 134である。Rust sourceとwireを変更していないためRust test／coverageは本変更の成立根拠へ流用せず`No Impact`とする。
...
是正後固定Commit `fb949110e071043eab6284d8f5117ce080c017b0`／Tree `97587f4e0c0350fcdd0a266805b45cbc3b192f3c`／Parent `4900edc923595bda98999b9e54d23d733324f02d`では、Agent／Architecture／Security、Document、Gap／ImpactおよびConformanceがすべて`Pass`／Finding 0となり、既知3 Findingは解消候補となった。しかしEvidence固定前の機械入力再照合で、一部確認がRepository基準未満のPATH上Node `v22.18.0`を使いながら自己確認記録が`v24.19.0`を示していた品質根拠の同一性不整合`QA-HOME-001`（Major、今回の変更によって発生）を親Agentが検出した。`fb94911`の監査集合も`Invalidated`／現在不流用とした。

次の固定Commit `320d74f8511500823f7b7b4f2ba9a582b62eae95`／Tree `98102ebe3664509c6aad653f38e5882f8fb55e56`／Parent `fb949110e071043eab6284d8f5117ce080c017b0`は記録を`v22.18.0`へ訂正したが、`.node-version`、package engineおよびMaintenanceが要求するNode.js 24.12以上を満たさないため、Agent／Architecture／Securityは`Fail`（`AG-HOME-R2-001` Major）、Documentは`Fail`（`DOC-HOME-R2-001` Major）、Gap／Impactは`Fail`（`GCI-HOME-002` Major）、Conformanceは`Fail / Not Eligible`となった。3件はすべて「今回の修正によって初めて確認可能」で、`QA-HOME-001`と同根である。`320d74f`の監査集合も`Invalidated`／現在不流用とし、同じ固定差分を基準適合の同梱Node.js `v24.19.0`で全数再実行した。数値とHashは上記の新しい共通機械入力に差し替え、現在状態は`Applied / Self-checked — pending independent re-review`とする。

固定Commit `e6fdbc6edb52bd757b86e2f40edadf9755692773`／Tree `f94010bb6c316480e1a796a6cc8e8d46bd0e0961`／Parent `320d74f8511500823f7b7b4f2ba9a582b62eae95`の監査集合は、Agent／Architecture／Securityが`Fail`（`AG-HOME-R3-001` Major）、Documentが`Fail`（`DOC-HOME-R3-001` Major）、Gap／Impactが`Fail`（`GCI-HOME-003` Major）、Conformanceが`Fail / Not Eligible`であった。3件はすべて「今回の修正によって新たに発生」で、更新後coverage値と旧記録から流用したstdout Hashが同じbyte列を指さない品質来歴不整合`QA-HOME-002`と同根である。原因は、旧の`402/481`とpayload `600550…`に、別の直接runner実行で得たstdout Hash `cc53…`を混在させたことであり、SHA-256衝突ではない。`e6fdbc6`の監査集合も`Invalidated`／現在不流用とした。runner自身にNode下限拒否とruntime Identity出力を追加し、基準適合Node `v24.19.0`の同一出力からcoverage、payload Hash、stdout byte数／Hashおよびstderrを再取得した。現在状態は`Applied / Self-checked — pending independent re-review`である。
