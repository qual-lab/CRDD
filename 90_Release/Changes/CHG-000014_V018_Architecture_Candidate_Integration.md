...
<a id="release-preparation-20260901"></a>

## 12. main統合とv0.18.0公開準備

2026-09-01、Qual-Labの人間の決定権限者はPR #32のmain統合、その後のv0.18.0タグ作成・リリース公開、正式署名済みRuntime配布物の添付を承認した。公開完了後は、未統合変更や利用中のworktreeを残すブランチを保護し、不要と確認できたローカル・リモートブランチを整理する。公開済みタグ・過去リリース・mainは削除しない。

[PR #32](https://github.com/qual-lab/CRDD/pull/32)は通常のmerge commitで統合した。sourceは`1a080a69c27318c93a4fa1419878bd38c844203a`、統合前mainは`8fe4c1cc97a97009cf3c0078303d8ccfdc73c67c`、統合後は`6a4f09d7623f49118650cbae79d6bd1c41f37a16`、Treeは`850527920cbb29704f4b189544963f68dff885e3`である。sourceと統合後のTreeは一致し、内容差分はない。mainのルールセットはPR経由、強制更新・削除禁止、未解決レビュー会話の解消を要求する。統合時に必須CI・必要承認数は設定されておらず、これをCI合格という根拠にはしない。

PR #32で統合した7CHGは未公開である。その後、人間が正式配布manifestの期限なし指定を選択したためCHG-000015を再開し、公開準備で判明したCheckerの誤検出をCHG-000017で是正する。公開準備のメタデータ遷移は次の範囲とし、実装変更は各CHGで別に追跡する。

- ルートの28正本文書（00～05、10～19、21～29、51～53）の実ヘッダーをStable化し、Released Baselineを除く。03本文中の候補ヘッダー例や非規範の将来候補は変更しない。19の旧固定件数「26」は実母集団との不一致を避けるため「対象の全正本文書」へ訂正する。
- README・CHANGELOG英日のv0.18.0案内を公開時にも残せる表示へ同期する。公式タグまたは不変Release参照だけが公開基準となり、branch、Stable表示、ソースZIPだけから採用先の有効化やRuntime起動可能性を推定しない。
