...
- manifestが署名するSource Commit Aと、manifestを加えた実行Commit Bを分離し、一般TaskのCandidate Revisionを実行前後に独立観測したBへ結合する。
...
Repository全体のTypeScript試験は最終Source候補を1345件まで拡張した。制限Process内では`taskkill.exe`を許可されない7件だけが安全側の終了未確認となり、残る1338件は合格した。同じProcess終了・回収境界を含む対象101件を通常のローカルユーザーProcessで再実行すると101件すべてが合格し、制限Processだけで失敗した7件も実終了を観測できた。Rustの通常試験、Clippy、Format、Native Coverage、TypeScript Coverage、Coordinator Checkおよび全体Checkerも合格した。この環境差を本番実装の成功へ補正せず、制限Process内の事実と通常Processで実終了を観測した根拠を分けて保持する。LLVM CoverageがWindows実子のBootstrap環境を変更するため、実子の完全環境一致は通常Rust試験で必須とし、Coverage実行では当該1件だけを非計測にして残るNative Sourceを全件集計する。
...
その後の署名済み一般Taskでは、配布検証がCommit A／Tree Aを正しく確認した一方、Runnerが隔離Candidateのbase RevisionにもAを要求し、manifest一件を加えた実行Commit Bを正しく基準にしたCandidateを拒否した。これはProvider出力ではなく二Commit署名手順とCandidate契約の接続漏れである。RunnerはTask前後に実行RepositoryのCommit B／Tree Bを独立観測し、CandidateをBへ照合するよう是正した。配布内容は引き続きAへ結合し、AとBの役割を混同しない。AとBが異なる正常例、Candidateが誤ってAをbaseにする異常例、実行中のRepository Revision変化、観測不能を契約試験へ追加した。
