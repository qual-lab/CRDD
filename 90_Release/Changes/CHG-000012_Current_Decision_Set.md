...
- Communication契約、v1 Architecture Candidate、バージョン表示、CHANGELOG、統合またはリリースを本変更だけで確定しない。
...
旧固定候補`dd617e7f15d413e363d041b0008922ebe89d811c`に対する確認結果は統合候補の現在判定へ流用しない。本変更はCommunicationとv1 Architecture Candidateを含む統合差分で再固定・再監査する。現在状態は`Ready for Verification`であり、統合候補の合格、対象Version、移行、CHANGELOG、main統合、公開およびReleaseは未確定である。
