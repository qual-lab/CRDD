...
| 区分 | 現在値 |
|---|---|
| 成立 | 設計対応、契約・結合試験、署名固定版の正常縦断2経路、うち1経路の正本採用、低Risk文書1件の自己適用と正本採用、認証主体の意味入口への伝播、Repository Binding単位の別Process排他、Lease取得後のfresh優先選択、Task Authorityの実行直前発行と未使用Authority失効、Task開始の`reserved / handoff_prepared / running`分離、Task別・種別別のexact Recovery相関、個別回復義務の耐久settlement、Queue／State／個別義務の途中中断から完了済みEffectを再発行しない再開、再計画上限と独立したfresh retry、終端request再送のEffect 0投影、親EOF取消、対話／スケジュール競合、Docker完了receiptによる冪等再入場を含む署名前の決定論的Project全体確認。技術候補`2518ecd`では正常経路をObjectiveごとの公開Processへ分離し、OSのProcess起動成功と有効なPID／標準入出力を確認した後だけ開始を記録して取消へ相関し、通知書込み完了、選定と開始の単一順序および全実行間のOperation ID一意性を検証可能にした |
| 未成立 | 認証済み公開MCP Clientからの実Provider経路、実Provider実行中の取消、および実Docker資源を用いたRecovery settlementの署名後最終E2E |
| 次Gate | Runtime実行Identityの固定と最終E2E、未評価事項のRelease処遇、収載・分類・移行・残存Riskの人間判断 |
...
[監査是正後の署名前検証](../../07_Quality/Verification_Results/2026-09-03_Project_Runtime_Audit_Remediation_Pre_Sign_Verification.md)では、技術候補`3661c3a`について制限Process試験1,566件、Windows実資源Gate 7件、関連試験147件、設計追跡および文書チェッカーがすべて成功した。Tombstone 64+1、回収後の古いReceipt再出現、物理Receiptと古いProject attemptの組合せ、Objective別MCP到達可能性、および部分再計画の完了・拒否を反証試験で固定した。過去の署名前検証は各記録時点のblobを保持し、現在候補の根拠へ上書きしない。この結果は独立再監査へ進める決定論的な技術候補の成立であり、残る実Provider・実Docker・認証済みMCP ClientのE2Eを代替しない。

同候補の独立再監査では前回指摘の解消を確認した一方、公開ProjectionのObjective別件数を配列へ展開する実装が、最大安全整数等の未信頼な内部結果によって例外または巨大割当へ至る是正起因のMajorを検出した。技術候補`45409e5`ではProject入力と同じObjective 128件、Task 1,024件の上限をAdapterとCoreの両方で固定長の算術検証へ接続し、Task 1,025件、最大安全整数および複数Objectiveの合計超過を例外なしで固定の不正結果へ閉じた。[公開件数境界に関する署名前検証](../../07_Quality/Verification_Results/2026-09-03_Project_Runtime_Projection_Bounds_Pre_Sign_Verification.md)で関連147件、制限Process 1,566件、Windows実資源Gate 7件および全決定論的確認の成功を記録した。この時点では独立再監査待ちであり、後段E2EまたはReleaseの成立を意味しなかった。

[最終署名前監査](../../07_Quality/Verification_Results/2026-09-03_Project_Runtime_Final_Pre_Sign_Audit.md)では、固定改訂版`661de97`に対する文書・規範、Runtime状態・回復・再計画、および公開MCP投影・例外境界の3系統がCritical 0、Major 0、Minor 0でPassした。前回Majorの解消、以前の回復・再計画是正の維持、設計からEvidenceまでの伝播および未完了表示を確認した。この結果は当該固定改訂版の履歴であり、後続の実Provider E2E検証器変更へ自動継承しない。
