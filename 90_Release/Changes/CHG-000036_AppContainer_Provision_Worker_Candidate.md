...
- workerは固定pipe引数1組だけを受理し、通常stdin／stdout互換入口と分離する。接続不能、request書込み不能、完了待機不能、response／終了状態不正を、それぞれ`native_provision_worker_connection_failed`、`native_provision_worker_request_write_failed`、`native_provision_worker_completion_unavailable`、`native_provision_worker_response_invalid`の固定Fail Closed結果へ分ける。いずれも秘密、Pathまたはraw OS errorを公開せず、Authority、Capabilityまたは観測候補を発行しない。
