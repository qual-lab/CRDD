...
- AppContainer最小probeはWorker起動成功、handle継承0、Codex親Job内で成立した。未署名supervisorはprocess生成前に`blocked`となる。固定commit `f5f2517`の署名済みrunではPA03／PR03 revision 3、worker exit 0、Job active process 0およびRegistry復元が同じrunで成立した。一時`LowBoxConsoleEnabled`値、回復記録、証明書、stagingおよびarchiveは終了後に残存0だった。
...
実装済み候補は、manifestのexact全field／時刻検査、同一handleの両artifact Hash、supervisor内部worker Hash、volume rootからleafまでのbounded parent handle、local fixed volume、cache-only Authenticode検証、Known Folder由来`LOCALAPPDATA`だけの環境block、一時Registry Effectのmutex／durable recovery／exact restore、Supervisor／Worker TokenUser binder、create-time Job、子孫禁止、終了後tree不存在、worker loaded image照合およびDLL mitigationである。mapped supervisor結合専用blockerはMinimum Trust Boundary採用により現sourceから除去した。固定commit／tree、正式Ed25519 Release鍵および別の一時Authenticode Identityを同じrunへ結合し、native entrypointからPA03／PR03、worker exit 0、Job／tree終了およびrollbackまで実測した。ただしこのrunはbinder実装前の固定commit `f5f2517`であり、現改訂版のbinder成立へ流用しない。別変更CHG-000037では、公式署名manifestへ結合した実Claude Code binaryの無通信`--version`起動を確認したが、Provider request、認証またはGateを成立させない。未完了は、現binderを含む正式署名runのNetwork非発火・全module集合、protected active／Provider Home、Mount Grant issuer／store／clock／失効、Runtime-owned Claude artifact verifier／固定image、Egress、OAuth／subscription条件および固定prompt request probeである。今後のTrust Store登録は自動承認せず、正式署名材料または目的・期間・追加／除去対象・残存riskを明示した別の人間承認を要する。これらの完了前にRelease・Gate open・Claude Code Provider requestへ進めない。保護対象の採用・統合、残存risk受容およびReleaseは人間の決定権限へ残す。
