...
- workerへhandleを継承しない。supervisorは`\\.\pipe\LOCAL\CRDD.Coordinator.<supervisor-pid>`のfirst instanceを作成し、owner、SYSTEMおよびAll Application PackagesだけのDACLと、このPipe objectだけのLow integrity mandatory labelを設定する。接続元process IDが自ら生成したworkerと一致する場合だけ通信する。AppContainerのLow integrity tokenがDACL許可だけでは既定Medium objectへ書き込めないMIC境界を明示し、OS、Registry、他objectまたは一般callerへLow integrity書込みを広げない。
