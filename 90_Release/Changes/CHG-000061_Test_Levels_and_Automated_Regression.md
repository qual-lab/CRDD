...
- Provider境界の状態・分岐Matrixを全件実装した後の限定実測で、Provider開始前の準備が固定Fakeより長く、短命なCapabilityの有効期間との関係を既存結合試験が再現していないことを検出した。件数網羅だけでは時間依存の成立条件を保証しないため、本番相当前処理時間とCapability lifetimeの境界反証、および準備失敗の安全な固定理由を結合試験契約へ追加した。原因の確定前に単一の外部境界信号から失効と断定せず、固定理由を観測する同一範囲の再実測を行う。
