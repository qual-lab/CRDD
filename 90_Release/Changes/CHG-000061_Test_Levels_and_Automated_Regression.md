...
- 同じ限定実測では、2TaskのAuthority操作と進捗参照が完全Identity観測を合計66回発火し、Source、Native成果物およびRepositoryの再検証だけで累計約273秒を要した。完全観測を省略せず、AuthorityまたはEffect境界でない進捗参照とread-only集計を、固定Session binding、期限、取消およびProcess停止だけを確認する生存確認へ分離した。完全Identity観測はAdmission前後、Task／Operation／Native／Provider呼出しの各境界に維持し、生存確認から新しいAuthorityまたはEffectを発行できないことを契約試験で固定した。
