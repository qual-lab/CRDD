...
6. 一般Taskは実行直前のRepository Commit B／Tree Bを独立観測し、隔離Candidateのbase RevisionをBへ照合する。manifest内のAをCandidateのbaseとして要求しない。Task終了後にBを再観測できない、またはCommit／Treeが変わった場合は、Candidateを回収して成功扱いにしない。
