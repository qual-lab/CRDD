...
|---|---|---|---|
| UX-000016: 利用能力 | 利用能力 | Same | Repositoryで必要な仕事能力を保持する |
| UX-000016: 利用可否 | 利用可否 | Same | 現在利用できるかを能力と分けて保持する |
| UX-000016: 決定権限 | 実行権限 | Rename | 能力を実行できるAuthorityとして保持する |
| UX-000016: 配布物 | 配布物 | Same | Toolを届ける単位として保持する |
| UX-000016: 配布目録 | 配布目録 | Same | 配布物の内容とIdentityを保持する |
| UX-000016: 実行環境との結合 | 実行環境との結合 | Same | ToolとRuntimeのBindingを保持する |
...
|---|---|---|---|---|
| UX-000016: 利用能力 | 利用能力（Capability） ID | 利用能力 | 利用能力（Capability） ID | Same。Repositoryで必要な仕事能力を保持する |
| UX-000016: 利用可否 | 利用能力（Capability）＋環境 | 利用可否 | 利用能力（Capability）＋環境 | Same。現在利用できるかを能力と分けて保持する |
| UX-000016: 決定権限 | 主体、対象範囲、有効時点で識別する | 実行権限 | 主体・対象・時点 | Rename。能力を実行できるAuthorityとして保持する |
| UX-000016: 配布物 | 配布内容の基点（Content Root） | 配布物 | 配布内容の基点（Content Root） | Same。Toolを届ける単位として保持する |
| UX-000016: 配布目録 | 配布目録のハッシュ（Manifest Hash） | 配布目録 | 配布目録のハッシュ（Manifest Hash） | Same。配布物の内容とIdentityを保持する |
| UX-000016: 実行環境との結合 | 結合識別子（Binding Identity） | 実行環境との結合 | 結合識別子（Binding Identity） | Same。ToolとRuntimeのBindingを保持する |
...
```text
[補足: Repository 改訂版（Revision）] --宣言する--> [O: 利用能力（Capability）]
...
[O: 実行権限（Authority）] --利用を許す--> [行動: 利用能力（Capability）の使用]
```
...
|---|---|---|
| UX-000016 | Tool配布情報を管理する主体がCapability・Distribution・Manifestを保ち、利用可否と実行環境との結合を観測する責任がAvailabilityとBindingを観測する | Repository正本を管理する主体または導入環境の信頼方針を決める主体がTool利用と実行Authorityを許可する |
...
- `UX-000016`: 版不一致・欠落実行基盤・改ざん配布目録（Manifest）を対応版と誤認する
...
|---|---|---|---|
| UX-000016 | 発見したツール／実行基盤を起動する直前 | 版不一致・欠落実行基盤・改ざん配布目録（Manifest）を対応版と誤認する | Commit・配布集合・配布目録（Manifest）・実行基盤の対応を検証する |
...
| Quality Analysis / IA（伴走） | 「Repositoryの仕事に必要な標準Toolを、版と根拠を取り違えず選ぶ。」の成立条件、重要な失敗および検証意図を保持する。 |
