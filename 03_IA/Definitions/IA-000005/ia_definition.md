...
|---|---|---|---|
| UX-000007: 変更前の能力 | 利用能力 | Rename | 利用者が失ってはならない成立済み能力として保持する |
| UX-000007: 公開契約 | 正式契約 | Rename | 能力を利用側へ約束するCanonical Contractとする |
| UX-000007: 利用側 | 利用側 | Same | 契約を消費する主体を保持する |
| UX-000007: 置換後の実装 | 置換先 | Rename | 能力の新しい所有先として保持する |
| UX-000007: 維持根拠 | 能力維持の根拠 | Rename | 置換後も能力が成立する証拠を保持する |
...
|---|---|---|---|---|
| UX-000007: 変更前の能力 | 名称と基準版の根拠（Baseline Evidence）で識別する | 利用能力 | 名称と基準版の根拠（Baseline Evidence）で識別する | Rename。利用者が失ってはならない成立済み能力として表示を明確にする |
| UX-000007: 公開契約 | 適用対象、所有者、改訂版で識別する | 正式契約 | 適用対象、所有者、改訂版で識別する | Rename。能力を利用側へ約束するCanonical表示へ揃える |
| UX-000007: 利用側 | 公開入口または責任で識別 | 利用側 | 公開入口または責任で識別 | Same。契約を消費する主体を保持する |
| UX-000007: 置換後の実装 | 正式契約（Contract）へ結ぶ | 置換先 | 正式契約（Contract）へ結ぶ | Rename。能力の新しい所有先として表示を明確にする |
| UX-000007: 維持根拠 | 利用側（Consumer）ごとに結ぶ | 能力維持の根拠 | 利用側（Consumer）ごとに結ぶ | Rename。置換後も能力が成立する証拠として表示を明確にする |
...
| 正式契約（Canonical Contract） | 能力の意味契約 | 適用対象、所有者、改訂版で識別する |
...
|---|---|---|
| UX-000007 | 変更担当が能力・契約・利用側・置換先・検証根拠を追跡する | 能力維持の責任主体が置換完了を確定し、Evidence不足の能力を削除済みと扱わない |
...
|---|---|---|---|
| UX-000007 | 変更を完了・公開可能と判断する直前 | 主経路だけ移行し副次利用側を取り残す | 旧能力・全利用側・置換根拠を閉じる |
...
| Quality Analysis / IA（伴走） | 「責務移動後も何が維持・変更・廃止されたかを利用側まで理解する。」の成立条件、重要な失敗および検証意図を保持する。 |
