...
|---|---|---|---|
| UX-000008: 実行基盤 | 実行基盤 | Same | 複数境界を含むRuntime全体を保持する |
| UX-000008: 境界 | 境界 | Same | 故障を局所化する境界を保持する |
| UX-000008: 回復経路 | 回復経路 | Same | 故障箇所に応じた安全なRecovery Pathを保持する |
| UX-000008: 利用可能能力 | 利用可能能力 | Same | 故障後も継続できるCapabilityを保持する |
| UX-000008: 故障 | 故障 | Same | どの境界で何が失敗したかを保持する |
...
|---|---|---|---|---|
| UX-000008: 実行基盤 | 環境の種類と検証対象で識別する | 実行基盤 | 環境の種類と検証対象で識別する | Same。複数境界を含むRuntime全体を保持する |
| UX-000008: 境界 | 実行基盤＋境界種別で識別する | 境界 | 実行基盤＋境界種別で識別し、利用可能能力へ結ぶ | Same。故障を局所化する境界を保持する |
| UX-000008: 回復経路 | 故障＋開始状態＋到達状態で識別する | 回復経路 | 故障＋開始状態＋到達状態で識別し、対象境界と回復後の確認へ結ぶ | Same。故障箇所に応じた安全なRecovery Pathを保持する |
| UX-000008: 利用可能能力 | 成立している境界と必要条件から導く | 利用可能能力 | 成立している境界と必要条件から導く | Same。故障後も継続できるCapabilityを保持する |
| UX-000008: 故障 | 境界・観測時点・Failure Identityで識別する | 故障 | 境界・観測時点・Failure Identityで識別し、原因が分かる範囲へ結ぶ | Same。どの境界で何が失敗したかを保持する |
...
| 境界 | 認証、起動、取消、結果取得、回復のどこを確認したか | 実行基盤＋境界種別で識別し、利用可能能力へ結ぶ |
| 故障 | 成立しなかった境界と観測根拠 | 境界・観測時点・Failure Identityで識別し、原因が分かる範囲へ結ぶ |
| 利用可能能力 | 故障後も続けられる仕事 | 成立している境界と必要条件から導く |
| 回復経路 | 故障した境界を安全に戻す方法 | 故障＋開始状態＋到達状態で識別し、対象境界と回復後の確認へ結ぶ |
...
|---|---|---|
| UX-000008 | 実行基盤の境界・故障・利用可能能力を観測する責任が境界・故障・利用可能能力を観測し、実行基盤の運用主体が回復状態を更新する | 運用責任者が縮退継続・停止・回復を決め、観測不能を利用可能と推定しない |
...
- `UX-000008`: 一律の失敗表示で無関係な能力まで停止する
...
|---|---|---|---|
| UX-000008 | 一つの失敗を全体障害と判断する直前 | 一律の失敗表示で無関係な能力まで停止する | 失敗した場所・影響範囲・利用可能性を分ける |
...
| Quality Analysis / IA（伴走） | 「実行基盤の一部が故障したとき、故障した境界と影響を受ける能力を見分け、利用できる範囲まで一律に停止しない。」の成立条件、重要な失敗および検証意図を保持する。 |
