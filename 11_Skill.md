...
- 解消済みで、現在の固定改訂版へ再発または残存していない誤り
- 承認済みの対象範囲、原則および委譲された決定権限によってAIが一意に修正できる事項
- 結果、影響、既知の制約、追跡情報等の報告だけであり、現在の採否または操作を求めない事項

過去に重大だった誤りは、是正結果と実際の影響を目立つ位置で報告できるが、重大だったことだけを理由に現在の判断要求へ戻してはならない。現在の判断集合が空の場合は、「現在、人間による判断は必要ありません」と明示し、進行確認、完了確認または形式的な承認を追加しない。
