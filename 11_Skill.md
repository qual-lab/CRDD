...
判断の再現、変更または検証に必要な場合は、根拠（Evidence）、解釈（Interpretation）、仮説（Hypothesis）、代替（Alternative）、意図（Intent）、不確実性（Uncertainty）、制約（Constraint）、トレードオフ（Trade-off）、判断（Decision）、判断理由（Rationale）、観測結果（Observed Result）および学び（Learning）から、工程固有の推論コンテキスト（Reasoning Context）を構成する。これは全項目を埋める共通Schemaではない。各工程は現在の判断を支える要素だけを用い、成果物へ吸収されると判断理由を再現できなくなる情報を保持する。

認知状態の変化を扱う認知推論（Cognitive Reasoning）、複数の機会や方向から対象を選ぶ選択推論（Choice Reasoning）等は、工程固有の推論パターンである。一つのパターンをCRDD全体の唯一の思考手順へ昇格せず、必要性、再利用価値、検証可能性および記録負荷から適用の深さを決める。内部推論全文、固定の点数、固定数の代替案または工程ごとの専用ファイルを要求しない。

推論コンテキストは、複数の有力候補から非自明な選択をする、成果物だけでは削除・変更可否を判断できない、利用者の理解・比較・判断可能性が成果を左右する、または後で結果を観測して仮説を更新する場合に用いる。誤字修正、一意な変換、既存判断を変えない機械的更新、成果物自身に理由が十分含まれる変更では新しく作らない。適用要否を判断する根拠が不足する場合は、仮説や内面を捏造せず、必要な判断所有者または確認する根拠を示す。

観測結果や学びによって、当時のIntent、Hypothesis、DecisionまたはRationaleを遡及上書きしない。当時の判断、新しい観測、支持・反証・更新後の仮説および現在有効な意図を区別し、どれが現在の変更判断を制約するかを取得可能にする。単なる最新記述への置換で履歴の意味を消さない。

AIへ渡す場合は、対象成果物、上位Intent、変更を制約するDecision／Trade-off、必要なEvidenceおよび現在有効なHypothesisを関係から選び、無関係な推論を全量投入しない。古い仮説を現在値にせず、競合する推論をAIが勝手に統合せず、情報不足を生成で補完しない。使用した推論コンテキストと対象改訂版を後から追跡できるようにする。これは新しいContext Databaseを要求せず、既存正本と関係から必要最小限を投影する契約である。
...
収束は、現在の許可された目的・判断と、その受入条件・検証義務に対して評価する。その範囲の採否、優先順位、設計方向、保持条件、リスク、実装範囲または検証方法を有意に変え得る未処置の不確実性がある間は、成果物またはスキル実行を収束済みとしてはならない。必須条件の未達や現在の影響が不明な事項を、重要度の低さや将来改善という名称で保留しない。

さらに改善できる可能性だけを理由に探索範囲を無制限に広げない。現在の完成に影響せず安全に独立保留できる候補は、根拠・担当責任者・再評価契機・保留影響を残して追跡する。新しい根拠が現在の前提や安全判断を崩す場合は、対象外として捨てず[収束判断と人間への移送](10_Agent.md#convergence-and-verification-selection)へ戻る。探索範囲、対象外、情報源の限界および残存不確実性を隠して、未知が存在しないものとしてはならない。
...
- 2.3が定める現在の判断に有意な影響を与え得る未処置の不確実性、必須条件の未達または現在の影響が不明な事項がない
...
- 将来は判断が必要でも、現在許可された作業の継続、現在のGate、停止、採否、重大リスク受容または不可逆なEffectに影響せず、前提成果物の確認後まで安全に独立保留できる事項。この場合は、担当責任者、再評価契機、保留影響および元の判断候補、FindingまたはEvidenceへ追跡可能にする

将来判断の条件または影響が不明な場合、単なる延期表示またはAIの推定だけで現在の判断集合から除外しない。再評価契機、期限または前提変化が到来した場合、およびリスクが重大化した場合は、その時点の対象改訂版から判断要否を再構成する。現在必要な重大リスク、不可逆性、強い不確実性、残存リスク受容または決定権限の競合を将来判断として退避しない。
...
人間の回答を受けた後は、回答者の決定権限、対象となる判断単位、対象範囲および有効な改訂版を確認し、判断を責務を持つ正本コンテキストまたはAuthority記録へ反映してから工程を再開する。会話履歴だけを判断の正本にせず、反映できなかった回答を適用済みとして扱わない。反映後は派生した現在の判断集合と再開地点を現在化し、対象範囲、改訂版、決定権限または判断条件が変わらない限り同じ判断を再要求しない。再開不能、誤った再質問、回答形式の誤解または別の正本への反映は、利用者の説明不足へ帰属させず、判断支援と工程接続のFinding候補として扱う。
...
handoff:
  ...
  quality_assurance:
    ...
    verification_design_reference: 07_Quality/03_Verification_Design.md
