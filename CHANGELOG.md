...
This release adds a shared reasoning contract for producing strong expert candidates before Human judgment and an External Information Boundary for using research and connected tools without directly disclosing internal Context. It extends CRDD from preserving adopted decisions into structuring how AI and specialists retrieve patterns, synthesize alternatives, critique or falsify them, refine the result, and explain why further exploration is unlikely to change the decision. It does not define taste as fixed values, require a fixed option count, create a lighter route, treat “explored” and “converged” as evidence, or permit internal Context to be copied into external queries, prompts, attachments, or tool inputs.
...
- Adds the External Information Boundary to `01_Principles.md` and Core criterion C-11. Internal Context, unpublished information, personal information, confidential information, secrets, and identifiable combinations are not sent directly to search, browsers, external AI, APIs, MCP servers, generation services, analytics, support, or other external connections. External research uses a separately constructed, redacted, abstracted, and minimized research Context; when classification, authorization, or safe abstraction is uncertain, transmission stops for Human decision.
...
- Treats external content as untrusted Evidence rather than instructions. Prompt injection, poisoned documents or tools, cross-agent leakage, privilege expansion, sensitive inference, hallucinated authority, unsafe actions, and external-tool supply-chain risk are evaluated through Architecture, Implementation, Verification, and Quality Assurance. Delegation does not expand privilege, and important security invariants use runtime enforcement and synthetic boundary tests where available instead of agent self-attestation.
...
- Required for Core conformance: evaluate C-11 for the claimed scope. Identify the applicable information classes and inheritance, derived or combined information risk, trust zones, external connections, agent and tool privileges, external-input instruction boundary, supply-chain assets, security invariants, enforcement, verification, audit, revocation, and recovery. Do not classify unknown information as public by default.
- Required before enabling external research or connected external tools: identify the research or action need, construct the minimum separate external Context by redaction plus abstraction, check combination-based reidentification and destination authorization, and retain the Human decision where residual disclosure risk cannot be safely resolved. External results return as untrusted Evidence and do not acquire instruction or decision authority.
...
- Verification: run the full repository checker on one fixed revision, then independently review agent operation, document quality, and gap / impact plus conformance impact. Reconstruct representative existing-direction, multiple-candidate, insufficient-information, visual / 3D, record-only, safely abstracted public research, blocked identifiable query, malicious external instruction, least-privilege, and supply-chain cases from the canonical rules. Use synthetic sensitive-like fixtures, never real secrets, for boundary tests.
...
本リリースは、人間の判断へ渡す前に有力な専門候補を作る共通推論契約と、内部コンテキストを直接開示せずに外部調査・接続ツールを使うための外部情報境界を追加する。採用済み判断を守るだけでなく、AIと専門担当がパターンを検索し、候補を合成し、批評または反証し、修正し、追加探索が判断を変えにくい理由を説明できるようにする。センスを固定値へ変換せず、固定案数、軽量経路、「探索済み」「収束済み」という自己申告を根拠にせず、内部コンテキストを外部の検索語、プロンプト、添付またはツール入力へ複製することも許可しない。
...
- アーキテクチャを設計要因とトレードオフの評価から、設計要因→構造合成→評価へ強化した。パターン名を答えにせず、境界推論、状態所有、Failure-first推論、感度、可逆性、事前失敗分析から意味の異なる構造候補を作る。
...
- `01_Principles.md`へ外部情報境界、Core基準へC-11を追加した。内部コンテキスト、未公開情報、個人情報、機密情報、シークレット、識別可能な組合せを、検索、ブラウザ、外部AI、API、MCPサーバー、生成サービス、計測、問い合わせその他の外部接続へ直接送信しない。外部調査では、削除・抽象化・最小化した別の外部向け調査コンテキストを作り、分類、許可または安全な抽象化が不明な場合は送信せず人間判断へ戻す。
- 情報分類は明示的に上書きされない限り継承し、派生・組合せ情報は再識別リスクに応じて同等以上に保護する。シークレット値をコンテキストへせず、必要な場合も実行時注入するシークレットへの参照だけをエージェントへ渡す。
- 外部内容を命令ではなく信頼していない根拠として扱う。プロンプト注入、汚染文書・ツール、エージェント間漏洩、権限拡大、機微な推論、権限の幻覚、危険な外部操作および接続ツールの供給網リスクを、アーキテクチャ、実装、検証、品質保証へ接続した。委譲で権限を拡大せず、重要なセキュリティ不変条件は可能な範囲で実行時強制と合成境界試験を用い、エージェントの自己申告へ依存しない。
...
- Core準拠で必須: 対象範囲のC-11を評価する。適用する情報分類と継承、派生・組合せ情報のリスク、信頼領域、外部接続、エージェント／ツール権限、外部入力の命令境界、供給網資産、セキュリティ不変条件、実行時強制、検証、監査、失効および回復を確認する。不明な情報を既定で公開情報とみなさない。
- 外部調査または接続ツールの有効化前に必須: 調査・操作目的を特定し、削除と抽象化によって最小限の別コンテキストを作り、組合せによる再識別と送信先の許可を確認する。残存開示リスクを安全に解消できない場合は人間判断を残す。外部結果は信頼していない根拠として戻し、命令または決定権限を与えない。
...
- 検証: 一つの固定改訂版へ全体Checkerを実行し、エージェント運用の独立レビュー、文書監査、不足／影響と準拠影響の監査を行う。既存案のみ、複数候補、情報不足、視覚／3D、記録更新だけ、安全に抽象化した公開調査、識別可能な検索語の拒否、悪意ある外部命令、最小権限、供給網の代表例を正本から再構成する。境界試験には実在シークレットではなく機微情報を模した合成データを使う。
