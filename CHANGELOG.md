...
The Project Runtime repository-binding lease now creates its acquisition marker exclusively at the final path, then writes, flushes, closes, and reads it back. A concurrent process that loses the exclusive create stops as unavailable without effect instead of mistaking the winner's transient file for recovery residue. Incomplete markers left by process loss and legacy temporary files still retain the existing exact recovery obligation.
...
Project RuntimeのRepository Binding単位Leaseは、取得中Markerを最終Pathへ排他的に作成し、write、flush、close、readbackで耐久化する。同時取得中の一時fileを回復残存と誤認して不要な手動回復を要求する競合を除き、別Processの同時取得は一方だけを所有者とし、他方をEffect 0の取得不可へ閉じる。Process消失で残る不完全Markerと旧実装の一時fileは通常競合へ丸めず、既存のexact Recovery境界を維持する。
