...
The official Source A-to-B release path now promotes the signed manifest as opaque stable bytes through a dedicated launcher command. It verifies the signature, Source A commit and tree, closed runtime dependency set, policy, native artifact, current HEAD, destination absence, byte length, and SHA-256 before and after the exclusive write. Manual editing, JSON reserialization, text pipelines, and newline normalization are no longer valid promotion paths. A failed postcondition removes only the unchanged file created by that operation; uncertain cleanup blocks the carrier commit and release.
...
公式ReleaseのSource AからBへの経路は、署名済みManifestを不透明な安定byteとして扱う専用Launcherへ固定する。署名、Source AのCommit／Tree、閉じたRuntime依存集合、Policy、Native成果物、現在HEAD、配置先不存在、byte数およびSHA-256を排他的書込みの前後に検証し、手動編集、JSON再serialize、text pipelineまたは改行正規化を正式経路にしない。昇格後の検査失敗では、そのOperationが作成し内容も変わっていないfileだけを回収し、cleanup不明ならCarrier CommitとReleaseを停止する。
