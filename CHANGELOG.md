...
- Required when context dependencies or managed artifact dependencies are used: inventory adopted sources and versions, local overrides, consumers, update procedures, verification impact, migration, recovery, and deferred risk. Ordinary and transitive implementation dependencies may use architecture, package-manager, lockfile, or SBOM records; promote them when they cross a shared or independently managed boundary or carry material risk. Missing consumer detail alone does not trigger the full contract. If the applicability of those boundaries cannot yet be determined, keep the evaluation pending rather than treating it as `Not Applicable` or fully contracted. Preserve the distinction between CRDD baseline adoption under Maintenance and project-specific dependency updates under Context Dependency.
...
- コンテキスト依存または管理対象成果物依存を使用する場合に必須: 採用している依存元と版、ローカル上書き、利用側、更新手順、検証影響、移行、復旧、延期時リスクを棚卸しする。通常・推移依存は既存の依存管理正本を使用でき、共有または独立管理の境界をまたぐ場合か、重大リスクがある場合に完全契約へ昇格する。利用側情報の不足だけでは昇格させず、境界への該当性を判定できない場合は確認待ちとする。CRDD基準版採用は保守、プロジェクト固有依存はコンテキスト依存を正本とする境界を維持する。
