...
AIの機能的な役割は固定エージェント構成を要求しない。対象の責務、必要な独立性および実行権限を評価し、次のモードを単一エージェントまたは複数の実行者へ割り当てる。使わないモードを形式的に割り当てない。
...
作って終わりにしない。実装・検証・運用・利用から得た制約、失敗、結果、仮説の支持・反証を、責務を持つ正本コンテキストへ戻す。未完了作業または採用済み変更への影響がある場合はロードマップまたは変更トレースへ接続し、影響がない場合は新しい追跡項目を作らない。
