 ...
 * @security N/A: DataDescriptorはAuthority、秘密値または信頼判断を扱わない。
 ...
 * @security N/A: PlainArraySnapshotはAuthority、秘密値または信頼判断を扱わない。
...
/**
 * snapshotOpenPlainRecordの処理を実行する。
 *
 * @responsibility snapshotOpenPlainRecordに対応する入力処理と結果生成を所有する。
 * @trace ARCH-000012
 * @input value: unknown
 * @returns Readonly<Record<string, unknown>> | nullを返す。
 * @precondition 「value: unknown」がsnapshotOpenPlainRecordの入力契約を満たす。
 * @postcondition snapshotOpenPlainRecordの責務を完了した結果だけを返す。
 * @effect N/A: snapshotOpenPlainRecordは入力と局所値だけを扱い、外部または共有Effectを発行しない。
 * @failure snapshotOpenPlainRecordは入力不正または下位処理の失敗を呼出し側へ返す。
 * @invariant snapshotOpenPlainRecordは入力から導いた結果以外の共有状態を変更しない。
 * @boundary N/A: snapshotOpenPlainRecordはProcess内の同一Subsystemで完結する。
 * @security N/A: snapshotOpenPlainRecordはAuthority、秘密値または信頼判断を扱わない。
 * @concurrency N/A: snapshotOpenPlainRecordは共有非同期状態を持たない同期処理である。
 */
