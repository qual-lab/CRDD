  ...
  try {
    ...
    function checkWorkLifecycleNavigation(): void {
      ...
      const aggregateIds = fs
        ...
        .filter((entry) =>
          lstatIfPresent(
            path.join(changesRoot, entry.name, "change.md"),
          )?.isFile(),
        )
      ...
      for (const version of fs.readdirSync(releasesRoot, {
        withFileTypes: true,
      })) {
    ...
    function checklistItemText(line: string): string | null {
      ...
      const result = /^- (?:N\/A|OPEN|FAIL): \S.*? — (?<text>\S.*)$/u.exec(
        line,
      );
    ...
    function templateVisibleChecklistError(
      ...
      if (
        bodyLines
          ...
          .some(
            (line) =>
              /^#{1,6}(?:\s|$)/u.test(line) || /^(?:=+|-+)\s*$/u.test(line),
          )
      )
    ...
    function checkUxRequirementAnalysis(): void {
      ...
      const experienceMapPath = path.join(
        root,
        "02_UX",
      ...
      const uxRootHandoffIsValid = (source: string, heading: string) => {
        ...
        const labels = section.split(/\r?\n/u).flatMap((line) => {
          ...
          return label && label !== "項目" && !/^-+$/u.test(label)
            ? [label]
            : [];
        });
      ...
      if (lstatIfPresent(discoveryAnalysisRoot)?.isDirectory()) {
        for (const entry of fs.readdirSync(discoveryAnalysisRoot, {
          ...
          if (!entry.isDirectory() || !/^EXP-[0-9]{6}$/u.test(entry.name))
            continue;
      ...
      const canonicalUxIds = new Set(
        uxIndex
          .split(/\r?\n/u)
          .map(
            (line) => line.match(/^\| (?:`|\[)(?<id>UX-[0-9]{6})/u)?.groups?.id,
          )
          .filter((value): value is string => Boolean(value)),
      );
      ...
      const canonicalRelationPairs = new Set(
        uxIndex.split(/\r?\n/u).flatMap((line) => {
          const uxId = line.match(/^\| (?:`|\[)(?<id>UX-[0-9]{6})/u)?.groups
            ?.id;
          if (!uxId) return [];
      ...
      for (const entry of fs.readdirSync(requirementsRoot, {
        ...
        if (!entry.isDirectory() || !/^REQ-[0-9]{6}$/u.test(entry.name))
          continue;
        const analysisPath = path.join(
          requirementsRoot,
          entry.name,
        ...
        const invalidDiscoveryInputLinks = analysisTargets.filter(
          (resolved) => {
        ...
        if (
          ...
          !hasExperienceChange ||
          uxIndex.split(`Analysis/${entry.name}/ux_analysis.md`).length - 1 !==
            1
        )
          add(
            "error",
        ...
        const doesRelationDiagramMatchTable =
          ...
          [...relationDiagramEntries]
            ...
            .every(
              (value, index) =>
                value === [...relationTableEntries].sort()[index],
            );
      ...
      if (lstatIfPresent(uxDefinitionsRoot)?.isDirectory()) {
        for (const entry of fs.readdirSync(uxDefinitionsRoot, {
          ...
          if (!entry.isDirectory() || !/^UX-[0-9]{6}$/u.test(entry.name))
            continue;
          const definitionPath = path.join(
            uxDefinitionsRoot,
            entry.name,
          ...
          if (
            ...
            definition.includes(
              "UXからArchitectureまたは検証への直接Handoff",
            ) ||
      ...
      if (
        ...
        [...analysisRelationPairs].some(
          (pair) => !canonicalRelationPairs.has(pair),
        )
      )
        add(
          "error",
      ...
      if (
        ...
        [...analysisJourneyPairs].some(
          (pair) => !canonicalJourneyPairs.has(pair),
        )
      )
        add(
          "error",
    ...
    function checkIaReconstruction(): void {
      ...
      const hasExactIaDownstream = (section: string | null) => {
        const labels = (section ?? "")
          ...
          .map(
            (line) =>
              /^\| (?<label>[^|]+) \| [^|]+ \|$/u.exec(line)?.groups?.label,
          )
          .filter(
            (label): label is string => Boolean(label) && label !== "接続先",
          );
        return (
      ...
      if (lstatIfPresent(iaAnalysisRoot)?.isDirectory()) {
        for (const entry of fs.readdirSync(iaAnalysisRoot, {
          ...
          const dispositionSection = exactSecondLevelSection(
            analysis,
            "5. IA処置",
          );
          ...
          for (const objectName of objectNames) {
            ...
            if (dispositions.length > 1) {
              ...
              if (transformationFamilies.size > 1)
                isAnalysisMappingInvalid = true;
            }
          }
          ...
          if (
            ...
            !dispositionSection ||
            ![...analysisPairs].some((pair) =>
              pair.startsWith(`${entry.name}|`),
            ) ||
      ...
      if (lstatIfPresent(iaDefinitionsRoot)?.isDirectory()) {
        for (const entry of fs.readdirSync(iaDefinitionsRoot, {
          ...
          if (!entry.isDirectory() || !/^IA-[0-9]{6}$/u.test(entry.name))
            continue;
          const definitionPath = path.join(
            iaDefinitionsRoot,
            entry.name,
          ...
          let isIdentityRelationMappingInvalid =
            identityRelationRows.length === 0;
          for (const [
            sourceObject,
      ...
      if (
        ...
        [...definitionMappingKeySet].some(
          (key) => !analysisMappingKeySet.has(key),
        )
      )
        add(
          "error",
    ...
    function checkUiReconstruction(): void {
      ...
      const uiProjectionTemplates = [
        ...
        path.join(
          ...
          "template",
          "04_UI",
          "03_Interaction_and_State_Model.md",
        ),
        path.join(
          root,
          "template",
      ...
      const canonicalIds = (
        ...
        for (const entry of fs.readdirSync(definitionsRoot, {
          ...
          if (
            ...
            lstatIfPresent(
              path.join(definitionsRoot, entry.name, file),
            )?.isFile()
          )
            ids.add(entry.name);
        return ids;
      ...
      if (lstatIfPresent(uiAnalysisRoot)?.isDirectory())
        for (const entry of fs.readdirSync(uiAnalysisRoot, {
          ...
          if (
            ...
            !requiredViewSections.every((heading) =>
              analysis.includes(heading),
            ) ||
            !dispositionSection ||
      ...
      if (lstatIfPresent(uiDefinitionsRoot)?.isDirectory())
        for (const entry of fs.readdirSync(uiDefinitionsRoot, {
          ...
          if (!entry.isDirectory() || !/^UI-[0-9]{6}$/u.test(entry.name))
            continue;
          const definitionPath = path.join(
            uiDefinitionsRoot,
            entry.name,
          ...
          const uxSection = exactSecondLevelSection(
            definition,
            "UX観点の分析結果",
          );
          const iaSection = exactSecondLevelSection(
            definition,
            "IA観点の分析結果",
          );
          ...
          if (
            ...
            ![...definitionUxUi].some((key) =>
              key.endsWith(`|${entry.name}`),
            ) ||
    ...
    function checkSpecReconstruction(): void {
      ...
      const specProjectionTemplates = [
        ...
        path.join(
          ...
          "template",
          "05_SPEC",
          "02_Use_Case_and_Behavior_Flow.md",
        ),
        ...
        path.join(
          ...
          "template",
          "05_SPEC",
          "05_Error_Effect_and_Recovery.md",
        ),
      ...
      const indexCountRows = tableRows(index, [
        "入力／成果",
        "件数",
        "現在の処置",
      ]);
      ...
      if (lstatIfPresent(specDefinitionsRoot)?.isDirectory())
        for (const entry of fs.readdirSync(specDefinitionsRoot, {
          ...
          const acceptanceSection = sectionBody(
            source,
            "## 受入条件と検証義務",
          );
          const uiPairLine =
          ...
          const analysisUxIds = new Set(
            [...uxInputSection.matchAll(/UX-[0-9]{6}/gu)].map(
              (match) => match[0],
            ),
          );
          const analysisIaIds = new Set(
            [...iaInputSection.matchAll(/IA-[0-9]{6}/gu)].map(
              (match) => match[0],
            ),
          );
          const formalUxIds = new Set(
      ...
      if (lstatIfPresent(correspondencePath)?.isFile()) {
        const correspondence = visibleMarkdownStructure(
          read(correspondencePath),
        );
        ...
        const hasSourcePhrase = (
          ...
          for (
            let index = 0;
            index <= normalizedFact.length - width;
            index += 1
          )
            if (
              normalizedSource.includes(
                normalizedFact.slice(index, index + width),
              )
            )
              return true;
          return false;
        ...
        for (const line of correspondence.split(/\r?\n/u)) {
          ...
          if (row) {
            ...
            const specSource = visibleMarkdownStructure(
              read(
                path.join(specDefinitionsRoot, row[2], "spec_definition.md"),
              ),
            );
            const uiFormal = sectionBody(uiSource, "## 正式入力と変換根拠");
            const specFormal = sectionBody(specSource, "## 正式入力と変換根拠");
            const uiContext = new Set(
              uiFormal.match(/(?:UX|IA)-[0-9]{6}/gu) ?? [],
            );
              ...
              evidenceRows.every(
                ([lens, uiEvidence, specEvidence, result, reason]) => {
                  ...
                  return (
                    ...
                    hasSourcePhrase(
                      uiFact,
                      sectionBody(uiSource, headings[0]),
                    ) &&
                    hasSourcePhrase(
                      specFact,
    ...
    function checkArchitectureReconstruction(): void {
      ...
      if (lstatIfPresent(analysisRoot)?.isDirectory())
        for (const entry of fs.readdirSync(analysisRoot, {
          withFileTypes: true,
        }))
          if (
            entry.isDirectory() &&
      ...
      for (const [kind, ids] of [
        ...
        for (const id of ids) {
          ...
          const evaluationDecisions: Readonly<
      ...
      if (lstatIfPresent(definitionRoot)?.isDirectory())
        for (const entry of fs.readdirSync(definitionRoot, {
          ...
          if (
            ...
            !requiredHeadings.every((heading) => source.includes(heading)) ||
            !requiredStructures.every((fragment) =>
      ...
      if (lstatIfPresent(componentModelPath)?.isFile()) {
        const componentSource = visibleMarkdownStructure(
          read(componentModelPath),
        );
      ...
      if (!lstatIfPresent(detailMapPath)?.isFile())
        ...
        const closureSection = sectionBody(
          source,
          "## 3. Architecture定義の閉包",
        );
      ...
      if (lstatIfPresent(detailRoot)?.isDirectory())
        for (const entry of fs.readdirSync(detailRoot, {
          ...
          if (!entry.isDirectory() || !/^[a-z0-9-]+$/u.test(entry.name))
            continue;
      ...
      if (
        ...
        [...registeredDetailAreas].some(
          (area) => !actualDetailAreas.has(area),
        ) ||
      ...
      if (
        ...
        [...analysisRelations].some(
          (relation) => !registryRelations.has(relation),
        )
      )
        add(
          "error",
    ...
    function checkQualityReconstruction(): void {
      ...
      if (
        ...
        phaseAnalysisPaths.some(
          (phasePath) => !lstatIfPresent(phasePath)?.isFile(),
        )
      ) {
        add(
          "error",
      ...
      const qualityRootChecklistSpecs = [
        ...
        [
          "03_Verification_Design.md",
          qualityVerificationDesignChecklistItemTexts,
        ],
      ...
      const qualityDefinitionPaths = fs
        .readdirSync(path.join(qualityRoot, "Definitions"), {
          withFileTypes: true,
        })
        .filter(
      ...
      if (
        !allowedRealityAuditStates.some((state) => state === realityAuditState)
      )
        add(
          "error",
      ...
      const isRealityStateAllowedForCenter =
        ...
        (isQualityDesignReady &&
          ["Pending — Not Started", "In Progress"].includes(
            realityAuditState,
          )) ||
      ...
      if (isQualityDesignReady || isQualityReady) {
        const qualityDesignRootFiles = qualityRootChecklistSpecs
          ...
          .filter(
            (fileName) =>
              fileName !== "05_Current_Implementation_Reality_Audit.md",
          );
      ...
      const phaseAnalyses = phaseAnalysisPaths.map((phasePath) =>
        read(phasePath),
      );
      const analysis = read(analysisPath);
      ...
      if (
        summarySourceGoalRelationEntries.length !==
        summarySourceGoalRelations.size
      )
        add(
          "error",
      ...
      if (
        ...
        [...physicalDetailUnits].some(
          (unit) => !analysisDetailUnits.has(unit),
        ) ||
      ...
      for (const definitionPath of physicalDefinitionPaths) {
        ...
        if (
          ...
          itemHeaderCells.some(
            (cell, index) => cell !== expectedItemColumns[index],
          )
        )
          add(
            "error",
        ...
        if (
          ...
          ["UT", "IT", "ST", "UAT"].some(
            (level) => !applicabilityRows.has(level),
          )
        )
          add(
            "error",
        ...
        const expectedPrimaryLevels = ["UT", "IT", "ST", "UAT"]
          .filter(
            (level) =>
              applicabilityRows.get(level)?.applicability === "Required",
          )
      ...
      if (
        definitionSourceRelationEntries.length !==
        definitionSourceRelations.size
      )
        add(
          "error",
      ...
      if (
        listedGoalLocalRelationEntries.length !== listedGoalLocalRelations.size
      )
        add(
          "error",
    ...
    function discoverProjectFiles(): Discovery {
          ...
          ? repositoryEntries.reason === "version_control_not_installed"
            ...
            : repositoryEntries.reason === "repository_not_found"
              ...
              : repositoryEntries.reason ===
                  ...
                  repositoryEntries.reason ===
                    "repository_entries_output_invalid"
                ? "list-failed"
      ...
      if (repositoryEntries.status === "completed") {
        ...
        const gitlinkEntries: GitlinkEntry[] = nestedEntries
          .filter(
            (entry) => !entry.conflicted && entry.contentIdentity !== null,
          )
          .map((entry) => ({
        ...
        const baselineSubmoduleState = {
          ...
          gitlink_indexed: isBaselineSubmodule
            ? isBaselineGitlinkIndexed
            : null,
    ...
    function underscoreCanOpen(
      ...
      return (
        flanking.left && (!flanking.right || flanking.isPreviousPunctuation)
      );
    }
    ...
    function uxFormalInputTargets(text: string): FormalInputTarget[] {
      ...
      const addCandidate = (
        ...
        const isPathRelevant = isDiscoveryPathCandidate(
          candidate,
          decoded.value,
        );
      ...
      for (const candidate of markdownLinkTargets(content))
    ...
    function loadFixedHistoryMigration(): Readonly<{
      ...
      try {
        ...
        for (const value of parsed.entries) {
          ...
          if (
            !new Set(["current", "fixed_history"]).has(
              entry.currentnessAtMigration,
            )
          )
    ...
    function resolveLinkWithFixedHistory(
      ...
      const migration =
        fixedHistoryMigration.byCurrentPath.get(currentSourcePath);
      if (!migration)
        return {
          source,
          raw,
    ...
    function checkPhaseDiagramDispositionContracts(): void {
      ...
      for (const [relativePath, diagrams] of phaseDiagramTemplateProfiles) {
        ...
        const sectionHeading = /^## (?:[0-9]+\.\s+)?基本図の処置$/mu.exec(
          content,
        );
        ...
        const headerCells =
          headerIndex < 0 ? [] : cells(lines[headerIndex] ?? "");
        if (
    ...
    const requestedScopes = scopeValues.map((value) =>
      path.resolve(root, value),
    );
    for (const scope of requestedScopes) {
      if (!isWithin(root, scope)) {
        cliError(`--scope must stay under the target root: ${scope}`);
      }
      if (pathContainsSymbolicLink(scope)) {
        cliError(
          `--scope must not traverse a symbolic link: ${relative(scope)}`,
        );
      }
      ...
      if (!fs.existsSync(scope)) {
        cliError(`--scope does not exist: ${scope}`);
      }
      if (
        discovery.baseline_submodule &&
        adoptedBaselineRoot &&
    ...
    if (docsRoot) {
      for (const name of fs.readdirSync(docsRoot)) {
        ...
        canonicalDocumentStates.push({
          ...
          releasedBaseline:
            content.match(/^Released Baseline:\s*(v[0-9]\S*)\s*$/m)?.[1] ??
            null,
        });
      }
    }
    ...
    if (
      ...
      for (const document of canonicalDocumentStates) {
        if (
          ...
          document.version === stableVersion
        ) {
          continue;
        }
        add(
      ...
      const readme = lstatIfPresent(readmePath)?.isFile()
        ? read(readmePath)
        : "";
      ...
      for (const file of allMarkdownFiles) {
        ...
        const isChangeTraceForStableRelease =
          /^99_Roadmap\/Changes\/CHG-[0-9]{6}\/change\.md$/u.test(
            relative(file),
          );
    ...
    if (
        ...
        const releases = starts.map((releaseStart) => {
          ...
          return {
            ...
            entries: markdown.entries.filter(
              (entry) =>
                entry.index >= releaseStart && entry.index < releaseEnd,
            ),
          };
        });
      ...
      > = {
        English: [
          ...
          [
            /^\s*[-*+]\s+Not required(?: for adopting projects)?:/u,
            "Not required",
          ],
    ...
    const manualStableIdRevisionPattern =
    ...
    for (const file of allMarkdownFiles) {
      ...
      for (let index = 0; index < lines.length - 2; index += 1) {
        ...
        const numeratorIndex = headers.findIndex((header) =>
          ["到達分岐数（分子）", "Covered Branches (Numerator)"].includes(
            header,
          ),
        );
        const denominatorIndex = headers.findIndex((header) =>
          ["対象分岐数（分母）", "Total Branches (Denominator)"].includes(
            header,
          ),
        );
    ...
    for (const file of allMarkdownFiles) {
      ...
      for (let index = 0; index < lines.length - 2; index += 1) {
        ...
        const missingStateColumns = stateColumns.filter(
          ([, target]) => target < 0,
        );
        ...
        for (let row = index + 2; row < lines.length; row += 1) {
          ...
          const progress =
            progressIndex >= 0 ? (cells[progressIndex] ?? "") : "";
          ...
          if (
            progressIndex >= 0 &&
            !["Identified", "Planned", "Applied", "Self-checked"].includes(
              progress,
            )
          ) {
            add(
          ...
          if (
            resolutionIndex >= 0 &&
            !["Open", "Resolved"].includes(resolution)
          ) {
            add(
              "error",
    ...
    for (const file of allFiles) {
      ...
      if (
        isEvidenceFile(file) &&
        isWithin(path.join(root, "99_Roadmap"), file)
      ) {
    ...
    if (shouldOutputJson && shouldOutputSummary) {
      request.writeOutput(`${JSON.stringify(report, null, 2)}\n`);
    } else if (shouldOutputJson) {
      request.writeOutput(`${JSON.stringify(findings, null, 2)}\n`);
    } else {
      ...
      request.writeOutput(
        `CRDD check: ${errors} error(s), ${warnings} warning(s)\n`,
      );
      if (shouldOutputSummary) {
        ...
        request.writeOutput(`Executed=${report.executed_at}\n`);
        request.writeOutput(
          `Repository=${report.repository_mode}; discovery=${report.discovery_source}; git_failure=${report.discovery_git_failure ?? "none"}\n`,
        );
        request.writeOutput(`Unchecked=${JSON.stringify(report.unchecked)}\n`);
        if (referenceMap) {
          request.writeOutput(`${JSON.stringify(referenceMap, null, 2)}\n`);
        }
      }
    }
    ...
    if (error instanceof CheckerCommandExit)
