import {
  ...
  type SemanticIr,
} from "../../../../crdd-domain-library/src/domain/semantic-coverage/index.ts";
...
type SemanticCoverageFinding = Readonly<{
  code: string;
  path: string;
  message: string;
}>;
...
export function mapSemanticDomainIssueToCheckerFinding(
  issue: DomainIssue,
...
export function compileSemanticIrFromRepository(
  ...
  return {
    ir: outcome.result,
    findings: outcome.issues.map(mapSemanticDomainIssueToCheckerFinding),
  };
}
...
export function createSemanticCoverageGraph(
  ...
  prerequisiteFindings: readonly SemanticCoverageFinding[],
): Readonly<{
