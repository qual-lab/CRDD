...
function buildPlan(
  ...
  if (!/^[a-f0-9]{64}$/u.test(activation.grant.providerHomeIdentityHash))
    return null;
  const providerContainerName = `crdd-claude-${activation.grant.providerHomeIdentityHash.slice(0, 16)}`;
  if (
    [
      internalNetworkName,
      egressNetworkName,
...
/**
 * prepareRuntimeOwnedClaudeDockerCandidateの処理を実行する。
 *
 * @responsibility prepareRuntimeOwnedClaudeDockerCandidateに対応する入力処理と結果生成を所有する。
 * @trace ARCH-000015
...
/**
 * prepareRuntimeOwnedClaudeDockerTaskCandidateの処理を実行する。
 *
 * @responsibility prepareRuntimeOwnedClaudeDockerTaskCandidateに対応する入力処理と結果生成を所有する。
 * @trace ARCH-000015
...
/**
 * cancelRuntimeOwnedClaudeDockerCandidateの処理を実行する。
 *
 * @responsibility cancelRuntimeOwnedClaudeDockerCandidateに対応する入力処理と結果生成を所有する。
 * @trace ARCH-000015
...
/**
 * consumeRuntimeOwnedClaudeDockerPlanForProcessControllerの処理を実行する。
 *
 * @responsibility consumeRuntimeOwnedClaudeDockerPlanForProcessControllerに対応する入力処理と結果生成を所有する。
 * @trace ARCH-000015
 * @input preparedCapability: unknown、managementCapability: unknown
...
/**
 * createIsolatedClaudeDockerRuntimeAdapterCandidateの処理を実行する。
 *
 * @responsibility createIsolatedClaudeDockerRuntimeAdapterCandidateに対応する入力処理と結果生成を所有する。
 * @trace ARCH-000015
 * @input dependencies: Omit<RuntimeState, "prepared" | "managementCapabilities">
...
/**
 * describeClaudeDockerRuntimeAdapterContractの処理を実行する。
 *
 * @responsibility describeClaudeDockerRuntimeAdapterContractに対応する入力処理と結果生成を所有する。
 * @trace ARCH-000015
