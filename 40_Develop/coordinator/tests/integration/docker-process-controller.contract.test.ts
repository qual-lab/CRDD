...
for (const dockerCleanupConfirmed of [true, false]) {
  /**
   * Windows Process Gate: 取消結合: 実子孫終了後の模擬Docker cleanup=${dockerCleanupConfirmed}を検証する。
   *
   * @responsibility Windows Process Gate: 取消結合: 実子孫終了後の模擬Docker cleanup=${dockerCleanupConfirmed}の合否判定を所有する。
   * @trace ERB-IT-002
   * @precondition Test Fileが構築するfixtureと入力を使用する。
   * @stimulus Windows Process Gate: 取消結合: 実子孫終了後の模擬Docker cleanup=${dockerCleanupConfirmed}の対象操作を実行する。
   * @observation 結果、状態、Effectおよび終了後条件を観測する。
   * @oracle Test本文のassertionが期待条件を満たす。
   * @cleanup Test本文または登録済みhookが作成資源を清掃する。
   * @boundary Adjacent 1 Block: Controller→stdio・signal・close→資源Observer
