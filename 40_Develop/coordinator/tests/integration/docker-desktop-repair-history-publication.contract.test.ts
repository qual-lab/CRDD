...
const bytesValue = Buffer.from('{"schema":"test","value":"a"}\n', "utf8");
const otherValue = Buffer.from('{"schema":"test","value":"b"}\n', "utf8");
...
function relativeImports(source: string) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    for (let cursor = index + 1; cursor < tokens.length; cursor += 1) {
      ...
      if (value.startsWith(".")) resultItems.push(value);
      break;
    }
  }
  return resultItems;
...
function productionDependencyClosure(entry: string) {
  ...
  while (pendingItems.length > 0) {
    const current = pendingItems.pop();
...
function observeFinalGlobalPublication(
  ...
  let isPreparation: boolean | null = null;
  try {
  ...
  const state =
    target?.equals(expectedTarget) === true && isPreparation === false
...
test("回復可能な公開: absent target + absent prepareはexact targetへ公開してprepareを残さない", (t) => {
  ...
  assert.equal(adapter.publish(TARGET, PREPARE, bytesValue), true);
  assert.equal(fs.readFileSync(value.target).equals(bytesValue), true);
...
test("回復可能な公開: same-byte foreign prepareは双方を変更せず拒否する", (t) => {
  ...
  const adapter = createRepairHistoryPublicationTestingAdapter(value.directory);
  assert.equal(adapter.publish(TARGET, PREPARE, bytesValue), false);
  assert.equal(fs.readFileSync(value.target).equals(bytesValue), true);
  assert.equal(fs.readFileSync(value.preparation).equals(bytesValue), true);
...
function verifyDifferentTarget(t: test.TestContext, isPrepare: boolean) {
  ...
  if (isPrepare) fs.writeFileSync(value.preparation, bytesValue);
  const adapter = createRepairHistoryPublicationTestingAdapter(value.directory);
  assert.equal(adapter.publish(TARGET, PREPARE, bytesValue), false);
  assert.equal(fs.readFileSync(value.target).equals(otherValue), true);
  assert.equal(fs.existsSync(value.preparation), isPrepare);
  if (isPrepare)
    assert.equal(fs.readFileSync(value.preparation).equals(bytesValue), true);
    ...
    value.directory,
    {
      observePlatformConfirmation: () => {
        if (!fs.existsSync(value.preparation))
          fs.writeFileSync(value.preparation, otherValue);
      },
    },
  );
  assert.equal(adapter.publish(TARGET, PREPARE, bytesValue), false);
...
test("回復可能な公開: Platform確認または最終shape観測が不明なら成功にしない", (t) => {
  ...
  assert.equal(
    ...
    }).publish(TARGET, PREPARE, bytesValue),
    false,
  );

  const prepareUnknown = fixture(t);
  fs.writeFileSync(prepareUnknown.target, bytesValue);
  ...
  fs.writeFileSync(targetUnknown.target, bytesValue);
  assert.equal(
...
function verifyFaultRecovery(
  ...
  const first = createRepairHistoryPublicationTestingAdapter(value.directory, {
    injectFault: (candidate: RepairHistoryPublicationFaultPoint) => {
      if (candidate === point && !isInjected) {
        isInjected = true;
        throw new Error(`injected_${point}`);
      }
    },
  });
  assert.equal(first.publish(TARGET, PREPARE, bytesValue), false);
  ...
  assert.equal(retry.publish(TARGET, PREPARE, bytesValue), true);
...
async function runRace(t: test.TestContext, contents: readonly Buffer[]) {
  ...
  const childItems = contents.map((bytes, index) => {
  ...
  const readyBeforeReleaseItems = childItems.map(({ ready }) =>
    fs.existsSync(ready),
  );
  ...
  const attempts = results.map((result, index) => {
    const input = childItems[index]?.input;
    assert.ok(input);
  ...
  return {
    ...
    results,
    attempts,
    readyBeforeRelease: readyBeforeReleaseItems,
  };
}
...
test("回復可能な公開: 異byteの実別Process競合は局所結果と最終共有状態を分離する", async (t) => {
  ...
  const loser = winner.equals(bytesValue) ? otherValue : bytesValue;
  const retry = createRepairHistoryPublicationTestingAdapter(value.directory);
  ...
  assert.throws(
    () =>
      observeFinalGlobalPublication(value, [
        { input: bytesValue, result: true },
        { input: otherValue, result: false },
      ]),
