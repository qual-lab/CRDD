...
test("完了済みRecoveryのsession handoffは8件で閉じ、9件目をEffect 0で拒否する", () => {
  const fixture = createKilledFullProductionRecoveryRoot("previous");
  const originalRoot = verifiedRoot(fixture.root);
  const reboundRoots = Array.from({ length: 9 }, (_unused, index) =>
    Object.freeze({
      ...originalRoot,
  ...
  try {
    ...
    const priorValues = fs.readdirSync(fixture.root).sort();
    assert.deepEqual(
      recoverRuntimeOwnedDockerTaskFromVerifiedRootWithObserver(
        fixture.recoveryId,
    ...
    assert.deepEqual(fs.readdirSync(fixture.root).sort(), priorValues);
    assert.equal(
