    ...
    execution: {
      runSingleTaskAttempt: async (input: {
        ...
        return {
          ...
          status: successful ? ("completed" as const) : ("blocked" as const),
          reason: successful ? "task_completed" : "transient_failure",
          effectState: "settled" as const,
          cleanupConfirmed: true,
          manualRecoveryRequired: false,
          processRestartRequired: false,
          candidateId: successful ? "candidate-full" : null,
...
test("human decision is one-time and resumes only through a fresh bounded plan", async (t) => {
  ...
  const common = { ...input, principalId: "principal-full", store };
