...
function lowS(value: Uint8Array) {
...
function frame(domain: string, value: Record<string, unknown>) {
...
function fixture() {
  ...
  const installationKeyId = hash(installationSpki),
    rootKeyId = hash(rootSpki),
    issuerKeyId = hash(issuerSpki);
  const caSeriesId = "7".repeat(32),
    enrollmentId = "4".repeat(32);
  const record = {
  ...
  const recordMessageCandidate =
    buildProvisioningRecordDomainMessageCandidate(record);
  if (
    recordMessageCandidate.status !== "candidate" ||
    !("message" in recordMessageCandidate) ||
    !Buffer.isBuffer(recordMessageCandidate.message)
  ) {
    assert.fail(
      `fixture record message was invalid: ${recordMessageCandidate.reason}`,
    );
  }
  const recordMessage = recordMessageCandidate.message;
    ...
    keys: [
      {
        keyId: installationKeyId,
        algorithm: "ECDSA-P256-SHA256",
        spkiDer: installationSpki.toString("base64url"),
        enrollmentCaId: caSeriesId,
        notBefore: "2026-01-01T00:00:00.000Z",
        notAfter: "2027-01-01T00:00:00.000Z",
      },
    ],
  };
  const recordRevocations = {
    ...
    signatures: [
      {
        ...
        signature: sign(
          null,
          frame(PROVISIONING_CA_DOMAINS.issuingCertificate, issuing),
          root.privateKey,
        ).toString("base64url"),
      },
    ],
    ...
    signatures: [
      {
        ...
        signature: sign(
          null,
          frame(PROVISIONING_CA_DOMAINS.revocationManifest, caRevocations),
          root.privateKey,
        ).toString("base64url"),
      },
    ],
    ...
    signatures: [
      {
        ...
        signature: sign(
          null,
          frame(INITIAL_ENROLLMENT_DOMAINS.certificate, certificate),
          issuer.privateKey,
        ).toString("base64url"),
      },
    ],
  ...
  const compile = (
    compiler: (value: unknown) => unknown,
    value: unknown,
  ): Buffer => {
  ...
  return {
    recordEnvelopeBytes: compile(
      compileProvisioningRecordEnvelopeCandidate,
      recordEnvelope,
    ),
    recordTrustAnchorSetBytes: compile(
      compileProvisioningTrustAnchorSetCandidate,
      recordKeyset,
    ),
    recordRevocationManifestBytes: compile(
      compileProvisioningRevocationManifestCandidate,
      recordRevocations,
    ),
    certificateBindings: [{ certificateEnvelope, issuingCertificateEnvelope }],
    provisioningCaRootTrustSet: rootTrustSet,
    provisioningCaRevocationEnvelope: caRevocationEnvelope,
...
test("enrollment, scope, key, CA series and dynamic mismatches fail closed", () => {
  const mutations: Array<(value: ReturnType<typeof fixture>) => void> = [
    (x) => {
      const binding = x.certificateBindings[0];
      assertPresent(binding);
      binding.certificateEnvelope.payload.enrollmentId = "8".repeat(32);
    },
    (x) => {
      const binding = x.certificateBindings[0];
      assertPresent(binding);
      binding.certificateEnvelope.payload.platformScopeId = "8".repeat(32);
    },
    (x) => {
      const binding = x.certificateBindings[0];
      assertPresent(binding);
      binding.issuingCertificateEnvelope.payload.caSeriesId = "8".repeat(32);
    },
  ];
  for (const mutate of mutations) {
    ...
    mutate(value);
    assert.equal(
      verifyProvisioningRecordEnrollmentBindingCandidate(value).status,
      "blocked",
    );
  }
  let calls = 0;
  const dynamic = fixture();
  Object.defineProperty(dynamic, "certificateBindings", {
    enumerable: true,
    get() {
      calls += 1;
      return [];
  ...
  assert.equal(
    verifyProvisioningRecordEnrollmentBindingCandidate(dynamic).status,
    "blocked",
  );
...
test("binding remains a non-authoritative candidate", () => {
  ...
  assert.equal(
    contract.runtimeOwnedTrustRollbackClockFilesystemAndActivation,
    "not_implemented",
  );
