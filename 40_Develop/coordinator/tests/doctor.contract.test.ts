...
test("欠落、重複、未知および不正なcheckをfail closedにする", () => {
  const base: DiagnosticCheck[] = REQUIRED_CHECK_IDS.map((id) => ({
    id,
    status: "confirmed",
    reason: null,
    followUp: null,
  }));
  assert.equal(evaluateReadiness(base.slice(1)).status, "blocked");
  assert.equal(evaluateReadiness([...base, base[0]]).status, "blocked");
  assert.equal(
    evaluateReadiness([
      ...base,
