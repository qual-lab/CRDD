...
test("対話Consoleは一つのRuntime契約だけがOS deviceを所有する", () => {
  ...
  const executableSources = ["bin", "scripts", "src"]
    ...
    .filter(
      (file) =>
        !file.endsWith("interactive-console.ts") &&
        !file.endsWith("interactive-console-reader.ts"),
    );
  ...
  assert.equal(readerSource.includes("process.stdin"), false);
  assert.equal(readerSource.includes('"\\\\\\\\.\\\\CONIN$"'), true);
...
test("固定reader子Processは単一OS readとdescriptor closeを分離する", async () => {
  ...
  {
    const observed = scenario({ closeConfirmed: false });
    const pending = readOwnedInteractiveConsoleLineOutcomeUsingAdapter(
      "win32",
      new AbortController().signal,
      observed.adapter,
  ...
  {
    const observed = scenario({ openFails: true });
    assert.deepEqual(
      await readOwnedInteractiveConsoleLineOutcomeUsingAdapter(
        "win32",
        new AbortController().signal,
        observed.adapter,
      ),
...
test("固定reader fixtureはdescriptor close失敗を成功へ流用しない", () => {
  const fixture = path.join(
    ...
    "tests",
    "fixtures",
    "interactive-console-owned-reader-process.ts",
  );
...
test("Node版GateはPATHをAuthorityにせずEffect前に停止する", () => {
  ...
  assert.match(
    readme,
    /"<absolute-preverified-node-24\.12\+-executable>" "<absolute-crdd-source-root>\\40_Develop\\coordinator\\scripts\\generate-release-key\.ts"/u,
  );
  assert.match(
    readme,
    /"<absolute-preverified-node-24\.12\+-executable>" "<absolute-crdd-source-root>\\40_Develop\\coordinator\\scripts\\sign-release-manifest\.ts"/u,
  );
  assert.match(
    readme,
    /"<absolute-preverified-node-24\.12\+-executable>" "<absolute-crdd-source-root>\\40_Develop\\coordinator\\bin\\coordinator\.ts" doctor/u,
  );
  assert.equal(readme.includes("$CRDD_NODE"), false);
  assert.equal(readme.includes("$CRDD_COORDINATOR"), false);
  assert.equal(
    /<absolute-preverified-node-24\.12\+-executable> 40_Develop\/coordinator\/scripts\/(?:generate-release-key|sign-release-manifest)\.ts/u.test(
      readme,
    ),
    false,
  );
