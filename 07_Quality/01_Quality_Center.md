...
v0.20.0 Candidateは、試験体系と自動回帰、実行知、Runtime責務分離、限定分散実行と統合結果、読み取り専用Project State、およびlocalhost限定MCP HTTPを対象とする。CHG-000061、CHG-000062およびCHG-000064は決定論的回帰、Windows実Process確認および独立一括監査を完了した。CHG-000063は、配布全体の利用グラフ、完全な引数式と値の由来、Authority・handle所有、および正規観測結果から公開・署名・昇格・回復結果までの伝播を限定グラフへ統合した。固定改訂版`392bd1ee`は正式4経路E2Eを4/4で完走したが、続くRecovery Matrixで、中立化したRuntime子Processから新設のDocker CLI Publisher Trustを実行する際のPowerShell初期化条件が未接続であることを検出した。ambient環境を継承せず、Native OS観測で検証した`USERPROFILE`をAuthenticode専用環境へ追加し、多段起動の実Docker CLI署名確認1件と制限Process全回帰1,985件（成功1,980件、明示skip 5件、失敗0件）を完了した。先に成功した4/4は旧Runtime実行Identityの結果であり、新候補へ流用しない。次は新しい固定候補を正式署名し、正式4経路E2EとRecovery Matrixを完走した後、その結果を含む改訂版へ独立再レビューと最終監査を行う。branch、本候補表示または署名前の検証だけではv0.20.0の公開、採用、準拠またはRuntime実行Authorityを成立させない。
