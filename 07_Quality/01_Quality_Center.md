...
| 対象 | 現在状態 | 根拠・次の処置 |
|---|---|---|
| v0.19 Project Runtime | 設計固定済み。耐久基盤、単一Task縦断、最大5件の複数Task制御が部分成立。実装・公開は未完了 | 9 Interface、9永続Record、13資源、4 Lock、7 Authority、9 Effect、7状態機械、52遷移／52対応、32不変条件、16失敗注入点を固定。State／Queue／Lease、Single Task Adapter接続、Dependency／競合予約、cleanup不明時停止、Lease取得途中・取得後の回復Coreを契約試験へ接続した。実Platform owner観測、公開CLI／MCP、再計画、人間判断、Integration／採用は未接続であり、次はPlatform観測と公開Objective Intakeである。[CHG-000057](../90_Release/Changes/CHG-000057_Minimum_AI_Native_Project_Runtime.md#8-現在状態と次のgate)を参照 |
