...
| 対象 | 現在状態 | 根拠・次の処置 |
|---|---|---|
| v0.19 Project Runtime | 正常縦断2経路、低Risk自己適用、および署名前の決定論的Project全体確認が成立。署名後E2E、独立確認、公開・Releaseは未完了 | 2経路のMCP Objective受理～Milestone受入、うち1経路と別の[自己適用](../90_Release/Changes/Evidence/CHG-000057_Project_Runtime_Self_Application_0acc157.md)で正本採用を確認した。固定Commit `fbd07de`では[署名前検証](Verification_Results/2026-09-03_Project_Runtime_Pre_Sign_Verification.md)として1,552件の制限Process試験と7件のWindows実資源Gate、別Process排他、親EOF取消、対話／スケジュール競合、型付きRecovery settlement、MCP意味相関およびProject settlementに結合したDocker Receipt消費を確認した。認証済み公開MCP Clientからの実Provider経路、実Provider実行中取消、実Docker資源のRecovery settlementは署名後の最終E2Eまで未完了であり、Project Runtime全体の`Pass`ではない。詳細と次Gateは[CHG-000057](../90_Release/Changes/CHG-000057_Minimum_AI_Native_Project_Runtime.md#8-現在状態と次のgate)を参照 |
