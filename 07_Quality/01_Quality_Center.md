...
v0.21.0のQuality設計は13定義、154 Local Itemまで拡張した。全SubsystemのTest Symbolと再照合した現在値は、一意なLocal Item 154件中119件が観測済み、35件が未観測である。残る35件はAutomated 2件、Hybrid 14件、Manual 19件である。Automated 2件は全Consumerを同一Snapshotで通す回帰と、実Dockerの別Session回復を確認する署名E2Eが実行責務を持つ。自動部分だけのPassを人間判断または実境界を含むLocal Item全体の成立へ読み替えず、工程Evidenceと人間受入を分けて扱う。
...
| Quality Definitions | 13定義、154 Local ItemをCanonical化済み | 119件観測済み、35件未観測。Automated 2件、Hybrid 14件、Manual 19件。定義済みであることを実装済み・試験済みへ読み替えない |
...
| Reality Audit | In Progress — 回帰／署名E2EとHybrid／Manual Evidence Pending | Coordinator／Project Runtime Pilotに加えて全Subsystemを照合し、残るAutomated 2件、Hybrid 14件、Manual 19件を処置する |
...
| Executed | 部分実行 | 局所試験結果を未観測35 Local Itemの回帰、署名E2E、人間判断または実境界Evidenceへ数えない |
...
Coordinator／Project Runtimeの17意味Pilotに加えて全Subsystemへ照合範囲を広げた。17意味では自動観測16件、手動UAT待ち1件である。全Local Itemでは119件観測済み、35件未観測であり、Quality Readyへ昇格しない。PT／LTは人間の明示許可がないため実行しない。
