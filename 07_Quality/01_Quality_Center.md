...
v0.20.0 Candidateは、試験体系と自動回帰、実行知、Runtime責務分離、限定分散実行と統合結果、読み取り専用Project State、およびlocalhost限定MCP HTTPを対象とする。CHG-000061、CHG-000062およびCHG-000064は決定論的回帰、Windows実Process確認および独立一括監査を完了した。CHG-000063の固定改訂版`98146b3b70295bc122784871233d0bd7cf58c423`は機械回帰を完了したが、二つの独立再レビューにより、配布全体の利用グラフ、完全な引数式と値の由来、Authority・handle所有、および正規観測結果から公開・署名結果までの伝播が未成立と判定された。是正固定版`f77b6e3fd88504cb6cb51bbdec8470eeaa79ef93`は、実配布対象と検証Tool対象を別のグラフとして実ソースから全数導出し、宣言集合との不足、余分、重複、無所属を拒否する。重点128件、制限Process 1,745件、Windows実Process 7件、静的検査およびRepository全体Checker（Error 0、Warning 0）は成功し、独立再レビュー待ちである。再レビュー後にRuntime実行Identityを生成・署名し、影響する正式署名E2Eを完走して、候補内容と最終配布物の一致を確認する。branch、本候補表示または署名前の検証だけではv0.20.0の公開、採用、準拠またはRuntime実行Authorityを成立させない。
...
| v0.20 Runtime責務分離 | Independent Re-review Pending | 固定改訂版`98146b3b70295bc122784871233d0bd7cf58c423`への独立再レビューは、file単位のtoken／prefix照合では配布全体の利用グラフ、完全な引数と値の由来、Authority・handle所有、および公開結果への伝播を証明しないため、Critical 0、Major 3、Moderate 3、Minor 1で不合格とした。是正固定版`f77b6e3fd88504cb6cb51bbdec8470eeaa79ef93`では、Runtime 14件と検証Tool 6件の実Process呼出しを実ソースから全数導出し、宣言集合との完全一致、正規flow、公開・署名利用側への伝播を検査する。重点128件、制限Process 1,745件、Windows実Process 7件、静的検査およびRepository全体Checker（Error 0、Warning 0）は成功した。[検証結果](Verification_Results/2026-09-06_V020_Public_Runtime_and_Bounded_Integration_Verification.md)を参照。独立再レビュー、正式署名および正式E2Eは未完了 |
