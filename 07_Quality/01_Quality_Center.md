...
| 対象 | 現在状態 | 根拠・次の処置 |
|---|---|---|
| v0.19 Project Runtime | 設計固定済み。公開Objective入口からTask実行、再計画／判断移送、判断Capabilityの置換・失効・独立Recovery、実候補の統合／明示採用までを開発候補として部分接続。公開・Releaseは未完了 | 9 Interface、9永続Record、13資源、4 Lock、7 Authority、9 Effect、7状態機械、52遷移／52対応、32不変条件、16失敗注入点を固定。State／Queue／Lease、Single Task Adapter、Windows owner観測、対話Lane優先、同一計画の再試行・部分再計画、Windows保護Rootの判断記録と独立Recovery Intent、MCP stdio Process、実Candidate Storeの統合・fresh base照合・明示採用・rollbackを契約試験と結合試験へ接続した。固定開発版の実Provider E2Eは、公開済みv0.18.1でDocker Recoveryの公開入口が到達不能だったため未成立である。現行Sourceでは入口文法と旧署名manifest配置の履歴検証を是正済みだが、到達可能な署名固定版による正式Recovery、実Provider候補の採用、認証済みMCP Clientおよび本番同等E2Eは未評価であり、Project Runtime全体の`Pass`ではない。[CHG-000057](../90_Release/Changes/CHG-000057_Minimum_AI_Native_Project_Runtime.md#8-現在状態と次のgate)を参照 |
| v0.18.1 Coordinator採用入口 | 公開済み。現行署名Identityの採用形態E2E、4経路4/4、Recovery 7シナリオ完了 | [現行Identityと検証結果](Verification_Results/2026-09-01_Coordinator_v0181_Runtime_Identity.md)、公式tag `v0.18.1`／Commit `14872bd19c3569a4c06752545a6057b2b4aaf3ab`。未評価範囲は利用制限として保持し、公開判断の残件へ戻さない |
