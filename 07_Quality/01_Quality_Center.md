...
| 新配置の機械検証 | 是正版2033dfeでCoordinator全1,469試験成功 | 取消の追加結合、Gitローカル除外のclose時刻・初期mode検証を含む。[初回失敗・是正・網羅率の対象と限界](Verification_Results/2026-08-31_Coordinator_Closure_Verification.md)を参照。先行するChecker 208/208、開発E2E 239/239、Rust上位33成功・2 ignoredは今回の新しい実行ではない |
...
| 最新Runtime全体の独立完成監査 | 一括確認済み、今回差分の是正・限定再確認完了 | 固定Tree `687699ed`へ全観点を実行。取消の本番共有経路への接続、是正1往復の残余不確実性、網羅率記録、現在状態の伝播を指摘。文書5件は3495bcdで、追加是正は7a7d8a6で限定再確認Pass。OS通知・実Docker取消回収は未確認であり、全体Passではない |
...
署名版45ea2acでは、[Claude実行／Codex確認の是正1往復](../90_Release/Changes/Evidence/CHG-000015_Remediation_45ea2ac.md)も完了し、独立再確認を通過した。欠陥を仕込んだ1件であり、通常実務の成功率、逆方向の実是正、実Provider取消の証明ではない。Process所有部品の抽出は限定独立再確認を通過し、2033dfeではTask→Controller→共有終了処理の追加結合も全体試験を通過した。追加差分の独立再確認は7a7d8a6で完了したが、OS通知・実Docker取消回収は未完了。変更後の実装を署名版45ea2acと同一とは扱わない。
