...
| 対象 | 現在状態 | 根拠・次の処置 |
|---|---|---|
| v0.19 Project Runtime | 設計固定済み。公開Objective入口からTask実行、再計画／判断移送、実候補の統合／明示採用までを開発候補として部分接続。公開・Releaseは未完了 | 9 Interface、9永続Record、13資源、4 Lock、7 Authority、9 Effect、7状態機械、52遷移／52対応、32不変条件、16失敗注入点を固定。State／Queue／Lease、Single Task Adapter、Windows owner観測、対話Lane優先、同一計画の再試行・部分再計画に加え、Windows保護Rootの判断記録、MCP stdio Process、実Candidate Storeの統合・fresh base照合・明示採用・rollbackを契約試験と結合試験へ接続した。判断Capabilityの置換・全失効・独立Recovery Intent、実Provider候補の採用、認証済みMCP Clientおよび本番同等E2Eは未評価であり、Project Runtime全体の`Pass`ではない。[CHG-000057](../90_Release/Changes/CHG-000057_Minimum_AI_Native_Project_Runtime.md#8-現在状態と次のgate)を参照 |
