...
| `ERB-IT-017` | 回復 | IT | Authentication／Durable Recovery | 子Process→Provider Home Kernel Lock→耐久Intent→fresh Process再入場 | Direct Boundary | 固定Provider Home Identity、active Intent、所有資源、不意のProcess終了 | Lockとactive Intentを持つProcessを終了し、fresh Processで同じ再認証を開始する | Process終了、Lock再取得、Recovery ID、資源所有権、Intent状態、後続Effect順序を記録する | 別Process終了後も同じRecovery IDを保持し、Kernel Lock再取得と所有資源不存在の確認後だけ再入場してsettledへ閉じる | ERB-IT-017、固定Identity、子終了、Lock再取得、Intent遷移、Docker Effect列、Oracleおよび終了後状態を保存する。Path、Credential、Secret、生Provider出力は保存しない | 所有Docker資源0。耐久記録はsettled。Kernel Lock解放確認 | Automated |
