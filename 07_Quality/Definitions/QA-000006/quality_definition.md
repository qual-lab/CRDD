...
| `ERB-ST-009` | 回復 | ST | Docker Repair／Recovery | 修復Identity→既知Runtime領域の段階退避→Docker restart→Engine readiness | System/E2E | exact Repair ID、`Docker/run`と`docker-secrets-engine`の既知2領域、停止中Docker、初回起動失敗、Engine readiness Observer、単一Probe Timeout、全体起動期限、署名版更新 | 同じRepair IDで各領域を順序付きに退避し、再起動・確認を継続する。初回起動が失敗して既知2領域のlockとProcess不在を確認できる場合は、同じInvocation内でContinuationへ移る | Repair ID、領域別Identity、各Effectの意図と結果、再起動試行、Engine readiness、同一実行内で計測できた起動所要時間の参考値、再入場回数を記録する | 未完了Effectを再発行せず、個別socketを削除せず、再起動confirmed後の一時的なProbe TimeoutはHost Effectなしで期限付き再観測する。期限満了後もProcess不在と既知2領域のexact lockが揃う場合だけ同じInvocationで段階修復し、2領域の退避と新世代確認が揃いEngine readyになった後だけ完了する | ERB-ST-009、Repair ID、領域別Identity Hash、追記記録chain、Effect確認、Engine readiness、起動所要時間または未計測、同一Invocation継続または再入場の結果、残る回復義務を保存する | native helper残存0。Process稼働中、部分退避、全体期限超過、観測不能、intent後中断では自動継続せず同じRepair IDの回復義務を保持する | Hybrid |
...
初回起動期限後にDocker Processが残存する実環境反例では、`ERB-ST-009`は同じRepair ID内で停止意図を耐久化し、停止Effectを一回だけ発行してProcess不存在をfreshに確認した後にだけ二領域退避へ進むことを要求する。既に不存在なら停止Effectは`not_issued`で閉じる。部分停止、観測不能、取消またはIdentity不一致ではrenameと再起動を0件にし、同じ回復義務を保持する。旧形式の3 Effect継続記録は、成立済み段階を再発行せず現行の4 Effectモデルへ読取り投影できることも検証する。
