...
| `ERB-ST-009` | 回復 | ST | Docker Repair／Recovery | 修復Identity→既知Runtime領域の段階退避→Docker restart→Engine readiness | System/E2E | exact Repair ID、`Docker/run`と`docker-secrets-engine`の既知2領域、停止中Docker、初回起動失敗、Engine readiness Observer、単一Probe Timeout、全体起動期限、署名版更新 | 同じRepair IDで各領域を順序付きに退避し、再起動・確認を継続する | Repair ID、領域別Identity、各Effectの意図と結果、再起動試行、Engine readiness、同一実行内で計測できた起動所要時間の参考値、再入場回数を記録する | 未完了Effectを再発行せず、個別socketを削除せず、再起動confirmed後の一時的なProbe TimeoutはHost Effectなしで期限付き再観測し、2領域の退避と新世代確認が揃いEngine readyになった後だけ完了する | ERB-ST-009、Repair ID、領域別Identity Hash、追記記録chain、Effect確認、Engine readiness、起動所要時間または未計測、再入場結果、残る回復義務を保存する | native helper残存0。部分退避、全体期限超過、観測不能、intent後中断では同じRepair IDの回復義務を保持する | Hybrid |
