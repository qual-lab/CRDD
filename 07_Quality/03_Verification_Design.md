...
Docker create結果不明を検証済みDesktop再起動後に収束させる経路は、Help表示だけでなく公開引数ParserとDispatcherを通して実到達できることを確認する。旧Release Rootは新旧manifest配置のexact一方だけを受理し、両配置併存、欠落、不正byteまたは由来不一致を拒否する。履歴検証を現在の実行Authorityへ流用せず、対象Taskより後の修復順序、exact Task Recovery ID、修復ID、二軸不存在および耐久化後の再入場までを一つの結合試験で確認する。
...
| PR-A-04 | 異常 | Lease取得中Markerの作成途中、物理Lock／Lock所有Marker／証跡／解放Markerの各境界、Queue owner結合前、Authority発行前／開始後のParent喪失、QueueだけまたはStateだけを更新した直後、および`recovery_required`からの再開要求。`maximumReplans: 0`も含める | 新規Task開始0。全資源のfreshな不存在を確認できた同期失敗だけを巻戻し済みとし、それ以外は決定論的な回復IDを返す。不完全Markerと所有不明Lockは変更せず手動回復へ閉じる。Authority発行前の`starting`はEffect 0で`ready`へ戻す。開始済みTaskはProject Operation IDからRuntime所有のexact Recovery Identityを解決し、欠落・重複・不一致を推測しない。再開ではClient指定値をAuthorityにせず、Project StateのTask別Recovery IdentityとQueueの非Authorityな適用IDを照合する。回復完了、対象資源不存在、専用Queue settlement、`settledRecoveryId`を持つfresh retry State、owner不存在、世代／Revision／容量／Dependency／Conflictの再確認後だけ新しいLeaseとTask Authorityを許可する。Queue／State片側更新後の中断では未完了側だけを再開してRecovery Effectを重複発行せず、再計画上限を回復義務へ適用しない |
