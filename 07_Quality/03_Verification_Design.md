...
MCP投影は列挙値と件数型だけでなく、Milestone状態、Objective／Task件数、最大128件のObjective別Task状態集計、Work Progress、Quality、人間判断要否、Recovery要否およびNext Actionの相関をCoreと同じ純粋規則で検証する。集約件数だけが一致する一方でObjectiveへのTask割当が成立しない反証例を拒否し、Objective別集計の総和が全体集計と一致することを確認する。Recovery中の`recover`、判断中の`human_decision`、統合中の検証Action、受入済みの`complete`を崩す反証例も拒否する。一方、Dependency Graphを持たないTransportでは`schedule_task`と`wait_for_task`の一方を推測せず、両方をCoreが選び得る通常Actionとして受理する。
