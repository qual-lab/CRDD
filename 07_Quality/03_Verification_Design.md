...
修復履歴のadoption、session handoffおよびclosureは同じ耐久公開処理を使用する。正常な初回公開に加え、準備fileだけ、公開targetと同一fileの準備残存、同じbyteだが別fileの準備残存、部分file、非通常file、未知名、競合する公開結果および観測不能を固定する。読取り専用inventoryが残存を削除せず非成功に保つことと、現在Authorityと期待byteを持つ対象限定persistだけが同一fileの残存を収束することを分けて確認する。各故障ではDocker／Provider／既存修復へのEffect 0、元記録とRecovery IDの不変、別候補の非削除および外側Lock／helperの解放を確認し、解放不明を成功にしない。
