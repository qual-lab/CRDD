...
最新の署名固定版`48515eb`でも4経路4/4、固定Workerの復旧7シナリオ、実TaskのCtrl+C取消と通常回収を確認した。再実測の対象・結果・限界は[48515ebの結果](#signed-e2e-48515eb)へ分離する。以下の`4f10201`の測定値と当時の独立確認は変更しない。
...
<a id="signed-e2e-48515eb"></a>

## 48515ebでの再実測

2026-09-01（日本時間）、追加境界試験と限定是正を固定したCommit `48515eb31a2b3f58ba450a2703c35b98e6487670`、Tree `6f14af76ca2a3e261e9e668701dadf3d49488769`で再実測した。実行環境と入口は上記と同じで、当該版の署名配布物を使用した。各経路の結果はmanifest SHA-256 `fb54f39738020cbe6010b62c9625b3be11e592962b3451cdb2aa101181d58e2b`、package content root SHA-256 `25dddf494bf38032c2a4b6fb2c051338091ca352b1feb850b3e41e9c7c7f3c0b`と同じCommit／Treeを返した。復旧は同じ配布物の入口から続けて実行し、保存envelopeのRepository情報だけを実行版の証明にしない。

| 確認 | UTC時刻（2026-08-31） | 観測結果 |
|---|---|---|
| 4経路 | 17:29:46.149Z～17:37:33.809Z | `signed_route_matrix_completed`、試行4・完了4・再試行0。全経路で内容完全一致、候補破棄、資源回収成立。是正往復なし、Repository不変、手動回復・再起動不要 |
| 固定Workerの復旧 | 17:37:34.087Z～17:37:51.870Z | `signed_recovery_matrix_verified`、7シナリオ。期待した異常停止、取消、回収観測不明からの回復、親消失後の子終了と新しい回復を確認。全体の資源回収成立、手動回復不要 |
| 実Task取消 | 17:38:26.138Z～17:39:14.973Z | `verified`。対象Claudeコンテナ1個の`running`を17:38:54.712Zに観測し、17:38:58.782Zに人間のCtrl+Cを1回受信。子CLIはexit 2、signal／launchErrorは`null` |

取消の依頼Hashは上記`c0198f6f…7a8`と同一で、測定scriptは同じ検証方法の固定版参照と保存名だけを更新した。公開結果は`coordinator_task_cancelled_after_provider_cleanup`、`cleanupConfirmed: true`、手動回復・再起動不要、候補未発行、Repository不変だった。測定側の`cancelObserved`、`containersAndNetworksAbsent`、`repositoryUnchanged`は`true`、出力超過・観測失敗は`false`。親の終了後確認でも子Processと対象Docker container／networkは不存在だった。新しい読取り専用入口による回復状態も`docker_task_runtime_state_clean`、手動回復不要、Docker回復ID・active bindingなしを返した。

先行試行`6698d38a-7c6c-4aae-8713-730b0fde867a`は、利用者が画面を閉じた後に開始記録だけが残り、完了結果を取得できなかった。成功件数へ含めない。回復状態と対象資源の不存在を確認して同じ署名版で再実行した。追加の署名鍵入力・自動課金経路・購入・修復は行っていない。

| 記録 | Repository-relativeな所在とSHA-256 |
|---|---|
| 4経路 | `.crdd/verification-results/5f7c6a73-7a55-42cd-b99d-4a07de523606/result.json` — `0cabb6b9132242e36617ef5ed4cc274cb2e1268150d6929d743643cff69a95a7` |
| 復旧 | `.crdd/verification-results/b09a00e4-40bb-4c56-841f-f6165a915153/result.json` — `3672cc88342c2449cc6fa59e90f10a9e0b8d7f2792735e9037ed5dd8cf15d290` |
| 実取消 | `.crdd/dogfooding/public-cancellation-48515eb.result.json` — `d72d05a89a9f15d25bf89e2338655b653d735be046cc30e1882f94decb114b08` |
| 取消測定script | `.crdd/dogfooding/verify-public-task-cancellation-48515eb.mjs` — `56387c5f9a4449586fe074fe486d9db693d78e5e01590899fa5635a3f48f596d` |

4経路・復旧では開始／結果／完了の識別情報と結果Hashを照合した。保存要約の`null`／`unknown`は元結果にない値を補完しないための表示であり、各途中状態の成功を意味しない。実取消もProvider内部の準備完了、全取消タイミング、全Providerへの保証ではなく、公開結果にない`effectStateUnknown`を`false`へ補完しない。旧版の実務有用性を再測定したものでもない。

今回閉じたのは変更後固定版の正式実測である。実装・試験の独立確認は[完成確認記録](2026-09-01_Coordinator_Completion_Review.md)を参照し、本追記と現在案内の限定独立確認は完了した。全体完成評価、統合・リリースは別に扱う。生記録・Host label・認証情報は公開文書へ転載しない。

### 再実測記録の限定確認

Wegenerが`48515eb`を基準とする7文書の凍結差分、3実測記録と測定scriptのHash、開始／結果／完了の対応、版・時刻・観測限界、参照先を独立確認した。実測主張の不整合はなく、Completion ReviewのTEST-01現在行に残った「正式実測が残件」という古い表示1件を検出した。是正方針を整合し、当該句だけ更新した後の限定再確認はPass、指摘解消・追加指摘0だった。再確認対象のCompletion Review SHA-256は`52f0f8036419c65880ba56f9cf57ee270684aecf36abc97144027788ddd0e8a0`であり、この後の確認完了表示の更新を含むHashとは扱わない。

共通Checkerは2026-08-31T17:44:25.992Zに全体を実行し、389文書・2,736リンク・919アンカー・固定履歴24件、エラー／警告0。確認者は同じ結果を利用した。再確認後の現在案内への完了反映と本節の追記は記録更新だけであり、署名版、実測値、過去結果、コード、全体完成判定を変えない。

今回の古い表示は、既存の現在状態・参照元照合の適用漏れとして同じCHG内で是正した。新しい規範や専用Checkerは追加せず、結果の正本と現在案内を対照する既存の確認を用いる。未完了の全体完成評価と統合・リリース判断はQuality Centerと既存CHGで追跡する。
